...
export function PullToRefresh({
  ...
  lastRefreshedLabel,
  refreshCount,
  isRefreshing: isExternallyRefreshing = false,
}: PullToRefreshProps) {
  ...
  const isRefreshing = isExternallyRefreshing || isRefreshingLocally;
  const displayPhase = isExternallyRefreshing ? 'refreshing' : phase;
  ...
  const handleTouchMove = useCallback(
    ...
    [hapticLight, isRefreshing, threshold],
  );
  ...
  }, [actionToast, hapticLight, phase, isRefreshing, onRefresh]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          height: isExternallyRefreshing ? INDICATOR_HEIGHT : pullDistance,
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            <div
              style={{
                transform: `rotate(${displayPhase === 'refreshing' ? 0 : rotation}deg)`,
                animation: displayPhase === 'refreshing' ? 'ptr-spin 0.7s linear infinite' : 'none',
              }}
            >
              <RefreshCw
                size={18}
                color={
                  displayPhase === 'triggered' || displayPhase === 'refreshing'
                    ? '#3B82F6'
                    : c.text3
                }
                strokeWidth={2.5}
            ...
            >
              {displayPhase === 'refreshing'
                ? 'Đang tải...'
                : displayPhase === 'triggered'
                  ? 'Thả để làm mới'
          ...
          {lastRefreshedLabel && displayPhase !== 'refreshing' && (
            <span
              style={{
                color: c.text3,
                fontSize: 9,
