...
import { SkeletonList, SkeletonPageList } from '@/shared/ui/SkeletonBlock';
import { EmptyState } from '@/shared/ui/EmptyState';
import { ErrorState } from '@/shared/ui/ErrorState';
import { OfflineBanner, Banner } from '@/shared/ui/OfflineBanner';
import { GatePanel } from './GatePanel';
import { SessionExpiredModal, ReauthSheet } from './SecurityOverlays';
    ...
    <div className="flex flex-col gap-6 pb-8">
      <div className="px-5">
        <p style={{ color: '#8B95B3', fontSize: 13, lineHeight: 1.6, marginBottom: 16 }}>
          5 state pattern chuẩn enterprise, match 100% visual style hiện tại. Chọn state để xem
          preview.
        </p>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
          {[
            ...
            <button
              key={s.id}
              onClick={() => setActiveState(s.id as StateType)}
      ...
      >
        ...
        >
          ...
          <span style={{ color: '#F0F4FF', fontSize: 15, fontWeight: 600 }}>
            Preview — {activeState}
          </span>
          <div className="w-8" />
        </div>

        {activeState === 'loading' && <SkeletonPageList title="Thị trường" rows={5} />}
        ...
        {activeState === 'error' && (
          <ErrorState
            title="Có lỗi xảy ra"
            message="Vui lòng thử lại. Nếu lỗi tiếp tục, hãy kiểm tra kết nối mạng."
            onAction={() => {}}
          />
        )}
          ...
          <div className="flex flex-col gap-3 py-4">
            ...
            <div className="px-4">
              <div
                ...
                style={{ background: '#141822', border: '1px solid #2A3356', opacity: 0.6 }}
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className="w-9 h-9 rounded-full flex items-center justify-center"
                    style={{ background: '#F7931A22' }}
                  >
                  ...
                  <span style={{ color: '#F0F4FF', fontSize: 14, fontFamily: 'monospace' }}>
                    $67,543.21
                  </span>
                </div>
        ...
        {activeState === 'gate' && (
          <div className="flex flex-col">
            <GatePanel type="kyc" onAction={() => navigate(`${prefix}/profile/kyc`)} />
          </div>
        )}
      </div>
      ...
      <div className="px-5">
        ...
        <div className="flex flex-col gap-3">
          <Banner
            variant="info"
            icon={<BarChart2 size={14} color="#3B82F6" />}
...
> = {
  ...
  watchlist: {
    ...
    empty: {
      title: 'Bạn chưa theo dõi cặp nào',
      subtitle: 'Thêm cặp giao dịch yêu thích để theo dõi biến động giá dễ dàng hơn.',
      cta: 'Thêm vào Watchlist',
      emoji: '⭐',
    },
  },
  orders: {
    ...
    empty: {
      ...
      subtitle: 'Bạn chưa đặt lệnh giao dịch nào. Bắt đầu ngay để tham gia thị trường!',
      cta: 'Bắt đầu giao dịch',
      emoji: '📊',
    },
  },
  transactions: {
    ...
    empty: {
      ...
      subtitle: 'Lịch sử giao dịch sẽ hiển thị ở đây sau khi bạn nạp, rút hoặc giao dịch.',
      cta: 'Nạp tiền',
      emoji: '💰',
    },
  },
  p2p: {
    ...
    path: '/p2p/my-orders',
    empty: {
      title: 'Bạn chưa có đơn P2P nào',
      subtitle: 'Mua bán crypto trực tiếp với người dùng khác — an toàn, nhanh chóng.',
      cta: 'Khám phá P2P',
      emoji: '🤝',
    },
  },
  notifications: {
    ...
    empty: {
      ...
      subtitle: 'Thông báo về giao dịch, bảo mật và cập nhật sẽ xuất hiện ở đây.',
      cta: 'Cài đặt thông báo',
      emoji: '🔔',
    },
  },
};
  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
      <div className="px-5">
        ...
        <div className="flex gap-2 mb-4">
          {(
            ...
            ] as { id: StateType; label: string; color: string }[]
          ).map((s) => (
            <button
              key={s.id}
      ...
      >
        ...
        {/* ─── STATE CONTENT ─── */}
        {activeState === 'loading' && <SkeletonList rows={6} />}
        ...
        {activeState === 'error' && (
          <ErrorState
            title="Có lỗi xảy ra"
            message="Vui lòng thử lại. Nếu lỗi tiếp tục, liên hệ hỗ trợ."
            onAction={() => {}}
          />
        )}
        ...
        {activeState === 'offline' && (
          <div className="flex flex-col">
            <div className="py-3">
              <OfflineBanner message="Mất kết nối. Đang hiển thị dữ liệu gần nhất." showStaleHint />
            </div>
      ...
      >
        <p style={{ color: '#F0F4FF', fontSize: 14, fontWeight: 700, marginBottom: 8 }}>
          Naming Convention
        </p>
        <div className="flex flex-col gap-1">
          {['Loading', 'Empty', 'Error', 'Offline', ...(hasGate ? ['Gate'] : [])].map((state) => (
            <div key={state} className="flex items-center gap-2 py-1">
              <span
                style={{ color: '#3B82F6', fontSize: 12, fontFamily: 'monospace', fontWeight: 600 }}
              >
                {config.label} – {state}
              </span>
            </div>
...
function SecurityOverlaysSection() {
  const navigate = useNavigate();
  const prefix = useRoutePrefix();
  const [activeOverlay, setActiveOverlay] = useState<'session' | 'reauth' | 'kyc' | '2fa'>(
    'session',
  );
      ...
      <div className="px-5">
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <p style={{ color: '#4A5568', fontSize: 12 }}>Input: Password hoặc mã 2FA (toggle)</p>
          </div>
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <button
                onClick={() => setActiveOverlay('kyc')}
                className="px-3 py-1.5 rounded-xl text-xs font-semibold"
                style={{
                  background:
                    activeOverlay === 'kyc' ? 'rgba(245,158,11,0.2)' : 'rgba(245,158,11,0.08)',
                  color: '#F59E0B',
            ...
            <p style={{ color: '#4A5568', fontSize: 12 }}>CTA: "Đi tới KYC" → /profile/kyc</p>
          </div>
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <button
                ...
                style={{
                  background:
                    activeOverlay === '2fa' ? 'rgba(59,130,246,0.2)' : 'rgba(59,130,246,0.08)',
                  color: '#3B82F6',
      ...
      {(activeOverlay === 'kyc' || activeOverlay === '2fa') && (
        ...
        >
          ...
          <GatePanel
            type={activeOverlay}
            onAction={() =>
              navigate(`${prefix}${activeOverlay === 'kyc' ? '/profile/kyc' : '/profile/security'}`)
            }
          />
        </div>
      )}
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 style={{ color: '#F0F4FF', fontSize: 20, fontWeight: 700 }}>
              02 – Enterprise UI States
            </h1>
            <p style={{ color: '#8B95B3', fontSize: 12 }}>
              UI-only • Không redesign • Match 100% style hiện tại
            </p>
          </div>
        </div>
      </div>
