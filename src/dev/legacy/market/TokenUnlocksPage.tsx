...
import { fmtCompact, fmtPct, fmtPrice, fmtNum } from '@/shared/lib/formatNumber';
import { FONT_SCALE, FONT_WEIGHT } from '@/shared/theme/legacyTypography';
import {
  TOKEN_UNLOCKS,
  VESTING_CATEGORY_CONFIG,
  IMPACT_CONFIG,
  type TokenUnlock,
} from '@/dev/mocks/market-p3-fixtures';
...
export function TokenUnlocksPage() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'Giá trị cao':
        return items.sort((a, b) => b.unlockValueUsd - a.unlockValueUsd);
      case 'Tác động lớn':
        return items.sort((a, b) => b.unlockPctCirculating - a.unlockPctCirculating);
      default:
        return items;
    }
  }, [sortBy, impactFilter]);
  ...
  const avgDilution =
    TOKEN_UNLOCKS.reduce((s, u) => s + u.unlockPctCirculating, 0) / TOKEN_UNLOCKS.length;
                ...
                {IMPACT_FILTERS.map((f) => {
                  const cfg =
                    f === 'all'
                      ...
                      : IMPACT_CONFIG[f as TokenUnlock['impactLevel']];
                  return (
                    <button
                      key={f}
                      onClick={() => {
            ...
            <div className="flex flex-col" style={{ gap: 4 }}>
              {filtered.map((unlock) => (
                <UnlockCard
                  ...
                  onToggle={() => {
                    setExpandedId(expandedId === unlock.id ? null : unlock.id);
                    hapticSelection();
                  }}
                  c={c}
                />
            ...
            <TrCard className="p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['high', 'medium', 'low'] as const).map((level) => {
                  ...
                  const totalValue = TOKEN_UNLOCKS.filter((u) => u.impactLevel === level).reduce(
                    (s, u) => s + u.unlockValueUsd,
                    0,
                  );
                  return (
                    <div
                  ...
                  {Object.entries(VESTING_CATEGORY_CONFIG).map(([key, cfg]) => {
                    ...
                    const maxTotal = Math.max(
                      ...Object.keys(VESTING_CATEGORY_CONFIG).map((k) =>
                        TOKEN_UNLOCKS.filter((u) => u.category === k).reduce(
                          (s, u) => s + u.unlockValueUsd,
                          0,
                        ),
                      ),
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p style={{ color: c.text3, fontSize: FONT_SCALE.xs, lineHeight: 1.6 }}>
                    Token unlock không đồng nghĩa token sẽ bị bán. Tuy nhiên, unlock lớn thường tạo
                    áp lực bán tiềm ẩn. Dữ liệu chỉ mang tính tham khảo. Hãy kết hợp với các yếu tố
                    khác khi ra quyết định.
                  </p>
                </div>
              </div>
            </TrCard>
...
function UnlockCard({
  ...
  return (
    <TrCard
      ...
      accentBorder={unlock.impactLevel === 'high' ? 'rgba(239,68,68,0.12)' : undefined}
    >
      <button onClick={onToggle} className="w-full text-left px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          >
            <span
              style={{ color: unlock.color, fontSize: FONT_SCALE.xs, fontWeight: FONT_WEIGHT.bold }}
            >
              {unlock.symbol.slice(0, 2)}
            </span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                style={{
                  color:
                    unlock.daysUntil <= 3 ? '#EF4444' : unlock.daysUntil <= 7 ? '#F59E0B' : c.text2,
                  fontSize: FONT_SCALE.micro,
                  fontWeight: FONT_WEIGHT.semibold,
                }}
              >
        ...
        <div className="px-4 pb-3 pt-1" style={{ borderTop: `1px solid ${c.borderSolid}` }}>
          ...
          {unlock.priceChange7d < -3 && (
            ...
            >
              ...
              <span style={{ color: '#EF4444', fontSize: FONT_SCALE.micro }}>
                Giá giảm {fmtPct(unlock.priceChange7d)} trong 7 ngày — có thể liên quan đến unlock
                sắp tới
              </span>
            </div>
          )}
