...
const KYC_LEVELS = [
  ...
  },
  {
    ...
    title: 'KYC Cấp 1',
    limits: 'Nạp: Không giới hạn\nRút: $20,000/ngày',
    features: ['Tất cả tính năng cơ bản', 'P2P Trading', 'Rút tiền cao hơn'],
    color: '#3B82F6',
  },
  {
    level: 2,
            ...
            >
              ...
              >
                <div
                  ...
                  style={{
                    background: isDone ? lv.color + '22' : c.surface2,
                    border: `2px solid ${isDone ? lv.color : c.borderSolid}`,
                  }}
                >
                  {isDone ? (
                    <CheckCircle size={20} color={lv.color} />
                  ) : (
                    <span style={{ color: c.text3, fontSize: 14, fontWeight: 700 }}>
                      {lv.level}
                    </span>
                  )}
                </div>
                <div className="flex-1 text-left">
                  <p style={{ color: isDone ? c.text1 : c.text2, fontSize: 15, fontWeight: 700 }}>
                    {lv.title}
                  </p>
                  <p style={{ color: c.text3, fontSize: 12 }}>
                    {isDone ? '✓ Đã hoàn thành' : 'Chưa xác minh'}
                  </p>
                </div>
                <ChevronRight
                  size={16}
              ...
              {isExpanded && (
                ...
                >
                  <div className="mt-3">
                    ...
                    {lv.limits.split('\n').map((line, i) => (
                      <p key={line} style={{ color: c.text1, fontSize: 13 }}>
                        • {line}
                      </p>
                    ))}
                  </div>
                  ...
                  {!isDone && lv.level === currentLevel + 1 && (
                    <button
                      className="w-full h-12 rounded-2xl flex items-center justify-center gap-2 font-semibold text-white text-sm"
                      style={{
