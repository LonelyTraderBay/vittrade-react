...
function PwStrength({ password }: { password: string }) {
  ...
  return (
    <div className="mt-2">
      <div className="flex gap-1 mb-2">
        {[0, 1, 2, 3].map((i) => (
          <div
            ...
            style={{ background: i < score ? colors[score] : c.borderSolid }}
          />
        ))}
      </div>
      ...
      {score > 0 && (
        <p style={{ color: colors[score], fontSize: 12, marginTop: 4 }}>Mật khẩu {labels[score]}</p>
      )}
    </div>
  );
}
...
export function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (!contact.trim())
      e.contact = regType === 'email' ? 'Vui lòng nhập email' : 'Vui lòng nhập số điện thoại';
    if (regType === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact))
      e.contact = 'Email không hợp lệ';
      ...
      <PageContent gap={16} padding="default">
        <div className="flex rounded-2xl p-1" style={{ background: c.surface2 }}>
          {(['email', 'phone'] as RegType[]).map((t) => (
            <button
              ...
              style={{
                background: regType === t ? c.chipActiveBg : 'transparent',
                color: regType === t ? c.chipActiveText : c.text2,
              }}
            >
        ...
        <button onClick={() => setAgreed(!agreed)} className="flex items-start gap-3 text-left">
          ...
          <span style={{ color: c.text2, fontSize: 13, lineHeight: 1.5 }}>
            Tôi đã đọc và đồng ý với <span style={{ color: '#3B82F6' }}>Điều khoản dịch vụ</span> và{' '}
            <span style={{ color: '#3B82F6' }}>Chính sách bảo mật</span> của VitTrade.
          </span>
        </button>
        {errors.agreed && (
          <p style={{ color: '#EF4444', fontSize: 12, marginTop: -8 }}>{errors.agreed}</p>
        )}
