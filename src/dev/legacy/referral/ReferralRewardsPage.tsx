...
import { φ, φSpace, φIcon } from '@/shared/lib/golden';
import {
  COMMISSION_RECORDS,
  REFERRAL_CHART_DATA,
  getReferralStats,
...
export function ReferralRewardsPage() {
  ...
  const filteredRecords = useMemo(
    () =>
      COMMISSION_RECORDS.filter((r) => tab === 'all' || r.type === tab).sort((a, b) => {
  ...
  const totalKycBonus = COMMISSION_RECORDS.filter(
    (r) => r.type === 'kyc_bonus' && r.status === 'completed',
  ).reduce((s, r) => s + r.amount, 0);
  const totalTradeComm = COMMISSION_RECORDS.filter(
    (r) => r.type === 'trade_commission' && r.status === 'completed',
  ).reduce((s, r) => s + r.amount, 0);
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      ...
      if (exportFormat === 'copy') {
        ...
        actionToast.success(`Đã sao chép! ${records.length} bản ghi đã sao chép vào clipboard`);
      } else {
        // Trigger download
        ...
        actionToast.success(`Xuất file thành công! ${records.length} bản ghi đã tải về dạng CSV`);
      }
  ...
  const handleSubmitDispute = () => {
    if (!disputeType || !disputeDesc.trim()) {
      actionToast.error('Thiếu thông tin — Vui lòng chọn loại vấn đề và mô tả chi tiết');
      return;
    }
    setIsSubmitting(true);
    setTimeout(() => {
      actionToast.success(
        'Đã gửi báo lỗi! Mã ticket: #DISP-' +
          (MOCK_DISPUTES.length + 1).toString().padStart(3, '0') +
          '. Chúng tôi sẽ xử lý trong 24–48h.',
      );
  ...
  return (
    <PageLayout>
      <Header
        ...
        back
        onBack={() => navigate(prefix + '/referral')}
      />
          ...
          <TrCard variant="hero" rounded="lg" className="p-5">
            <div className="flex items-center justify-between mb-1">
              ...
              >
                ...
                <span style={{ color: c.portfolioTextDim, fontSize: φ.xs, fontWeight: 600 }}>
                  {currentTier.name}
                </span>
                <span style={{ color: c.portfolioTextMuted, fontSize: φ.xs, opacity: 0.7 }}>
                  ({currentTier.nameEn})
                </span>
              </div>
            </div>
            ...
            <SectionHeader
              ...
              right={
                <span style={{ color: '#10B981', fontSize: φ.sm, fontWeight: 600 }}>
                  +{fmtUsd(stats.thisMonthCommission)} tháng này
                </span>
              }
            />
                ...
                filteredRecords.map((record, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        borderBottom:
                          i < filteredRecords.length - 1 ? `1px solid ${c.divider}` : 'none',
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p style={{ color: c.text3, fontSize: φ.xs, lineHeight: 1.5 }}>
                    ...
                    />{' '}
                    bên cạnh mỗi giao dịch để báo lỗi. Đội ngũ hỗ trợ sẽ xử lý trong 24–48 giờ làm
                    việc.
                  </p>
                </div>
              </div>
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <p style={{ color: c.text3, fontSize: φ.xs }}>
              Dùng cho kê khai thuế / kế toán cá nhân
            </p>
          </div>
        </div>

        {/* Date Range */}
        <p style={{ color: c.text2, fontSize: φ.sm, fontWeight: 600, marginBottom: 8 }}>
          Khoảng thời gian
        </p>
          ...
          {EXPORT_DATE_RANGES.map((r) => (
            <button
              ...
              style={{
                ...
                fontSize: φ.sm,
                fontWeight: exportRange === r.id ? 600 : 400,
              }}
            >
        ...
        <div className="flex gap-2 mb-5">
          {[
            { id: 'csv' as const, label: 'Tải CSV', icon: Download, desc: 'File .csv tải về máy' },
            {
              ...
              label: 'Sao chép',
              icon: Copy,
              desc: 'Copy dữ liệu vào clipboard',
            },
          ].map((f) => (
            <button
              key={f.id}
            ...
            >
              ...
              <p
                style={{
                  color: exportFormat === f.id ? c.chipActiveText : c.text1,
                  fontSize: φ.sm,
                  fontWeight: 600,
                  marginTop: 6,
                }}
        ...
        <div className="rounded-xl p-3 mb-5" style={{ background: c.surface2 }}>
          <p style={{ color: c.text3, fontSize: φ.xs, fontWeight: 600, marginBottom: 4 }}>
        ...
        <div className="flex items-start gap-2 mt-4 px-1">
          ...
          <p style={{ color: c.text3, fontSize: φ.xs, lineHeight: 1.5 }}>
            Dữ liệu xuất ra chỉ mang tính chất tham khảo. VitTrade không cung cấp tư vấn thuế. Vui
            lòng liên hệ cơ quan thuế hoặc kế toán viên cho các nghĩa vụ cụ thể.
          </p>
        </div>
      </BottomSheetV2>
      ...
      >
        ...
        {disputeRecord && (
          ...
          >
            <div
              ...
              style={{
                background:
                  disputeRecord.type === 'kyc_bonus'
                    ? 'rgba(139,92,246,0.12)'
                    : 'rgba(16,185,129,0.12)',
              }}
            >
              {disputeRecord.type === 'kyc_bonus' ? (
                <Award size={φ.base} color="#8B5CF6" />
              ) : (
                <TrendingUp size={φ.base} color="#10B981" />
              )}
            </div>
            <div className="flex-1 min-w-0">
              <p style={{ color: c.text1, fontSize: φ.sm, fontWeight: 600 }} className="truncate">
                {disputeRecord.friendName}
              </p>
              <p style={{ color: c.text3, fontSize: φ.xs }}>
                {disputeRecord.action} • {disputeRecord.date}
              </p>
            </div>
            <p
              style={{
          ...
          {DISPUTE_TYPES.map((dt) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  style={{
                    color: disputeType === dt.id ? c.chipActiveText : c.text1,
                    fontSize: φ.sm,
                    fontWeight: 600,
                  }}
                >
        ...
        <button
          ...
          style={{
            minHeight: 52,
            background:
              !disputeType || !disputeDesc.trim()
                ? c.surface2
                : isSubmitting
                  ? 'rgba(245,158,11,0.15)'
                  : 'linear-gradient(135deg, #F59E0B, #D97706)',
            color:
              !disputeType || !disputeDesc.trim() ? c.text3 : isSubmitting ? '#F59E0B' : '#000',
            fontSize: φ.base,
            fontWeight: 700,
            boxShadow:
              !disputeType || !disputeDesc.trim() || isSubmitting
                ? 'none'
      ...
      {/* ═══════════════════════════════════════════
          ...
          ═══════════════════════════════════════════ */}
      <BottomSheetV2
        open={showDisputeHistory}
        onClose={() => setShowDisputeHistory(false)}
        title="Lịch sử báo lỗi"
      >
        {MOCK_DISPUTES.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <p style={{ color: c.text3, fontSize: φ.sm, marginTop: 4 }}>
              Tất cả hoa hồng đang hoạt động bình thường
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {MOCK_DISPUTES.map((dispute) => {
              ...
              const typeLabel =
                DISPUTE_TYPES.find((dt) => dt.id === dispute.type)?.label ?? dispute.type;
