...
export function ReferralFriendDetailPage() {
  ...
  const timeline = useMemo(() => (friend ? getFriendTimeline(friend.id) : []), [friend]);
  const commissions = useMemo(() => (friend ? getFriendCommissions(friend.id) : []), [friend]);
    ...
    return (
      <PageLayout>
        <Header
          ...
          back
          onBack={() => navigate(prefix + '/referral/history')}
        />
        <div className="flex-1 flex flex-col items-center justify-center px-8">
          <AlertCircle size={40} color={c.text3} style={{ opacity: 0.4, marginBottom: 12 }} />
          <p style={{ color: c.text2, fontSize: 16, fontWeight: 600, textAlign: 'center' }}>
            Không tìm thấy bạn bè
          </p>
          <p style={{ color: c.text3, fontSize: 13, textAlign: 'center', marginTop: 4 }}>
            ID không hợp lệ hoặc đã bị xóa
          </p>
          <button
        ...
        <PageContent gap="default">
          ...
          >
            <div
              ...
              style={{
                background:
                  'radial-gradient(circle, ' + statusCfg.color + '18 0%, transparent 65%)',
              }}
            />
            ...
            <div className="flex items-center gap-4 mb-4 relative z-10">
              ...
              >
                <span style={{ color: statusCfg.color, fontSize: φ.lg, fontWeight: 700 }}>
                  {friend.avatar}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <p
                    style={{ color: c.portfolioTextDim, fontSize: 18, fontWeight: 700 }}
                    className="truncate"
                  >
                    {friend.name}
                  </p>
            ...
            <div className="grid grid-cols-3 gap-2 relative z-10">
              {[
                {
                  label: 'Hoa hồng',
                  value: totalFromFriend > 0 ? '+' + fmtUsd(totalFromFriend) : '—',
                  color: '#10B981',
                },
                {
                  label: 'Khối lượng',
                  value: friend.totalVolume > 0 ? '$' + friend.totalVolume.toLocaleString() : '—',
                  color: '#3B82F6',
                },
                {
                  label: 'Giao dịch',
                  value: (friend.tradeCount ?? 0).toString(),
                  color: '#F59E0B',
                },
              ].map((s) => (
                <div
                ...
                >
                  ...
                  <p style={{ color: c.portfolioTextMuted, fontSize: φ.xs, marginTop: 2 }}>
                    {s.label}
                  </p>
                </div>
              ))}
          ...
          {friend.status === 'pending_kyc' && (
            ...
            >
              <button
                onClick={() =>
                  actionToast.success(`Đã gửi nhắc nhở — Nhắc ${friend.name} hoàn tất KYC`)
                }
          ...
          {friend.status === 'kyc_done' && (
            ...
            >
              ...
              <p style={{ color: c.text2, fontSize: φ.sm, lineHeight: 1.5 }}>
                {friend.name} đã hoàn tất KYC nhưng chưa giao dịch. Khi bạn bè bắt đầu trade, bạn sẽ
                nhận hoa hồng {currentTier.commission}% phí GD.
              </p>
            </div>
          )}
                  ...
                  {timeline.map((event, idx) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center shrink-0" style={{ width: 36 }}>
                        ...
                        {idx < timeline.length - 1 && (
                          <div
                            ...
                            style={{ background: c.divider, minHeight: 20 }}
                          />
                        )}
                      </div>
                      ...
                      <div className="flex-1 pb-4 min-w-0">
                        ...
                        <p
                          style={{ color: c.text3, fontSize: φ.xs, lineHeight: 1.4, marginTop: 2 }}
                        >
                          {event.description}
                        </p>
                      </div>
              ...
              <SectionHeader
                ...
                right={
                  <span style={{ color: c.text3, fontSize: φ.xs }}>
                    {completedComm.length} giao dịch
                  </span>
                }
              />
              <TrCard overflow>
                {commissions.slice(0, 8).map((record, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        borderBottom:
                          i < Math.min(commissions.length, 8) - 1
                            ? '1px solid ' + c.divider
                            : 'none',
                ...
                {commissions.length > 8 && (
                  ...
                  >
                    <span style={{ color: c.primary, fontSize: φ.sm, fontWeight: 600 }}>
                      Xem tất cả hoa hồng
                    </span>
            ...
            <TrCard className="px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <p style={{ color: c.text2, fontSize: φ.sm, lineHeight: 1.5 }}>
                  ...
                  phí GD của {friend.name}. Hoa hồng cộng real-time vào ví USDT, không giới hạn thời
                  gian.
                </p>
              </div>
            </TrCard>
