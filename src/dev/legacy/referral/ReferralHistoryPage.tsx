...
const STATUS_CONFIG: Record<
  string,
  { label: string; color: string; bg: string; icon: typeof UserCheck }
> = {
...
export function ReferralHistoryPage() {
  ...
  const filteredFriends = REFERRAL_FRIENDS.filter((f) => filter === 'all' || f.status === filter)
  ...
  const filters: { id: FilterType; label: string; count: number }[] = [
    { id: 'all', label: 'Tất cả', count: REFERRAL_FRIENDS.length },
    {
      id: 'active_trader',
      label: 'Đang GD',
      count: REFERRAL_FRIENDS.filter((f) => f.status === 'active_trader').length,
    },
    {
      id: 'kyc_done',
      label: 'Đã KYC',
      count: REFERRAL_FRIENDS.filter((f) => f.status === 'kyc_done').length,
    },
    {
      id: 'pending_kyc',
      label: 'Chờ KYC',
      count: REFERRAL_FRIENDS.filter((f) => f.status === 'pending_kyc').length,
    },
  ];
                ...
                {filteredFriends.map((friend) => {
                  ...
                  return (
                    <TrCard
                      ...
                      as="button"
                      onClick={() => navigate(prefix + '/referral/friend/' + friend.id)}
                    >
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        >
                          <span style={{ color: c.text2, fontSize: φ.base, fontWeight: 700 }}>
                            {friend.avatar}
                          </span>
                        </div>
                        {/* Name + Status */}
                        ...
                        <div className="text-right shrink-0">
                          <p
                            style={{
                              color: friend.totalCommission > 0 ? '#10B981' : c.text3,
                              fontSize: φ.body,
                              fontWeight: 700,
                              fontFamily: 'monospace',
                            }}
