...
import { fmtAmount, fmtUsd, fmtFee } from '@/shared/lib/formatNumber';
...
import {
  ...
  type PriceAlert,
  type TxRecord,
} from '@/dev/legacy/trading/components/ConvertP2Components';
...
const ASSETS = [
  { symbol: 'USDT', name: 'Tether USD', balance: 12450.8, color: '#26A17B' },
  { symbol: 'BTC', name: 'Bitcoin', balance: 0.23451, color: '#F7931A' },
  { symbol: 'ETH', name: 'Ethereum', balance: 3.521, color: '#627EEA' },
          ...
          {filtered.map((asset) => (
            <button
              ...
              style={{
                ...
                background: selected === asset.symbol ? 'rgba(59,130,246,0.08)' : 'transparent',
              }}
            >
              <div
              ...
              >
                <span style={{ color: asset.color, fontSize: FS.micro, fontWeight: 700 }}>
              ...
              <div className="text-right">
                <p style={{ color: c.text2, fontSize: FS.sm, fontFamily: 'monospace' }}>
                  {fmtAmount(asset.balance)}
                </p>
                <p style={{ color: c.text3, fontSize: FS.xs }}>
                  {fmtUsd(asset.balance * PRICES[asset.symbol], { decimals: 0 })}
                </p>
              </div>
              ...
              return (
                <React.Fragment key={`${hop.from}-${hop.to}`}>
                  ...
                  {i === 0 && (
                    ...
                    >
                      <div
                        ...
                        style={{
                          background: (fromData?.color ?? '#666') + '22',
                          border: `2px solid ${fromData?.color ?? '#666'}55`,
                        }}
                      >
                        <span
                          style={{ color: fromData?.color, fontSize: FS.micro, fontWeight: 700 }}
                        >
                          {hop.from.slice(0, 3)}
                        </span>
                      </div>
                      <span style={{ color: c.text1, fontSize: FS.micro, fontWeight: 600 }}>
                        {hop.from}
                      </span>
                    </motion.div>
                  )}
                  ...
                  >
                    <div className="w-full flex items-center">
                      <div
                        ...
                        style={{
                          background: `linear-gradient(90deg, ${fromData?.color ?? '#666'}44, ${toData?.color ?? '#666'}44)`,
                        }}
                      />
                      <motion.div
                        initial={{ scale: 0 }}
                  ...
                  >
                    ...
                    >
                      <span style={{ color: toData?.color, fontSize: FS.micro, fontWeight: 700 }}>
                        {hop.to.slice(0, 3)}
                      </span>
                    </div>
                    <span
                      style={{
                        color: isLast ? c.text1 : c.text2,
                        fontSize: FS.micro,
          ...
          <div className="flex gap-2 mt-3">
            ...
            >
              ...
              >
                {route.hops
                  .map((h) => h.from)
                  .concat(route.hops[route.hops.length - 1].to)
                  .join(' → ')}
              </p>
              <p style={{ color: '#10B981', fontSize: FS.micro }}>
                Phí: {(route.totalFee * 100).toFixed(2)}%
              </p>
            </div>
            <div
              className="flex-1 rounded-xl px-3 py-2"
              style={{ background: c.surface2, opacity: 0.6 }}
            >
...
function ConvertReviewSheet({
  ...
  const rows = [
    {
      label: 'Loại lệnh',
      value: mode === 'market' ? 'Market' : mode === 'limit' ? 'Limit' : 'Scheduled',
    },
    {
      label: 'Tỷ giá',
      value: `1 ${fromAsset} = ${(mode === 'limit' && limitRate ? limitRate : rate).toFixed(6)} ${toAsset}`,
    },
    ...
    { label: 'Tối thiểu nhận', value: `${fmtAmount(minReceive)} ${toAsset}` },
    {
      ...
      color: priceImpact > 0.1 ? '#F59E0B' : '#10B981',
    },
    ...(route.type === 'multi-hop'
      ? [
          {
      ...
      : []),
    ...(route.savings > 0
      ? [{ label: 'Tiết kiệm', value: `${route.savings.toFixed(2)}%`, color: '#10B981' }]
      : []),
  ];
        ...
        >
          ...
          <div className="px-5 pt-2 pb-4 flex items-center justify-between">
            <h3 style={{ color: c.text1, fontSize: FS.lg, fontWeight: 700 }}>
              {mode === 'limit'
                ? 'Xác nhận Limit Convert'
                : mode === 'schedule'
                  ? 'Xác nhận lịch tự động'
                  : 'Xác nhận chuyển đổi'}
            </h3>
            <button onClick={onClose} className="active:scale-95 transition-transform">
              <X size={20} color={c.text2} />
          ...
          {/* Amount summary */}
          <div
            className="mx-5 rounded-2xl p-4 flex flex-col gap-3"
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{
                    background: fromData.color + '22',
                    border: `1.5px solid ${fromData.color}44`,
                  }}
                >
                  <span style={{ color: fromData.color, fontSize: FS.micro, fontWeight: 700 }}>
                    {fromAsset.slice(0, 3)}
                  </span>
                </div>
                <div>
              ...
              <p style={{ color: c.text3, fontSize: FS.xs, fontFamily: 'monospace' }}>
                ≈ {fmtUsd(fromAmount * PRICES[fromAsset])}
              </p>
            </div>
            <div className="flex justify-center">
              <ArrowDownUp size={16} color={c.text3} />
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className="w-9 h-9 rounded-full flex items-center justify-center"
                  style={{
                    background: toData.color + '22',
                    border: `1.5px solid ${toData.color}44`,
                  }}
                >
                  <span style={{ color: toData.color, fontSize: FS.micro, fontWeight: 700 }}>
                    {toAsset.slice(0, 3)}
                ...
                <div>
                  ...
                  >
                    {mode === 'limit' ? '≈ ' : ''}
                    {fmtAmount(toAmount)} {toAsset}
                  </p>
                </div>
              </div>
              <p style={{ color: c.text3, fontSize: FS.xs, fontFamily: 'monospace' }}>
                ≈ {fmtUsd(toAmount * PRICES[toAsset])}
              </p>
            </div>
          </div>
          ...
          <div className="px-5 pt-4 pb-2 flex flex-col gap-3">
            {rows.map((row) => (
              <div key={row.label} className="flex justify-between items-center">
                ...
                <span
                  style={{
                    color: (row as any).color ?? c.text1,
                    fontSize: FS.sm,
                    fontWeight: 600,
                    fontFamily: 'monospace',
                  }}
          ...
          {mode === 'market' && (
            <div
              className="mx-5 mt-2 rounded-xl px-4 py-3 flex items-center gap-2"
              style={{
...
function ConvertReceipt({
  ...
  const subtitle =
    mode === 'limit'
      ? 'Lệnh sẽ thực hiện khi đạt tỷ giá mục tiêu'
      : mode === 'schedule'
        ? 'Hệ thống sẽ tự chuyển đổi theo lịch'
        : timestamp;
    ...
    >
      ...
      >
        ...
        >
          ...
          <h3 style={{ color: c.text1, fontSize: FS.lg, fontWeight: 700, marginBottom: 4 }}>
            {title}
          </h3>
        ...
        <div className="px-5 py-4 flex items-center justify-center gap-3">
          <div className="text-right">
            <p style={{ color: c.text2, fontSize: FS.xs }}>Từ</p>
            <p
              style={{ color: c.text1, fontSize: FS.lg, fontWeight: 700, fontFamily: 'monospace' }}
            >
              {fmtAmount(fromAmount)}
            </p>
        ...
        <div
          className="mx-5 rounded-xl px-4 py-3 flex items-center gap-2"
          style={{ background: c.surface2, border: `1px solid ${c.borderSolid}` }}
        >
          <span style={{ color: c.text3, fontSize: FS.xs, flex: 1 }}>
...
export function ConvertPage() {
  ...
  const effectiveRate =
    mode === 'limit' && parseFloat(limitRate) > 0 ? parseFloat(limitRate) : marketRate;
  ...
  const priceImpact =
    fromAmountNum * fromPrice > 50000 ? 0.12 : fromAmountNum * fromPrice > 10000 ? 0.06 : 0.03;
  ...
  const handleSlippageSelect = (val: number) => {
    setSlippage(val);
    setShowCustomSlippage(false);
    setCustomSlippage('');
  };
  const handleCustomSlippageApply = () => {
    const v = parseFloat(customSlippage);
    if (v > 0 && v <= 50) setSlippage(v);
  };
  const handleSelectFavorite = (pair: { from: string; to: string }) => {
  ...
  const toggleFavorite = () => {
    ...
    setFavorites(
      exists
        ? favorites.filter((f) => !(f.from === fromAsset && f.to === toAsset))
        : [...favorites, { from: fromAsset, to: toAsset }],
    );
  };
  ...
  const activeAlertCount = alerts.filter(
    (a) => a.from === fromAsset && a.to === toAsset && a.active,
  ).length;
  ...
  return (
    <PageLayout>
      ...
      <AnimatePresence>
        {showReview && (
          <ConvertReviewSheet
            ...
            route={smartRoute}
            mode={mode}
            limitRate={parseFloat(limitRate) || undefined}
      ...
      <AnimatePresence>
        {selectedTx && (
          <TxDetailSheet key="tx-detail" tx={selectedTx} onClose={() => setSelectedTx(null)} />
        )}
      </AnimatePresence>
        ...
        >
          ...
          <motion.div variants={fadeUp}>
            <div className="flex items-center justify-between mb-2">
              ...
              >
                <Star
                  size={12}
                  color={isFavorited ? '#F59E0B' : c.text3}
              ...
              {favorites.map((pair) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: isActive ? `${c.primaryAlpha08}` : c.surface2,
          ...
          <motion.div variants={fadeUp}>
            ...
            >
              <RefreshCw
                size={14}
                color={rateCountdown <= 5 ? '#EF4444' : '#3B82F6'}
              ...
              <div className="flex-1">
                {rateLoading ? (
                  ...
                  <InverseRateToggle
                    ...
                    to={toAsset}
                    rate={marketRate}
                    inversed={inverseRate}
                    onToggle={() => setInverseRate(!inverseRate)}
                  />
                )}
              </div>
              <div className="flex items-center gap-2">
          ...
          <motion.div variants={fadeUp}>
            <TrCard rounded="lg" className="p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span style={{ color: c.text3, fontSize: FS.xs }}>
                  ...
                  <span style={{ color: c.text2, fontFamily: 'monospace' }}>
                    {fmtAmount(fromAssetData.balance)} {fromAsset}
                  </span>
                </span>
              </div>
              <div className="flex items-center gap-3 mb-3">
                ...
                >
                  <div
                    ...
                    style={{ background: fromAssetData.color + '22' }}
                  >
                    <span
                      style={{ color: fromAssetData.color, fontSize: FS.micro, fontWeight: 700 }}
                    >
                      {fromAsset.slice(0, 3)}
                    </span>
                  </div>
                  <span style={{ color: c.text1, fontSize: FS.body, fontWeight: 700 }}>
                    {fromAsset}
                  </span>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2">
                  {pctButtons.map((pct) => (
                    <button
                      ...
                      onClick={() =>
                        setFromAmount(
                          ((fromAssetData.balance * pct) / 100).toFixed(
                            fromAsset === 'USDT' ? 2 : 6,
                          ),
                        )
                      }
                ...
                {fromAmountNum > 0 && (
                  <span style={{ color: c.text3, fontSize: FS.xs, fontFamily: 'monospace' }}>
                    ≈ {fmtUsd(fromAmountUsd)}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-3 mt-2">
                <span style={{ color: c.text3, fontSize: FS.xs }}>
                  Min: {fmtUsd(MIN_CONVERT_USD, { decimals: 0 })}
                </span>
                <span style={{ color: c.text3, fontSize: FS.xs }}>
                  Max: {fmtUsd(MAX_CONVERT_USD, { decimals: 0 })}
                </span>
              </div>
            </TrCard>
          </motion.div>
            ...
            <TrCard rounded="lg" className="p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span style={{ color: c.text3, fontSize: FS.xs }}>
                  Số dư:{' '}
                  <span style={{ color: c.text2, fontFamily: 'monospace' }}>
                    {fmtAmount(toAssetData.balance)} {toAsset}
                  </span>
                </span>
              </div>
              <div className="flex items-center gap-3">
                ...
                >
                  <div
                    className="w-7 h-7 rounded-full flex items-center justify-center"
                    style={{ background: toAssetData.color + '22' }}
                  >
                    <span style={{ color: toAssetData.color, fontSize: FS.micro, fontWeight: 700 }}>
                      {toAsset.slice(0, 3)}
                    </span>
                  </div>
                  <span style={{ color: c.text1, fontSize: FS.body, fontWeight: 700 }}>
                ...
                <div className="flex-1 text-right">
                  <p
                    style={{
                      color: fromAmountNum > 0 ? '#10B981' : c.text3,
                      fontSize: FS.input,
                      fontWeight: 700,
                      fontFamily: 'monospace',
                    }}
          ...
          <motion.div variants={fadeUp}>
            <div
              className="flex items-center gap-2 overflow-x-auto -mx-5 px-5"
              style={{ scrollbarWidth: 'none' }}
            >
              <button
                ...
                style={{
                  background: showChart ? `${c.primaryAlpha08}` : c.surface2,
                  border: `1px solid ${showChart ? c.primary : c.borderSolid}`,
                  color: showChart ? c.primary : c.text3,
                  fontSize: FS.xs,
                  fontWeight: 600,
                }}
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 min-h-9 rounded-xl active:scale-95 transition-transform shrink-0"
                style={{
                  background: showOrderBook ? `${c.primaryAlpha08}` : c.surface2,
                  border: `1px solid ${showOrderBook ? c.primary : c.borderSolid}`,
                  color: showOrderBook ? c.primary : c.text3,
                  fontSize: FS.xs,
                  fontWeight: 600,
                }}
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 min-h-9 rounded-xl active:scale-95 transition-transform shrink-0"
                style={{
                  background: showPairInfo ? `${c.primaryAlpha08}` : c.surface2,
                  border: `1px solid ${showPairInfo ? c.primary : c.borderSolid}`,
                  color: showPairInfo ? c.primary : c.text3,
                  fontSize: FS.xs,
                  fontWeight: 600,
                }}
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 min-h-9 rounded-xl active:scale-95 transition-transform shrink-0"
                style={{
                  background: showAdvanced ? `${c.primaryAlpha08}` : c.surface2,
                  border: `1px solid ${showAdvanced ? c.primary : c.borderSolid}`,
                  color: showAdvanced ? c.primary : c.text3,
                  fontSize: FS.xs,
                  fontWeight: 600,
                }}
              >
            ...
            {mode === 'limit' && (
              ...
              >
                <TrCard rounded="lg" className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <div
                      className="flex items-center gap-1 flex-1 rounded-xl px-3"
                      style={{
                        background: c.surface2,
                        border: `1px solid ${c.borderSolid}`,
                        height: 44,
                    ...
                    >
                      ...
                      <input
                        type="number"
                        inputMode="decimal"
                        placeholder={marketRate.toFixed(6)}
                        value={limitRate}
                      ...
                      />
                      <span style={{ color: c.text3, fontSize: FS.xs, marginLeft: 4 }}>
                        {toAsset}
                      </span>
                    </div>
                  </div>
                  ...
                  {parseFloat(limitRate) > 0 && (
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      >
                        ...
                        {(((parseFloat(limitRate) - marketRate) / marketRate) * 100).toFixed(2)}% so
                        với market
                      </span>
                    </div>
                  )}
            ...
            {mode === 'schedule' && (
              ...
              >
                <TrCard rounded="lg" className="p-4">
                  ...
                  <p style={{ color: c.text2, fontSize: FS.xs, marginBottom: 8 }}>
                    Tần suất chuyển đổi
                  </p>
                  ...
                  <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span style={{ color: c.text1, fontSize: FS.xs, fontWeight: 600 }}>
                        Xem trước lịch
                      </span>
                    </div>
                    <div className="flex flex-col gap-2">
                      {[
                        {
                          ...
                          value:
                            fromAmountNum > 0 ? `${fmtAmount(fromAmountNum)} ${fromAsset}` : '—',
                        },
                        {
                          label: 'Tần suất',
                          value: FREQ_OPTIONS.find((f) => f.key === scheduleFrequency)?.label ?? '',
                        },
                        {
                      ...
                      {MOCK_SCHEDULES.map((sc) => (
                        <TrCard key={sc.id} className="px-4 py-3">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span style={{ color: '#10B981', fontSize: FS.xs, fontWeight: 600 }}>
                              {fmtUsd(sc.totalSpent)}
                            </span>
                          </div>
                        </TrCard>
                      ))}
          ...
          <motion.div variants={fadeUp}>
            <TrCard className="px-4 py-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span style={{ color: c.text2, fontSize: FS.xs, fontWeight: 600 }}>
                    {fromAsset}/{toAsset} · 24h
                  </span>
                  {(() => {
                  ...
                  {rateLoading ? (
                    ...
                    <>
                      {[
                        {
                          label: 'Tỷ giá',
                          value: `1 ${fromAsset} = ${effectiveRate.toFixed(6)} ${toAsset}`,
                        },
                        {
                          label: `Phí (${(smartRoute.totalFee * 100).toFixed(2)}%)`,
                          value: fmtFee(feeUsdt),
                        },
                        { label: 'Tối thiểu nhận', value: `${fmtAmount(minReceive)} ${toAsset}` },
                        {
                          label: 'Price Impact',
                        ...
                        ...(smartRoute.savings > 0
                          ? [
                              {
                                label: 'Smart Route tiết kiệm',
                                value: `${smartRoute.savings.toFixed(2)}%`,
                                color: '#8B5CF6',
                              },
                            ]
                          : []),
          ...
          <AnimatePresence>
            {isInsufficientBalance && (
              ...
              >
                ...
                <div>
                  <span style={{ color: '#EF4444', fontSize: FS.sm, fontWeight: 600 }}>
                    Số dư không đủ
                  </span>
                  <p style={{ color: '#EF4444', fontSize: FS.xs, opacity: 0.8 }}>
                    Có: {fmtAmount(fromAssetData.balance)} {fromAsset} · Cần:{' '}
                    {fmtAmount(fromAmountNum)} {fromAsset}
                  </p>
                </div>
              </motion.div>
            )}
            {isBelowMin && (
              ...
              >
                ...
                <span style={{ color: '#F59E0B', fontSize: FS.sm }}>
                  Tối thiểu: {fmtUsd(MIN_CONVERT_USD, { decimals: 0 })}
                </span>
              </motion.div>
            )}
            {isAboveMax && (
              ...
              >
                <AlertCircle size={16} color="#EF4444" />
                <span style={{ color: '#EF4444', fontSize: FS.sm }}>
                  Giới hạn: {fmtUsd(MAX_CONVERT_USD, { decimals: 0 })} / lần
                </span>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            {showAdvanced && (
              ...
              >
                <AdvancedSettingsCard
                  ...
                  onToggleMev={() => setMevProtection(!mevProtection)}
                  deadline={txDeadline}
                  onSetDeadline={setTxDeadline}
                  autoSlippage={autoSlippage}
                  onToggleAutoSlippage={() => setAutoSlippage(!autoSlippage)}
                />
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <motion.div variants={fadeUp}>
            <TrCard rounded="lg" className="p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                >
                  <span style={{ color: c.primary, fontSize: FS.xs, fontWeight: 600 }}>
                    {showCustomSlippage ? 'Đóng' : 'Tùy chỉnh'}
                  </span>
                </button>
              </div>
              <div className="flex gap-2">
                {slippageOptions.map((s) => (
                  <button
                    key={s}
                    onClick={() => handleSlippageSelect(s)}
                    className="px-3 min-h-9 rounded-xl active:scale-95 transition-transform"
                    style={{
                ...
                {!slippageOptions.includes(slippage) && (
                  <div
                    className="px-3 min-h-9 rounded-xl flex items-center"
                    style={{
              ...
              <AnimatePresence>
                {showCustomSlippage && (
                  ...
                  >
                    ...
                    <button
                      onClick={handleCustomSlippageApply}
                      disabled={
                        !customSlippage ||
              ...
              {slippage > 3 && (
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ color: '#F59E0B', fontSize: FS.xs }}>
                    Slippage cao có thể dẫn đến giá không thuận lợi
                  </span>
                </div>
              )}
            </TrCard>
          ...
          <motion.div variants={fadeUp}>
            <button
              ...
              style={{
                fontWeight: 600,
                background: canConvert ? c.primary : c.surface2,
                color: canConvert ? '#fff' : c.text3,
              }}
            >
              {canConvert ? (
                <>
                  {mode === 'market' && (
                    <>
                      <Zap className="w-5 h-5" /> Xem trước & Chuyển đổi
                    </>
                  )}
                  {mode === 'limit' && (
                    <>
                      <Target className="w-5 h-5" /> Đặt lệnh Limit
                    </>
                  )}
                  {mode === 'schedule' && (
                    <>
                      <Calendar className="w-5 h-5" /> Tạo lịch tự động
                    </>
                  )}
                </>
              ) : fromAmountNum <= 0 ? (
          ...
          <motion.div variants={fadeUp}>
            <div className="flex items-center justify-between mb-2 px-1">
              <p style={{ color: c.text1, fontSize: FS.body, fontWeight: 700 }}>
                Giao dịch gần đây
              </p>
              <div className="flex items-center gap-3">
                <button
            ...
            <TrCard overflow>
              ...
              {MOCK_HISTORY.length === 0 && (
                <div className="flex flex-col items-center py-8">
                  ...
                  <p style={{ color: c.text3, fontSize: FS.sm, marginTop: 8 }}>
