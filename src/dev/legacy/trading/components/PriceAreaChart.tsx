...
export function PriceAreaChart({ data, isPositive, timeframe }: PriceAreaChartProps) {
  const c = useThemeColors();
  const points = TIMEFRAME_POINTS[timeframe] ?? 48;
  const chartData = useMemo(
    () =>
      data.slice(-points).map((price, index) => ({
        price,
        time: `${index + 1}`,
      })),
    [data, points],
  ...
  if (chartData.length === 0) {
    return <div style={{ color: c.text3, fontSize: 12 }}>ChÆ°a cÃ³ dá»¯ liá»‡u biá»ƒu Ä‘á»“.</div>;
  }
