...
export function WebAdvancedOrders({
  open,
  onClose,
  livePrice,
  symbol,
  baseAsset,
}: WebAdvancedOrdersProps) {
  ...
  const ocoCalc = useMemo(() => {
    ...
    return {
      ...
      slPnl,
      rr,
      tpDist: ((tp - livePrice) / livePrice) * 100,
      slDist: ((sl - livePrice) / livePrice) * 100,
    };
  ...
  const InputField = ({
    ...
    <div>
      <label
        style={{ color: c.text3, fontSize: 10, fontWeight: 600, display: 'block', marginBottom: 3 }}
      >
        {label}
      </label>
      <div
  ...
  return (
    <div
      className="absolute right-0 top-0 z-30 flex flex-col rounded-lg shadow-xl overflow-hidden"
      style={{
        width: 340,
        maxHeight: 520,
        background: panelBg,
            ...
            {ocoCalc && (
              ...
              >
                <div className="flex justify-between py-0.5">
                  ...
                  >
                    +${Math.abs(ocoCalc.tpPnl).toFixed(2)} ({ocoCalc.tpDist > 0 ? '+' : ''}
                    {ocoCalc.tpDist.toFixed(2)}%)
                  </span>
                </div>
                <div className="flex justify-between py-0.5">
                  ...
                  >
                    -${Math.abs(ocoCalc.slPnl).toFixed(2)} ({ocoCalc.slDist > 0 ? '+' : ''}
                    {ocoCalc.slDist.toFixed(2)}%)
                  </span>
                </div>
                <div
                ...
                >
                  ...
                  >
                    1:{ocoCalc.rr.toFixed(2)}
                  </span>
                </div>
                {ocoCalc.rr < 1 && (
                  <div
                    className="flex items-center gap-1 mt-1 px-1.5 py-1 rounded"
                    style={{
                  ...
                  >
                    ...
                    <span style={{ color: '#F59E0B', fontSize: 9 }}>
                      R/R &lt; 1:1 — rủi ro cao hơn lợi nhuận kỳ vọng
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <span style={{ color: c.text3, fontSize: 8 }}>
                TWAP: Chia lệnh lớn thành nhiều phần nhỏ, thực hiện đều trong khoảng thời gian để
                giảm tác động giá.
              </span>
            </div>

            <InputField
              label={`Tổng khối lượng (${baseAsset})`}
              value={twapTotal}
              onChange={setTwapTotal}
              suffix={baseAsset}
            />
            ...
            <InputField
              label="Biên độ giá tối đa (%)"
              value={twapVariance}
              onChange={setTwapVariance}
              suffix="%"
              placeholder="0.1"
            />
            ...
            {twapCalc && (
              ...
              >
                ...
                <div className="flex justify-between py-0.5">
                  ...
                  <span
                    style={{ color: c.text2, fontSize: 10, fontVariantNumeric: 'tabular-nums' }}
                  >
        ...
        {orderType === 'iceberg' && (
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <span style={{ color: c.text3, fontSize: 8 }}>
                Iceberg: Ẩn tổng khối lượng thực, chỉ hiện một phần trên sổ lệnh. Giảm tác động thị
                trường cho lệnh lớn.
              </span>
            </div>

            <InputField
              label="Giá giới hạn (USDT)"
              value={icePrice}
            ...
            />
            <InputField
              label={`Tổng khối lượng thực (${baseAsset})`}
              value={iceTotal}
              onChange={setIceTotal}
              suffix={baseAsset}
            />
            <InputField
              label={`Khối lượng hiển thị (${baseAsset})`}
              value={iceVisible}
              onChange={setIceVisible}
              suffix={baseAsset}
            />
              ...
              >
                ...
                <div className="mt-2 flex items-end gap-0.5 h-8">
                  <div
                    ...
                    style={{ height: '40%', background: 'rgba(59,130,246,0.3)' }}
                  />
                  <div className="flex-1 rounded-t relative" style={{ height: '100%' }}>
                    <div
                      className="absolute bottom-0 left-0 right-0 rounded-t"
                      style={{
                        height: `${100 - iceCalc.hiddenPct}%`,
