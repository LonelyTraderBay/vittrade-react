...
import { OCOOrderForm } from '@/features/trading';
import {
  PositionDashboard,
  type Position,
} from '@/dev/legacy/trading/components/PositionDashboard';
import { PositionSizingCalculator } from '@/dev/legacy/trading/components/PositionSizingCalculator';
...
export function RiskManagementDemoPage() {
  ...
  const handlePositionClick = (position: Position) => {
    actionToast.info(`${position.symbol} — chi tiết vị thế đang ở chế độ demo.`);
  };

  const handleCalculatorApply = (amount: number) => {
    setShowCalcSheet(false);
    hapticSuccess();
    actionToast.success(`Đã áp dụng — Khối lượng ${amount.toFixed(6)} BTC đã được điền vào form`);
  };
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p style={{ fontSize: FONT_SCALE.xs, color: c.text3, lineHeight: 1.6 }}>
                3 công cụ quản lý rủi ro chuyên nghiệp giúp bảo vệ vốn và tối ưu hóa lợi nhuận. Đây
                là foundation quan trọng nhất cho enterprise trading platform.
              </p>
            </div>
          </div>
        </TrCard>
        ...
        <TrCard className="p-4">
          ...
          <div className="flex flex-col gap-3">
            {[
              {
                icon: '🎯',
                title: 'Quản lý rủi ro tốt hơn',
                desc: 'Không bị over-leverage, bảo vệ tài khoản',
              },
