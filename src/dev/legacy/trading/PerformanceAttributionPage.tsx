...
// Deterministic fallback data. The performance API contract will replace this
// adapter; keeping the fallback stable prevents UI/test behaviour from
// changing between renders or CI runs.
const RETURNS_DATA = Array.from({ length: 30 }, (_, i) => {
  const marketReturn = (Math.sin((i + 1) * 1.7) * 0.5 + 0.2) * 3;
  const alpha = (Math.cos((i + 1) * 1.3) * 0.5 - 0.1) * 2;
...
const DRAWDOWN_DATA = Array.from({ length: 30 }, (_, i) => {
  const equity = 5000 + (650 / 30) * i + Math.sin((i + 1) * 1.9) * 110 - 100;
  const peak = Math.max(...RETURNS_DATA.slice(0, i + 1).map((point) => 5000 + point.cumTotal * 50));
  const drawdown = i === 0 ? 0 : -(2.5 + ((i * 7) % 11) * 0.6 + Math.abs(Math.sin(i)) * 1.5);
...
const MONTE_CARLO_PATHS = Array.from({ length: 50 }, (_, pathIdx) => {
  return Array.from({ length: 30 }, (_, day) => ({
    day: day + 1,
    path: pathIdx,
    value: 5000 * Math.exp((0.13 / 30) * day + Math.sin(pathIdx * 0.8 + day * 0.6) * 0.03),
  }));
});
...
export function PerformanceAttributionPage() {
  ...
  const negativeDrawdowns = DRAWDOWN_DATA.filter((d) => d.drawdown < 0);
  const avgDD =
    negativeDrawdowns.reduce((sum, d) => sum + d.drawdown, 0) /
    Math.max(negativeDrawdowns.length, 1);
