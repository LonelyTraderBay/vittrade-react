...
import {
  ...
  MARGIN_ACCOUNT,
  CRYPTO_PAIRS,
} from '@/dev/mocks/trading-screen-fixtures';
import { fmtUsd, fmtPrice, fmtPct, fmtSignedUsd, fmtAmount } from '@/shared/lib/formatNumber';
import { CTAButton } from '@/shared/ui/CTAButton';
import { TrCard, TrCardStat } from '@/shared/ui/TrCard';
import { FONT_SCALE, FONT_WEIGHT, LETTER_SPACING } from '@/shared/theme/legacyTypography';
import { ICON_SIZE, ICON_STROKE } from '@/shared/theme/icons';
import { ALPHA, OPACITY, withAlpha } from '@/shared/theme/colors';
import {
  MarginLevelAlert,
  PriceComparison,
  MarginRequirement,
  ...
  LeverageWarning,
} from '@/dev/legacy/trading/components/MarginSafetyComponents';
import {
  AppropriatenessTest,
...
} from '@/dev/legacy/trading/components/RegulatoryCompliance';
...
export function MarginTradingPage() {
  ...
  // The `as` keeps the declared union as the expression type so region checks below stay exhaustive
  const userRegion: 'EU' | 'UK' | 'SG' | 'US' | 'OTHER' = 'OTHER' as
    'EU' | 'UK' | 'SG' | 'US' | 'OTHER';
  ...
  /* ─── Margin level color ─── */
  const mlColor =
    MARGIN_ACCOUNT.marginLevel > 150 ? c.buy : MARGIN_ACCOUNT.marginLevel > 100 ? c.warn : c.sell;
  ...
  return (
    <PageLayout>
      ...
      <PageContent gap="default">
        ...
        <ClientCategoryBanner category={userCategory} onRequestUpgrade={() => {}} />
        ...
        <TrCard variant="hero" className="p-5">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <p style={{ color: c.portfolioTextMuted, fontSize: FONT_SCALE.xs, marginBottom: 4 }}>
                Tổng vốn ký quỹ
              </p>
              <p
                style={{
            ...
            >
              ...
              >
                {fmtSignedUsd(totalUnrealizedPnl)}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            <TrCardStat>
              <p style={{ color: c.portfolioTextMuted, fontSize: FONT_SCALE.micro }}>
                Margin đã dùng
              </p>
              <p
                style={{
                  color: '#F59E0B',
                  fontSize: FONT_SCALE.sm + 1,
                  fontWeight: FONT_WEIGHT.bold,
                  fontFamily: 'monospace',
                  marginTop: 2,
                }}
          ...
          <div>
            <div className="flex justify-between mb-2">
              <span style={{ color: c.portfolioTextDim, fontSize: FONT_SCALE.xs }}>
                Margin Level
              </span>
              <span
                style={{
            ...
            <div
              ...
              style={{ background: `rgba(255,255,255,${OPACITY.muted})` }}
            >
              <div
        ...
        {tab === 'trade' && (
          <div className="flex flex-col gap-3">
            ...
            <PriceComparison markPrice={markPrice} lastPrice={lastPrice} indexPrice={indexPrice} />

            {/* ═══ IMR/MMR Disclosure ═══ */}
            {positionSize > 0 && (
              <MarginRequirement leverage={leverage} positionSize={positionSize} mode={mode} />
            )}
            ...
            <TrCard className="p-4">
              ...
              <div className="flex items-end gap-2">
                ...
                <span style={{ color: c.text3, fontSize: FONT_SCALE.xs, marginBottom: 3 }}>
                  USDT
                </span>
              </div>
            </TrCard>
            ...
            <div className="flex rounded-2xl p-1 gap-1" style={{ background: c.surface2 }}>
              <button
                ...
                style={{
                  ...
                  border:
                    side === 'long'
                      ? `1px solid ${withAlpha('#10B981', ALPHA.border)}`
                      : '1px solid transparent',
                  boxShadow:
                    side === 'long' ? `0 2px 8px ${withAlpha('#10B981', ALPHA.muted)}` : 'none',
                }}
              >
                <TrendingUp
                  size={ICON_SIZE.sm}
                  color={side === 'long' ? '#10B981' : c.text3}
                  strokeWidth={ICON_STROKE.standard}
                />
                <span
                  style={{
              ...
              <button
                ...
                style={{
                  ...
                  border:
                    side === 'short'
                      ? `1px solid ${withAlpha('#EF4444', ALPHA.border)}`
                      : '1px solid transparent',
                  boxShadow:
                    side === 'short' ? `0 2px 8px ${withAlpha('#EF4444', ALPHA.muted)}` : 'none',
                }}
              >
                <TrendingDown
                  size={ICON_SIZE.sm}
                  color={side === 'short' ? '#EF4444' : c.text3}
                  strokeWidth={ICON_STROKE.standard}
                />
                <span
                  style={{
            ...
            <TrCard
              as="button"
              hover
              onClick={() => setShowLeverageSheet(!showLeverageSheet)}
              className="p-4 flex items-center justify-between w-full"
            >
              <div className="flex items-center gap-3">
                <div
                  className="w-8 h-8 rounded-lg flex items-center justify-center"
                  style={{ background: withAlpha('#F59E0B', ALPHA.soft) }}
                >
              ...
              <TrCard variant="inner" className="p-4">
                  ...
                  {LEVERAGE_OPTIONS.map((lev) => (
                    <button
                      ...
                      style={{
                        ...
                        boxShadow:
                          leverage === lev
                            ? `0 2px 8px ${withAlpha(c.primary, ALPHA.border)}`
                            : 'none',
                ...
                {leverage >= 20 && (
                  <div
                    ...
                    style={{
                      background: withAlpha('#EF4444', ALPHA.muted),
                      border: `1px solid ${withAlpha('#EF4444', ALPHA.muted)}`,
                    }}
                  >
                    <AlertTriangle
                      size={ICON_SIZE.sm}
                      ...
                      strokeWidth={ICON_STROKE.standard}
                    />
                    <span style={{ color: '#EF4444', fontSize: FONT_SCALE.xs }}>
                      Đòn bẩy cao = rủi ro thanh lý cao
                    </span>
                  </div>
                )}
              </TrCard>
                ...
                <button
                  onClick={() =>
                    setAmount(
                      String(
                        (
                          (MARGIN_ACCOUNT.availableMargin * leverage) /
                          (pair?.price ?? 67543)
                        ).toFixed(6),
                      ),
                    )
            ...
            <TrCard variant="inner" className="px-4 py-3 flex flex-col gap-2">
              {[
                {
                  label: 'Margin khả dụng',
                  value: fmtUsd(MARGIN_ACCOUNT.availableMargin),
                  color: c.text1,
                },
                {
                  ...
                  value:
                    amount && parseFloat(amount) > 0
                      ? fmtPrice(
                          parseFloat(price) *
                            (side === 'long' ? 1 - 0.9 / leverage : 1 + 0.9 / leverage),
                        )
                      : '—',
                  color: '#EF4444',
                },
                {
                  label: 'Phí giao dịch (0.05%)',
                  value:
                    amount && parseFloat(amount) > 0
                      ? fmtUsd(parseFloat(amount || '0') * (pair?.price ?? 0) * 0.0005)
                      : '—',
                  color: c.text2,
                },
              ].map((row) => (
          ...
          <div className="flex flex-col gap-3">
            ...
            {positions.map((pos) => {
              ...
              const accentColor = isProfit
                ? withAlpha('#10B981', ALPHA.muted)
                : withAlpha('#EF4444', ALPHA.muted);
              const accentBorderColor = isProfit
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-x-3 gap-y-3 mb-3">
                    {[
                      ...
                      {
                        label: 'Liq. Price',
                        value: fmtPrice(pos.liquidationPrice),
                        color: '#EF4444',
                      },
                      {
                        label: 'Margin Ratio',
                        value: `${pos.marginRatio}%`,
                        color: pos.marginRatio > 10 ? '#10B981' : '#F59E0B',
                      },
                    ].map((d) => (
                      <div key={d.label}>
                        <p style={{ color: c.text3, fontSize: FONT_SCALE.micro, marginBottom: 2 }}>
