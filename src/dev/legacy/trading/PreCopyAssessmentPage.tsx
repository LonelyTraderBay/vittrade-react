/* eslint-disable @typescript-eslint/no-unused-vars -- legacy demo page quarantined from production routes */
/**
 * ══════════════════════════════════════════════════════════════
...
import { COPY_TRADERS } from '@/dev/mocks/trading-screen-fixtures';
...
export function PreCopyAssessmentPage() {
  ...
  const getRecommendedAllocation = () => {
    if (scorePercent < 40)
      return { percent: 0, reason: 'Không nên copy cho đến khi hiểu rõ hơn về rủi ro' };
    if (scorePercent < 55) return { percent: 5, reason: 'Bắt đầu với số tiền nhỏ để học hỏi' };
    if (scorePercent < 70)
      return { percent: 10, reason: 'Phân bổ vừa phải, phù hợp với risk tolerance' };
          ...
          <div className="flex flex-col gap-5">
            ...
            >
              ...
              <div>
                ...
                <p style={{ color: c.text2, fontSize: 12, lineHeight: 1.5 }}>
                  Theo quy định MiFID II Art. 25.3, chúng tôi cần đánh giá sự phù hợp của Copy
                  Trading với kiến thức, kinh nghiệm và mục tiêu đầu tư của bạn.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ background: c.primaryAlpha12, border: `2px solid ${c.primary}` }}
                >
                ...
                <div className="flex-1">
                  ...
                  <p style={{ color: c.text3, fontSize: 11 }}>
                    ROI: +{provider.totalPnlPct.toFixed(1)}% · Max DD:{' '}
                    {provider.maxDrawdown.toFixed(1)}% · Risk:{' '}
                    <span
                      style={{
                        color:
                ...
                {[
                  {
                    ...
                    desc: '7 câu hỏi về kinh nghiệm & rủi ro',
                    duration: '3 phút',
                  },
                  {
                    step: 2,
                    icon: BookOpen,
                    title: 'Đọc tài liệu',
                    desc: '3 tài liệu về cơ chế & rủi ro',
                    duration: '6 phút',
                  },
                  {
                    step: 3,
                    icon: Target,
                    title: 'Nhận kết quả',
                    desc: 'Đánh giá độ phù hợp & khuyến nghị',
                    duration: '1 phút',
                  },
                ].map((item) => {
            ...
            >
              <div className="flex gap-2 items-start">
                ...
                <div>
                  <p
                    style={{ color: c.warningText, fontSize: 11, fontWeight: 600, marginBottom: 2 }}
                  >
                    Lưu ý quan trọng
                  </p>
                  <ul
                    className="space-y-1"
                    style={{ color: c.warningText, fontSize: 10, lineHeight: 1.4 }}
                  >
                ...
                <div className="space-y-2">
                  {q.options.map((option) => (
                    <button
                      ...
                      style={{
                        background: answers[q.id] === option.value ? c.primaryAlpha08 : c.surface2,
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            borderColor: answers[q.id] === option.value ? c.primary : c.border,
                          }}
                        >
            ...
            >
              <div className="flex gap-2 items-start">
                ...
                <p style={{ color: c.primary, fontSize: 11, lineHeight: 1.4 }}>
                  Bạn cần đọc và hiểu <strong>tất cả 3 tài liệu</strong> trước khi tiếp tục. Mỗi tài
                  liệu sẽ được đánh dấu hoàn thành sau 3 giây.
                </p>
              </div>
            </div>
        ...
        {step === 'results' && (
          <div className="flex flex-col gap-5">
            ...
            <div
              ...
              style={{
                background: appropriateness.color + '15',
                border: `2px solid ${appropriateness.color}`,
              }}
            >
              <div
              ...
              <p style={{ color: c.text2, fontSize: 12 }}>
                Điểm đánh giá:{' '}
                <strong style={{ color: appropriateness.color }}>
                  {score}/{maxScore}
                </strong>{' '}
                  ...
                  <div className="flex gap-2 items-start">
                    ...
                    <div>
                      ...
                      <p style={{ color: c.warningText, fontSize: 11, lineHeight: 1.4 }}>
                        Dựa trên đánh giá, Copy Trading với provider này không phù hợp với kiến thức
                        và kinh nghiệm hiện tại của bạn. Chúng tôi khuyến nghị bạn học thêm về Copy
                        Trading hoặc chọn provider có risk level thấp hơn.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2 items-start">
                  ...
                  <div>
                    ...
                    <p style={{ color: c.primary, fontSize: 10, lineHeight: 1.4 }}>
                      Đây là lần đầu bạn copy. Theo quy định, bạn cần chờ <strong>24 giờ</strong>{' '}
                      trước khi có thể bắt đầu copy. Điều này giúp bạn có thời gian suy nghĩ kỹ về
                      quyết định.
                    </p>
                  </div>
                </div>
            ...
            <span>
              ...
              {step === 'questions' &&
                (allQuestionsAnswered
                  ? 'Tiếp tục'
                  : `Trả lời câu hỏi (${Object.keys(answers).length}/${QUESTIONS.length})`)}
              {step === 'education' &&
                (allDocsCompleted
                  ? 'Xem kết quả'
                  : `Đọc tài liệu (${completedDocs.size}/${EDUCATION_DOCS.length})`)}
              {step === 'results' &&
                (appropriateness.level === 'unsuitable'
                  ? 'Quay lại danh sách'
                  : 'Tiếp tục cấu hình')}
