...
import { ProviderApplicationPage } from '@/dev/legacy/trade/ProviderApplicationPage';
...
describe('ProviderApplicationPage', () => {
  ...
  it('should render intro step with benefits, responsibilities and requirements', () => {
    ...
    expect(screen.getByText(/kiếm performance fee từ những người copy bạn/i)).toBeInTheDocument();
  ...
  it('should block requirements step until all criteria are met', async () => {
    ...
    expect(screen.getByText(/Bạn chưa đáp ứng tất cả các yêu cầu/i)).toBeInTheDocument();
  ...
  it('should require both disclosure and fiduciary consents', async () => {
    ...
    expect(
      screen.getByText(/Conflict of interest \(nếu trade coin mình hold\)/i),
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole('button', { name: 'Tiếp tục' })).toBeDisabled();
    await user.click(
      screen.getByRole('button', { name: /Tôi cam kết công khai tất cả thông tin trên/i }),
    );
    expect(screen.getByRole('button', { name: 'Tiếp tục' })).toBeDisabled();
  ...
  it('should review entered data and require terms acceptance before submit', async () => {
    ...
    const consentButton = screen.getByRole('button', { name: /Tôi đã đọc và đồng ý với/i });
    expect(consentButton).toHaveAttribute('aria-pressed', 'false');
    await user.click(consentButton);
    expect(consentButton).toHaveAttribute('aria-pressed', 'true');
  ...
  it('should submit the application and navigate back to the list', async () => {
    ...
    await waitFor(() => {
      expect(alertSpy).toHaveBeenCalledWith(expect.stringContaining('Đơn đăng ký đã được gửi'));
    });
    await waitFor(() => {
