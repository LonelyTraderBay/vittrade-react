...
import { CopyPerformancePage } from '@/dev/legacy/trading/CopyPerformancePage';
...
describe('CopyPerformancePage', () => {
  ...
  it('should show equity curve section with gap reasons on overview tab', () => {
    ...
    expect(
      screen.getByText(/Slippage: Copy orders thực thi chậm hơn 0\.5-3s/i),
    ).toBeInTheDocument();
  ...
  it('should list the 3 trade comparisons on the trades tab', async () => {
    ...
    expect(screen.getByText(/So sánh từng giao dịch giữa bạn và provider/i)).toBeInTheDocument();
