...
import { SafetyEducationPage } from '@/dev/legacy/trade/SafetyEducationPage';
...
describe('SafetyEducationPage', () => {
  ...
  it('should list all 5 common scam types on the default tab', () => {
    ...
    expect(screen.getByText(/Đọc kỹ guide này trước khi copy bất kỳ ai/i)).toBeInTheDocument();
  ...
  it('should fill and submit the report provider form', async () => {
    ...
    expect(alertSpy).toHaveBeenCalledWith('Report submitted! Team sẽ review trong 24-48h.');

    // False-report warning
    expect(
      screen.getByText(/Reports giả mạo có thể dẫn đến tài khoản của bạn bị khóa/i),
    ).toBeInTheDocument();
