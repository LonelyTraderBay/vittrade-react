...
describe('DisputeResolutionPage', () => {
  ...
  it('should offer 5 complaint types with descriptions and selection state', async () => {
    ...
    expect(misconductBtn).toHaveStyle({ background: 'var(--tr-surface-2)' }); // unselected
    await user.click(misconductBtn);
    await waitFor(() => {
      expect(misconductBtn).toHaveStyle({ background: 'var(--tr-primary-alpha-12)' });
    });
  });
  ...
  it('should show the active case with status, timeline and escalation', async () => {
    ...
    expect(screen.getByText(/my copy filled at \$68,750 \(0\.36% slippage\)/i)).toBeInTheDocument();
