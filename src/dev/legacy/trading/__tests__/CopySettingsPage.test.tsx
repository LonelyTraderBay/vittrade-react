...
describe('CopySettingsPage', () => {
  ...
  it('should show the ratio control only for Fixed mode', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole('button', { name: 'Smart' })).toHaveStyle({
        background: 'var(--tr-primary)',
      });
    });
  ...
  it('should toggle notification preferences and channels', async () => {
    ...
    const rows = ['Trades mới', 'Thay đổi P/L', 'Cảnh báo rủi ro', 'Cập nhật provider'].map(
      (label) => getCard(label),
    );

    expect(
      within(rows[0]).getByText('Thông báo mỗi khi provider mở/đóng lệnh'),
    ).toBeInTheDocument();
    expect(
      within(rows[3]).getByText('Thông báo khi provider thay đổi chiến lược'),
    ).toBeInTheDocument();

    rows.slice(0, 3).forEach((row) => {
      expect(within(row).getByRole('button')).toHaveStyle({ background: 'var(--tr-primary)' });
    });
    expect(within(rows[3]).getByRole('button')).toHaveStyle({ background: 'var(--tr-border)' });

    // Toggle provider updates on
    await user.click(within(rows[3]).getByRole('button'));
    await waitFor(() => {
      expect(within(rows[3]).getByRole('button')).toHaveStyle({ background: 'var(--tr-primary)' });
    });
    ...
    expect(emailBtn).toHaveStyle({ background: 'var(--tr-primary-alpha-08)' });
    expect(pushBtn).toHaveStyle({ background: 'var(--tr-primary-alpha-08)' });

    // Turning off email updates its style
    await user.click(emailBtn);
    await waitFor(() => {
      expect(emailBtn).toHaveStyle({ background: 'var(--tr-surface-2)' });
    });
  });
  ...
  it('should accept emergency contact details', async () => {
    ...
    expect(screen.getByText(/Người liên hệ khẩn cấp sẽ được thông báo/i)).toBeInTheDocument();
  ...
  it('should toggle public portfolio visibility', async () => {
    ...
    expect(toggle).toHaveStyle({ background: 'var(--tr-border)' });

    await user.click(toggle);
    await waitFor(() => {
      expect(toggle).toHaveStyle({ background: 'var(--tr-primary)' });
    });
  });
