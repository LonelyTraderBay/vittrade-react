...
import { CRYPTO_PAIRS } from '@/dev/mocks/trading-screen-fixtures';
...
import { fmtUsd, fmtPct, fmtSignedUsd } from '@/shared/lib/formatNumber';

interface FuturesMetricRow {
  label: string;
  value: string;
  color?: string;
}
...
export function FuturesPage() {
  ...
  const incomingLeverage = (location.state as { newLeverage?: number } | null)?.newLeverage;
  ...
  return (
    <PageLayout>
      {showSuccess && (
        <div
          className="fixed top-24 left-5 right-5 z-50 rounded-2xl px-4 py-3 flex items-center gap-3"
          style={{
            background: c.surface,
            border: `1px solid ${side === 'long' ? '#10B981' : '#EF4444'}`,
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p style={{ color: c.text2, fontSize: 12 }}>
              {pair.symbol} • {leverage}x • {positionSize.toFixed(2)} USDT
            </p>
          </div>
      ...
      <Header variant="custom" breadcrumb>
        ...
        >
          ...
          <button
            onClick={() => navigate(`${prefix}/trade/advanced-chart/${pairId}`)}
        ...
        <div className="px-5 mt-3 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <TrCard key={s.label} className="p-3 text-center">
                <p
                  style={{ color: s.color, fontSize: 20, fontWeight: 700, fontFamily: 'monospace' }}
                >
                  {s.value}
                </p>
          ...
          {orderType === 'limit' && (
            <div
              ...
              onClick={() => {}}
            >
              <label style={{ color: c.text3, fontSize: 12, display: 'block', paddingTop: 8 }}>
                Giá giới hạn (USDT)
              </label>
              <input
                type="number"
          ...
          <div
            ...
            style={{
              background: c.surface,
              border: `1.5px solid ${marginNum > 0 ? (side === 'long' ? 'rgba(16,185,129,0.4)' : 'rgba(239,68,68,0.4)') : c.borderSolid}`,
            }}
          >
            <label style={{ color: c.text3, fontSize: 12, display: 'block', paddingTop: 4 }}>
              Ký quỹ (USDT)
            </label>
          ...
          <div className="flex gap-2">
            {[10, 25, 50, 100].map((pct) => (
              <button
                key={pct}
                onClick={() => setMargin(((5000 * pct) / 100).toString())}
          ...
          <div className="flex gap-2">
            <button
              ...
              style={{
                background: tpEnabled ? 'rgba(16,185,129,0.12)' : c.surface2,
                color: tpEnabled ? '#10B981' : c.text2,
                border: `1px solid ${tpEnabled ? 'rgba(16,185,129,0.3)' : c.borderSolid}`,
              }}
            >
              <Target size={14} />
              Take Profit
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-2 rounded-2xl text-xs font-semibold"
              style={{
                background: slEnabled ? 'rgba(239,68,68,0.1)' : c.surface2,
                color: slEnabled ? '#EF4444' : c.text2,
                border: `1px solid ${slEnabled ? 'rgba(239,68,68,0.2)' : c.borderSolid}`,
              }}
            >
              <Shield size={14} />
              Stop Loss
          ...
          {tpEnabled && (
            ...
            >
              <label style={{ color: '#10B981', fontSize: 12, display: 'block', paddingTop: 4 }}>
                Take Profit (USDT)
              </label>
              <input
                type="number"
          ...
          {slEnabled && (
            ...
            >
              <label style={{ color: '#EF4444', fontSize: 12, display: 'block', paddingTop: 4 }}>
                Stop Loss (USDT)
              </label>
              <input
                type="number"
          ...
          <button
            ...
            style={{
              ...
              boxShadow: canOpen
                ? `0 4px 20px ${side === 'long' ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}`
                : 'none',
              fontSize: 16,
            }}
          >
          ...
          {/* Risk warning */}
          <div
            className="flex items-start gap-2 px-3 py-3 rounded-2xl"
            style={{
          ...
          >
            ...
            <p style={{ color: '#F59E0B', fontSize: 12, lineHeight: 1.5 }}>
              Giao dịch hợp đồng tương lai có rủi ro cao. Bạn có thể mất toàn bộ ký quỹ. Chỉ giao
              dịch số tiền bạn có thể chấp nhận mất.
            </p>
          </div>
        </div>
      )}
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span
                      ...
                      style={{
                        background:
                          pos.side === 'long' ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)',
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {(
                    [
                      ...
                      {
                        label: 'Giá thanh lý',
                        value: `$${pos.liquidPrice.toLocaleString()}`,
                        color: '#EF4444',
                      },
                    ] satisfies FuturesMetricRow[]
                  ).map((r) => (
          ...
          <TrCard className="p-4">
            ...
            {(
              [
                ...
                {
                  label: 'Ký qu đang dùng',
                  value: `$${MOCK_POSITIONS.reduce((s, p) => s + p.margin, 0).toFixed(2)}`,
                },
                {
                  label: 'Ký quỹ khả dụng',
                  value: `$${(5000 - MOCK_POSITIONS.reduce((s, p) => s + p.margin, 0)).toFixed(2)}`,
                },
                {
                  label: 'PnL chưa thực hiện',
                  value: `+$${MOCK_POSITIONS.reduce((s, p) => s + p.pnl, 0).toFixed(2)}`,
                  color: '#10B981',
                },
              ] satisfies FuturesMetricRow[]
            ).map((row) => (
