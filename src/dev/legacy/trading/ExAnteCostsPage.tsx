...
import { fmtUsd, fmtPct } from '@/shared/lib/formatNumber';
...
export function ExAnteCostsPage() {
  ...
  // Calculate totals
  const oneOffCosts = EXAMPLE_COSTS.filter((c) => c.category === 'one-off').reduce(
    (s, c) => s + c.amountEur,
    0,
  );
  const recurringCosts = EXAMPLE_COSTS.filter((c) => c.category === 'recurring').reduce(
    (s, c) => s + c.amountEur,
    0,
  );
  const incidentalCosts = EXAMPLE_COSTS.filter((c) => c.category === 'incidental').reduce(
    (s, c) => s + c.amountEur,
    0,
  );
        ...
        >
          ...
          <div>
            ...
            <p style={{ color: c.info, fontSize: 10, lineHeight: 1.4, opacity: 0.9 }}>
              This document shows all costs you will pay before investing. Required by EU regulation
              for retail clients.
            </p>
          </div>
        </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div
                        ...
                        style={{ background: CATEGORY_CONFIG['one-off'].color }}
                      />
                ...
                <TrCard className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div
                        className="w-3 h-3 rounded"
                        style={{ background: CATEGORY_CONFIG['recurring'].color }}
                      />
                      <span style={{ color: c.text1, fontSize: 13, fontWeight: 600 }}>
                        Recurring Costs
                      </span>
                ...
                <TrCard className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div
                        className="w-3 h-3 rounded"
                        style={{ background: CATEGORY_CONFIG['incidental'].color }}
                      />
                      <span style={{ color: c.text1, fontSize: 13, fontWeight: 600 }}>
                        Incidental Costs
                      </span>
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p style={{ color: c.text3, fontSize: 11, lineHeight: 1.4 }}>
                      How much costs reduce your returns over {holdingPeriod}{' '}
                ...
                <div className="rounded-lg p-3" style={{ background: c.warningBg }}>
                  <div className="flex gap-2">
                    ...
                    <p style={{ color: c.warningText, fontSize: 10, lineHeight: 1.4 }}>
                      Example: If the investment returns 8% per year, after costs you would receive
                      approximately {(8 - riy).toFixed(2)}% per year.
                    </p>
                  </div>
                </div>
              </TrCard>
              ...
              <div className="space-y-2">
                ...
                >
                  <span style={{ color: c.text3, fontSize: 11 }}>
                    Recurring Costs ({holdingPeriod} years)
                  </span>
                  <span style={{ color: c.text1, fontSize: 12, fontWeight: 600 }}>
                ...
                <div
                  className="flex items-center justify-between p-2.5 rounded-lg mt-3"
                  style={{ background: '#EF4444' + '15' }}
                >
                  ...
                  <span style={{ color: '#EF4444', fontSize: 16, fontWeight: 700 }}>
                    ...
                    {(
                      oneOffCosts +
                      recurringCosts * holdingPeriod +
                      incidentalCosts * holdingPeriod
                    ).toLocaleString()}
                  </span>
                </div>
              </div>
