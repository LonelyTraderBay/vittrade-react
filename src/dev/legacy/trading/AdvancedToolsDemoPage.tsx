...
import {
  ...
  type OrderBookLevel,
  type LadderOrder,
} from '@/dev/legacy/trading/components/LadderTrading';
import {
  BulkOperations,
  type BulkOrder,
  type BulkModification,
} from '@/dev/legacy/trading/components/BulkOperations';
import {
  ShortcutsReference,
  ...
  ShortcutToast,
  type ShortcutAction,
} from '@/dev/legacy/trading/components/KeyboardShortcuts';
...
export function AdvancedToolsDemoPage() {
  ...
  const handleBulkModify = (orderIds: string[], modification: BulkModification) => {
    hapticSuccess();
    const details =
      modification.type === 'shift_price'
        ? `price shifted ${modification.shiftPct ?? 0}%`
        : `amount set to ${modification.newAmount ?? 0} BTC`;
    actionToast.success(`Orders Modified — ${orderIds.length} orders: ${details}`);
  };
    ...
    <PageLayout>
      ...
      >
        <LadderTrading
          ...
          onModifyOrder={(id, price) =>
            actionToast.success(`Order Modified — #${id} @ $${price.toLocaleString()}`)
          }
        />
      </BottomSheetV2>
