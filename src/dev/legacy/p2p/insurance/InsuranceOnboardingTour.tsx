...
export function InsuranceOnboardingTour({
  ...
  // This legacy preview uses a fixed sample variant and session-only progress.
  const variant = { id: 'A' as const };
  const [currentStep, setCurrentStep] = useState(0);
  const trackConversion = (_eventName: string) => {};
  ...
  const advanceTourProgress = () => setCurrentStep((stepIndex) => Math.min(stepIndex + 1, 4));
  const retreatTourProgress = () => setCurrentStep((stepIndex) => Math.max(stepIndex - 1, 0));
