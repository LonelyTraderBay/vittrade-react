...
export function P2PAMLScreeningPage() {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'pass':
        return { color: '#10B981', icon: CheckCircle, label: 'Pass' };
      case 'review':
        return { color: '#F59E0B', icon: Clock, label: 'Review' };
      case 'fail':
        return { color: '#EF4444', icon: AlertTriangle, label: 'Fail' };
      default:
        return { color: c.text3, icon: FileText, label: 'N/A' };
    }
  };
          ...
          {AML_STATUS.checks.map((check, i) => {
            ...
            return (
              <div
                key={i}
                className="p-4"
                style={{ borderBottom: isLast ? 'none' : `1px solid ${c.borderSolid}` }}
              >
                <div className="flex items-start gap-3">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                    style={{ background: hexToRgba(config.color, 12) }}
                  >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span
                        ...
                        style={{ background: hexToRgba(config.color, 15), color: config.color }}
                      >
                        {config.label}
                      </span>
                    </div>
        ...
        >
          ...
          <div>
            ...
            <p style={{ color: c.text2, fontSize: 10, lineHeight: 1.5 }}>
              Chúng tôi thực hiện kiểm tra AML định kỳ để tuân thủ quy định chống rửa tiền. Nếu có
              vấn đề, team Compliance sẽ liên hệ.
            </p>
          </div>
        </div>
      </div>
