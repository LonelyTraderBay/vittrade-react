...
import { fmtVnd, fmtCompact } from '@/shared/lib/formatNumber';
...
import { InsuranceOnboardingTour } from '@/dev/legacy/p2p/insurance/InsuranceOnboardingTour';
...
export function P2PInsuranceFundPage() {
  ...
  const [showOnboardingTour, setShowOnboardingTour] = useState(() => {
    ...
    try {
      const seen = browserStorage.session.getItem('p2p_insurance_tour_seen');
      return !seen;
    } catch {
      return true;
    }
  ...
  const handleCloseTour = () => {
    ...
    try {
      browserStorage.session.setItem('p2p_insurance_tour_seen', '1');
    } catch {
