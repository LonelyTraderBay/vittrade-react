        ...
        <TrCard rounded="lg" className="overflow-hidden">
          {settings.map((setting, idx) => (
            <div
              ...
              className="p-4"
              style={{
                borderBottom: idx === settings.length - 1 ? 'none' : `1px solid ${c.borderSolid}`,
              }}
            >
              <div className="mb-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={channel.id}
                    onClick={() => handleToggle(setting.id, channel.id as 'push' | 'email' | 'sms')}
                    className="p-2 rounded-lg flex flex-col items-center gap-1"
                    style={{
                      background: setting.channels[channel.id as keyof typeof setting.channels]
                        ? hexToRgba('#10B981', 12)
                        : c.surface2,
                      border: `1px solid ${setting.channels[channel.id as keyof typeof setting.channels] ? '#10B981' : 'transparent'}`,
                    }}
                  >
                    <channel.icon
                      size={16}
                      color={
                        setting.channels[channel.id as keyof typeof setting.channels]
                          ? '#10B981'
                          : c.text3
                      }
                    />
                    <span
                      style={{
                        color: setting.channels[channel.id as keyof typeof setting.channels]
                          ? '#10B981'
                          : c.text3,
                        fontSize: 10,
                        fontWeight: 600,
