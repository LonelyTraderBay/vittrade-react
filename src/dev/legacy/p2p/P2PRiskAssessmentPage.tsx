...
export function P2PRiskAssessmentPage() {
  ...
  const riskColor =
    RISK_PROFILE.overall === 'low'
      ? '#10B981'
      : RISK_PROFILE.overall === 'medium'
        ? '#F59E0B'
        : '#EF4444';
    ...
    <PageLayout>
      ...
      <div className="px-5 py-4">
        <TrCard
          rounded="lg"
          className="p-5"
          style={{ background: `linear-gradient(135deg, ${riskColor} 0%, ${riskColor}CC 100%)` }}
        >
          <div className="text-center mb-4">
            ...
            <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: φ.xs }}>
              Risk Score: {RISK_PROFILE.score}/100
            </p>
          </div>
        </TrCard>
      </div>
      ...
      <div className="px-5 mb-6">
        ...
        >
          ...
          <p style={{ color: c.text2, fontSize: 10, lineHeight: 1.5 }}>
            Risk score được tính dựa trên KYC level, transaction history, AML screening, và
            behavioral patterns.
          </p>
        </div>
      </div>
        ...
        <TrCard rounded="lg" className="overflow-hidden">
          {RISK_PROFILE.factors.map((factor, idx) => (
            <div
              ...
              style={{
                borderBottom:
                  idx === RISK_PROFILE.factors.length - 1 ? 'none' : `1px solid ${c.borderSolid}`,
              }}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="text-right">
                  <p style={{ color: '#10B981', fontSize: φ.xs, fontWeight: 700 }}>
                    +{factor.score}
                  </p>
                </div>
              </div>
