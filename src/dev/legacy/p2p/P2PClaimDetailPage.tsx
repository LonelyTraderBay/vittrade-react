...
import { fmtVnd } from '@/shared/lib/formatNumber';
...
import { ClaimExpiryWarning } from '@/dev/legacy/p2p/insurance/InsuranceStates';
import { ClaimSLATracker } from '@/dev/legacy/p2p/insurance/ClaimSLATracker';
import { ClaimBenchmarks } from '@/dev/legacy/p2p/insurance/ClaimBenchmarks';
...
const MOCK_CLAIMS_DB: Record<string, ClaimDetail> = {
  ic001: {
    ...
    reviewerNotes: [
      ...
      },
      {
        ...
        content: 'Phê duyệt chi trả 85% theo tier Pro. Merchant sẽ bị cảnh cáo và giảm hạng.',
        timestamp: '2026-02-19 16:00',
      },
      {
        id: 'rn3',
        author: 'Hệ thống',
        role: 'Tự động',
        content: 'Đã chuyển 12.750.000 VND vào ví nội bộ. Mã giao dịch: TXN-INS-20260220-001.',
        timestamp: '2026-02-20 10:00',
      },
    ],
  },
  ic002: {
    ...
    timeline: [
      ...
      },
      {
        ...
        status: 'evidence_added',
        title: 'Bằng chứng bổ sung',
        description: 'Ảnh chụp thông báo chargeback từ ngân hàng',
        timestamp: '2026-02-23 17:00',
        actor: 'Bạn',
      },
      {
        id: 't3',
        status: 'reviewing',
        title: 'Bắt đầu điều tra',
        description: 'Đang xác minh với ngân hàng và buyer',
    ...
    evidence: [
      {
        id: 'e1',
        type: 'screenshot',
        name: 'chargeback_notice.png',
        size: '980 KB',
        uploadedAt: '2026-02-23 17:00',
      },
      {
        id: 'e2',
        type: 'document',
    ...
    reviewerNotes: [
      {
        ...
        author: 'Lê Văn C',
        role: 'Chuyên viên hỗ trợ',
        content: 'Đang liên hệ buyer để xác minh. Chờ phản hồi từ ngân hàng về lệnh chargeback.',
  ...
  ic003: {
    ...
    description:
      'Hệ thống tự động phân xử nhầm cho phía buyer trong khi seller đã cung cấp đầy đủ bằng chứng thanh toán.',
    amount: 50_000_000,
    currency: 'VND',
    status: 'approved',
    ...
    timeline: [
      ...
      },
      {
        ...
        status: 'reviewing',
        title: 'Bắt đầu điều tra',
        description: 'Xem xét log hệ thống dispute',
      ...
      },
      {
        ...
        status: 'approved',
        title: 'Đã chấp thuận',
        description: 'Xác nhận lỗi hệ thống, phê duyệt hoàn tiền 85%',
    ...
    evidence: [
      {
        id: 'e1',
        type: 'screenshot',
        name: 'dispute_result.png',
        size: '750 KB',
        uploadedAt: '2026-02-22 11:25',
      },
    ],
    reviewerNotes: [
      {
        ...
        author: 'Phạm Văn D',
        role: 'Kỹ sư hệ thống',
        content: 'Xác nhận bug trong logic auto-resolve dispute. Đã tạo ticket fix #BUG-4521.',
        timestamp: '2026-02-22 16:00',
      },
      {
        id: 'rn2',
        author: 'Trần Thị B',
        role: 'Trưởng nhóm Claims',
        content: 'Phê duyệt hoàn tiền 85% = 42.500.000 VND. Đang chờ xử lý chi trả.',
        timestamp: '2026-02-23 09:30',
      },
    ],
  },
  ic004: {
    ...
    timeline: [
      ...
      },
      {
        ...
        status: 'reviewing',
        title: 'Bắt đầu điều tra',
        description: 'Xem xét log giao dịch',
      ...
      },
      {
        ...
        title: 'Từ chối',
        description:
          'Giao dịch đã được xử lý thành công sau khi kết nối lại. Coin đã giải phóng đúng hạn.',
    ...
    reviewerNotes: [
      {
        ...
        role: 'Chuyên viên hỗ trợ',
        content:
          'Kiểm tra log cho thấy giao dịch đã hoàn tất lúc 08:42 sau khi reconnect. Coin đã giải phóng cho buyer. Không phát sinh thiệt hại thực tế.',
        timestamp: '2026-02-21 09:00',
      },
    ],
  },
  ic005: {
    ...
    reason: 'fraud',
    description:
      'Merchant cung cấp thông tin thanh toán giả mạo, yêu cầu chuyển khoản ngoài nền tảng.',
    amount: 25_000_000,
    currency: 'VND',
    status: 'pending',
    ...
    evidence: [
      {
        id: 'e1',
        type: 'screenshot',
        name: 'fake_payment_info.png',
        size: '1.1 MB',
        uploadedAt: '2026-02-25 09:18',
      },
      {
        id: 'e2',
        type: 'screenshot',
        name: 'chat_evidence.png',
        size: '900 KB',
        uploadedAt: '2026-02-25 09:20',
      },
    ],
    reviewerNotes: [],
  },
...
export function P2PClaimDetailPage() {
  ...
  const handleSubmitAppeal = () => {
    if (appealReason.trim().length < 20) {
      toast.error('Lý do kháng nghị quá ngắn', {
        description: 'Vui lòng mô tả chi tiết tối thiểu 20 ký tự',
      });
      return;
    }
  ...
  const handleToggleNotifications = () => {
    ...
    hapticSelection();
    toast.success(!notiEnabled ? 'Đã bật thông báo' : 'Đã tắt thông báo', {
      description: !notiEnabled
        ? 'Bạn sẽ nhận thông báo khi có cập nhật'
        : 'Không nhận thông báo cho claim này',
    });
  };
  ...
  const handleCancelClaim = () => {
    ...
    toast.success('Đã hủy yêu cầu', {
      description: 'Bạn có thể gửi lại yêu cầu mới bất kỳ lúc nào',
    });
    navigate(-1);
  };
  ...
  const handleDownloadReceipt = () => {
    ...
    const txnRef =
      claim.reviewerNotes
        .find((n) => n.content.includes('TXN-INS'))
        ?.content.match(/TXN-INS-\d+-\d+/)?.[0] || 'TXN-INS-UNKNOWN';
    ...
    <PageLayout>
      ...
      <div className="px-5 py-4 flex flex-col" style={{ gap: φSpace[5] }}>
        ...
        <TrCard className="p-5" style={{ border: `1.5px solid ${statusCfg.color}25` }}>
          ...
          >
            ...
            {claim.paidAmount && (
              ...
              >
                <span
                  style={{ color: '#10B981', fontSize: φ.sm, fontWeight: 600, lineHeight: 1.5 }}
                >
                  Đã chi trả
                </span>
                <span
        ...
        <TrCard className="p-4">
          ...
          <p style={{ color: c.text1, fontSize: φ.body, lineHeight: 1.6 }}>{claim.description}</p>
        </TrCard>
        ...
        {/* ── Section Content ── */}
        {activeSection === 'timeline' && <TimelineSection timeline={claim.timeline} c={c} />}
        ...
        <div className="flex flex-col gap-3">
          <button
            onClick={() => {
              hapticSelection();
              navigate(`${prefix}/p2p/order/${claim.orderId.replace('P2P-', '')}`);
            }}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span style={{ color: c.text1, fontSize: φ.body, fontWeight: 600, lineHeight: 1.5 }}>
                Xem đơn hàng gốc
              </span>
            </div>
          ...
          <button
            onClick={() => {
              hapticSelection();
              toast.info('Đang mở trung tâm hỗ trợ');
            }}
            className="flex items-center justify-between w-full p-4 rounded-2xl"
            style={{ background: c.surface, border: `1px solid ${c.cardBorder}` }}
          >
          ...
          {claim.status === 'paid' && (
            <CTAButton variant="success" onClick={handleDownloadReceipt}>
      ...
      >
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <p style={{ color: c.text2, fontSize: φ.body, lineHeight: 1.6, textAlign: 'center' }}>
            Bạn sẽ mất toàn bộ bằng chứng đã nộp. Có thể gửi yêu cầu mới trong vòng 7 ngày kể từ
            ngày giao dịch.
          </p>
        </div>
      ...
      >
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <p style={{ color: c.text2, fontSize: φ.body, lineHeight: 1.6, textAlign: 'center' }}>
            Bạn có 14 ngày kể từ ngày từ chối để gửi kháng nghị. Hãy mô tả rõ lý do và bằng chứng
            của bạn.
          </p>
        </div>
...
function TimelineSection({ timeline, c }: TimelineSectionProps) {
  const getEventIcon = (status: string): { icon: React.ElementType; color: string } => {
    switch (status) {
      ...
      case 'evidence_added':
        return { icon: Upload, color: '#8B5CF6' };
      case 'reviewing':
        return { icon: Search, color: '#3B82F6' };
      case 'note_added':
            ...
            <TrCard key={note.id} className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div
                  ...
                  style={{
                    background: isSystem ? 'rgba(16,185,129,0.1)' : 'rgba(59,130,246,0.1)',
                  }}
                >
                  <span
                    style={{
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span
                      style={{ color: c.text1, fontSize: φ.sm, fontWeight: 700, lineHeight: 1.5 }}
                    >
                      {note.author}
                    </span>
                    <span
