...
const getOverallGrade = (
  score: number,
): { grade: string; label: string; color: string; desc: string } => {
  if (score >= 90)
    return {
      grade: 'A+',
      label: 'Xuất sắc',
      color: '#10B981',
      desc: 'Bảo vệ tối đa — đủ điều kiện tier Elite',
    };
  if (score >= 75)
    return { grade: 'A', label: 'Tốt', color: '#3B82F6', desc: 'Bảo vệ mạnh — đang ở tier Pro' };
  if (score >= 60)
    return {
      grade: 'B',
      label: 'Khá',
      color: '#8B5CF6',
      desc: 'Bảo vệ trung bình — cần cải thiện bảo mật',
    };
  if (score >= 40)
    return {
      grade: 'C',
      label: 'Trung bình',
      color: '#F59E0B',
      desc: 'Bảo vệ cơ bản — nhiều yếu tố cần cải thiện',
    };
  return {
    grade: 'D',
    label: 'Yếu',
    color: '#EF4444',
    desc: 'Bảo vệ yếu — cần hoàn tất KYC và bật bảo mật',
  };
};
      ...
      <PageContent>
        <div className="px-5 py-4 flex flex-col" style={{ gap: φSpace[5] }}>
                ...
                {[
                  {
                    ...
                    action: () => {
                      hapticSelection();
                      navigate(`${prefix}/profile/settings`);
                    },
                    color: '#EF4444',
                  },
                  {
                  ...
                  >
                    <div className="flex items-center gap-2.5 flex-1">
                      ...
                      />
                      <span
                        style={{ color: c.text1, fontSize: φ.sm, fontWeight: 500, lineHeight: 1.5 }}
                      >
                        {item.label}
                      </span>
                    </div>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {tier.name}
                            {tier.isCurrent && (
                              <span style={{ fontSize: 10, marginLeft: 6, color: '#3B82F6' }}>
                                ← HIỆN TẠI
                              </span>
                            )}
                          </span>
          ...
          >
            ...
            <p style={{ color: c.text3, fontSize: 11, lineHeight: 1.5 }}>
              Điểm bảo vệ được tính dựa trên nhiều yếu tố và cập nhật hàng ngày. Điểm không ảnh
              hưởng đến khả năng giao dịch, chỉ xác định mức bảo hiểm khi xảy ra sự cố.
            </p>
          </div>
