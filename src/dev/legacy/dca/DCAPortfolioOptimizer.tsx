...
import {
  ...
  type FrontierPoint,
} from '@/dev/legacy/dca/services/DCAPortfolioOptimizerService';
...
function computeDrift(
  ...
  total: number;
  perAsset: { symbol: string; current: number; target: number; diff: number; color: string }[];
} {
  const allSymbols = Array.from(
    new Set([...current.map((a) => a.symbol), ...target.map((a) => a.symbol)]),
  );
  let totalDrift = 0;
  const perAsset = allSymbols.map((symbol) => {
    ...
    return {
      ...
      target: tgtW,
      diff,
      color: cur?.color ?? tgt?.color ?? '#8B95B3',
    };
  });
...
function AllocationComparisonRow({
  symbol,
  currentWeight,
  optimalWeight,
  color,
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1.5">
        {/* Current */}
        <div className="flex items-center gap-2">
          <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10, width: 42, flexShrink: 0 }}>
            Hiện tại
          </span>
          <div
    ...
    <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {subtitle && (
            <span style={{ color: c.text3, fontSize: 11, lineHeight: 1.3 }}>{subtitle}</span>
          )}
        </div>
      </div>
      {trailing}
    ...
    >
      ...
      <div className="p-3.5">
        <div className="flex items-center gap-2 mb-2.5">
          ...
          <span style={{ color: c.text3, fontSize: 10, fontWeight: 600, letterSpacing: 0.2 }}>
            {label}
          </span>
        </div>
        <p
...
function FrontierComparisonCard({
  indices,
  frontier,
}: {
  indices: number[];
  ...
  const selected = indices.map((idx, i) => ({
    ...frontier[idx],
  ...
  const radarData = radarMetrics.map((metric) => {
    ...
    selected.forEach((s) => {
      ...
      switch (metric) {
        case 'Return':
          row[`p${s.idx}`] = (s.return_ / maxReturn) * 100;
          break;
        case 'Sharpe':
          row[`p${s.idx}`] = (s.sharpe / maxSharpe) * 100;
          break;
        case 'Diversity':
          row[`p${s.idx}`] = portfolioOptimizerService.getDiversificationScore(s.allocations);
          break;
        case 'Low Risk':
          row[`p${s.idx}`] = Math.max(0, 100 - (s.risk / maxRisk) * 100 + 30);
          break;
        case 'Stability':
          row[`p${s.idx}`] = Math.max(0, 100 + risk.maxDrawdown * 1.5);
          break;
      }
    });
    return row;
    ...
    <TrCard className="overflow-hidden">
      <div className="p-5 pb-0">
        <SectionHeader
          ...
          title={`So sánh ${selected.length} chiến lược`}
          subtitle="Radar 5 trục + bảng chi tiết"
          color="#3B82F6"
        />
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={radarData} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
            ...
            <PolarAngleAxis
              key="polar-angle"
              dataKey="metric"
              tick={{ fill: ct.axis, fontSize: 10, fontWeight: 600 }}
            />
            {selected.map((s, i) => (
              <Radar
                key={`radar-${s.idx}-${i}`}
                name={`${s.label ?? 'Portfolio'} #${s.idx + 1}`}
                dataKey={`p${s.idx}`}
                stroke={s.color}
                fill={s.color}
                fillOpacity={ct.radarFill}
                strokeWidth={2}
          ...
          <table className="w-full" style={{ minWidth: 280 }}>
              ...
              <tr style={{ background: c.surface2 }}>
                ...
                {selected.map((s, i) => (
                  <th
                    ...
                    style={{ color: s.color, fontSize: 10, fontWeight: 700 }}
                  >
                    {s.label}
                  </th>
                ))}
            ...
            <tbody>
              {[
                ...
                {
                  label: 'Max Drawdown',
                  get: (s: (typeof selected)[0]) =>
                    `${portfolioOptimizerService.getRiskMetrics(s.allocations).maxDrawdown.toFixed(1)}%`,
                },
              ].map((row, rowIdx) => (
                <tr
                  key={`row-${row.label}`}
                  style={{
                    borderTop: `1px solid ${c.divider}22`,
                    background: rowIdx % 2 === 0 ? 'transparent' : `${c.surface2}44`,
                  }}
                >
                  <td
                    className="py-3 px-3"
                    style={{ color: c.text2, fontSize: 11, fontWeight: 500 }}
                  >
                    {row.label}
                  </td>
                  {selected.map((s, i) => (
                    <td
                      key={`td-${s.idx}-${row.label}`}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.4, marginBottom: 8 }}>
            Danh mục đã lệch {drift.toFixed(1)}% so với phân bổ mục tiêu (ngưỡng: {threshold}%). Xem
            xét tái cân bằng.
          </p>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-[10px] flex items-center justify-center hover:opacity-90 transition-opacity active:scale-[0.98]"
            style={{ background: c.surface2 }}
          >
        ...
        <div className="px-5 pb-6">
          ...
          <button
            onClick={() => {
              setEnabled(!enabled);
              toast.success(enabled ? 'Đã tắt drift alert' : 'Đã bật drift alert');
            }}
          ...
          >
            ...
            <div
              ...
              style={{ background: enabled ? '#F59E0B' : c.surface2 }}
            >
              <motion.div
            ...
            {DRIFT_THRESHOLDS.map((t) => (
              <button
                ...
                onClick={() => {
                  ...
                  toast.success(`Ngưỡng drift: ${t.label}`);
                }}
              ...
              >
                <span
                  style={{
                    color: threshold === t.value ? '#F59E0B' : c.text1,
                    fontSize: 14,
                    fontWeight: 700,
                  }}
                >
          ...
          <div className="flex items-start gap-2 p-3 rounded-xl" style={{ background: c.surface2 }}>
            ...
            <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
              Khi tổng drift vượt ngưỡng, bạn sẽ nhận cảnh báo và gợi ý tái cân bằng. Drift được
              tính theo chênh lệch tuyệt đối giữa phân bổ hiện tại và mục tiêu.
            </p>
          </div>
        </div>
      </motion.div>
...
function ExportShareSheet({
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.text(
        `${portfolioLabel} | Sharpe: ${sharpe.toFixed(2)} | Diversification: ${diversScore}/100`,
        margin,
        margin + 14,
      );
      pdf.text(
        `Generated: ${new Date().toLocaleDateString('vi-VN')} ${new Date().toLocaleTimeString('vi-VN')}`,
        margin,
        margin + 19,
      );
  ...
  const handleCopySummary = () => {
    ...
    navigator.clipboard
      .writeText(text)
      .then(() => toast.success('Đã sao chép tóm tắt'))
  ...
  const actions = [
    ...
    {
      icon: Mail,
      label: 'Gửi email advisor',
      desc: 'Mở email với nội dung sẵn',
    ...
    },
    {
      ...
      desc: exporting ? 'Đang xuất...' : 'Tải file phân tích đầy đủ',
          ...
          {actions.map((a) => {
            ...
            return (
              ...
              >
                ...
                >
                  {a.loading ? (
                    <Loader2 size={18} color={a.color} className="animate-spin" />
                  ) : a.done ? (
                    <CheckCircle size={18} color={a.color} />
                  ) : (
...
export default function DCAPortfolioOptimizer() {
  ...
  // Drift computation
  const drift = useMemo(() => computeDrift(CURRENT_PORTFOLIO, optimal.allocations), []);
  ...
  const dcaVsHodl = lastBacktest
    ? ((lastBacktest.dcaValue - lastBacktest.hodlValue) / lastBacktest.hodlValue) * 100
    : 0;
  ...
  const toggleCompare = (idx: number) => {
    setComparedIndices((prev) => {
      ...
      if (prev.length >= 5) {
        toast.error('Tối đa 5 chiến lược');
        return prev;
      }
      return [...prev, idx];
    });
    ...
    <PageLayout>
            ...
            <div className="px-5 pt-5 pb-3">
              <div className="flex items-start justify-between mb-1">
                <div>
                  ...
                  <p style={{ color: '#FFFFFF', fontSize: 20, fontWeight: 800, lineHeight: 1.2 }}>
                    Hiện tại vs Tối ưu
                  </p>
                </div>
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  >
                    ...
                    <span
                      style={{
                        color:
                          diversScore >= 50 ? '#34D399' : diversScore >= 30 ? '#FBBF24' : '#F87171',
                        fontSize: 18,
                        fontWeight: 800,
                        fontVariantNumeric: 'tabular-nums',
                        lineHeight: 1,
              ...
              <div className="grid grid-cols-3">
                ...
                <div className="py-3 px-3 text-center">
                  ...
                  <p
                    style={{
                      color:
                        drift.total >= driftThreshold
                          ? '#F87171'
                          : drift.total >= driftThreshold * 0.6
                            ? '#FBBF24'
                            : '#34D399',
                      fontSize: 18,
                      fontWeight: 800,
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (!compareMode) setComparedIndices([selectedFrontierIdx]);
                      }}
                    ...
                    >
                      ...
                      <span
                        style={{
                          color: compareMode ? '#3B82F6' : c.text3,
                          fontSize: 11,
                          fontWeight: 600,
                        }}
                      >
                  ...
                  <TrCard className="overflow-hidden">
                    <div className="px-5 pt-4 pb-2">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        />
                        <span style={{ color: c.text2, fontSize: 11, fontWeight: 500 }}>
                          Risk-Return Scatter
                        </span>
                      </div>
                      <p style={{ color: c.text3, fontSize: 11, lineHeight: 1.5 }}>
                        {compareMode
                          ? 'Bấm vào các điểm để chọn/bỏ chọn so sánh (tối đa 5).'
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: 0 }}>
                              ...
                              return (
                                ...
                                >
                                  <p style={{ color: c.text1, fontSize: 12, fontWeight: 700 }}>
                                    {d.label} {isCompared ? '(selected)' : ''}
                                  </p>
                                  <p style={{ color: c.text2, fontSize: 11 }}>
                                    Risk: {d.x}% · Return: {d.y}%
                                  </p>
                                  <p style={{ color: '#8B5CF6', fontSize: 11, fontWeight: 600 }}>
                                    Sharpe: {d.sharpe}
                                  </p>
                                </div>
                              );
                          ...
                          <Scatter
                            ...
                            fill="#8B5CF6"
                            onClick={(data: any) => handleFrontierClick(data.idx)}
                            isAnimationActive={false}
                            shape={(props: any) => {
                              const { cx, cy, payload, index } = props;
                              const ptIdx = payload?.idx ?? 0;
                              const isActive = compareMode
                                ? comparedIndices.includes(ptIdx)
                                : ptIdx === selectedFrontierIdx;
                              const compIdx = compareMode ? comparedIndices.indexOf(ptIdx) : -1;
                              const fillColor =
                                compareMode && compIdx >= 0
                                  ? COMPARE_COLORS[compIdx % COMPARE_COLORS.length]
                                  : isActive
                                    ? ct.scatterActive
                                    : ct.scatterInactive;
                              ...
                              return (
                                <circle
                                  key={`scatter-pt-${ptIdx}-${index}`}
                                  cx={cx}
                                  cy={cy}
                                  r={r}
                                  fill={fillColor}
                                  stroke={isActive ? ct.scatterStroke : 'none'}
                    ...
                    {frontier.map((p, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p
                            style={{
                              color: isSelected ? accentColor : c.text1,
                              fontSize: 12,
                              fontWeight: 700,
                            }}
                          >
                  ...
                  {!compareMode && (
                    <>
                      <TrCard className="overflow-hidden">
                        ...
                        <div className="px-5 pt-5 pb-4 flex items-center justify-between">
                          ...
                          >
                            ...
                            >
                              Sharpe {selectedFrontier.sharpe.toFixed(2)}
                            </span>
                          </div>
                        </div>
                        ...
                        >
                          <div className="grid grid-cols-2">
                            <div
                              ...
                              style={{ borderRight: `1px solid ${c.divider}33` }}
                            >
                        ...
                        <div className="px-5 pb-5 flex flex-col gap-3">
                          {selectedFrontier.allocations.map((a, idx) => (
                            ...
                            >
                              ...
                              >
                                <span style={{ color: a.color, fontSize: 10, fontWeight: 700 }}>
                                  {a.symbol}
                                </span>
                              </div>
                      ...
                      {suggestions.length > 0 && (
                        <TrCard className="overflow-hidden">
                          <button
                            className="flex items-center justify-between w-full px-5 pt-5 pb-0"
                            onClick={() => setShowSuggestions(!showSuggestions)}
                          >
                            <SectionHeader
                              icon={Sparkles}
                              title={`Gợi ý tối ưu (${suggestions.length})`}
                              color="#F59E0B"
                            />
                            <motion.div
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <p
                                          style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}
                                        >
                                          {s.reason}
                                        </p>
                                      </div>
                  ...
                  <TrCard className="overflow-hidden">
                    <div className="p-5 pb-0">
                      <SectionHeader
                        ...
                        subtitle="Hệ số tương quan giữa các tài sản"
                        color="#3B82F6"
                      />
                    </div>
                    <div className="px-5 pt-2 pb-4">
                      <p
                        style={{ color: c.text3, fontSize: 11, marginBottom: 16, lineHeight: 1.5 }}
                      >
                        Tương quan càng thấp (xanh) = diversification tốt. Càng cao (đỏ) = di chuyển
                        cùng hướng.
                      </p>
                      <div className="overflow-x-auto -mx-1">
                        <div style={{ minWidth: 300 }}>
                          ...
                          {CORR_ASSETS.map((row, rowIdx) => (
                            ...
                            >
                              <div style={{ width: 40 }}>
                                <span style={{ color: c.text2, fontSize: 10, fontWeight: 700 }}>
                                  {row}
                                </span>
                              </div>
                              {CORR_ASSETS.map((col) => {
                                ...
                                const corr = isDiag
                                  ? 1
                                  : portfolioOptimizerService.getCorrelation(row, col);
                                const bg = isDiag
                                  ? c.surface2
                                  : corr >= 0.7
                                    ? `rgba(239,68,68,${0.08 + corr * 0.18})`
                                    : corr >= 0.4
                                      ? `rgba(245,158,11,${0.06 + corr * 0.14})`
                    ...
                    <div
                      ...
                      style={{ borderTop: `1px solid ${c.divider}22` }}
                    >
                      {[
                        { label: 'Thấp (<0.4)', color: 'rgba(16,185,129,0.3)' },
                        { label: 'TB (0.4–0.7)', color: 'rgba(245,158,11,0.3)' },
                        { label: 'Cao (>0.7)', color: 'rgba(239,68,68,0.3)' },
                      ].map((l) => (
                  ...
                  <TrCard className="overflow-hidden">
                    <div className="p-5">
                      <SectionHeader
                        ...
                        subtitle="Đánh giá mức đa dạng hóa"
                        color="#10B981"
                      />
                      <div className="flex items-center gap-4 mb-5">
                        ...
                        <div className="flex-1">
                          ...
                          <p
                            style={{ color: c.text3, fontSize: 11, lineHeight: 1.5, marginTop: 4 }}
                          >
                            {diversScore >= 50
                              ? 'Tài sản có tương quan thấp, giảm rủi ro tổng thể.'
                              : diversScore >= 30
                                ? 'Xem xét thêm tài sản tương quan thấp.'
                                : 'Tập trung vào tài sản tương quan cao — rủi ro lớn.'}
                          </p>
                        </div>
                      </div>
                      <div
                ...
                <div className="flex flex-col gap-4">
                  <TrCard className="overflow-hidden">
                    <div className="px-5 pt-5 pb-2">
                      <SectionHeader
                        ...
                        subtitle="So sánh chiến lược đầu tư định kỳ"
                        color="#10B981"
                      />
                      <p
                        style={{ color: c.text3, fontSize: 11, marginBottom: 12, lineHeight: 1.5 }}
                      >
                        So sánh chiến lược DCA với mua một lần dựa trên dữ liệu lịch sử.
                      </p>
                    </div>
                            ...
                            content={({ payload, label }) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {payload.map((p: any) => (
                                    <p key={p.dataKey} style={{ color: p.color, fontSize: 11 }}>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      {
                        label: 'DCA Final',
                        value: fmtVND(lastBacktest.dcaValue),
                        color: '#10B981',
                      },
                      {
                        label: 'HODL Final',
                        value: fmtVND(lastBacktest.hodlValue),
                        color: '#F59E0B',
                      },
                      {
                        label: 'DCA trội hơn',
                        value: `${dcaVsHodl >= 0 ? '+' : ''}${dcaVsHodl.toFixed(1)}%`,
                        color: dcaVsHodl >= 0 ? '#8B5CF6' : '#EF4444',
                      },
                    ].map((stat, idx) => (
                      <motion.div
                        key={stat.label}
                ...
                <div className="flex flex-col gap-4">
                  <TrCard className="overflow-hidden">
                    <div className="p-5 pb-0">
                      <SectionHeader
                        ...
                        subtitle={selectedFrontier.label}
                        color="#EF4444"
                      />
                    </div>
                    <div className="px-5 pb-5">
                      <div className="grid grid-cols-2 gap-3">
                        <RiskMetricCard
                          label="Biến động/năm"
                          value={`${riskMetrics.annualizedVolatility.toFixed(1)}%`}
                          color="#F59E0B"
                          description="Dao động giá hàng năm"
                          delay={0}
                        />
                        <RiskMetricCard
                          label="Max Drawdown"
                          ...
                          color="#EF4444"
                          description="Sụt giảm tối đa từ đỉnh"
                          delay={0.05}
                        />
                        <RiskMetricCard
                          label="Sharpe Ratio"
                          ...
                          color="#8B5CF6"
                          description="Return/Risk hiệu chỉnh"
                          delay={0.1}
                        />
                        <RiskMetricCard
                          label="Sortino Ratio"
                          ...
                          color="#3B82F6"
                          description="Chỉ tính downside risk"
                          delay={0.15}
                        />
                        <RiskMetricCard
                          label="VaR 95%"
                          value={`${riskMetrics.valueAtRisk95.toFixed(1)}%`}
                          color="#EC4899"
                          description="Thua lỗ tối đa 95% thời gian"
                          delay={0.2}
                        />
                        <RiskMetricCard
                          label="VaR 99%"
                          value={`${riskMetrics.valueAtRisk99.toFixed(1)}%`}
                          color="#EF4444"
                          description="Thua lỗ tối đa 99% thời gian"
                          delay={0.25}
                        />
                        <RiskMetricCard
                          label="Beta"
                          ...
                          color="#10B981"
                          description="Biến động so với thị trường"
                          delay={0.3}
                        />
                        <RiskMetricCard
                          label="Calmar Ratio"
                  ...
                  <TrCard className="overflow-hidden">
                    <div className="p-5">
                      <SectionHeader
                        ...
                        subtitle="Phân tích rủi ro giá trị"
                        color="#3B82F6"
                      />
                      <div className="flex flex-col gap-3">
                        {[
                          { pct: 95, color: '#EC4899', val: riskMetrics.valueAtRisk95 },
                          { pct: 99, color: '#EF4444', val: riskMetrics.valueAtRisk99 },
                        ].map((item, idx) => (
                          <motion.div
                            key={item.pct}
                          ...
                          >
                            ...
                            >
                              <span style={{ color: item.color, fontSize: 15, fontWeight: 800 }}>
                                {item.pct}
                              </span>
                            </div>
                            <div className="flex-1">
                              ...
                              >
                                Trong {item.pct}% trường hợp, thua lỗ hàng ngày không vượt quá{' '}
                                {Math.abs(item.val).toFixed(1)}%
                              </p>
                            </div>
                          </motion.div>
                        ))}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
                        Các chỉ số dựa trên dữ liệu lịch sử. Hãy luôn đa dạng hóa và chỉ đầu tư số
                        tiền bạn chấp nhận mất.
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <div className="fixed left-0 right-0 flex justify-center px-5 z-10" style={{ bottom: 92 }}>
        <div className="flex gap-3">
          ...
          <button
            onClick={() => setShowDriftSettings(true)}
            className="w-12 h-[48px] rounded-[14px] flex items-center justify-center shrink-0 hover:opacity-90 transition-opacity active:scale-[0.98]"
            style={{ background: c.primary }}
          >
          ...
          <button
            onClick={() => navigate(`${routePrefix}/dca/rebalance/config`)}
      ...
      <AnimatePresence>
        {showExportSheet && (
          <ExportShareSheet
            ...
            portfolioLabel={selectedFrontier.label ?? 'Optimal'}
            sharpe={selectedFrontier.sharpe}
      ...
      <AnimatePresence>
        {showDriftSettings && (
          <DriftSettingsSheet
            onClose={() => setShowDriftSettings(false)}
            threshold={driftThreshold}
            setThreshold={setDriftThreshold}
