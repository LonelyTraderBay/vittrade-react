...
import { DCAOverviewCard, DCAOverviewData } from '@/features/dca/components/DCAOverviewCard';
...
export default function DCAOverviewDemo() {
  ...
  const notifyDemoAction = (message: string) => () => setDemoNotice(message);
  const demoActions = {
    onCreatePlan: notifyDemoAction('Tạo kế hoạch DCA demo.'),
    onPauseAll: notifyDemoAction('Tạm dừng kế hoạch DCA demo.'),
    onViewChart: notifyDemoAction('Mở biểu đồ DCA demo.'),
    onViewHistory: notifyDemoAction('Mở lịch sử DCA demo.'),
  };
  ...
  return (
    <PageLayout>
      ...
      <PageContent gap="loose">
        ...
        >
          <DCAOverviewCard
            ...
            onSparklineTap={notifyDemoAction('Đã chọn biểu đồ giá.')}
            actions={demoActions}
          />
        </Section>
        ...
        >
          <DCAOverviewCard data={lossData} sparklineData={sparklineLoss} actions={demoActions} />
        </Section>
        ...
        >
          <DCAOverviewCard data={errorData} actions={demoActions} />
        </Section>
        ...
        >
          <DCAOverviewCard
            data={noNextData}
            onClick={notifyDemoAction('Đã chọn danh mục không có lịch mua tiếp theo.')}
          />
        </Section>
        ...
        >
          <DCAOverviewCard data={largeData} sparklineData={sparklineProfit} actions={demoActions} />
        </Section>
      ...
      {demoNotice && (
        <p role="status" className="px-5 pb-4 text-sm" style={{ color: c.text2 }}>
          {demoNotice}
        </p>
      )}
