...
} from '@/dev/legacy/dca/services/DCARebalanceService';
...
function CoinBadge({ symbol, size = 36 }: { symbol: string; size?: number }) {
  ...
  return (
    ...
    >
      <span style={{ color, fontSize: size * 0.33, fontWeight: 700 }}>
        {(symbol || '?').slice(0, 3)}
      </span>
    </div>
  );
...
function PreviewSimulationSheet({
  ...
  const totalBuy = activeTrades
    .filter((t) => t.action === 'buy')
    .reduce((s, t) => s + t.tradeAmount, 0);
  const totalSell = activeTrades
        ...
        <div className="px-5 pb-5 overflow-y-auto" style={{ maxHeight: 'calc(85vh - 160px)' }}>
              ...
              return (
                ...
                >
                  ...
                  <div
                    style={{
                      height: 2,
                      background: isHold ? c.divider : isBuy ? '#10B981' : '#EF4444',
                    }}
                  />
                  <div className="p-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      {!isHold && (
                        <div className="text-right">
                          ...
                          >
                            {isBuy ? '+' : '-'}$
                            {Math.abs(trade.tradeAmount).toLocaleString(undefined, {
                      ...
                      {isHold && <span style={{ color: c.text3, fontSize: 12 }}>—</span>}
                    </div>
                  </div>
                </motion.div>
              );
          ...
          >
            ...
            <p style={{ color: '#3B82F6', fontSize: 11, lineHeight: 1.5 }}>
              Đây là mô phỏng dựa trên giá hiện tại. Giá thực tế khi thực hiện giao dịch có thể khác
              do biến động thị trường và slippage.
            </p>
          </div>
        </div>
...
export default function DCARebalanceConfig() {
  ...
  const handleUpdateTarget = useCallback(
    (id: string, field: keyof AllocationTarget, value: any) => {
  ...
  const handleConfirmSave = async () => {
    ...
    const config = rebalanceService.createConfig({
      userId: 'demo-user',
      strategy,
      targets: targets.map(({ id: _id, ...rest }) => rest),
      driftThreshold,
      frequency,
      nextRebalanceDate:
  ...
  return (
    <PageLayout>
      ...
      <PageContent gap="relaxed" grow>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p style={{ color: c.text2, fontSize: 12, lineHeight: 1.5 }}>
                Duy trì tỷ lệ phân bổ tài sản theo mục tiêu. Hệ thống tự động mua/bán khi danh mục
                lệch khỏi ngưỡng.
              </p>
            </div>
          </div>
        ...
        <TrCard className="p-5">
            ...
            trailing={
              <button
                ...
                style={{
                  ...
                  color: '#8B5CF6',
                  opacity: availableCoins.length === 0 ? 0.4 : 1,
                }}
              >
                <Plus size={14} />
                  ...
                  .map((t) => (
                    ...
                    >
                      <div
                        ...
                        style={{ background: getCoinColor(t.symbol) }}
                      />
                      <span style={{ color: c.text1, fontSize: 13, fontWeight: 600, flex: 1 }}>
                        {t.symbol}
                      </span>
                      <span
                        style={{
          ...
          <motion.div
            ...
            style={{
              border: `1px solid ${isValidTotal ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)'}`,
            }}
          >
            <div className="flex items-center gap-2">
              {isValidTotal ? (
        ...
        <div className="flex flex-col gap-3">
          <AnimatePresence mode="popLayout">
            {targets.map((target, idx) => {
              ...
              return (
                <motion.div key={target.id} layout {...cardMotion}>
                  <TrCard className="overflow-hidden">
                    {/* Accent top bar */}
                    <motion.div
                      style={{ height: 3 }}
                      animate={{
                        background: `linear-gradient(90deg, ${coinColor}60, ${coinColor})`,
                      }}
                    ...
                    <div className="p-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="relative">
                            <select
                              value={target.symbol}
                              onChange={(e) =>
                                handleUpdateTarget(target.id, 'symbol', e.target.value)
                              }
                            ...
                            >
                              ...
                              {COIN_OPTIONS.filter(
                                (s) => s === target.symbol || !usedSymbols.has(s),
                              ).map((s) => (
                      ...
                      <div className="mb-3">
                        ...
                        <RangeSlider
                          ...
                          max={100}
                          color={coinColor}
                          onChange={(v) => handleUpdateTarget(target.id, 'targetPercent', v)}
                        />
                      </div>
                          ...
                          <button
                            onClick={() =>
                              handleUpdateTarget(
                                target.id,
                                'tolerance',
                                Math.max(1, (target.tolerance || 5) - 1),
                              )
                            }
                          ...
                          <button
                            onClick={() =>
                              handleUpdateTarget(
                                target.id,
                                'tolerance',
                                Math.min(20, (target.tolerance || 5) + 1),
                              )
                            }
                            className="w-7 h-7 rounded-lg flex items-center justify-center"
                            style={{ background: c.surface, border: `1px solid ${c.divider}` }}
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{ background: isActive ? 'rgba(139,92,246,0.15)' : c.surface2 }}
                  >
                  ...
                  <div
                    ...
                    style={{ borderColor: isActive ? '#8B5CF6' : c.divider }}
                  >
                    {isActive && (
                      <motion.div
                        className="w-2.5 h-2.5 rounded-full"
                        style={{ background: '#8B5CF6' }}
                        layoutId="strategy-dot"
                      />
                    )}
                  </div>
                </motion.button>
              ...
              <TrCard className="p-5">
                ...
                <div className="flex items-center justify-between mb-1 mt-2">
                  ...
                  <span style={{ color: c.text3, fontSize: 11 }}>
                    Trigger khi drift {'>'} {driftThreshold}%
                  </span>
                </div>
                <RangeSlider
        ...
        <div>
          ...
          >
            ...
            <span
              style={{ color: c.text2, fontSize: 13, fontWeight: 600, flex: 1, textAlign: 'left' }}
            >
              Cài đặt nâng cao
            </span>
            <motion.div
                ...
                <TrCard className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-9 h-9 rounded-xl flex items-center justify-center"
                        style={{ background: autoExecute ? 'rgba(16,185,129,0.1)' : c.surface2 }}
                      >
                    ...
                    <button
                      ...
                      className="relative w-[52px] h-[30px] rounded-full transition-all"
                      style={{
                    ...
                    {autoExecute && (
                      ...
                      >
                        ...
                        <p style={{ color: '#B45309', fontSize: 11, lineHeight: 1.4 }}>
                          Hệ thống sẽ tự động thực hiện giao dịch khi danh mục lệch. Bạn có thể tắt
                          bất kỳ lúc nào.
                        </p>
                      </motion.div>
                    )}
      ...
      <div
        className="fixed left-0 right-0 flex flex-col items-center px-5 z-10"
        style={{ bottom: 92 }}
      >
        <div className="flex gap-3">
          {/* Preview button */}
          ...
          >
            ...
            <span style={{ color: 'white', fontSize: 14, fontWeight: 600 }}>Xem trước</span>
          </button>
