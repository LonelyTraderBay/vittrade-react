 ...
 * Rebalance Frequency
 */
export type RebalanceFrequency = 'weekly' | 'biweekly' | 'monthly' | 'quarterly';
...
class DCARebalanceService {
   ...
   */
  analyzeDrift(targets: AllocationTarget[], holdings: PortfolioHolding[]): DriftAnalysis[] {
   ...
   */
  createRebalancePlan(config: RebalanceConfig, holdings: PortfolioHolding[]): RebalancePlan {
    const totalValue = holdings.reduce((sum, h) => sum + h.value, 0);
    const driftAnalysis = this.analyzeDrift(config.targets, holdings);
  ...
  getHistoryStats(configId: string): {
    ...
    const avgTimeBetweenRebalances =
      sortedEntries.length > 1 ? totalTimeDiff / (sortedEntries.length - 1) : 0;
