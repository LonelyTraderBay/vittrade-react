...
const AUDIT_CATEGORIES: AuditCategory[] = [
    ...
    items: [
      {
        ...
        description: 'Mật khẩu mạnh, đủ độ dài và phức tạp',
        status: 'pass',
        statusLabel: 'Tốt',
        weight: 12,
        icon: Lock,
      },
      {
        id: 'password-age',
        title: 'Tuổi mật khẩu',
        description: 'Đổi mật khẩu lần cuối 45 ngày trước',
        status: 'warning',
        statusLabel: 'Nên đổi',
        weight: 8,
        fixRoute: '/w/profile/security/change-password',
        fixLabel: 'Đổi mật khẩu',
        icon: Clock,
      },
      {
        id: '2fa-enabled',
        title: 'Xác thực 2 bước (2FA)',
        description: 'Google Authenticator đã kích hoạt',
        status: 'pass',
        statusLabel: 'Đã bật',
        weight: 20,
        icon: Shield,
      },
      {
        ...
        description: '2 passkey đã đăng ký (MacBook + iPhone)',
        status: 'pass',
        statusLabel: 'Đã bật',
        weight: 10,
        icon: Fingerprint,
      ...
      {
        id: 'sms-2fa',
        title: 'Xác thực SMS backup',
        description: 'SMS backup đã cấu hình (+84 ••• 1234)',
        status: 'pass',
        statusLabel: 'Đã bật',
        weight: 5,
        icon: Smartphone,
  ...
  },
  {
    ...
    items: [
      {
        id: 'anti-phishing',
        title: 'Mã chống giả mạo',
        description: 'Mã anti-phishing đã được cài đặt trong email',
        status: 'pass',
        statusLabel: 'Đã bật',
        weight: 10,
        icon: Mail,
      },
      {
        ...
        description: 'KYC Level 2 đã hoàn thành',
        status: 'pass',
      ...
      },
      {
        ...
        description: '2 thiết bị tin tưởng — nên kiểm tra định kỳ',
        status: 'warning',
        statusLabel: 'Kiểm tra',
        weight: 5,
        fixRoute: '/w/profile/devices',
        fixLabel: 'Xem thiết bị',
        icon: Monitor,
      },
      {
        id: 'login-notifications',
        title: 'Thông báo đăng nhập',
        description: 'Chưa bật thông báo khi đăng nhập từ thiết bị mới',
        status: 'fail',
        statusLabel: 'Chưa bật',
        weight: 5,
        fixRoute: '/w/profile/settings',
        fixLabel: 'Bật ngay',
        icon: Bell,
      },
    ],
    ...
    items: [
      {
        ...
        description: '4 địa chỉ đã whitelist, whitelist đang bật',
        status: 'pass',
        statusLabel: 'Đã bật',
        weight: 15,
        icon: FileKey,
      ...
      {
        id: 'withdrawal-2fa',
        title: '2FA cho rút tiền',
        description: 'Mỗi lệnh rút đều yêu cầu xác nhận 2FA',
        status: 'pass',
        statusLabel: 'Đã bật',
        weight: 10,
        icon: Shield,
      },
      {
        id: 'api-withdrawal',
        title: 'API rút tiền',
        description: 'Không có API key nào có quyền rút tiền',
        status: 'pass',
        statusLabel: 'An toàn',
        weight: 5,
        icon: Key,
    ...
    items: [
      {
        ...
        description: '2 lần đáng ngờ trong 7 ngày qua — cần kiểm tra',
        status: 'warning',
        statusLabel: 'Cần kiểm tra',
        weight: 5,
        fixRoute: '/w/profile/security/login-activity',
        fixLabel: 'Xem chi tiết',
        icon: Globe,
      },
      {
        id: 'api-keys',
        title: 'Quản lý API Keys',
        description: '1 API key đang hoạt động, không có quyền rút tiền',
        status: 'info',
        statusLabel: 'Ổn',
        weight: 3,
        fixRoute: '/w/profile/api',
        fixLabel: 'Kiểm tra',
        icon: Key,
      },
      {
        ...
        description: 'Session timeout: 30 phút không hoạt động',
        status: 'pass',
        statusLabel: 'Đã bật',
        weight: 4,
        icon: Clock,
...
/* ═══ Scoring ═══ */
function calculateScore(categories: AuditCategory[]): {
  score: number;
  maxScore: number;
  items: { pass: number; fail: number; warning: number; total: number };
} {
  let score = 0;
  let maxScore = 0;
  let pass = 0,
  ...
  for (const cat of categories) {
    for (const item of cat.items) {
      ...
      if (item.status === 'pass') {
        ...
        score += Math.round(item.weight * 0.5);
        warning++;
      } else if (item.status === 'info') {
        score += Math.round(item.weight * 0.8);
      } else {
        fail++;
      }
    }
...
function getRiskLevel(score: number): {
  ...
  if (score >= 90)
    return {
      ...
      description: 'Tài khoản được bảo vệ tốt nhất. Tiếp tục duy trì.',
    };
  if (score >= 75)
    return {
      level: 'good',
      ...
      description: 'Tài khoản khá an toàn. Hoàn thành các mục còn lại để đạt mức xuất sắc.',
    };
  if (score >= 55)
    return {
      level: 'moderate',
      ...
      description: 'Một số biện pháp bảo mật quan trọng chưa được bật. Nên xử lý sớm.',
    };
  if (score >= 30)
    return {
      level: 'poor',
      ...
      description: 'Tài khoản có nguy cơ cao. Cần bật 2FA và whitelist ngay lập tức.',
    };
  return {
    level: 'critical',
    label: 'Nguy hiểm',
    ...
    description: 'Tài khoản gần như không có bảo mật. Cần hành động NGAY.',
  };
}
...
export function WebSecurityAuditPage() {
  ...
  const failedItems = AUDIT_CATEGORIES.flatMap((cat) =>
    cat.items.filter((i) => i.status === 'fail'),
  );
  const warningItems = AUDIT_CATEGORIES.flatMap((cat) =>
    cat.items.filter((i) => i.status === 'warning'),
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p style={{ color: c.text3, fontSize: WEB_FONT.xs }}>
              Bảo mật &gt; Kiểm tra bảo mật tài khoản
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col" style={{ gap: 24 }}>
            ...
            <div className="flex items-center" style={{ gap: 32 }}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2" style={{ marginBottom: 8 }}>
                  ...
                  >
                    <span style={{ color: risk.color, fontSize: WEB_FONT.md, fontWeight: 700 }}>
                      {risk.label}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex" style={{ gap: 16 }}>
                  {[
                    ...
                    <div key={s.label} className="flex items-center gap-2">
                      ...
                      <span style={{ color: s.color, fontSize: WEB_FONT.md, fontWeight: 700 }}>
                        {s.value}
                      </span>
          ...
          {(failedItems.length > 0 || warningItems.length > 0) && (
            <div
              style={card({
                borderColor:
                  failedItems.length > 0 ? 'rgba(239,68,68,0.2)' : 'rgba(245,158,11,0.2)',
              })}
            >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div style={{ textAlign: 'left' }}>
                      ...
                      <p style={{ color: c.text3, fontSize: WEB_FONT.xs }}>
                        {catPassed}/{catTotal} đạt yêu cầu
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    {/* Mini progress */}
                    <div
                      style={{ width: 80, height: 4, borderRadius: 2, background: c.borderSolid }}
                    >
                      <div
                        style={{
                          width: `${(catPassed / catTotal) * 100}%`,
                          height: '100%',
                          borderRadius: 2,
                    ...
                    {isExpanded ? (
                      <ChevronRight
                        size={16}
                        color={c.text3}
                        style={{ transform: 'rotate(90deg)', transition: 'transform 0.2s' }}
                      />
                    ) : (
                      ...
                      return (
                        <div key={item.id}>
                          ...
                          <div className="flex items-center" style={{ padding: '12px 0', gap: 14 }}>
                            ...
                            <div
                              className="flex items-center justify-center shrink-0"
                              style={{ width: 32, height: 32, borderRadius: 8, background: sc.bg }}
                            >
              ...
              {[
                {
                  ...
                  desc: 'Nên đổi mật khẩu mỗi 60-90 ngày để giảm rủi ro',
                  priority: 'Trung bình',
                  color: '#F59E0B',
                },
                {
                  title: 'Bật thông báo đăng nhập',
                  desc: 'Nhận email/push khi có đăng nhập mới từ thiết bị lạ',
                  priority: 'Cao',
                  color: '#EF4444',
                },
                {
                  title: 'Kiểm tra thiết bị tin tưởng',
                  desc: 'Xem lại danh sách thiết bị, gỡ bỏ thiết bị không còn sử dụng',
                  priority: 'Thấp',
                  color: '#6B7280',
                },
                {
                  title: 'Sử dụng Passkey cho mọi thiết bị',
                  desc: 'Đăng ký passkey trên tất cả thiết bị thường dùng',
                  priority: 'Thấp',
                  color: '#6B7280',
                },
              ].map((r, i) => (
                <div
                  ...
                  style={{ padding: '10px 14px', borderRadius: 10, background: c.bg }}
                >
                  <div
                    className="shrink-0"
                    style={{
          ...
          <div style={card()}>
            <h3
              style={{ color: c.text1, fontSize: WEB_FONT.md, fontWeight: 600, marginBottom: 14 }}
            >
              Phân tích điểm số
            </h3>
            <div className="flex flex-col" style={{ gap: 10 }}>
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: WEB_FONT.xs, lineHeight: 1.5 }}>
              ...
              <span style={{ color: c.text1, fontWeight: 500 }}>Whitelist (+15pt)</span> có ảnh
              hưởng lớn nhất. Quét định kỳ để đảm bảo tài khoản luôn an toàn.
            </p>
          </div>
        </div>
      </div>
