...
function CopyHash({ text }: { text: string }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ color: copied ? c.success : c.text3, fontSize: 10 }}>
        {copied ? 'Đã chép' : 'Copy'}
      </span>
    </button>
  );
...
export function PendingDepositsPage() {
  const c = useThemeColors();
  const { refresh, lastRefreshedLabel, refreshCount } = useLoadingState({
    initialDelay: 400,
  });
        ...
        prev.map((d) => {
          if (d.status === 'confirming' && d.confirmations < d.requiredConfirmations) {
            ...
            return {
              ...
              status:
                newConf >= d.requiredConfirmations
                  ? ('credited' as const)
                  : ('confirming' as const),
  ...
  const pendingCount = deposits.filter(
    (d) => d.status === 'confirming' || d.status === 'processing',
  ).length;
      ...
      <PageContent gap="default">
                    ...
                    {(dep.status === 'confirming' || dep.status === 'processing') && (
                      <div className="mb-3">
                        ...
                        <div className="flex justify-between mt-1">
                          {Array.from({ length: Math.min(dep.requiredConfirmations, 12) }).map(
                            (_, i) => {
                              const blockIdx =
                                dep.requiredConfirmations > 12
                    ...
                    {dep.status === 'credited' && (
                      ...
                      >
                        ...
                        <span style={{ color: c.success, fontSize: 12, fontWeight: 600 }}>
                          Đã ghi nhận vào ví — {dep.confirmations}/{dep.requiredConfirmations} xác
                          nhận
                        </span>
                      </div>
                    )}
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-1">
                          <span style={{ color: c.primary, fontSize: 11, fontFamily: 'monospace' }}>
                            {dep.txHash}
                          </span>
        ...
        >
          ...
          <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.6 }}>
            Số xác nhận cần thiết phụ thuộc vào mạng blockchain. Nạp dưới mức tối thiểu sẽ không
            được ghi nhận. Liên hệ hỗ trợ nếu giao dịch chưa xuất hiện sau 1 giờ.
          </p>
        </div>
      </PageContent>
    </PageLayout>
