          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-6">
            ...
            <div className="text-center">
              ...
              <p style={{ color: c.text2, fontSize: 14, lineHeight: 1.6 }}>
                Lệnh mua {fmtAmount(cryptoAmount)} {selectedCrypto} đã được đặt. Tiền sẽ vào ví
                trong vài phút.
              </p>
            </div>
            ...
            <TrCard className="w-full p-4 flex flex-col gap-2.5">
              {[
                ...
                {
                  label: 'Nhận về',
                  value: `${cryptoAmount < 1 ? cryptoAmount.toFixed(6) : cryptoAmount.toFixed(4)} ${selectedCrypto}`,
                },
          ...
          >
            <div className="text-center mb-4">
              ...
              <div className="flex items-center justify-center gap-2 mt-1">
                ...
                >
                  <span style={{ color: selectedCryptoData.color, fontSize: 10, fontWeight: 700 }}>
                ...
                >
                  {cryptoAmount < 1 ? cryptoAmount.toFixed(6) : cryptoAmount.toFixed(4)}{' '}
                  {selectedCrypto}
                </p>
              </div>
            </div>
            ...
            {[
              ...
              {
                label: 'Tỷ giá',
                value: `1 ${selectedCrypto} = ${cryptoPrice.toLocaleString('vi-VN')} VND`,
              },
              ...
              >
                <span style={{ color: c.text2, fontSize: 13 }}>{row.label}</span>
                <span
                  style={{ color: (row as any).color ?? c.text1, fontSize: 13, fontWeight: 600 }}
                >
                  {row.value}
                </span>
              </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p style={{ color: c.text2, fontSize: 12, marginTop: 4, lineHeight: 1.5 }}>
                  Sau khi xác nhận, bạn sẽ được chuyển đến app {selectedPaymentData.name} để hoàn
                  tất thanh toán. Lệnh mua sẽ được xử lý tự động.
                </p>
              </div>
            </div>
          </TrCard>

          <CTAButton onClick={handleConfirm} loading={isProcessing} variant="primary">
            {isProcessing
              ? 'Đang xử lý...'
              : `Xác nhận thanh toán ${vndNum.toLocaleString('vi-VN')} VND`}
          </CTAButton>
    ...
    <PageLayout>
      ...
      {showCryptoPicker && (
        <div
          ...
          onClick={() => setShowCryptoPicker(false)}
        >
          <div
          ...
          >
            ...
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setShowCryptoPicker(false)}>
            ...
            {CRYPTO_OPTIONS.map((crypto) => (
              <button
                ...
                style={{
                  background:
                    selectedCrypto === crypto.symbol ? 'rgba(59,130,246,0.08)' : c.surface2,
                  border: `1px solid ${selectedCrypto === crypto.symbol ? '#3B82F6' : c.borderSolid}`,
                }}
              >
                <div
                  className="w-10 h-10 rounded-full flex items-center justify-center"
                ...
                >
                  <span style={{ color: crypto.color, fontSize: 10, fontWeight: 700 }}>
                ...
                <div className="text-right">
                  <p style={{ color: c.text2, fontSize: 12 }}>
                    {CRYPTO_PRICES_VND[crypto.symbol].toLocaleString('vi-VN')} VND
                  </p>
                </div>
      ...
      <PageContent gap="default">
        ...
        <TrCard rounded="lg" className="p-5">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <input
              ...
              placeholder="0"
              value={
                vndAmount ? parseFloat(vndAmount.replace(/,/g, '')).toLocaleString('vi-VN') : ''
              }
              onChange={(e) => {
          ...
          <div className="flex gap-2 flex-wrap mb-4">
            {PRESET_AMOUNTS.map((amt) => (
              <button
                key={amt}
                onClick={() => setVndAmount(amt.toString())}
                className="px-3 py-1.5 rounded-xl text-xs font-semibold"
                style={{
                  background: vndAmount === amt.toString() ? '#3B82F6' : c.surface2,
                  color: vndAmount === amt.toString() ? '#fff' : c.text2,
                  border: `1px solid ${vndAmount === amt.toString() ? '#3B82F6' : c.borderSolid}`,
                }}
              >
          ...
          >
            <div>
              ...
              <p
                style={{ color: '#10B981', fontSize: 16, fontWeight: 700, fontFamily: 'monospace' }}
              >
                {vndNum > 0
                  ? cryptoAmount < 1
            ...
            <button
              onClick={() => setShowCryptoPicker(true)}
            ...
            >
              <div
                ...
                style={{ background: selectedCryptoData.color + '22' }}
              >
                <span style={{ color: selectedCryptoData.color, fontSize: 8, fontWeight: 700 }}>
                  {selectedCrypto.slice(0, 3)}
                </span>
              </div>
          ...
          <div className="flex flex-col gap-2 mb-4">
            {PAYMENT_METHODS.filter((p) => p.type === 'bank').map((pm) => (
              <button
                ...
                className="flex items-center gap-3 p-3 rounded-2xl w-full"
                style={{
                  background: selectedPayment === pm.id ? 'rgba(59,130,246,0.08)' : c.surface,
                  border: `1.5px solid ${selectedPayment === pm.id ? '#3B82F6' : c.borderSolid}`,
                }}
              >
                <div
        ...
        <TrCard className="p-4 flex flex-col gap-2">
          {[
            ...
            <div key={row.label} className="flex justify-between">
              <span style={{ color: c.text2, fontSize: 13 }}>{row.label}</span>
              <span style={{ color: (row as any).color ?? c.text1, fontSize: 13, fontWeight: 600 }}>
                {row.value}
              </span>
            </div>
          ))}
        ...
        {/* Buy button */}
        <CTAButton onClick={() => setStep('confirm')} disabled={!canBuy} variant="success">
          {canBuy ? (
            <div className="contents">
              Mua {selectedCrypto} <ArrowRight size={18} />
            </div>
          ) : (
            'Nhập số tiền mua'
          )}
