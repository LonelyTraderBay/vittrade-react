      ...
      <PageContent gap="default">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div
              ...
              style={{
                ...
                border: `2px solid ${currentTier.color}44`,
              }}
            >
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <TrCard key={s.label} className="p-3 text-center">
              <p
                style={{ color: s.color, fontSize: φ.sm, fontWeight: 700, fontFamily: 'monospace' }}
              >
                {s.value}
              </p>
        ...
        >
          ...
          <p style={{ color: '#D97706', fontSize: 11, lineHeight: 1.6 }}>
            Yêu cầu rút trên <strong>{fmtUsd(10000)}</strong> sẽ cần xem xét thủ công (lên đến 24
            giờ). Rút trên <strong>{fmtUsd(50000)}</strong> cần xác minh video call.
          </p>
        </div>
                    ...
                    <div className="px-4 pb-4">
                      <div className="rounded-xl p-3 mb-3" style={{ background: c.surface2 }}>
                        {[
                          {
                            label: 'Rút/ngày',
                            value: tier.dailyLimit > 0 ? fmtUsd(tier.dailyLimit) : '—',
                          },
                          {
                            label: 'Rút/tháng',
                            value: tier.monthlyLimit > 0 ? fmtUsd(tier.monthlyLimit) : '—',
                          },
                          {
                            label: 'Giao dịch đơn',
                            value: tier.singleTxLimit > 0 ? fmtUsd(tier.singleTxLimit) : '—',
                          },
                        ].map((row) => (
                          <div
                            key={row.label}
                      ...
                      <div className="flex flex-col gap-1 mb-3">
                        {tier.requirements.map((req) => (
                          <div key={req} className="flex items-center gap-2">
                            ...
                            <span style={{ color: !isLocked ? c.text2 : c.text3, fontSize: 11 }}>
                              {req}
                            </span>
                          </div>
                        ))}
        ...
        <TrCard className="p-4">
          ...
          {[
            {
              q: 'Hạn mức reset khi nào?',
              a: 'Hạn mức ngày reset lúc 00:00 UTC. Hạn mức tháng reset ngày 1 hàng tháng.',
            },
            {
              q: 'Rút vượt hạn mức thì sao?',
              a: 'Yêu cầu rút sẽ bị từ chối. Bạn có thể nâng cấp KYC hoặc chờ hạn mức reset.',
            },
            {
              q: 'Phí rút có tính vào hạn mức?',
              a: 'Không. Hạn mức chỉ tính số tiền thực rút, không bao gồm phí mạng.',
            },
          ].map((faq, i) => (
            <div
              key={i}
