...
function StatusIcon({ status }: { status: 'fail' | 'warn' | 'pass' }) {
  ...
  if (status === 'pass') {
    return <CheckCircle2 className="size-4" style={{ color: c.success }} />;
  }
  if (status === 'warn') {
    return (
      <AlertCircle
  ...
  return <XCircle className="size-4" style={{ color: c.danger }} />;
}
...
export function IssueComparisonTable() {
  ...
  return (
    <div className="rounded-2xl p-5" style={{ backgroundColor: c.surface }}>
      <h3 className="text-base font-bold mb-4" style={{ color: c.text }}>
        📊 Compliance Comparison Matrix
      </h3>

      {/* Score Summary */}
      <div className="grid grid-cols-4 gap-2 mb-4">
        <div className="rounded-xl p-3 text-center" style={{ backgroundColor: c.danger + '15' }}>
          <div className="text-xs font-medium mb-1" style={{ color: c.textSec }}>
            Original
          </div>
          <div className="text-2xl font-bold" style={{ color: c.danger }}>
            {originalScore}
          </div>
        ...
        >
          ...
          <div className="text-2xl font-bold" style={{ color: c.success }}>
            {variantAScore}
          </div>
          <div className="text-[10px]" style={{ color: c.textTer }}>
            /100 ★
        ...
        <div className="rounded-xl p-3 text-center" style={{ backgroundColor: c.buyAlpha10 }}>
          <div className="text-xs font-medium mb-1" style={{ color: c.textSec }}>
            Variant B
          </div>
          <div className="text-2xl font-bold" style={{ color: c.success }}>
        ...
        <div className="rounded-xl p-3 text-center" style={{ backgroundColor: '#F59E0B15' }}>
          <div className="text-xs font-medium mb-1" style={{ color: c.textSec }}>
            Variant C
          </div>
          ...
          <tbody>
            {ISSUES.map((issue, idx) => (
              <tr key={idx} className="border-t" style={{ borderColor: c.border }}>
                <td className="py-3">
                  <div className="text-sm font-medium mb-0.5" style={{ color: c.text }}>
                    {issue.category}
                  </div>
      ...
      >
        <div className="flex items-center gap-1.5">
          <CheckCircle2 className="size-3.5" style={{ color: c.success }} />
        ...
        <div className="flex items-center gap-1.5">
          <XCircle className="size-3.5" style={{ color: c.danger }} />
          <span className="text-xs" style={{ color: c.textSec }}>
            Non-compliant
          </span>
        </div>
