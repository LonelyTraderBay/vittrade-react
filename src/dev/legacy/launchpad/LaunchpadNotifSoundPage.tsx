...
import {
  ...
  saveNotifSoundSettings,
  SOUND_TYPES,
  type NotifSoundSettings,
  type NotifSoundCategory,
...
export function LaunchpadNotifSoundPage() {
  ...
  const updateSettings = useCallback(
    (updater: (prev: NotifSoundSettings) => NotifSoundSettings) => {
      setSettings((prev) => {
  ...
  const setCategorySound = (catId: string, soundType: NotifSoundCategory['soundType']) => {
    updateSettings((s) => ({
      ...s,
      categories: s.categories.map((cat) => (cat.id === catId ? { ...cat, soundType } : cat)),
    }));
  };
  ...
  const setCategoryVolume = (catId: string, volume: number) => {
    updateSettings((s) => ({
      ...s,
      categories: s.categories.map((cat) => (cat.id === catId ? { ...cat, volume } : cat)),
    }));
  };
      ...
      <PageContent gap="default">
            ...
            {settings.masterEnabled && (
              <div className="mt-2">
                ...
                <input
                  ...
                  style={{
                    background: `linear-gradient(to right, #6366F1 ${settings.masterVolume}%, rgba(255,255,255,0.15) ${settings.masterVolume}%)`,
                  }}
                />
              </div>
            )}
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-0" style={{ borderTop: `1px solid ${c.border}` }}>
                      ...
                      <div className="mt-3 mb-3">
                        <p style={{ color: c.text3, fontSize: 10, marginBottom: 6 }}>
                          Kiểu âm thanh
                        </p>
                          ...
                          {SOUND_TYPES.map((st) => (
                            <button
                              ...
                              onClick={() =>
                                setCategorySound(
                                  cat.id,
                                  st.value as NotifSoundCategory['soundType'],
                                )
                              }
                              className="px-3 py-1.5 rounded-lg"
                              style={{
                                background:
                                  cat.soundType === st.value ? cat.color + '15' : c.surface2,
                                border: `1px solid ${cat.soundType === st.value ? cat.color + '40' : 'transparent'}`,
                                color: cat.soundType === st.value ? cat.color : c.text3,
                                fontSize: 11,
                                fontWeight: cat.soundType === st.value ? 600 : 400,
                              }}
                            >
                              {st.label}
                            </button>
                      ...
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          className="w-full h-1.5 rounded-full appearance-none cursor-pointer"
                          style={{
                            background: `linear-gradient(to right, ${cat.color} ${cat.volume}%, ${c.surface2} ${cat.volume}%)`,
                          }}
                        />
                      </div>
                      ...
                      >
                        ...
                        <span
                          style={{
                            color: isPlaying ? '#10B981' : c.text2,
                            fontSize: 11,
                            fontWeight: 500,
                          }}
                        >
        ...
        >
          ...
          <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
            Âm thanh chỉ phát khi ứng dụng đang hoạt động. Thông báo push ngoài app sử dụng cài đặt
            hệ thống thiết bị.
          </p>
        </div>
...
function ToggleSwitch({
  ...
  return (
    ...
    >
      <div
        className="absolute top-0.5 rounded-full bg-white transition-transform duration-200 shadow"
