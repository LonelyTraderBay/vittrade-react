...
import {
  MOCK_VESTING_SCHEDULES,
  calculateVestingStats,
...
> = {
  linear: {
    ...
    desc: 'Mở khóa đều theo tháng',
  },
  cliff: {
    label: 'Cliff',
    color: '#F59E0B',
    bg: 'rgba(245,158,11,0.12)',
    desc: 'Khóa đến cliff date',
  },
  milestone: {
    label: 'Milestone',
    color: '#10B981',
    bg: 'rgba(16,185,129,0.12)',
    desc: 'Mở khóa theo milestone',
  },
  custom: {
    label: 'Custom',
    color: '#8B5CF6',
    bg: 'rgba(139,92,246,0.12)',
    desc: 'Lịch riêng custom',
  },
};
    ...
    <PageLayout>
      ...
      <PageContent gap="default">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p style={{ color: c.text1, fontSize: 11, fontWeight: 600 }}>
                    Next unlock: {stats.nextUnlock.date}
                  </p>
                  <p style={{ color: c.text3, fontSize: 10 }}>
                    ~{stats.nextUnlock.amount} tokens · {stats.nextUnlock.projects} projects
                  </p>
                </div>
              </div>
            )}
        ...
        <TabBar
          ...
          active={TABS.find((t) => t.key === tab)?.label || TABS[0].label}
        ...
        {tab === 'schedules' && (
          <SchedulesTab schedules={schedules} onSelect={setSelectedSchedule} />
        )}
...
function TimelineTab({ schedules }: { schedules: VestingSchedule[] }) {
  ...
  const allUnlocks = activeSchedules
    .flatMap((schedule) => schedule.unlocks.map((unlock) => ({ schedule, unlock })))
            ...
            <div className="flex flex-col items-center">
              ...
              {idx < allUnlocks.length - 1 && (
                <div
                  ...
                  style={{ background: c.borderSolid, minHeight: 24 }}
                />
              )}
            </div>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{
                        background: schedule.projectLogoColor + '22',
                        color: schedule.projectLogoColor,
                      }}
                    >
                      {schedule.projectLogo}
                    </div>
                    <span style={{ color: c.text1, fontSize: 13, fontWeight: 600 }}>
                      {schedule.projectSymbol}
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p style={{ color: c.text3, fontSize: 10 }}>
                      {unlock.percentage}% · {unlock.milestone || 'Scheduled unlock'}
                    </p>
                  </div>
                  {unlock.claimedAt && (
                    <span
      ...
      <button className="w-full text-left p-4" onClick={() => onSelect(schedule)}>
        ...
        <div className="flex items-start gap-3 mb-3">
          <div
            ...
            style={{
              background: schedule.projectLogoColor + '22',
              border: `2px solid ${schedule.projectLogoColor}44`,
              color: schedule.projectLogoColor,
            }}
          >
            {schedule.projectLogo}
        ...
        <div className="mb-3">
          <div className="flex justify-between mb-1">
            ...
            <span
              style={{
                color: schedule.vestingProgress === 100 ? '#10B981' : c.text1,
                fontSize: 11,
                fontWeight: 600,
              }}
            >
        ...
        {schedule.status === 'active' && schedule.nextUnlockDate !== '—' && (
          ...
          >
            ...
            <span style={{ color: c.text2, fontSize: 11 }}>
              Next unlock: <strong>{schedule.nextUnlockDate}</strong> ·{' '}
              {schedule.nextUnlockAmount.toLocaleString()} tokens
            </span>
          </div>
        )}
      </button>
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <PageSection label="Contract Details" accentColor={schedule.projectLogoColor}>
            <div className="rounded-2xl p-3 flex flex-col gap-2" style={{ background: c.surface2 }}>
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => onCopy(schedule.tokenAddress, 'token')}
            ...
            <div className="flex flex-col gap-2">
              {schedule.unlocks.map((unlock, idx) => (
                <UnlockRow
                  ...
                  unlock={unlock}
                  schedule={schedule}
                  isLast={idx === schedule.unlocks.length - 1}
                />
              ))}
            </div>
          </PageSection>
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
              {scheduleType.desc}. Vesting schedule bắt đầu {schedule.startDate} và kết thúc{' '}
              {schedule.endDate}.
            </p>
          </div>
        </div>
  ...
  return (
    <div className="flex gap-2">
      ...
      <div className="flex flex-col items-center pt-1">
        ...
        {!isLast && (
          <div className="w-0.5 flex-1 mt-1" style={{ background: c.borderSolid, minHeight: 16 }} />
        )}
      </div>
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <span style={{ color: c.text3, fontSize: 11 }}>
              {unlock.date} · {unlock.percentage}%
            </span>
            {unlock.milestone && (
              <span style={{ color: c.text2, fontSize: 10, fontStyle: 'italic' }}>
                {unlock.milestone}
              </span>
            )}
          </div>
