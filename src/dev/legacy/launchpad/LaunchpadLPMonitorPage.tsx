...
export function LaunchpadLPMonitorPage() {
  ...
  const totalLiquidityUSD = pools.reduce(
    (sum, p) => sum + parseFloat(p.liquidityUSD.replace(/[$,]/g, '')),
    0,
  );
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: 'Pools', value: pools.length, color: '#3B82F6' },
                {
                  ...
                  value: '$' + (totalLiquidityUSD / 1000000).toFixed(2) + 'M',
                  color: '#10B981',
                },
              ].map((s) => (
                <div
    ...
    <TrCard overflow hover>
      <button className="w-full text-left p-4" onClick={() => onSelect(pool)}>
        ...
        <div className="flex gap-2 mb-3">
          <div
            className="flex-1 flex items-center gap-1.5 px-2 py-1.5 rounded-xl"
            style={{
              background:
                pool.priceChange24h >= 0 ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)',
            }}
          >
          ...
          <div
            ...
            style={{
              background:
                pool.liquidityChange24h >= 0 ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)',
            }}
          >
            ...
            <span
              style={{
                color: pool.liquidityChange24h >= 0 ? '#10B981' : '#EF4444',
                fontSize: 11,
                fontWeight: 600,
              }}
            >
              {pool.liquidityChange24h > 0 ? '+' : ''}
              {pool.liquidityChange24h}% liq
            </span>
          </div>
        </div>
        ...
        >
          ...
          <span style={{ color: c.text2, fontSize: 11 }}>
            <strong>{pool.lockedLiquidity}%</strong> locked{' '}
            {pool.lockedUntil && `until ${pool.lockedUntil}`}
          </span>
        </div>
            ...
            {pool.alerts.map((alert) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: alertConfig.color + '08',
                    border: `1px solid ${alertConfig.color}22`,
                  }}
                >
                  <alertConfig.icon
                    size={12}
...
function PoolDetailSheet({ pool, onClose }: { pool: LiquidityPool; onClose: () => void }) {
  const c = useThemeColors();
