...
import {
  ...
  type SwapRoute,
  type BridgeTxRecord,
} from '@/dev/mocks/launchpad-fixtures';
...
export function LaunchpadIDOBridgePage() {
  ...
  const targetChain = project
    ? BRIDGE_NETWORKS.find((n) => n.name.toLowerCase().includes(project.chain.toLowerCase())) ||
      BRIDGE_NETWORKS[2]
    : BRIDGE_NETWORKS[2];
          ...
          <>
            ...
            <TrCard className="p-4">
              ...
              >
                ...
                <div className="flex-1 text-left">
                  ...
                  <p style={{ color: c.text3, fontSize: 11 }}>
                    Gas: {sourceChain.gasEstimate} · {sourceChain.avgTime}
                  </p>
                </div>
              ...
              <div
                ...
                style={{
                  background: `${targetChain.color}08`,
                  border: `1px solid ${targetChain.color}20`,
                }}
              >
                <div
                  className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold"
            ...
            <TrCard className="p-4">
              ...
              >
                ...
                <div className="flex items-center gap-2 px-3" style={{ background: c.surface2 }}>
                  <button
                    onClick={() =>
                      setInputAmount(Math.min(project.maxBuy, MOCK_USER.usdtBalance).toString())
                    }
                    className="px-2 py-0.5 rounded-lg"
                    style={{
              ...
              <div className="flex gap-2">
                {[100, 250, 500, 1000].map((v) => (
                  <button
                    ...
                    style={{
                      background:
                        inputAmount === v.toString() ? 'rgba(59,130,246,0.1)' : c.surface2,
              ...
              {numAmount > 0 && (
                <div
                  className="mt-3 rounded-xl p-2.5 flex items-center justify-between"
            ...
            >
              ...
              <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
                Giao dịch cross-chain cần thời gian xử lý. Giá có thể thay đổi trong quá trình
                bridge. Luôn kiểm tra địa chỉ contract trước khi giao dịch.
              </p>
            </div>
          </>
        )}
        ...
        {step === 'routes' && (
          <>
            ...
            <p style={{ color: c.text3, fontSize: 12, marginBottom: 4 }}>
              {routes.length} routes tìm thấy cho ${numAmount.toLocaleString()} USDT →{' '}
              {project.symbol}
            </p>
            ...
            <button
              onClick={() =>
                navigate(`${prefix}/launchpad/bridge-compare`, {
                  state: {
            ...
            >
              ...
              <span style={{ color: '#6366F1', fontSize: 12, fontWeight: 600 }}>
                So sánh chi tiết tất cả routes
              </span>
        ...
        {step === 'review' && selectedRoute && (
          <>
            ...
            <TrCard className="p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="text-center">
                  <div
                    ...
                    style={{
                      background: sourceChain.color + '15',
                      border: `1px solid ${sourceChain.color}25`,
                    }}
                  >
                    <span style={{ color: sourceChain.color, fontSize: 12, fontWeight: 700 }}>
                      {sourceChain.icon}
                    </span>
                  </div>
                  <p
                ...
                <div className="text-center">
                  <div
                    className="w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-1"
                    style={{
                      background: project.logoColor + '15',
                      border: `1px solid ${project.logoColor}25`,
                    }}
                  >
                    <span style={{ color: project.logoColor, fontSize: 12, fontWeight: 700 }}>
                      {project.logo}
                    </span>
                  </div>
                  <p
                  ...
                  <p style={{ color: c.text3, fontSize: 10 }}>
                    {project.symbol} · {targetChain.name}
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl p-3 mb-3" style={{ background: c.surface2 }}>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span style={{ color: c.text3, fontSize: 11 }}>
                    Đường đi ({selectedRoute.path.length} bước)
                  </span>
                </div>
                {selectedRoute.path.map((hop, i) => (
                  <div key={i} className="flex items-center gap-2 py-1">
                    ...
                    <span className="ml-auto" style={{ color: c.text3, fontSize: 10 }}>
                      {hop.dex} · {hop.chain}
                    </span>
                  </div>
                ))}
              ...
              <div className="flex flex-col gap-1.5">
                {(
                  [
                    ...
                    {
                      label: 'Price impact',
                      value: `${selectedRoute.priceImpact}%`,
                      color: selectedRoute.priceImpact > 1 ? '#EF4444' : c.text1,
                    },
                    { label: 'Slippage tolerance', value: `${slippage}%` },
                    { label: 'Thời gian ước tính', value: selectedRoute.estimatedTime },
                  ] satisfies Array<{ label: string; value: string; color?: string }>
                ).map((r) => (
                  <div
                    key={r.label}
            ...
            >
              ...
              <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
                Cross-chain swap có rủi ro về giá, thời gian xử lý và lỗi kỹ thuật. Chỉ tham gia với
                số tiền bạn sẵn sàng chấp nhận rủi ro.
              </p>
            </div>
          </>
        )}
                  ...
                  >
                    ...
                    >
                      {isDone ? (
                        ...
                        <s.icon
                          size={16}
                          color={isCurrent ? '#3B82F6' : c.text3}
                    ...
                    {isDone && (
                      <span className="ml-auto" style={{ color: '#10B981', fontSize: 10 }}>
                        Done
                      </span>
                    )}
                  </div>
          ...
          <>
            <div className="text-center py-6">
              ...
              <p style={{ color: c.text2, fontSize: 13, lineHeight: 1.5 }}>
                Bạn đã swap ${numAmount.toLocaleString()} USDT thành{' '}
                {selectedRoute.estimatedOutput.toLocaleString()} {project.symbol}
              </p>
            </div>
            ...
            <TrCard className="p-4">
              {[
                ...
                {
                  label: 'Nhận',
                  value: `${selectedRoute.estimatedOutput.toLocaleString()} ${project.symbol}`,
                },
                { label: 'Route', value: selectedRoute.provider },
                { label: 'Tổng phí', value: selectedRoute.totalFee },
            ...
            <div className="flex gap-2">
              <button
                onClick={() => {
                  ...
                  const record: BridgeTxRecord = {
                    ...
                    routeHops: selectedRoute.path.length,
                  ...
                  navigate(`${prefix}/launchpad/bridge-order/${record.id}`, {
              ...
              <CTAButton
                className="flex-1"
                variant="success"
                onClick={() => navigate(`${prefix}/launchpad/${project.id}`)}
              >
                Xem dự án
              </CTAButton>
            </div>
      ...
      <button className="w-full text-left p-4" onClick={onSelect}>
        ...
        <div className="flex items-end justify-between mb-3">
          <div>
            ...
            <p style={{ color: '#10B981', fontSize: 20, fontWeight: 800, fontFamily: 'monospace' }}>
              {route.estimatedOutput.toLocaleString()}{' '}
              <span style={{ fontSize: 12 }}>{projectSymbol}</span>
            </p>
          </div>
        ...
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p
              style={{
                color: route.priceImpact > 1 ? '#EF4444' : c.text1,
                fontSize: 11,
                fontWeight: 600,
              }}
            >
...
function SlippageSettingsSheet({
  slippage,
  onApply,
  onClose,
}: {
          ...
          {value > 5 && (
            ...
            >
              ...
              <span style={{ color: '#F59E0B', fontSize: 11 }}>
                Slippage cao có thể dẫn đến mất giá lớn
              </span>
            </div>
          )}

          <CTAButton onClick={() => onApply(value)}>Áp dụng ({value}%)</CTAButton>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onBack} style={{ color: '#3B82F6', fontSize: 12, fontWeight: 600 }}>
          Giao dịch mới
        </button>
      </div>
          ...
          {history.map((tx) => {
            ...
            return (
              <TrCard
                ...
                className="p-4"
                onClick={() =>
                  navigate(`${prefix}/launchpad/bridge-order/${tx.id}`, {
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="px-2 py-0.5 rounded-lg" style={{ background: st.bg }}>
                    <span style={{ color: st.color, fontSize: 10, fontWeight: 600 }}>
                      {st.label}
                    </span>
                  </div>
                </div>
                ...
                {tx.status === 'failed' && tx.failReason && (
                  <div
                    className="mt-2 rounded-lg p-2 flex items-start gap-1.5"
