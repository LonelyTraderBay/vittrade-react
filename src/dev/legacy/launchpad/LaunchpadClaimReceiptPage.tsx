...
import { CountdownTimer, RiskDisclosure } from '@/dev/legacy/launchpad/LaunchpadComponents';
...
export function LaunchpadClaimReceiptPage() {
  ...
  const receiptDetails: { label: string; value: string; color?: string; mono?: boolean }[] = [
  ...
  return (
    <PageLayout>
      ...
      <PageContent gap="default">
        ...
        {/* Tab: Lịch sử */}
        {tab === 'Lịch sử' && (
          <ClaimHistoryTab history={receipt.claimHistory} token={receipt.rewardToken} />
        )}
...
function ClaimHistoryTab({ history, token }: { history: ClaimHistoryEntry[]; token: string }) {
  const c = useThemeColors();
