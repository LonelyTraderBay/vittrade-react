...
import {
  ...
  BRIDGE_NETWORKS,
  type BridgeRouteOption,
  type RouteComparisonResult,
} from '@/dev/mocks/launchpad-fixtures';
import { RiskDisclosure } from '@/dev/legacy/launchpad/LaunchpadComponents';
...
export function LaunchpadBridgeComparePage() {
  ...
  const sortedRoutes = useMemo(() => {
    ...
    switch (sortMode) {
      ...
      case 'speed':
        return routes.sort((a, b) => a.estimatedSeconds - b.estimatedSeconds);
      case 'security':
      ...
      <PageContent gap="default">
          ...
          <div className="mb-4">
            ...
            {sortedRoutes.map((route) => {
              const pct =
                minOutput === maxOutput
                  ? 100
                  : ((route.outputAmount - minOutput) / (maxOutput - minOutput)) * 60 + 40;
                ...
                <div key={route.id + '-out'} className="flex items-center gap-2 mb-1.5">
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        background: isBest ? '#10B981' : route.providerColor + '60',
                      }}
                    >
                      <span
                        style={{
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        background: isBest ? '#10B981' : '#EF444460',
                      }}
                    >
                      <span
                        style={{
                ...
                <div key={route.id + '-speed'} className="flex items-center gap-2 mb-1.5">
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${Math.max(pct, 15)}%`,
                        background: isBest ? '#10B981' : '#3B82F660',
                      }}
                    >
                      <span
                        style={{
        ...
        {loading && (
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-3" style={{ color: c.text2, fontSize: 13 }}>
              Cập nhật routes...
            </span>
          </div>
        )}
        ...
        <StickyFooter>
          <div className="flex items-center gap-3 mb-2">
            <div
              ...
              style={{
                background: selectedRouteObj.providerColor + '22',
                color: selectedRouteObj.providerColor,
                fontSize: 9,
                fontWeight: 700,
              }}
            >
            ...
            <div className="flex-1">
              <p style={{ color: c.text1, fontSize: 12, fontWeight: 600 }}>
                {selectedRouteObj.provider}
              </p>
              <p style={{ color: c.text3, fontSize: 10 }}>
                {selectedRouteObj.outputAmount.toLocaleString()} {outputToken} ·{' '}
                {selectedRouteObj.totalFeeUSD} fee · {selectedRouteObj.estimatedTime}
              </p>
            </div>
          </div>
...
function ChainBadge({ chain }: { chain: string }) {
  ...
  return (
    <div
      ...
      style={{
        background: (net?.color || '#6366F1') + '22',
        border: `1.5px solid ${net?.color || '#6366F1'}44`,
      }}
    >
...
function RouteCard({
  ...
  const outputDiff =
    bestOutput > 0 ? (((route.outputAmount - bestOutput) / bestOutput) * 100).toFixed(2) : '0';
  ...
  return (
    <TrCard
      ...
      style={{
        border: selected
          ? `1.5px solid ${route.providerColor}50`
          : route.recommended
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="relative">
            <div
              ...
              style={{
                background: route.providerColor + '18',
                border: `1.5px solid ${route.providerColor}30`,
              }}
            >
              <span style={{ color: route.providerColor, fontSize: 11, fontWeight: 800 }}>
                {route.providerIcon}
              </span>
            </div>
            {rank === 1 && (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5">
              ...
              {route.recommended && (
                <div
                  className="px-1.5 py-px rounded-md"
                  style={{ background: 'rgba(99,102,241,0.1)' }}
                >
                  <span style={{ color: '#6366F1', fontSize: 8, fontWeight: 700 }}>
                    KHUYEN NGHI
                  </span>
                </div>
              )}
            ...
            <div className="flex flex-wrap gap-1 mb-2">
              ...
              {route.tags
                .filter((t) => !['Best output', 'Lowest fee', 'Fastest'].includes(t))
                .map((t) => (
            ...
            <div className="flex items-center gap-4">
              <div>
                ...
                <p
                  style={{
                    color: isBestOutput ? '#10B981' : c.text1,
                    fontSize: 14,
                    fontWeight: 800,
                    fontFamily: 'monospace',
                  }}
                ...
                {!isBestOutput && <p style={{ color: '#EF4444', fontSize: 9 }}>{outputDiff}%</p>}
              </div>
              <div>
                <p style={{ color: c.text3, fontSize: 9 }}>Phi</p>
                <p
                  style={{
                    color: isBestFee ? '#10B981' : c.text1,
                    fontSize: 13,
                    fontWeight: 700,
                    fontFamily: 'monospace',
                  }}
              ...
              <div>
                ...
                <p
                  style={{
                    color: isBestSpeed ? '#10B981' : c.text1,
                    fontSize: 13,
                    fontWeight: 700,
                  }}
                >
        ...
        <div className="px-4 pb-4" style={{ borderTop: `1px solid ${c.border}` }}>
                ...
                <React.Fragment key={i}>
                  ...
                  <div className="flex items-center gap-0.5 shrink-0">
                    ...
                    <div
                      ...
                      style={{ background: route.providerColor + '10' }}
                    >
                      <span style={{ color: route.providerColor, fontSize: 8, fontWeight: 600 }}>
                        {hop.dex}
                      </span>
                    </div>
                    <ArrowRight size={10} color={c.text3} />
          ...
          <div className="flex flex-col gap-0">
            {[
              {
                label: 'Output',
                value: `${route.outputAmount.toLocaleString()} ${comparison.outputToken}`,
                color: isBestOutput ? '#10B981' : undefined,
              },
              ...
              { label: 'Bridge fee', value: `$${route.bridgeFee.toFixed(2)}` },
              {
                ...
                color: isBestFee ? '#10B981' : undefined,
              },
              {
                label: 'Price impact',
                value: `${route.priceImpact}%`,
                color: route.priceImpact > 0.3 ? '#F59E0B' : '#10B981',
              },
              { label: 'Slippage tolerance', value: `${route.slippage}%` },
              {
                label: 'Thời gian',
                value: route.estimatedTime,
                color: isBestSpeed ? '#10B981' : undefined,
              },
              { label: 'Hops', value: `${route.hops}` },
              {
                ...
                color:
                  route.securityScore >= 90
                    ? '#10B981'
                    : route.securityScore >= 80
                      ? '#F59E0B'
                      : '#EF4444',
              },
        ...
        <div className="px-5 pb-6 flex flex-col gap-4">
          ...
          <div className="text-center py-3">
            <div
              ...
              style={{
                background: route.providerColor + '18',
                border: `2px solid ${route.providerColor}30`,
              }}
            >
              <span style={{ color: route.providerColor, fontSize: 14, fontWeight: 800 }}>
                {route.providerIcon}
              </span>
            </div>
          ...
          {route.priceImpact > 0.3 && (
            ...
            >
              ...
              <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
                Price impact ({route.priceImpact}%) cao hơn bình thường. Kết quả thực tế có thể khác
                so với dự kiến.
              </p>
            </div>
          )}
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
              Đây là chế độ mô phỏng. Giao dịch thực tế có thể có kết quả khác tùy điều kiện thị
              trường.
            </p>
          </div>

          <CTAButton onClick={onExecute}>Xác nhận Bridge qua {route.provider}</CTAButton>
        </div>
      </div>
    </div>
  );
