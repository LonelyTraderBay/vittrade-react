...
import {
  loadBridgeHistory,
  createBridgeOrderDetail,
  getBridgePollingSequence,
  generateBridgeEventLog,
  type BridgeTxRecord,
  type BridgeOrderDetail,
  type BridgePollingStatus,
  type BridgeWsEvent,
  type WsEventLevel,
...
export function LaunchpadBridgeOrderPage() {
  ...
  const locationState = location.state as { scenario?: 'success' | 'slow' | 'fail' } | null;
  const scenario = locationState?.scenario ?? 'success';

  const [bridgeTx] = useState<BridgeTxRecord | null>(() => {
  ...
  const [confirmations, setConfirmations] = useState<{ current: number; required: number } | null>(
    null,
  );
        ...
        <PageContent>
          <TrCard className="p-8 text-center">
            ...
            <p style={{ color: c.text3, fontSize: 12, marginTop: 4 }}>
              Giao dich bridge khong ton tai hoac da het han.
            </p>
          </TrCard>
        </PageContent>
      </PageLayout>
  ...
  const allStatuses: BridgePollingStatus[] = [
    'initiated',
    'approved',
    'bridging',
    'confirming',
  ...
  const filteredEvents =
    logFilter === 'all' ? wsEvents : wsEvents.filter((e) => e.level === logFilter);
    ...
    <PageLayout>
      ...
      <PageContent gap="default">
        ...
        <TrCard variant={isComplete ? undefined : 'hero'} className="p-5 relative overflow-hidden">
          {!isComplete && !isFailed && (
            <div
              ...
              style={{
                background: `radial-gradient(circle, ${statusCfg.color}33 0%, transparent 65%)`,
              }}
            />
          )}
          ...
          <div className="flex flex-col items-center text-center relative z-10">
            ...
            <div className="flex items-center gap-3 mt-3">
              <div className="text-center">
                <p style={{ color: isComplete ? c.text3 : 'rgba(255,255,255,0.5)', fontSize: 10 }}>
                  Ban
                </p>
                <p
                  style={{
            ...
            {isPolling && !isComplete && !isFailed && (
              ...
              >
                ...
                <span
                  style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11, fontFamily: 'monospace' }}
                >
                  ETA ~{etaSeconds}s
                </span>
          ...
          <div className="relative">
            ...
            {order.steps
              ...
              .map((step, i) => {
                ...
                const isDone =
                  stepIdx < currentIdx ||
                  (step.status === order.status && (isComplete || isFailed));
                ...
                const StepIcon = isFailStep
                  ? AlertCircle
                  : isDone
                    ? CheckCircle
                    : isActive
                      ? STATUS_CONFIG[step.status]?.icon || CircleDot
                      : CircleDot;

                return (
                  <div
                    key={step.status + i}
                    className="flex gap-3 relative mb-1"
                  ...
                  >
                    ...
                    <div
                      className="flex-1 rounded-xl p-2.5 mb-1"
                      style={{
                        background: isActive
                          ? `${statusCfg.color}08`
                          : isFailStep
                    ...
                    >
                      <div className="flex items-center justify-between mb-0.5">
                        <span
                          style={{
                            color: isFailStep ? '#EF4444' : c.text1,
                            fontSize: 12,
                            fontWeight: 600,
                          }}
                        >
                      ...
                      <p
                        style={{
                          color: isFailStep ? '#EF4444' : c.text3,
                          fontSize: 10,
                          lineHeight: 1.4,
                        }}
                      >
        ...
        <TrCard className="overflow-hidden">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span style={{ color: c.text3, fontSize: 10 }}>{wsEvents.length} events</span>
            </div>
            <div className="flex items-center gap-1.5">
              {logExpanded && wsEvents.length > 0 && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (logPaused) handleResumeLog();
                    else setLogPaused(true);
                  }}
          ...
          {logExpanded && (
            <div>
              ...
              >
                {(['all', 'info', 'success', 'warning', 'error', 'debug', 'system'] as const).map(
                  (level) => (
                    <button
                      key={level}
                      ...
                      style={{
                        background:
                          logFilter === level
                            ? level === 'all'
                              ? 'rgba(59,130,246,0.1)'
                              : `${WS_LEVEL_COLORS[level]}15`
                            : c.surface2,
                        color:
                          logFilter === level
                            ? level === 'all'
                              ? '#3B82F6'
                              : WS_LEVEL_COLORS[level]
                            : c.text3,
                        fontSize: 10,
        ...
        <TrCard className="p-4">
          ...
          <div className="flex flex-col gap-0">
            {[
              ...
              {
                label: 'Price Impact',
                value: `${order.priceImpact}%`,
                color: order.priceImpact > 1 ? '#F59E0B' : '#10B981',
              },
              { label: 'Gas', value: order.gasCost },
              { label: 'Tổng phí', value: order.totalFee },
              { label: 'Source Tx', value: order.sourceTxHash, mono: true },
              ...(order.retryCount > 0
                ? [{ label: 'So lan thu lai', value: `${order.retryCount}` }]
                : []),
            ].map((r: { label: string; value: string; color?: string; mono?: boolean }) => (
              <div
                key={r.label}
        ...
        {isComplete && (
          <div className="flex gap-2">
            <button
              onClick={() => navigate(`${prefix}/launchpad/${bridgeTx.projectId}`)}
              className="flex-1 h-12 rounded-2xl font-bold flex items-center justify-center gap-1.5 hover:opacity-90 transition-opacity active:scale-[0.98]"
              style={{
                background: c.surface2,
                color: c.text2,
        ...
        >
          ...
          <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
            Đây là chế độ mô phỏng. Trạng thái được cập nhật tự động mỗi vài giây. Giao dich khong
            duoc gui len blockchain that.
          </p>
        </div>
...
function WsLogEntry({ event }: { event: BridgeWsEvent }) {
  ...
  return (
    <div className="group">
      <div
        className="flex items-start gap-1.5 py-0.5 px-1 rounded hover:bg-white/[0.02] cursor-pointer"
          ...
          <div className="rounded px-2 py-1" style={{ background: '#161B22' }}>
            {Object.entries(event.data).map(([k, v]) => (
              <div key={k} className="flex gap-2">
                ...
                <span style={{ color: '#C9D1D9', fontSize: 8, fontFamily: 'monospace' }}>
                  {String(v)}
                </span>
              </div>
            ))}
