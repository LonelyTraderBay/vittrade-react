...
import {
  MOCK_WHITELIST_ENTRIES,
  MOCK_WHITELIST_PROJECTS,
  type WhitelistEntry,
  type WhitelistProject,
...
const STATUS_CONFIG: Record<
  string,
  { label: string; color: string; icon: typeof CheckCircle; bg: string }
> = {
  whitelisted: {
    label: 'Whitelisted',
    color: '#10B981',
...
export function LaunchpadWhitelistCheckerPage() {
  ...
  return (
    <PageLayout>
      ...
      {/* Entry detail sheet */}
      {selectedEntry && (
        <WhitelistDetailSheet entry={selectedEntry} onClose={() => setSelectedEntry(null)} />
      )}

      {/* Project detail sheet */}
      {selectedProject && (
        <ProjectWhitelistSheet project={selectedProject} onClose={() => setSelectedProject(null)} />
      )}
      ...
      <PageContent gap="default">
        ...
        {tab === 'upcoming' && (
          <UpcomingTab projects={upcomingProjects} onSelect={setSelectedProject} />
        )}
      ...
      <button className="w-full text-left p-4" onClick={() => onSelect(entry)}>
        ...
        <div className="flex items-start gap-3 mb-3">
          <div
            ...
            style={{
              background: entry.projectLogoColor + '22',
              border: `2px solid ${entry.projectLogoColor}44`,
              color: entry.projectLogoColor,
            }}
          >
            {entry.projectLogo}
          </div>
              ...
              <>
                ...
                <div className="flex items-center justify-between">
                  <span style={{ color: c.text3, fontSize: 10 }}>
                    Used: ${entry.allocationUsed.toLocaleString()}
                  </span>
...
function UpcomingWhitelistCard({
  project,
  onSelect,
}: {
      ...
      <button className="w-full text-left p-4" onClick={() => onSelect(project)}>
        ...
        <div className="flex items-start gap-3 mb-3">
          <div
            className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 text-lg font-bold"
            style={{
              background: project.projectLogoColor + '22',
              border: `2px solid ${project.projectLogoColor}44`,
              color: project.projectLogoColor,
            }}
          >
            {project.projectLogo}
          </div>
        ...
        <div className="mt-3 pt-3 border-t" style={{ borderColor: c.borderSolid }}>
          {canApply ? (
            ...
            <div
              className="flex items-center gap-2 px-3 py-2 rounded-xl"
              style={{
                background: 'rgba(139,149,179,0.08)',
                border: '1px solid rgba(139,149,179,0.15)',
              }}
            >
...
function WhitelistDetailSheet({ entry, onClose }: { entry: WhitelistEntry; onClose: () => void }) {
  const c = useThemeColors();
  const statusConfig = STATUS_CONFIG[entry.status];
      ...
      >
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div
            ...
            style={{ background: statusConfig.bg, border: `1px solid ${statusConfig.color}33` }}
          >
            <statusConfig.icon size={32} color={statusConfig.color} className="mx-auto mb-2" />
            <p style={{ color: statusConfig.color, fontSize: 16, fontWeight: 700 }}>
              {statusConfig.label}
            </p>
          </div>
          ...
          {entry.status === 'pending' && (
            ...
            >
              ...
              <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
                Application đang chờ xử lý. Hoàn thành tất cả requirements để tăng cơ hội được chấp
                nhận.
              </p>
            </div>
          )}
...
function ProjectWhitelistSheet({
  project,
  onClose,
}: {
  project: WhitelistProject;
    ...
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      ...
      >
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          {/* Apply CTA */}
          {project.userStatus === 'can_apply' && <CTAButton>Apply for Whitelist</CTAButton>}
        </div>
      </div>
    </div>
  );
