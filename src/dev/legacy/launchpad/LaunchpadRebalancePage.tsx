...
export function LaunchpadRebalancePage() {
  ...
  const suggestions = useMemo(
    () => generateRebalanceSuggestions(assetsWithTargets),
    [assetsWithTargets],
  );
  ...
  const pieData = assetsWithTargets.map((a) => ({
    name: a.symbol,
    value: a.currentPercent,
    color: a.color,
  }));
  const targetPieData = assetsWithTargets.map((a) => ({
    name: a.symbol,
      ...
      <PageContent gap="default" grow>
          ...
          <div className="flex gap-2">
            {REBALANCE_STRATEGIES.map((s) => (
              ...
              >
                ...
                <p
                  style={{
                    color: selectedStrategy === s.id ? s.color : c.text1,
                    fontSize: 11,
                    fontWeight: 700,
                  }}
                >
        ...
        <TrCard className="p-3">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1">
              <p style={{ color: c.text3, fontSize: 9, textAlign: 'center', marginBottom: 4 }}>
                Hiện tại
              </p>
              <ResponsiveContainer width="100%" height={100}>
                <PieChart>
                  ...
                  >
                    {pieData.map((entry, i) => (
                      <Cell key={`cur-${i}`} fill={entry.color} />
                    ))}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
                ...
                <PieChart>
                  ...
                  >
                    {targetPieData.map((entry, i) => (
                      <Cell key={`tgt-${i}`} fill={entry.color} />
                    ))}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={barData} layout="vertical" barGap={2}>
              ...
              <YAxis
                ...
                type="category"
                dataKey="name"
                tick={{ fontSize: 10, fill: c.text1, fontWeight: 600 }}
                tickLine={false}
                axisLine={false}
                width={40}
              />
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <span style={{ color: sg.color, fontSize: 10, fontWeight: 800 }}>
                        {sg.symbol.slice(0, 2)}
                      </span>
                    </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {sg.action !== 'hold' && (
                          <span style={{ color: cfg.color, fontSize: 10, fontWeight: 600 }}>
                            {sg.deviation > 0 ? '-' : '+'}
                            {Math.abs(sg.deviation).toFixed(1)}%
                          </span>
                        )}
                      </div>
                    </div>
                    {sg.action !== 'hold' && (
                      <div className="text-right">
                        ...
                        <p style={{ color: c.text3, fontSize: 9 }}>
                          {sg.suggestedAmount < 1
                            ? sg.suggestedAmount.toFixed(4)
                            : sg.suggestedAmount.toFixed(2)}{' '}
                          {sg.symbol}
                        </p>
                      </div>
                    )}
        ...
        <TrCard className="p-3">
          ...
          <div className="flex flex-col gap-0">
            {[
              {
                label: 'Số giao dịch cần thực hiện',
                value: `${suggestions.filter((s) => s.action !== 'hold').length} tx`,
              },
              ...
              >
                ...
                <span style={{ color: row.color || c.text1, fontSize: 11, fontWeight: 600 }}>
                  {row.value}
                </span>
              </div>
            ))}
        ...
        >
          ...
          <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
            Đây là đề xuất tự động dựa trên tỷ lệ mục tiêu. Giá token có thể thay đổi giữa lúc xem
            và lúc thực hiện. Luôn kiểm tra lại trước khi giao dịch.
          </p>
        </div>
      </PageContent>
