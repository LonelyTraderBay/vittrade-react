...
import {
  generateMockEventLog,
  formatEventLogForClipboard,
  formatEventLogAsJSON,
  formatEventLogAsCSV,
  type EventLogEntry,
  type EventLogLevel,
...
export function LaunchpadEventLogPage() {
  ...
  const handleCopyToClipboard = useCallback(async () => {
    ...
    switch (exportFormat) {
      case 'json':
        content = formatEventLogAsJSON(eventsToExport);
        break;
      case 'csv':
        content = formatEventLogAsCSV(eventsToExport);
        break;
      default:
        content = formatEventLogForClipboard(eventsToExport);
    }
    try {
                ...
                {isExpanded && (
                  <div className="px-3 pb-3 pt-0" style={{ borderTop: `1px solid ${c.border}` }}>
                    <div className="flex flex-col gap-0 mt-2">
                      {event.details && <DetailRow label="Chi tiet" value={event.details} />}
                      {event.txHash && (
                        <DetailRow
                          label="TxHash"
                          value={event.txHash}
                      ...
                      {event.chain && <DetailRow label="Chain" value={event.chain} />}
                      {event.contractAddress && (
                        <DetailRow label="Contract" value={event.contractAddress} mono />
                      )}
                      {event.gasUsed && <DetailRow label="Gas used" value={event.gasUsed} mono />}
                      {event.blockNumber && (
                        <DetailRow
                          label="Block"
                          value={`#${event.blockNumber.toLocaleString()}`}
                          mono
                          action={
                            <button className="p-1">
      ...
      >
        ...
        <div className="px-5 pb-6 flex flex-col gap-4">
          ...
          <div
            ...
            style={{ background: c.surface2, maxHeight: 100 }}
          >
            <pre
              style={{
                color: c.text3,
