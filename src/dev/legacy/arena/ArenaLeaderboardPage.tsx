...
import { SkeletonRow } from '@/shared/ui/SkeletonBlock';
...
function Podium({
  ...
  entries: { avatar: string; name: string; value: string; rank: number }[];
}) {
      ...
      <div className="flex flex-col items-center flex-1">
        ...
        <div
          ...
          style={{
            ...
            fontSize: 30,
            border: `2px solid ${RANK_COLORS[0]}`,
            boxShadow: `0 0 20px ${hexToRgba(RANK_COLORS[0], 33)}`,
          }}
        >
          {entries[0].avatar}
        </div>
...
export function ArenaLeaderboardPage() {
  ...
  const sortedCreators = useMemo(() => {
    ...
    switch (metric) {
      case 'fair_play':
        return list.sort((a, b) => b.fairPlayScore - a.fairPlayScore);
      case 'popularity':
        return list.sort((a, b) => b.creator.followers - a.creator.followers);
      case 'win_rate':
        return list.sort((a, b) => b.winRate - a.winRate);
      case 'activity':
        return list.sort((a, b) => b.activity - a.activity);
      case 'completion':
        return list.sort((a, b) => b.completionQuality - a.completionQuality);
      default:
        return list;
    }
  }, [metric]);
  ...
  const sortedPlayers = useMemo(() => {
    ...
    switch (metric) {
      case 'fair_play':
        return list.sort((a, b) => b.fairPlayScore - a.fairPlayScore);
      case 'popularity':
        return list.sort((a, b) => b.totalChallenges - a.totalChallenges);
      case 'win_rate':
        return list.sort((a, b) => b.winRate - a.winRate);
      case 'activity':
  ...
  const sortedTeams = useMemo(() => {
    ...
    switch (metric) {
      ...
      case 'win_rate':
        return list.sort((a, b) => b.winRate - a.winRate);
      case 'activity':
        return list.sort((a, b) => b.totalWins - a.totalWins);
      case 'completion':
        return list.sort((a, b) => b.fairPlayScore - a.fairPlayScore);
      default:
        return list;
  ...
  function getCreatorMetricValue(entry: (typeof sortedCreators)[0]): string {
    switch (metric) {
      ...
      case 'popularity':
        return fmtPoints(entry.creator.followers);
      case 'win_rate':
        return `${entry.winRate}%`;
      case 'activity':
        return `${entry.activity} rooms`;
      case 'completion':
        return `${entry.completionQuality}%`;
    }
  }
  ...
  function getPlayerMetricValue(entry: (typeof sortedPlayers)[0]): string {
    switch (metric) {
      case 'fair_play':
        return `${entry.fairPlayScore}%`;
      case 'popularity':
        return `${entry.totalChallenges}`;
      case 'win_rate':
        return `${entry.winRate}%`;
      case 'activity':
        return `${entry.streak} streak`;
      case 'completion':
          ...
          <TrCard className="p-4 flex items-center gap-3" accentBorder="rgba(139,92,246,0.2)">
            ...
            <div className="flex-1">
              ...
              <p style={{ color: c.text3, fontSize: φ.xs }}>
                {MY_ARENA_STATS.wins} wins · {MY_ARENA_STATS.totalChallenges} challenges
              </p>
            </div>
            <div className="text-right">
              <p style={{ color: '#8B5CF6', fontSize: φ.md, fontWeight: 700 }}>
                #{MY_ARENA_STATS.rank}
              </p>
              <p style={{ color: '#F59E0B', fontSize: φ.xs, fontWeight: 600 }}>
                {fmtPoints(MY_ARENA_STATS.pointsEarned)} pts
              </p>
            </div>
          </TrCard>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-4">
              ...
              <TrCard overflow>
                {(mainTab === 'creators'
                  ...
                  const avatar = isCreator
                    ? (entry as (typeof sortedCreators)[0]).creator.avatar
                    : (entry as any).avatar;
                  const name = isCreator
                    ? (entry as (typeof sortedCreators)[0]).creator.name
                    : isPlayer
                      ? (entry as (typeof sortedPlayers)[0]).name
                      : (entry as (typeof sortedTeams)[0]).teamName;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {isCreator &&
                          (entry as (typeof sortedCreators)[0]).creator.fairPlayBadge && (
                            <span
                  ...
                  {sortedCreators
                    ...
                    .map((entry, i) => (
                      <button
                        ...
                        onClick={() => {
                          navigate(`${prefix}/arena/creator/${entry.creator.id}`);
                          hapticSelection();
                        }}
                        className="flex items-center gap-3 px-4 py-3.5 w-full text-left active:opacity-70"
                        style={{
                          borderBottom: i < 1 ? `1px solid ${c.divider}` : 'none',
                          minHeight: 52,
                        }}
                      >
                        <div
            ...
            {mainTab === 'players' && (
              <div className="mb-4">
                <SectionHeader
                  title="Người chơi đáng tin nhất"
                  accent
                  accentColor="#10B981"
                  mb={8}
                />
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span style={{ color: c.text3, fontSize: 10 }}>
                              {p.totalChallenges} challenges
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-1">
                          <Shield size={12} color="#10B981" />
                          <span style={{ color: '#10B981', fontSize: φ.xs, fontWeight: 700 }}>
                            {p.fairPlayScore}%
                          </span>
                        </div>
                      </div>
            ...
            <div className="mt-2">
              <TrCard className="p-3 flex items-start gap-2">
                ...
                <p style={{ color: c.text3, fontSize: φ.xs, lineHeight: 1.5 }}>
                  Bảng xếp hạng dựa trên chất lượng, sự công bằng và độ tin cậy — không nhấn mạnh số
                  tiền. Arena Points không phải tài sản tài chính.
                </p>
              </TrCard>
            </div>
          </div>
