...
export type GovernanceStatus =
  'draft' | 'missing_rules' | 'missing_referee' | 'high_risk' | 'publish_blocked' | 'publish_ready';
...
export function RuleClarityCard({ score }: { score: number }) {
  ...
  const tier: RiskTier = score >= 70 ? 'low' : score >= 40 ? 'medium' : 'high';
          ...
          return (
            <div
              ...
              style={{
                ...
                border: active
                  ? `1.5px solid ${hexToRgba(riskStyle.text, 20)}`
...
export function PublishEligibilityPanel({
  ...
  return (
    <TrCard
      className="p-4"
      accentBorder={allPassed ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <span
          ...
          style={{
            background: hexToRgba(cfg.color, 15),
            color: cfg.color,
            fontSize: 10,
            fontWeight: 700,
          }}
...
export function RoomSafetySnapshotCard({ snapshot }: { snapshot: SafetySnapshot }) {
  ...
  const rows: { label: string; value: string; icon: React.ReactNode }[] = [
    ...
    { label: 'Resolution', value: snapshot.resolution, icon: <Scale size={12} color={c.text3} /> },
    {
      ...
      icon: <FileWarning size={12} color={c.text3} />,
    },
    { label: 'Privacy', value: snapshot.privacy, icon: <Lock size={12} color={c.text3} /> },
    {
      label: 'Void rule',
      value: snapshot.voidRule || '—',
      icon: <AlertTriangle size={12} color={c.text3} />,
    },
  ];
...
export function GovernanceHintBanner({
  ...
  return (
    ...
    >
      ...
      <p style={{ color: c.text2, fontSize: φ.xs, lineHeight: 1.5 }}>{text}</p>
    </div>
  );
}
...
const JOIN_STYLE_NOTES: Record<
  string,
  { label: string; note: string; icon: React.ReactNode; type: 'info' | 'warning' | 'success' }
> = {
  public: {
    label: 'Public room',
    note: 'Room công khai cần luật rõ ràng hơn để tránh tranh chấp từ người lạ. Rule Clarity Score sẽ được áp dụng nghiêm hơn.',
    icon: <Eye size={14} color="#3B82F6" />,
    type: 'info',
  },
  invite_only: {
    label: 'Invite only',
    note: 'Phù hợp khi bạn đã biết đối thủ. Hỗ trợ referee / mutual confirm tốt hơn vì các bên quen nhau.',
    icon: <Lock size={14} color="#10B981" />,
    type: 'success',
  },
  unlisted: {
    label: 'Unlisted',
    note: 'Room không xuất hiện trong danh sách khám phá, chỉ ai có link mới vào. Ít tranh chấp hơn nhưng cũng ít người tham gia.',
    icon: <Link2 size={14} color="#F59E0B" />,
    type: 'warning',
  },
};
...
export function EligibilityNoteCard({ joinStyle }: { joinStyle: string }) {
  ...
  if (!info) return null;
  return <GovernanceHintBanner text={info.note} type={info.type} />;
}
...
export function ParticipantGovernanceCard({
  ...
  const risk = resolutionRisk(
    method === 'Tự động'
      ? 'auto'
      : method === 'Mutual Confirm'
        ? 'mutual'
        : method === 'Referee'
          ? 'referee'
...
export function ParticipantPreviewSheet({
    ...
    <div className="flex flex-col gap-4" style={{ padding: isMobile ? 16 : 20 }}>
      ...
      <div>
        <div className="flex items-center gap-1.5 flex-wrap mb-2">
          ...
          <span
            className="px-2.5 py-1 rounded-lg"
            style={{ background: c.chipBg, color: c.chipText, fontSize: 10, fontWeight: 600 }}
          >
            {data.matchFormat}
          </span>
          <span
            ...
            <TrCard className="p-4">
              ...
              <div className="flex flex-col gap-3">
                {[
                  ...
                  {
                    label: 'Bằng chứng',
                    value: data.evidenceRequired ? 'Bắt buộc' : 'Không yêu cầu',
                  },
          ...
          <div className="flex-1 flex flex-col gap-4">
            <TrCard className="p-4">
              ...
              <div className="flex flex-col gap-2">
                <div
                  className="flex items-center justify-between"
                  style={{ paddingBottom: 6, borderBottom: `1px solid ${c.divider}` }}
                >
  ...
  return (
      ...
      customHeader={
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span style={{ color: c.text1, fontSize: φ.body, fontWeight: 700 }}>Xem trước</span>
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: active
                      ? `1.5px solid ${c.chipActiveBorder}`
                      : '1.5px solid transparent',
                  }}
    ...
    >
      {/* Device label */}
      <div
        className="flex items-center justify-center gap-2 py-2 -mx-5"
        style={{ background: c.surface2 }}
      >
        {isMobile ? (
          <Smartphone size={12} color={c.text3} />
        ) : (
      ...
      {isMobile ? (
        <div style={{ maxWidth: 390, margin: '0 auto', width: '100%' }}>{previewContent}</div>
      ) : (
