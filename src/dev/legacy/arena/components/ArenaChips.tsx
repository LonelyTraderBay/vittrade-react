...
import type { ChallengeState, ParticipantLayout } from '@/dev/mocks/arena-fixtures';
...
export function FormatChip({ format, size = 'sm', className }: ChipProps & { format: FormatType }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 ${px} ${py} rounded-lg ${className ?? ''}`}
      style={{
...
export function ResolutionChip({
  ...
  size = 'sm',
  className,
}: ChipProps & { resolution: ResolutionType }) {
...
export type TrustBadgeType =
  'points_only' | 'fair_play' | 'high_trust' | 'new_creator' | 'verified_locked';
