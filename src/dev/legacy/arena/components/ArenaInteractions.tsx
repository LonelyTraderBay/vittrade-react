...
import { fmtPoints } from '@/dev/mocks/arena-fixtures';
...
const DISPUTE_REASONS = [
  {
    id: 'wrong_result',
    label: 'Kết quả không chính xác',
    desc: 'Dữ liệu nguồn hoặc cách tính bị sai',
  },
  {
    id: 'wrong_winner',
    label: 'Người thắng sai',
    desc: 'Kết quả đúng nhưng xác định người thắng sai',
  },
  {
    id: 'evidence_fraud',
    label: 'Bằng chứng giả mạo',
    desc: 'Bằng chứng bị chỉnh sửa hoặc không hợp lệ',
  },
  {
    id: 'rule_violation',
    label: 'Vi phạm luật chơi',
    desc: 'Bên thắng vi phạm luật nhưng vẫn được công nhận',
  },
  {
    id: 'void_condition',
    label: 'Đáng lẽ phải void',
    desc: 'Điều kiện void đã xảy ra nhưng challenge không bị void',
  },
          ...
          <div
            ...
            style={{
              background:
                step === s
                  ? '#EF4444'
                  : i < ['reason', 'detail', 'preview'].indexOf(step)
                    ? '#EF4444'
                    : c.chipBg,
              opacity:
                step === s ? 1 : i < ['reason', 'detail', 'preview'].indexOf(step) ? 0.5 : 0.3,
            }}
          />
        ))}
      </div>
        ...
        {step === 'reason' && (
          <div className="flex flex-col gap-3">
            <p style={{ color: c.text2, fontSize: φ.xs, lineHeight: 1.5, marginBottom: 4 }}>
              Tranh chấp khác với báo cáo vi phạm. Dùng khi bạn tin rằng{' '}
              <strong style={{ color: c.text1 }}>kết quả bị sai</strong>, không phải khi bị lừa đảo.
            </p>
            ...
            <TrCard className="p-3">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right shrink-0 ml-3">
                  ...
                  >
                    {fmtPoints(prizePool)} pts
                  </p>
                  {currentResult && (
                    <p style={{ color: c.text3, fontSize: 10 }}>Kết quả: {currentResult}</p>
                  )}
                </div>
              </div>
            </TrCard>
            ...
            {/* Reason selection */}
            <p style={{ color: c.text1, fontSize: φ.sm, fontWeight: 600, marginTop: 4 }}>
            ...
            {DISPUTE_REASONS.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <p style={{ color: c.text1, fontSize: φ.xs, fontWeight: 600, marginBottom: 2 }}>
                      {r.label}
                    </p>
        ...
        {step === 'detail' && (
          <div className="flex flex-col gap-3">
            <button
              ...
              className="flex items-center gap-1 active:opacity-70 self-start"
              style={{ color: c.text3, fontSize: φ.xs, minHeight: 28 }}
            >
            ...
            <p style={{ color: c.text2, fontSize: φ.xs, lineHeight: 1.5 }}>
              Giải thích vì sao bạn cho rằng kết quả không đúng. Cung cấp càng nhiều thông tin càng
              tốt (ít nhất 20 ký tự).
            </p>
          ...
          <div className="flex flex-col gap-3">
            ...
            <TrCard className="p-4" accentBorder="rgba(239,68,68,0.2)">
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span style={{ color: '#EF4444', fontSize: φ.xs, fontWeight: 600 }}>
            ...
            >
              ...
              <span style={{ color: c.text2, fontSize: φ.xs, lineHeight: 1.6 }}>
                Tôi xác nhận thông tin trên là chính xác. Tôi hiểu rằng tranh chấp vô căn cứ có thể
                ảnh hưởng đến điểm Trust Score.
              </span>
            </button>
...
export function LeaveChallengeSheet({
  ...
  const handleLeave = () => {
    ...
    actionToast.success(
      isBeforeDeadline
        ? TOAST.ARENA.CHALLENGE_LEFT_FULL_REFUND
        : TOAST.ARENA.CHALLENGE_LEFT_PARTIAL_REFUND,
    );
    setConfirmed(false);
    onClose();
  };
  ...
  return (
    <BottomSheetV2 open={open} onClose={handleClose} title="Rời challenge">
      <p style={{ color: c.text2, fontSize: φ.xs, lineHeight: 1.5, marginBottom: 12 }}>
        Bạn sắp rời khỏi <strong style={{ color: c.text1 }}>{challengeTitle}</strong>. Hành động này
        không thể hoàn tác.
      </p>

      {/* Refund breakdown */}
      <TrCard className="p-4 mb-3">
        <p style={{ color: c.text1, fontSize: φ.sm, fontWeight: 600, marginBottom: 12 }}>
          Thông tin hoàn điểm
        </p>
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-between">
            ...
            <span
              style={{
                color: isBeforeDeadline ? '#10B981' : '#F59E0B',
                fontSize: φ.sm,
                fontWeight: 700,
              }}
            >
      ...
      {/* Policy info */}
      <TrCard
        className="p-3 flex items-start gap-2 mb-4"
        accentBorder={isBeforeDeadline ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}
      >
        <Info
          size={13}
          color={isBeforeDeadline ? '#10B981' : '#F59E0B'}
...
const EVIDENCE_TYPES: { id: EvidenceType; label: string; icon: typeof Camera; desc: string }[] = [
  {
    ...
    icon: Image,
    desc: 'Screenshot kết quả từ nguồn chính thức',
  },
  {
    id: 'photo',
    label: 'Ảnh chụp',
    icon: Camera,
    desc: 'Ảnh chụp bằng camera (bàn phím, sự kiện…)',
  },
...
export function EvidenceUploadSheet({ open, onClose, challengeTitle }: EvidenceUploadSheetProps) {
  const c = useThemeColors();
  const { hapticSelection, hapticSuccess } = useHaptic();
  const actionToast = useActionToast();
  ...
  const simulateUpload = (type: EvidenceType) => {
    ...
    const interval = setInterval(() => {
      ...
      if (prog >= 100) {
        ...
        setItems((prev) =>
          prev.map((it) => (it.id === id ? { ...it, status: 'uploaded', progress: 100 } : it)),
        );
      } else {
        setItems((prev) =>
          prev.map((it) => (it.id === id ? { ...it, progress: Math.min(100, prog) } : it)),
        );
      }
    }, 400);
  ...
  const EVIDENCE_ICONS: Record<EvidenceType, typeof Camera> = {
    screenshot: Image,
    photo: Camera,
    video: Video,
    link: Link2,
      ...
      <div className="px-5 pb-8">
        {step === 'select' && (
          <div className="flex flex-col gap-3">
            ...
            {/* Link input */}
            <div>
              <p style={{ color: c.text1, fontSize: φ.xs, fontWeight: 600, marginBottom: 4 }}>
                Hoặc dán link
              </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <Icon
                            size={16}
                            color={item.status === 'uploaded' ? '#10B981' : '#3B82F6'}
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p
                            style={{ color: c.text1, fontSize: 10, fontWeight: 600 }}
                            className="truncate"
                          >
                            {item.label}
                          </p>
                        ...
                        <button
                          ...
                          className="w-7 h-7 rounded-md flex items-center justify-center active:opacity-70"
                          style={{ background: c.chipBg }}
                        >
            ...
            <CTAButton onClick={handleSubmit} disabled={!allUploaded}>
              <div className="flex items-center gap-2 justify-center">
                <Upload size={16} /> Nộp bằng chứng (
                {items.filter((i) => i.status === 'uploaded').length})
              </div>
            </CTAButton>
          </div>
        )}
...
export function ChatSafetyBanner({ type = 'warning' }: { type?: 'warning' | 'info' }) {
  ...
  return (
    <div
      className="flex items-start gap-2 px-3 py-2 rounded-xl mx-2 mb-2"
...
export function ChatLinkWarningInline() {
  return (
    <div
      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg mb-1 inline-flex"
  ...
  return (
    <BottomSheetV2 open={open} onClose={handleClose} title="Báo cáo tin nhắn">
      ...
      <TrCard className="p-3 mb-4">
        ...
        <p
          style={{ color: c.text1, fontSize: φ.xs, lineHeight: 1.5, fontStyle: 'italic' }}
      ...
      <p style={{ color: c.text1, fontSize: φ.sm, fontWeight: 600, marginBottom: 8 }}>
        Lý do báo cáo
      </p>
