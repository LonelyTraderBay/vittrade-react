    ...
    <BottomSheetV2 open={open} onClose={onClose} title="Biên nhận kết quả">
      <div className="flex flex-col gap-4">
        ...
        >
          ...
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: φ.xs, marginBottom: 4 }}>Kết quả</p>
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                ...
                style={{ color: c.text1, fontSize: φ.sm, fontWeight: 600, lineHeight: 1.4 }}
              >
                {data.challengeTitle}
              </span>
            </div>
              ...
              return (
                <div key={p.name} className="flex items-center gap-3" style={{ minHeight: 40 }}>
                  ...
                  <span
                    style={{
                      color:
                        p.pointsChange > 0 ? '#10B981' : p.pointsChange < 0 ? '#EF4444' : c.text3,
                      fontSize: φ.xs,
                      fontWeight: 700,
                  ...
                  >
                    {p.pointsChange > 0 ? '+' : ''}
                    {fmtPoints(Math.abs(p.pointsChange))}
                  </span>
                </div>
              );
            })}
        ...
        <TrCard className="p-4">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className="text-right flex-1"
                style={{ color: c.text1, fontSize: φ.xs, lineHeight: 1.4 }}
              >
          ...
          <TrCard className="p-4">
            ...
            <div className="flex flex-col gap-2">
              {[
                {
                  label: 'Gross Pool',
                  value: fmtPoints(data.feeBreakdown.grossPool),
                  color: c.text1,
                  bold: true,
                },
                {
                  label: `Phí vận hành (${data.feeBreakdown.platformFeePct}%)`,
                  value: `−${fmtPoints(data.feeBreakdown.platformFeeAmt)}`,
                  color: '#EF4444',
                },
                ...(data.feeBreakdown.creatorCutPct > 0
                  ? [
                      {
                        label: `Creator cut (${data.feeBreakdown.creatorCutPct}%)`,
                        value: `−${fmtPoints(data.feeBreakdown.creatorCutAmt)}`,
                        color: '#EF4444',
                      },
                    ]
                  : []),
                ...(data.feeBreakdown.consolationPct > 0
                  ? [
                      {
                        label: `Thưởng an ủi (${data.feeBreakdown.consolationPct}%)`,
                        value: `−${fmtPoints(data.feeBreakdown.consolationAmt)}`,
                        color: '#8B5CF6',
                      },
                    ]
                  : []),
                {
                  label: 'Net Pool (đã phân bổ)',
                  value: fmtPoints(data.feeBreakdown.netPool),
                  color: '#10B981',
                  bold: true,
                },
              ].map((row, i) => (
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-start gap-2">
              ...
              <p style={{ color: c.text3, fontSize: 10, lineHeight: 1.5 }}>
                Arena Points (pts) chỉ dùng trong hệ thống Open Arena. Pts{' '}
                <span style={{ fontWeight: 700 }}>không phải tài sản tài chính</span>, không có giá
                trị tiền tệ, không thể quy đổi ra tiền thật.
              </p>
            </div>
            <div className="flex items-start gap-2">
              <CheckCircle2 size={10} color="#10B981" className="shrink-0 mt-0.5" />
              <p style={{ color: c.text3, fontSize: 10, lineHeight: 1.5 }}>
                Biên nhận được tạo tự động, không thể chỉnh sửa, và được ghi nhận vào sổ điểm minh
                bạch.
              </p>
            </div>
            <div className="flex items-start gap-2">
              ...
              <p style={{ color: c.text3, fontSize: 10, lineHeight: 1.5 }}>
                Nếu bạn cho rằng kết quả không chính xác, hãy sử dụng tính năng "Tranh chấp kết quả"
                trong vòng 48 giờ.
              </p>
            </div>
          </div>
...
export function buildReceiptFromResolution(
  ...
  if (resolution.resultProposal) {
    ...
    challenge.participants.forEach((p, i) => {
      participants.push({
        ...
        pointsChange:
          i === 0 ? Math.round(challenge.prizePool * 0.6) : -Math.round(challenge.prizePool * 0.3),
      });
    });
  }

  const lastSettled = resolution.timeline.filter((t) => t.done);
  const settlementTime = lastSettled.length > 0 ? lastSettled[lastSettled.length - 1].date : '—';
  ...
  return {
    ...
    feeBreakdown: (() => {
      ...
      const conPct =
        challenge.consolationEnabled && challenge.consolationPct ? challenge.consolationPct : 0;
