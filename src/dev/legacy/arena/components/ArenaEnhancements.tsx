...
export function LockedBalanceBreakdown({
  totalBalance,
  lockedBalance,
  activeChallenges,
}: LockedBalanceBreakdownProps) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <div className="flex items-baseline gap-0.5" style={{ fontFamily: 'monospace' }}>
        ...
        <span style={{ color, fontSize, marginLeft: 1, marginRight: 1 }}>:</span>
        ...
        <div className="flex gap-2">
          {canEdit && (
            <button
              ...
              style={{
                background: c.accentAlpha08,
                border: `1px solid ${c.accentAlpha15}`,
                color: c.accent,
                fontSize: φ.xs,
                fontWeight: 600,
                minHeight: 44,
...
export function CancelRoomSheet({
  open,
  onClose,
  challengeTitle,
  slotsFilled,
      ...
      >
        <div
          ...
          style={{
            ...
            border: confirmed
              ? `2px solid ${isVoid ? '#F59E0B' : '#EF4444'}`
              : `2px solid ${c.borderSolid}`,
          }}
        >
          {confirmed && <CheckCircle2 size={14} color="#fff" />}
        </div>
        <span style={{ color: c.text2, fontSize: φ.xs, lineHeight: 1.6 }}>
          Tôi hiểu rằng {isVoid ? 'void' : 'hủy'} sẽ hoàn trả tất cả entry points và không thể hoàn
          tác.
        </span>
      </button>
...
export function StepUpAuthSheet({
  open,
  onClose,
  onSuccess,
  actionLabel,
  riskLevel = 'high',
}: StepUpAuthSheetProps) {
  const c = useThemeColors();
  const { hapticSelection, hapticSuccess } = useHaptic();
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: hexToRgba(riskColor, 8),
          border: `1px solid ${hexToRgba(riskColor, 20)}`,
        }}
      >
        ...
        <div className="contents">
          ...
          {authMethod === 'pin' && (
            <div className="contents">
              ...
              {error && (
                <p
                  style={{ color: '#EF4444', fontSize: φ.xs, textAlign: 'center', marginBottom: 8 }}
                >
                  {error}
                </p>
              )}
