...
import { SkeletonCard, SkeletonRow } from '@/shared/ui/SkeletonBlock';
...
import { PolicyInfoLink } from '@/dev/legacy/arena/components/ArenaSafetyTrust';
import {
  TrustBreakdownSheet,
  buildTrustBreakdownFromCreator,
} from '@/dev/legacy/arena/components/ArenaSafetyTrust';
import { ReportDialog } from '@/dev/legacy/arena/components/ArenaModeration';
...
export function ArenaCreatorPage() {
  ...
  if (!creator) {
    return (
      <PageLayout>
        ...
        <EmptyState
          icon={AlertTriangle}
          title="Không tìm thấy creator"
          subtitle="Creator không tồn tại"
        />
      </PageLayout>
    );
  }
  ...
  const creatorRooms = ARENA_ROOMS.filter(
    (r) => creatorModes.some((m) => m.id === r.modeId) || r.creator.name === creator.name,
  );
        ...
        <PageContent padding="compact" gap="default">
            ...
            <TrCard className="p-5">
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  {
                    label: 'Modes',
                    value: creator.modesCreated.toString(),
                    color: '#3B82F6',
                    tappable: false,
                  },
                  {
                    label: 'Phòng HT',
                    value: (creator.completedRooms || creator.totalChallenges).toString(),
                    color: '#8B5CF6',
                    tappable: false,
                  },
                  {
                    label: 'Clone',
                    value: fmtPoints(creator.totalClones || 0),
                    color: '#10B981',
                    tappable: false,
                  },
                  {
                  ...
                  s.tappable ? (
                    <button
                      ...
                      style={{
                        ...
                        border: `1.5px solid ${hexToRgba(s.color, 25)}`,
                        minHeight: 44,
                      }}
                    >
                      <p
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={() => {
                  navigate(`${prefix}/arena/trust/${creator.id}`);
                  hapticSelection();
                }}
                  ...
                  },
                  {
                    ...
                    value:
                      creator.disputeRate !== undefined
                        ? `${(creator.disputeRate * 100).toFixed(1)}%`
                        : '—',
                    color: (creator.disputeRate || 0) < 0.05 ? '#10B981' : '#EF4444',
                    icon: Flag,
                  },
                  {
                    label: 'Hoàn thành',
                    value:
                      creator.completionRate !== undefined ? `${creator.completionRate}%` : '—',
                    color: '#3B82F6',
                    icon: CheckCircle2,
                  },
                  {
                    label: 'Đánh giá CĐ',
                    value:
                      creator.communityRating !== undefined ? `${creator.communityRating}/5` : '—',
                    color: '#F59E0B',
                    icon: Star,
                  },
                ].map((s) => (
              ...
              <div className="contents">
                {creatorModes.length === 0 ? (
                  <EmptyState
                    icon={Zap}
                    title="Chưa có mode"
                    subtitle="Creator chưa tạo mode nào"
                  />
                ) : (
                  <TrCard overflow>
                    {creatorModes.map((mode, i) => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            borderBottom:
                              i < creatorModes.length - 1 ? `1px solid ${c.divider}` : 'none',
                            minHeight: 52,
                          }}
                        >
                          <div
                            ...
                            style={{
                              background: hexToRgba(tpl?.color || '#3B82F6', 15),
                              fontSize: 18,
                            }}
                          >
                            {tpl?.icon}
                ...
                {creatorModes.length > 0 && (
                  <div className="flex gap-2 mt-4">
                    <button
                      onClick={() => {
                        navigate(`${prefix}/arena/mode/${creatorModes[0].id}`);
                        hapticSelection();
                      }}
              ...
              <div className="contents">
                {liveRooms.length === 0 ? (
                  <EmptyState
                    ...
                    subtitle="Hiện chưa có phòng nào đang mở"
                  />
                ) : (
                  <TrCard overflow>
                    {liveRooms.map((room, i) => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            borderBottom:
                              i < liveRooms.length - 1 ? `1px solid ${c.divider}` : 'none',
                            minHeight: 52,
                          }}
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <p style={{ color: c.text3, fontSize: φ.xs }}>
                              {room.slotsFilled}/{room.slotsTotal} · {fmtPoints(room.entryPoints)}{' '}
                              pts
                            </p>
                          </div>
                          <span
                            className="px-2.5 py-1 rounded-lg shrink-0 ml-2"
                            style={{
                              background: hexToRgba(st.color, 15),
                              color: st.color,
                              fontSize: 10,
                              fontWeight: 600,
                            }}
                ...
                {historyRooms.length === 0 ? (
                  <div className="contents">
                    <TrCard className="p-5">
                      <div className="flex flex-col items-center text-center gap-3 py-4">
                        ...
                        <p style={{ color: c.text2, fontSize: φ.sm }}>
                          Chưa có lịch sử phòng hoàn thành. Các challenge đã kết thúc sẽ hiển thị ở
                          đây.
                        </p>
                      </div>
                    </TrCard>
                    ...
                    <TrCard className="mt-3" overflow>
                      {[
                        {
                          title: 'BTC $68K Predict — Tuần 7',
                          slots: '50/50',
                          pts: '4,800 pts',
                          status: 'Hoàn tất',
                        },
                        {
                          title: 'ETH Merge Predict',
                          slots: '30/30',
                          pts: '2,700 pts',
                          status: 'Hoàn tất',
                        },
                        {
                          title: 'Altcoin Battle — Tuần 5',
                          slots: '40/40',
                          pts: '7,200 pts',
                          status: 'Hoàn tất',
                        },
                  ...
                  <TrCard overflow>
                    {historyRooms.map((room, i) => (
                      <div
                        ...
                        style={{
                          borderBottom:
                            i < historyRooms.length - 1 ? `1px solid ${c.divider}` : 'none',
                        }}
                      >
                        <div className="flex-1 min-w-0">
                          <p
