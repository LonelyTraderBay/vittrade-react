...
import { StatusChip, TrustBadge } from '@/dev/legacy/arena/components/ArenaChips';
import {
  ArenaEmptyRooms,
  ArenaEmptyJoined,
  ArenaEmptySavedModes,
  ArenaEmptyDrafts,
  ArenaEmptyHistory,
  ArenaLoadingSkeleton,
} from '@/dev/legacy/arena/components/ArenaStates';
import { ArenaPageFooter } from '@/dev/legacy/arena/components/ArenaPageFooter';
import { RewardAnalyticsCard } from '@/dev/legacy/arena/components/ArenaRewardComponents';
...
const MY_ROOMS = ARENA_CHALLENGES.filter(
  (ch) =>
    ch.challengeState === 'open' || ch.challengeState === 'live' || ch.challengeState === 'full',
).slice(0, 3);

// Joined = challenges I participate in
const JOINED_CHALLENGES = ARENA_CHALLENGES.filter(
  (ch) =>
    ch.challengeState === 'live' ||
    ch.challengeState === 'pending_result' ||
    ch.challengeState === 'open',
).slice(0, 4);
...
const DRAFT_CHALLENGES: {
  ...
  {
    id: 'draft002',
    title: 'DeFi Quiz Night (chưa hoàn thành)',
    format: 'Bracket',
    updated: '25/02 18:00',
    entryPoints: 200,
  },
...
const HISTORY_CHALLENGES = ARENA_CHALLENGES.filter(
  (ch) => ch.challengeState === 'resolved' || ch.challengeState === 'canceled',
);
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          <span style={{ color: c.text3, fontSize: φ.xs }}>
            {ch.slotsFilled}/{ch.slotsTotal}
          </span>
          <span style={{ color: c.text3, fontSize: 10 }}>·</span>
          <span style={{ color: '#F59E0B', fontSize: φ.xs, fontWeight: 600 }}>
            {fmtPoints(ch.entryPoints)} pts
          </span>
        </div>
      </div>
...
export function MyArenaPage() {
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <ArenaLoadingSkeleton variant="home" />
      ) : (
        <PageContent padding="compact" gap="default">
            ...
            <TrCard className="p-0 overflow-hidden">
              ...
              <div className="px-5 pt-5 pb-4">
                <div className="flex items-center justify-between mb-1">
                  <p
                    style={{ color: c.text3, fontSize: φ.sm, fontWeight: 600, letterSpacing: 0.3 }}
                  >
                    Arena Points
                  </p>
                  <button
                    onClick={() => {
                      navigate(`${prefix}/arena/points`);
                      hapticSelection();
                    }}
              ...
              <div className="px-5 py-4 flex items-center gap-4">
                ...
                <button
                  onClick={() => {
                    navigate(`${prefix}/rewards?tab=arena`);
                    hapticSelection();
                  }}
                  ...
                  <TrCard overflow>
                    {MY_ROOMS.map((ch, i) => (
                      <div
                        key={ch.id}
                        style={{
                          borderBottom: i < MY_ROOMS.length - 1 ? `1px solid ${c.divider}` : 'none',
                        }}
                      >
                        <ChallengeRow
                          ch={ch}
                          onPress={() => {
                            navigate(`${prefix}/arena/challenge/${ch.id}`);
                            hapticSelection();
                          }}
                        />
                      </div>
                  ...
                  <TrCard overflow>
                    {JOINED_CHALLENGES.map((ch, i) => (
                      <div
                        ...
                        style={{
                          borderBottom:
                            i < JOINED_CHALLENGES.length - 1 ? `1px solid ${c.divider}` : 'none',
                        }}
                      >
                        <ChallengeRow
                          ch={ch}
                  ...
                  <TrCard overflow>
                    {SAVED_MODES.map((mode, i) => (
                      <button
                        ...
                        style={{
                          borderBottom:
                            i < SAVED_MODES.length - 1 ? `1px solid ${c.divider}` : 'none',
                          minHeight: 64,
                        }}
                      >
                        <div className="flex-1 min-w-0">
                  ...
                  <TrCard overflow>
                    {DRAFT_CHALLENGES.map((draft, i) => (
                      <button
                        ...
                        style={{
                          borderBottom:
                            i < DRAFT_CHALLENGES.length - 1 ? `1px solid ${c.divider}` : 'none',
                          minHeight: 64,
                        }}
                      >
                        <div
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0 ml-2">
                          ...
                          <span style={{ color: '#F59E0B', fontSize: φ.xs, fontWeight: 600 }}>
                            {fmtPoints(draft.entryPoints)} pts
                          </span>
                        </div>
                      </button>
                    ))}
                    ...
                    {HISTORY_CHALLENGES.map((ch, i) => {
                      return (
                        <button
                          ...
                          style={{
                            borderBottom:
                              i < HISTORY_CHALLENGES.length - 1 ? `1px solid ${c.divider}` : 'none',
                            minHeight: 64,
                          }}
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <span style={{ color: c.text3, fontSize: 10 }}>·</span>
                              <span style={{ color: c.text3, fontSize: φ.xs }}>
                                {fmtPoints(ch.prizePool)} pts pool
                              </span>
                              <span style={{ color: c.text3, fontSize: 10 }}>·</span>
                              <span style={{ color: c.text3, fontSize: φ.xs }}>
                                {ch.slotsFilled}/{ch.slotsTotal}
          ...
          {stats.modesCreated > 0 && (
            <div>
              <SectionHeader
                title={`Mode đã tạo (${stats.modesCreated})`}
                accent
                accentColor="#8B5CF6"
              />
              <TrCard
          ...
          {stats.rewardHistory && (
            <div>
              ...
              <RewardAnalyticsCard
                ...
                onViewChallenge={(id) => {
                  navigate(`${prefix}/arena/challenge/${id}`);
                  hapticSelection();
                }}
              />
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <TrCard
                ...
                as="button"
                onClick={() => {
                  navigate(`${prefix}/profile/arena/reports`);
                  hapticSelection();
                }}
              ...
              >
                ...
                <div className="flex-1 text-left">
                  ...
                  <p style={{ color: c.text3, fontSize: φ.xs }}>
                    Theo dõi tiến trình xử lý báo cáo
                  </p>
                </div>
                <ChevronRight size={16} color={c.text3} />
              </TrCard>
              <TrCard
                ...
                as="button"
                onClick={() => {
                  navigate(`${prefix}/profile/arena/blocked`);
                  hapticSelection();
                }}
                className="flex items-center gap-3 p-4 w-full active:opacity-70"
                style={{ minHeight: 52 }}
