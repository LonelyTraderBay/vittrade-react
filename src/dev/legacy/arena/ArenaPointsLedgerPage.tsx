  ...
  return (
    <PageLayout>
      ...
      <PageContent gap="default">
        ...
        <TrCard className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p
                style={{ color: c.text1, fontSize: φ.md, fontWeight: 700, fontFamily: 'monospace' }}
              >
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <p style={{ color: '#10B981', fontSize: φ.xs, fontWeight: 600 }}>
                  +{fmtPoints(MY_ARENA_STATS.pointsEarned)}
                </p>
                <p style={{ color: c.text3, fontSize: 9 }}>Đã nhận</p>
              </div>
              <div className="text-right">
                <p style={{ color: '#EF4444', fontSize: φ.xs, fontWeight: 600 }}>
                  -{fmtPoints(MY_ARENA_STATS.pointsSpent)}
                </p>
        ...
        {/* ─── Entry count ─── */}
        <p style={{ color: c.text3, fontSize: φ.xs }}>{filtered.length} bản ghi</p>
            ...
            {filtered.map((entry, i) => {
              ...
              return (
                <button
                  key={entry.id}
                  onClick={() => {
                    navigate(`${prefix}/arena/ledger/entry/${entry.id}`);
                    hapticSelection();
                  }}
                ...
                >
                  ...
                  <div className="flex flex-col items-end shrink-0 ml-2">
                    <span
                      style={{
                        color:
                          entry.amount > 0 ? '#10B981' : entry.amount < 0 ? '#EF4444' : c.text3,
                        fontSize: φ.sm,
                        fontWeight: 700,
                    ...
                    >
                      {entry.amount > 0 ? '+' : ''}
                      {entry.amount === 0 ? '0' : fmtPoints(Math.abs(entry.amount))}
                    </span>
        ...
        <TrCard className="p-3 flex items-start gap-2">
          ...
          <p style={{ color: c.text3, fontSize: φ.xs, lineHeight: 1.5 }}>
            Mọi thay đổi Arena Points đều được ghi lại với mã tham chiếu duy nhất. Arena Points
            không phải tài sản tài chính và không có giá trị tiền tệ.
          </p>
        </TrCard>
