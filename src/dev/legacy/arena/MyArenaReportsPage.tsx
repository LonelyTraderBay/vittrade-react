...
import { Flag, AlertTriangle, Filter, Info } from 'lucide-react';
...
import { ReportCaseRow } from '@/dev/legacy/arena/components/ArenaModerationCases';
...
import { ARENA_REPORT_CASES, type ReportCaseStatus } from '@/dev/mocks/arena-fixtures';
...
export function MyArenaReportsPage() {
  ...
  const filteredCases =
    filter === 'all' ? ARENA_REPORT_CASES : ARENA_REPORT_CASES.filter((r) => r.status === filter);
        ...
        <PageContent padding="compact">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              {
                label: 'Đã giải quyết',
                value: ((statusCounts.action_taken || 0) + (statusCounts.closed || 0)).toString(),
                color: '#10B981',
              },
            ].map((s) => (
          ...
          <div className="flex gap-1.5 overflow-x-auto -mx-5 px-5 no-scrollbar">
            {FILTER_CHIPS.map((chip) => {
              ...
              const count =
                chip.id === 'all' ? ARENA_REPORT_CASES.length : statusCounts[chip.id] || 0;
              return (
                <button
                  key={chip.id}
                  onClick={() => {
              ...
              <TrCard overflow>
                {filteredCases.map((reportCase, i) => (
                  <ReportCaseRow
                    ...
                    onPress={() => {
                      navigate(`${prefix}/profile/arena/reports/${reportCase.id}`);
                      hapticSelection();
                    }}
          ...
          {filteredCases.length > 0 && (
            <p style={{ color: c.text3, fontSize: φ.xs, textAlign: 'center' }}>
              {filteredCases.length} báo cáo
              {filter !== 'all' ? ` (${FILTER_CHIPS.find((c) => c.id === filter)?.label})` : ''}
            </p>
          )}
        </PageContent>
      )}
