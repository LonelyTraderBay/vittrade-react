        ...
        return (
          <div key={rung.id} className="flex items-center gap-2 h-8">
            ...
            <div className="flex-1 relative h-5">
              <div
                className="absolute inset-y-0 left-0 h-full rounded-r-lg flex items-center px-2"
                style={{
...
export function SavingsLadderPage() {
  ...
  const weightedAPY =
    rungs.length > 0 ? rungs.reduce((s, r) => s + r.apy * r.amount, 0) / totalAllocated : 0;
  const estimatedAnnualInterest = rungs.reduce((s, r) => s + (r.amount * r.apy) / 100, 0);
  const nextMaturity =
    rungs.length > 0 ? [...rungs].sort((a, b) => a.lockDays - b.lockDays)[0] : null;
  const totalRungs = rungs.length;
  const avgLockDays =
    rungs.length > 0 ? Math.round(rungs.reduce((s, r) => s + r.lockDays, 0) / rungs.length) : 0;
  ...
  const liquidityScore = useMemo(() => {
    ...
    const shortTermPct =
      (rungs.filter((r) => r.lockDays <= 30).reduce((s, r) => s + r.amount, 0) / totalAllocated) *
      100;
  ...
  const liquidityColor =
    liquidityScore >= 70 ? '#10B981' : liquidityScore >= 40 ? '#F59E0B' : '#EF4444';
  const liquidityLabel =
    liquidityScore >= 70 ? 'Cao' : liquidityScore >= 40 ? 'Trung bình' : 'Thấp';
  ...
  const handleAddProduct = useCallback(
    (product: (typeof AVAILABLE_PRODUCTS)[0]) => {
  ...
  return (
    <PageLayout variant={showConfirmSheet ? 'default' : 'default'}>
      ...
      <BottomSheetV2
        open={!!showRungDetail}
        onClose={() => setShowRungDetail(null)}
        title="Chi tiết bậc ladder"
      >
        {showRungDetail &&
          (() => {
            ...
            return (
              <div className="flex flex-col gap-4">
                ...
                <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
                  ...
                  <BottomSheetRow
                    label="Dự kiến lãi"
                    value={fmtUsd((((r.amount * r.apy) / 100) * r.lockDays) / 365)}
                    valueColor="#10B981"
                  />
                  <BottomSheetRow
                    label="Tự động gia hạn"
                    value={r.autoRenew ? 'Bật' : 'Tắt'}
                ...
                {r.autoRenew && (
                  ...
                  >
                    ...
                    <p style={{ color: c.text2, fontSize: FONT_SCALE.micro, lineHeight: 1.5 }}>
                      Khi đáo hạn, vốn + lãi sẽ tự động gửi lại cùng sản phẩm với APY tại thời điểm
                      đó. Bạn có thể tắt tự động gia hạn bất kỳ lúc nào trước ngày đáo hạn.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  <CTAButton
                    variant="secondary"
                    onClick={() => {
                      handleRemoveRung(r.id);
                      setShowRungDetail(null);
                    }}
                    className="flex-1"
                  >
      ...
      <BottomSheetV2
        open={showAddRung}
        onClose={() => setShowAddRung(false)}
        title="Thêm bậc ladder"
      >
          ...
          {['USDT', 'BTC', 'ETH', 'SOL', 'AVAX'].map((asset) => {
            ...
            return (
              <div key={asset}>
                <button
                  onClick={() => {
                    setExpandedProduct(isExpanded ? null : asset);
                    hapticLight();
                  }}
                ...
                >
                  ...
                  {isExpanded ? (
                    <ChevronUp size={ICON_SIZE.sm} color={c.text3} />
                  ) : (
                    <ChevronDown size={ICON_SIZE.sm} color={c.text3} />
                  )}
                </button>
                {isExpanded && (
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
            ...
            <BottomSheetRow
              ...
              value={fmtUsd(estimatedAnnualInterest)}
              valueColor="#10B981"
            />
            <BottomSheetRow
              label="Đáo hạn gần nhất"
              value={nextMaturity?.maturityDate ?? '-'}
              valueColor="#F59E0B"
            />
            <BottomSheetRow
              label="Thanh khoản"
              value={`${liquidityScore}/100 (${liquidityLabel})`}
              valueColor={liquidityColor}
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5 max-h-[200px] overflow-y-auto">
            {rungs.map((r, i) => (
              ...
              >
                ...
                >
                  <span style={{ color: r.color, fontSize: 8, fontWeight: FONT_WEIGHT.bold }}>
                    {i + 1}
                  </span>
                </div>
                <span style={{ color: c.text1, fontSize: FONT_SCALE.xs, flex: 1 }}>
                  {r.product}
                </span>
                <span
                  style={{
                ...
                <span
                  style={{ color: '#10B981', fontSize: FONT_SCALE.micro, fontFamily: 'monospace' }}
                >
                  {r.apy}%
                </span>
              </div>
          ...
          {unallocated > 1 && (
            ...
            >
              ...
              <p style={{ color: c.text2, fontSize: FONT_SCALE.micro, lineHeight: 1.5 }}>
                Còn {fmtUsd(unallocated)} chưa được phân bổ. Bạn có muốn thêm bậc hoặc điều chỉnh
                không?
              </p>
            </div>
          )}
          ...
          >
            <AlertTriangle size={ICON_SIZE.sm} color="#F59E0B" className="mt-0.5 shrink-0" />
            <p style={{ color: c.text2, fontSize: FONT_SCALE.micro, lineHeight: 1.5 }}>
              Sản phẩm cố định không thể rút sớm hoặc sẽ mất lãi nếu rút trước hạn. APY có thể thay
              đổi khi gia hạn. Đây không phải lời khuyên đầu tư.
            </p>
          </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {t.intervals.length > 0 && (
                        <p style={{ color: c.text3, fontSize: FONT_SCALE.micro, marginTop: 3 }}>
                          {t.intervals.length} bậc · APY TB:{' '}
                          {t.intervals
                            .reduce((s, iv) => s + (iv.apy * iv.pctAlloc) / 100, 0)
                            .toFixed(1)}
                          %
                        </p>
                      )}
            ...
            {rungs.length === 0 ? (
              <div className="flex flex-col items-center py-10 gap-3">
                ...
                <p style={{ color: c.text2, fontSize: FONT_SCALE.sm }}>
                  Chưa có bậc nào. Chọn template hoặc thêm thủ công.
                </p>
              </div>
            ) : (
              <div className="flex flex-col gap-2">
                {rungs.map((rung, i) => (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        background: rung.color + '22',
                        border: `1.5px solid ${rung.color}40`,
                      }}
                    >
                      <span
                        style={{
          ...
          {rungs.length === 0 ? (
            ...
            <>
              ...
              <PageSection label="Lịch trình đáo hạn">
                <div className="flex flex-col gap-2">
                  {[...rungs]
                    .sort((a, b) => a.lockDays - b.lockDays)
                    .map((rung) => (
                      <div
                        key={rung.id}
                      ...
                      >
                        <div
                          className="w-10 h-12 rounded-xl flex flex-col items-center justify-center shrink-0"
                          style={{
                            background: rung.color + '15',
                            border: `1px solid ${rung.color}30`,
                          }}
                        >
                          <span
                            style={{
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span style={{ color: c.text1, fontSize: FONT_SCALE.xs }}>
                              {rung.maturityDate}
                            </span>
                          </div>
                          <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        {fmtUsd(
                          rungs.reduce(
                            (s, r) => s + (((r.amount * r.apy) / 100) * r.lockDays) / 365,
                            0,
                          ),
                        )}
                      </span>
                ...
                <div className="flex flex-col gap-2">
                  {(() => {
                    const byAsset: Record<
                      string,
                      { total: number; count: number; color: string; avgApy: number }
                    > = {};
                    rungs.forEach((r) => {
                      if (!byAsset[r.asset])
                        byAsset[r.asset] = { total: 0, count: 0, color: r.color, avgApy: 0 };
                      byAsset[r.asset].total += r.amount;
                      byAsset[r.asset].count += 1;
                      byAsset[r.asset].avgApy += r.apy;
                    });
                    Object.keys(byAsset).forEach((k) => {
                      byAsset[k].avgApy /= byAsset[k].count;
                    });
                        ...
                        return (
                          <div key={asset}>
                            ...
                            >
                              <div
                                className="h-full rounded-full"
                                style={{ background: data.color, width: `${pct}%` }}
                              />
                            </div>
                          </div>
                        );
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          >
                            {dayRungs.length} bậc · {pct.toFixed(0)}%
                          </p>
              ...
              <TrCard className="p-4" accentBorder={`${liquidityColor}40`}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    >
                      {fmtUsd(
                        rungs.filter((r) => r.lockDays <= 30).reduce((s, r) => s + r.amount, 0),
                      )}
                    </p>
                  </div>
                  <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    >
                      {fmtUsd(
                        rungs.filter((r) => r.lockDays === 60).reduce((s, r) => s + r.amount, 0),
                      )}
                    </p>
                  </div>
                  <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    >
                      {fmtUsd(
                        rungs.filter((r) => r.lockDays >= 90).reduce((s, r) => s + r.amount, 0),
                      )}
                    </p>
                  </div>
                </div>
