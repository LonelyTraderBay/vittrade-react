...
const SUGGESTIONS: Suggestion[] = [
  ...
  },
  {
    ...
    description:
      'SOL Cố định 30D sắp đáo hạn (22/03). ETH Linh hoạt đang có APY 4.2% — cao hơn SOL Linh hoạt (3.8%). Nên xem xét chuyển.',
    reasoning:
      'SOL Cố định đáo hạn còn 13 ngày. ETH Linh hoạt APY vượt SOL Linh hoạt 0.4%. Rủi ro tương đương (thấp). Thanh khoản ETH tốt hơn.',
    impact: '+0.4% APY khi chuyển',
    impactValue: 0.4,
    impactType: 'apy_gain',
    fromProduct: 'SOL Cố định 30D',
    toProduct: 'ETH Linh hoạt',
    fromAPY: 6.5,
    toAPY: 4.2,
    action: 'Xem chi tiết',
  ...
  },
  {
    ...
    description:
      'USDT Linh hoạt của bạn chưa bật lãi kép tự động. Bật lãi kép có thể tăng thu nhập thêm ~$12/năm với số dư hiện tại.',
    reasoning:
      'Số dư $3,500 × 4.5% APY. Lãi kép hằng ngày tăng hiệu quả thêm ~0.34% APY thực tế. Không có rủi ro thêm.',
    impact: '+~$12/năm',
    impactValue: 12,
    impactType: 'earning_boost',
    toProduct: 'USDT Linh hoạt',
    toAPY: 4.5,
    action: 'Bật lãi kép',
  ...
  },
  {
    ...
    description:
      'Tỷ trọng USDT (68%) cao hơn mục tiêu (60%). Nên xem xét tái cân bằng để giảm rủi ro tập trung.',
    reasoning:
      'Drift USDT +8% so với target. Drift BTC -5%. Tái cân bằng giúp đa dạng hóa và giảm rủi ro single-asset.',
  ...
  },
  {
    ...
    description:
      'AVAX Cố định 90D vừa ra mắt với APY 7.2% — cao nhất trong các sản phẩm tương đương rủi ro trung bình.',
    reasoning:
      'APY 7.2% cao hơn trung bình nhóm locked 90D (5.8%). AVAX on-chain staking backing. Hạn mức còn 85%.',
    impact: 'APY 7.2% — cao nhất nhóm',
    impactValue: 7.2,
    impactType: 'apy_gain',
    toProduct: 'AVAX Cố định 90D',
    toAPY: 7.2,
    action: 'Xem sản phẩm',
    confidence: 65,
    createdAt: '06/03/2026 11:30',
    tags: ['Moi', 'AVAX', 'Locked'],
  },
  {
    id: 'sg6',
    type: 'risk_alert',
    ...
    description:
      'APY SOL Cố định giảm từ 7.5% xuống 6.5%. Xu hướng giảm có thể tiếp tục. Cần đánh giá lại vị thế.',
    reasoning:
...
const MARKET_SIGNALS: MarketSignal[] = [
  ...
  {
    id: 'ms2',
    title: 'Ethereum Shanghai upgrade hoàn thành — staking yield ổn định',
    type: 'neutral',
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        style={{
          color,
          fontSize: size === 'sm' ? FONT_SCALE.micro : FONT_SCALE.xs,
...
export function SavingsSmartSuggestionsPage() {
  ...
  const highCount = suggestions.filter(
    (s) => s.priority === 'high' && s.status !== 'dismissed',
  ).length;
  ...
  const handleViewDetail = useCallback(
    (sg: Suggestion) => {
      setSuggestions((prev) =>
        prev.map((s) =>
          s.id === sg.id && s.status === 'new' ? { ...s, status: 'viewed' as const } : s,
        ),
      );
      setShowDetail(sg);
      hapticSelection();
    ...
    <PageLayout>
            ...
            return (
              <div className="flex flex-col gap-4">
                ...
                <div
                  ...
                  style={{
                    background:
                      sg.impactValue >= 0 ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)',
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      style={{
                        color: sg.impactValue >= 0 ? '#10B981' : '#EF4444',
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.bold,
                      }}
                    >
                ...
                <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
                  ...
                  {sg.expiresAt && (
                    <BottomSheetRow label="Hết hạn" value={sg.expiresAt} valueColor="#F59E0B" />
                  )}
                ...
                >
                  ...
                  <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
                    Đây là gợi ý dựa trên phân tích dữ liệu. Không phải lời khuyên tài chính. APY và
                    điều kiện thị trường có thể thay đổi. Luôn đánh giá trước khi hành động.
                  </p>
                </div>
            ...
            const trendIcon =
              trend.trend === 'up'
                ...
                : trend.trend === 'down'
                  ...
                  : ArrowRight;
            const trendColor =
              trend.trend === 'up' ? '#10B981' : trend.trend === 'down' ? '#EF4444' : '#F59E0B';
            const trendLabel =
              trend.trend === 'up' ? 'Tăng' : trend.trend === 'down' ? 'Giảm' : 'On định';
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${trend.color}44`,
                    }}
                  >
                    <span
                      style={{
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="p-2.5 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    <p
                      style={{
                        color:
                          trend.prediction > trend.currentAPY
                            ? '#10B981'
                            : trend.prediction < trend.currentAPY
                              ? '#EF4444'
                              : '#F59E0B',
                ...
                >
                  ...
                  <p style={{ color: c.text2, fontSize: FONT_SCALE.micro, lineHeight: 1.5 }}>
                    Dự báo APY dựa trên xu hướng 30 ngày và điều kiện on-chain. Kết quả thực tế có
                    thể khác biệt.
                  </p>
                </div>
              </div>
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs }}>
              Chuyển đến trang tương ứng để hoàn tất.
            </p>
          </div>
          <button onClick={() => setShowAppliedToast(false)}>
      ...
      {showDismissedToast && (
        ...
        >
          ...
          <button onClick={() => setShowDismissedToast(false)}>
            <X size={ICON_SIZE.sm} color={c.text3} />
          </button>
        </div>
      )}
      ...
      <TrCard variant="hero" rounded="lg" className="mx-5 mt-4 p-5">
        ...
        <div className="flex gap-3">
          <TrCardStat className="flex-1">
            ...
            <p
              style={{ color: '#EF4444', fontSize: FONT_SCALE.base, fontWeight: FONT_WEIGHT.bold }}
            >
              {highCount}
            </p>
          </TrCardStat>
          <TrCardStat className="flex-1">
            ...
            <div className="flex items-center gap-1">
              ...
              >
                {APY_TRENDS.filter((t) => t.trend === 'up').length} tăng
              </p>
            </div>
          </TrCardStat>
          <TrCardStat className="flex-1">
            <p style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>Tin hiệu</p>
            <p style={{ color: '#3B82F6', fontSize: FONT_SCALE.sm, fontWeight: FONT_WEIGHT.bold }}>
          ...
          {filteredSuggestions.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {filteredSuggestions.map((sg) => {
                ...
                const prioInfo = PRIORITY_CONFIG[sg.priority];
                return (
                  <TrCard key={sg.id} hover className="p-4" onClick={() => handleViewDetail(sg)}>
                    {/* Header row */}
