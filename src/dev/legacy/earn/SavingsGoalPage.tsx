...
import { useLoadingState } from '@/dev/legacy/earn/hooks/useLoadingState';
import { TrCard, TrCardStat } from '@/shared/ui/TrCard';
import { CTAButton } from '@/shared/ui/CTAButton';
import { fmtUsd, fmtNum } from '@/shared/lib/formatNumber';
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div style={{ fontSize: 13, fontWeight: 600, color: c.text1 }}>{milestone.label}</div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <span style={{ fontSize: 12, color: c.text3 }}>/ {fmtUsd(goal.targetAmount)}</span>
      </div>
          ...
          <div key={m.id} className="flex items-center gap-1">
            ...
            {m.id !== goal.milestones[goal.milestones.length - 1].id && (
              <div
                ...
                style={{ background: m.unlocked ? goal.color : c.border, minWidth: 12 }}
              />
            )}
          </div>
        ))}
...
export function SavingsGoalPage() {
  ...
  const totalTarget = useMemo(
    () => goals.filter((g) => g.status === 'active').reduce((s, g) => s + g.targetAmount, 0),
    [goals],
  );
  const totalCurrent = useMemo(
    () => goals.filter((g) => g.status === 'active').reduce((s, g) => s + g.currentAmount, 0),
    [goals],
  );
  const totalProgress = totalTarget > 0 ? (totalCurrent / totalTarget) * 100 : 0;
  const totalMilestones = useMemo(
    () => goals.reduce((s, g) => s + g.milestones.length, 0),
    [goals],
  );
  const unlockedMilestones = useMemo(
    () => goals.reduce((s, g) => s + g.milestones.filter((m) => m.unlocked).length, 0),
    [goals],
  );
          ...
          <>
            ...
            <TrCard variant="hero" className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <div style={{ fontSize: 11, color: c.text3, fontWeight: 600 }}>Tổng tiến độ</div>
                  <div
                    style={{
                      fontSize: 24,
                      fontWeight: 700,
                  ...
                  <div style={{ fontSize: 12, color: c.text2 }}>mục tiêu {fmtUsd(totalTarget)}</div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: c.text3, marginTop: 2 }}>
                      Đặt auto-contribute hàng tháng giúp bạn đạt mục tiêu nhanh hơn 2.5x so với
                      đóng góp thủ công.
                    </div>
                  </div>
                </div>
              ...
              <TrCard className="p-3">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: c.text3, marginTop: 2 }}>
                      Mỗi milestone đạt được sẽ mở khóa phần thưởng APY boost, badge hoặc điểm
                      thưởng.
                    </div>
                  </div>
                </div>
      ...
      >
                ...
                {['3', '6', '12', '24', '36'].map((m) => (
                  <button
                    ...
                    style={{
                      ...
                      border:
                        goalMonths === m
                          ? `1.5px solid ${selectedTemplate.color}`
                          : `1px solid ${c.border}`,
              ...
              <div>
                ...
                {goalTarget && monthlyAmount && (
                  <div style={{ fontSize: 11, color: c.text3, marginTop: 4 }}>
                    Ước tính hoàn thành trong{' '}
                    {Math.ceil(Number(goalTarget) / Number(monthlyAmount))} tháng
                  </div>
                )}
              </div>
            )}
            ...
            <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
              ...
              <div className="flex gap-2">
                {[25, 50, 75, 100].map((p) => (
                  ...
                  >
                    <div style={{ fontSize: 12, fontWeight: 700, color: selectedTemplate.color }}>
                      {p}%
                    </div>
        ...
        {createStep === 2 && selectedTemplate && (
          <div className="flex flex-col gap-3 pb-4">
            ...
            <BottomSheetRow
              label="Tự động đóng góp"
              value={autoContribute ? 'Bật' : 'Tắt'}
              valueColor={autoContribute ? '#10B981' : c.text3}
            />
            {autoContribute && (
              <BottomSheetRow label="Số tiền/tháng" value={fmtUsd(Number(monthlyAmount))} />
            )}
            ...
            >
              ...
              <span style={{ fontSize: 11, color: c.text2 }}>
                Số tiền trong mục tiêu sẽ được tính lãi suất theo sản phẩm đã chọn. Bạn có thể rút
                bất cứ lúc nào.
              </span>
            </div>
            ...
            return (
              <div className="flex flex-col gap-4 pb-4">
                ...
                <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
                  <BottomSheetRow
                    label="Ngày bắt đầu"
                    value={new Date(selectedGoal.startDate).toLocaleDateString('vi-VN')}
                  />
                  <BottomSheetRow
                    label="Ngày mục tiêu"
                    value={new Date(selectedGoal.targetDate).toLocaleDateString('vi-VN')}
                  />
                  <BottomSheetRow label="Sản phẩm" value={selectedGoal.linkedProduct} />
                  <BottomSheetRow
                    label="APY"
                    value={`${selectedGoal.linkedProductAPY}%`}
                    valueColor="#10B981"
                  />
                  {selectedGoal.autoContribute && (
                    <BottomSheetRow
                      label="Auto-contribute"
                      value={`${fmtUsd(selectedGoal.monthlyContribution)}/tháng`}
                    />
                  )}
                </div>
