...
import { fmtUsd, fmtAmount } from '@/shared/lib/formatNumber';
...
export function AutoCompoundSettingsPage() {
  ...
  const toggleCompound = (id: string) => {
    hapticSelection();
    setPositions((prev) =>
      prev.map((p) => (p.id === id ? { ...p, autoCompound: !p.autoCompound } : p)),
    );
  };

  const updateFrequency = (id: string, freq: 'daily' | 'weekly' | 'monthly') => {
    hapticLight();
    setPositions((prev) => prev.map((p) => (p.id === id ? { ...p, compoundFrequency: freq } : p)));
  };

  const updateThreshold = (id: string, threshold: number) => {
    setPositions((prev) =>
      prev.map((p) => (p.id === id ? { ...p, compoundThreshold: threshold } : p)),
    );
  };
  ...
  const totalCompounded = positions.reduce((s, p) => {
    const usdVal =
      p.asset === 'USDT'
        ? p.totalCompounded
        : p.asset === 'BTC'
          ? p.totalCompounded * 67543
          : p.asset === 'ETH'
            ? p.totalCompounded * 2800
            ...
            return (
              <div className="flex flex-col gap-4">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p style={{ color: c.text3, fontSize: 11 }}>Lãi tự động cộng vào gốc</p>
                    </div>
                  </div>
                          ...
                          >
                            <div
                              ...
                              style={{
                                ...
                                background:
                                  pos.compoundFrequency === freq.id ? '#10B981' : 'transparent',
                              }}
                            >
                              {pos.compoundFrequency === freq.id && (
                      ...
                      <div className="flex gap-2">
                        {thresholds.map((t) => (
                          <button
                            ...
                            style={{
                              background:
                                pos.compoundThreshold === t ? 'rgba(16,185,129,0.12)' : c.chipBg,
                ...
                <div className="rounded-2xl p-3" style={{ background: c.surface2 }}>
                  <BottomSheetRow label="APY cơ bản" value={`${pos.apy}%`} />
                  <BottomSheetRow
                ...
                {pos.compoundCount > 0 && (
                  <div className="rounded-2xl p-3" style={{ background: c.surface2 }}>
                    <BottomSheetRow
                      label="Tổng đã compound"
                      value={`${fmtAmount(pos.totalCompounded)} ${pos.asset}`}
                    />
                ...
                {/* Save CTA */}
                <CTAButton onClick={saveSettings}>Lưu cài đặt</CTAButton>
              </div>
            );
          })()}
      </BottomSheetV2>
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: 12, marginTop: 4, lineHeight: 1.6 }}>
              Lãi kép tự động cộng phần lãi kiếm được vào số gốc, giúp bạn kiếm lãi trên cả lãi —
              tạo hiệu ứng tăng trưởng nhanh hơn theo thời gian.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: 'Compound hàng ngày có thể tăng APY thêm 0.03-0.15%',
              },
              {
                icon: Clock,
                color: '#3B82F6',
                title: 'Hoàn toàn tự động',
                desc: 'Hệ thống tự compound theo tần suất bạn chọn',
              },
              {
                icon: Shield,
                color: '#8B5CF6',
                title: 'Không phí phát sinh',
                desc: 'Auto-compound miễn phí, không tính phí giao dịch',
              },
              {
                icon: Zap,
                color: '#F59E0B',
                title: 'Ngưỡng tùy chỉnh',
                desc: 'Chỉ compound khi lãi đạt mức tối thiểu bạn đặt',
              },
            ].map((item, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p style={{ color: c.text3, fontSize: 11, lineHeight: 1.4, marginTop: 2 }}>
                    {item.desc}
                  </p>
                </div>
              </div>
          ...
          >
            ...
            <span style={{ color: '#F59E0B', fontSize: 11, lineHeight: 1.5 }}>
              Auto-compound chỉ áp dụng cho sản phẩm linh hoạt (flexible). Sản phẩm cố định (locked)
              trả lãi cuối kỳ, không hỗ trợ compound giữa kỳ.
            </span>
          </div>
        </div>
      </BottomSheetV2>
      ...
      <PageContent gap="default">
        ...
        <TrCard variant="hero" rounded="lg" className="p-5">
          ...
          <div className="flex gap-3">
            ...
            <TrCardStat className="flex-1">
              ...
              <p style={{ color: c.text1, fontSize: 18, fontWeight: 700 }}>{totalCompoundEvents}</p>
            </TrCardStat>
          </div>
        </TrCard>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span style={{ color: c.text3, fontSize: 11 }}>
                            {pos.compoundFrequency === 'daily'
                              ? 'Hàng ngày'
                              : pos.compoundFrequency === 'weekly'
                                ? 'Hàng tuần'
                                : 'Hàng tháng'}
                          </span>
                        </div>
          ...
          <TrCard className="p-4">
            ...
            <div className="flex gap-3">
              <div className="flex-1 rounded-xl p-3 text-center" style={{ background: c.surface2 }}>
                ...
                <p
                  style={{ color: c.text1, fontSize: 16, fontWeight: 700, fontFamily: 'monospace' }}
                >
                  $45.00
                </p>
              ...
              >
                <p style={{ color: '#10B981', fontSize: 10, fontWeight: 600 }}>
                  Compound hàng ngày
                </p>
                <p
                  style={{
            ...
            >
              Hiệu quả compound tăng theo thời gian và số vốn lớn hơn. Số liệu mô phỏng, APY thực tế
              có thể thay đổi.
            </p>
          </TrCard>
        </PageSection>
        ...
        >
          ...
          <span style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
            Auto-compound chỉ khả dụng cho sản phẩm <strong>linh hoạt</strong>. Sản phẩm cố định trả
            lãi cuối kỳ và không hỗ trợ compound giữa kỳ. Phí compound: miễn phí.
          </span>
        </div>
      </PageContent>
    </PageLayout>
