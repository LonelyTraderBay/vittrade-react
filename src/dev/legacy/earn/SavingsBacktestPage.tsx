...
> = {
  conservative: {
    label: 'An toàn',
    desc: 'Ưu tiên stablecoin và linh hoạt, rủi ro thấp',
    icon: Shield,
    color: '#10B981',
    riskLevel: 'Thấp',
    slots: [
  ...
  balanced: {
    label: 'Cân bằng',
    desc: 'Pha trộn linh hoạt và cố định, rủi ro trung bình',
    icon: Target,
    color: '#3B82F6',
  ...
  aggressive: {
    label: 'Tăng trưởng',
    desc: 'Ưu tiên cố định dài hạn, APY cao, rủi ro cao hơn',
    icon: Zap,
    color: '#F59E0B',
    riskLevel: 'Cao',
    slots: [
  ...
  custom: {
    label: 'Tùy chỉnh',
    desc: 'Tự tạo phân bổ theo ý bạn',
    icon: Sliders,
    color: '#8B5CF6',
    riskLevel: 'Tùy chỉnh',
    slots: [
...
function runBacktest(
  ...
  const variance = [
    0.15, -0.08, 0.22, -0.12, 0.18, -0.05, 0.25, -0.1, 0.12, -0.15, 0.2, -0.06, 0.14, -0.09, 0.19,
    -0.11, 0.16, -0.07, 0.23, -0.13, 0.17, -0.04, 0.21, -0.08,
  ];
  ...
  for (let i = 0; i < months; i++) {
    ...
    monthlyBreakdown.push({
      ...
      returnPct: Math.round(monthReturnPct * 1000) / 1000,
      apy: weightedAPY + v * weightedAPY * 0.3,
    });

    if (value > maxValue) maxValue = value;
    const dd = ((maxValue - value) / maxValue) * 100;
    if (dd > maxDrawdown) maxDrawdown = dd;
    if (monthReturnPct > 0) monthsPos++;
    else monthsNeg++;
...
function AllocationRing({ slots, size = 100 }: { slots: AllocationSlot[]; size?: number }) {
  ...
  const gradientStr = segments.map((s) => `${s.color} ${s.start}% ${s.end}%`).join(', ');
...
function AmountSlider({
  ...
  return (
    <div className="relative w-full" style={{ height: 24 }}>
      ...
      />
      <div
        className="absolute top-0.5 w-5 h-5 rounded-full border-2 shadow-md"
...
export function SavingsBacktestPage() {
  ...
  return (
    <PageLayout>
      ...
      <BottomSheetV2
        open={!!showSlotDetail}
        onClose={() => setShowSlotDetail(null)}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
                  ...
                  <BottomSheetRow
                    ...
                    value={fmtUsd((initialAmount * sl.percentage) / 100)}
                  />
                  <BottomSheetRow
                    label="Dự kiến lãi/năm"
                    value={fmtUsd((((initialAmount * sl.percentage) / 100) * sl.avgAPY) / 100)}
                    valueColor="#10B981"
                  />
                </div>
              </div>
      ...
      <BottomSheetV2
        open={showMonthlySheet}
        onClose={() => setShowMonthlySheet(false)}
        title="Lãi hằng tháng"
      >
        {result && (
        ...
        <PageContent padding="compact" gap="default">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p style={{ color: c.text3, fontSize: FONT_SCALE.micro, marginTop: 3 }}>
                        {p.slots.length} sản phẩm · APY TB:{' '}
                        {p.slots
                          .reduce((s, sl) => s + (sl.avgAPY * sl.percentage) / 100, 0)
                          .toFixed(1)}
                        %
                      </p>
                    </div>
            ...
            <TrCard className="p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                  ...
                  >
                    {fmtUsd((initialAmount * weightedAPY) / 100)}
                  </p>
                </div>
              </div>
            </TrCard>
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
              Kết quả mô phỏng dựa trên dữ liệu APY lịch sử và giả định ổn định. Hiệu suất quá khứ
              không đảm bảo kết quả tương lai. Đây không phải lời khuyên đầu tư.
            </p>
          </div>
          ...
          {!result ? (
            ...
            <>
              ...
              <TrCard
                className="p-4"
                accentBorder={
                  result.totalReturn >= 0 ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'
                }
              >
                <div className="flex items-center gap-2 mb-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    <p
                      style={{
                        color: result.sharpeRatio > 2 ? '#10B981' : '#F59E0B',
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.bold,
                      }}
                    >
                ...
                <div style={{ height: 200 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      <ChartGradientDefs
                        key="gradient-defs"
                        gradients={[{ id: 'fillGrowth', color: '#10B981', opacityFrom: 0.2 }]}
                      />
                      ...
                      <ReferenceLine
                        ...
                        strokeDasharray="4 4"
                        label={{
                          value: `Vốn: ${fmtUsd(initialAmount)}`,
                          fill: '#F59E0B',
                          fontSize: 9,
                          position: 'right',
                        }}
                ...
                <div style={{ height: 160 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      <ChartGradientDefs
                        key="gradient-defs"
                        gradients={[{ id: 'fillCumInt', color: '#8B5CF6', opacityFrom: 0.25 }]}
                      />
                      <CartesianGrid key="grid" strokeDasharray="3 3" stroke={c.divider} />
                      <XAxis
                        key="x"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <TrCard className="p-3">
                  ...
                  <p
                    style={{
                      color: result.monthsNegative > 0 ? '#EF4444' : c.text1,
                      fontSize: FONT_SCALE.lg,
                      fontWeight: FONT_WEIGHT.bold,
                    }}
                  >
                    {result.monthsNegative}
                  </p>
                  <p style={{ color: c.text3, fontSize: FONT_SCALE.micro, marginTop: 2 }}>
                    Tháng tệ nhất
                  </p>
              ...
              >
                <AlertTriangle size={ICON_SIZE.sm} color="#F59E0B" className="mt-0.5 shrink-0" />
                <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
                  Kết quả mô phỏng dựa trên dữ liệu lịch sử và giả định APY ổn định. Hiệu suất thực
                  tế có thể khác biệt do biến động thị trường, thay đổi APY, và các yếu tố khác.
                </p>
              </div>
            </>
          )}
      ...
      {tab === 'compare' && (
        <PageContent padding="compact" gap="default">
                ...
                >
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                      ...
                      <p
                        style={{
                          color: cr.maxDrawdown > 1 ? '#EF4444' : '#F59E0B',
                          fontSize: FONT_SCALE.xs,
                          fontWeight: FONT_WEIGHT.bold,
                        }}
                      >
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
              Chiến lược Tăng trưởng có APY hiệu quả cao nhất nhưng cũng có Drawdown lớn hơn. Chiến
              lược An toàn ổn định hơn nhưng lãi thấp hơn. Chọn theo khả năng chịu rủi ro của bạn.
            </p>
          </div>
        </PageContent>
      )}
