...
const MODE_CONFIG: Record<
  AutoPilotMode,
  {
    ...
    switchMinGain: string;
    maxConcentration: string;
  }
> = {
  conservative: {
    label: 'An toàn',
    desc: 'Stablecoin-first, DCA chậm, rebalance ít, chỉ switch khi APY chênh nhiều',
    icon: Shield,
    color: '#10B981',
    dcaFreq: 'Hằng tháng',
  ...
  balanced: {
    label: 'Cân bằng',
    desc: 'Đa dạng hóa, DCA trung bình, rebalance tự động, switch linh hoạt',
    icon: Target,
    color: '#3B82F6',
    dcaFreq: 'Hằng tuần',
  ...
  growth: {
    label: 'Tăng trưởng',
    desc: 'Ưu tiên APY cao, DCA nhanh, rebalance thường xuyên, switch nhanh',
    icon: Zap,
    color: '#F59E0B',
    dcaFreq: 'Hằng ngày',
...
export function SavingsAutoPilotPage() {
  ...
  const totalSavedAPY = actions
    .filter((a) => a.impactValue > 0 && a.type !== 'dca_executed')
    .reduce((s, a) => s + a.impactValue, 0);
  const totalDCA = actions
    .filter((a) => a.type === 'dca_executed' && a.status === 'executed')
    .reduce((s, a) => s + a.impactValue, 0);
  ...
  const performanceMetrics: PerformanceMetric[] = [
    ...
    {
      label: 'Tối ưu APY',
      value: `+${totalSavedAPY.toFixed(1)}%`,
      change: totalSavedAPY,
      icon: Sparkles,
      color: '#F59E0B',
    },
  ];
  ...
  const handleSkipAction = useCallback(
    (id: string) => {
      setActions((prev) =>
        prev.map((a) => (a.id === id ? { ...a, status: 'skipped' as const } : a)),
      );
      hapticLight();
      setShowActionDetail(null);
    },
  ...
  const updateConfig = useCallback(
    <K extends keyof AutoPilotConfig>(key: K, val: AutoPilotConfig[K]) => {
  ...
  const TABS = [
    ...
    {
      id: 'actions' as const,
      label: `Hoạt động (${pendingCount > 0 ? pendingCount : executedCount})`,
    },
  ...
  return (
    <PageLayout>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
                Quản lý tự động toàn bộ tiết kiệm
              </p>
            </div>
          </div>
          ...
          >
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.6 }}>
              AutoPilot sẽ tự động thực hiện DCA, tái cân bằng, tối ưu APY và quản lý rủi ro theo
              chế độ <strong style={{ color: modeInfo.color }}>{modeInfo.label}</strong>.
            </p>
          </div>
          ...
          <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
            ...
            <BottomSheetRow
              label="DCA"
              value={config.dcaEnabled ? 'Bật' : 'Tắt'}
              valueColor={config.dcaEnabled ? '#10B981' : '#EF4444'}
            />
            <BottomSheetRow
              label="Rebalance"
              value={config.rebalanceEnabled ? 'Bật' : 'Tắt'}
              valueColor={config.rebalanceEnabled ? '#10B981' : '#EF4444'}
            />
            <BottomSheetRow
              label="Smart Switch"
              value={config.smartSwitchEnabled ? 'Bật' : 'Tắt'}
              valueColor={config.smartSwitchEnabled ? '#10B981' : '#EF4444'}
            />
            <BottomSheetRow
              label="Phê duyệt thủ công"
              value={config.approvalRequired ? 'Bật' : 'Tắt'}
              valueColor={config.approvalRequired ? '#3B82F6' : '#F59E0B'}
            />
          </div>
          ...
          {config.approvalRequired && (
            ...
            >
              ...
              <p style={{ color: c.text2, fontSize: FONT_SCALE.micro, lineHeight: 1.5 }}>
                Chế độ phê duyệt thủ công đang bật. Mọi hành động chuyển sản phẩm sẽ cần bạn xác
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.micro, lineHeight: 1.5 }}>
              AutoPilot tự động giao dịch với tài sản của bạn. APY và điều kiện thị trường thay đổi
              liên tục. Đây không phải lời khuyên tài chính. Bạn có thể tạm dừng hoặc tắt bất kỳ lúc
              nào.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
                Các hành động tự động sẽ dừng lại
              </p>
            </div>
          </div>
          ...
          >
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.6 }}>
              Khi tạm dừng: DCA sẽ không thực hiện, rebalance/switch tự động sẽ dừng lại. Các vị thế
              hiện tại vẫn giữ nguyên, lãi vẫn được tính bình thường.
            </p>
          </div>
      ...
      <BottomSheetV2
        ...
        onClose={() => setShowActionDetail(null)}
      ...
      {showToast && (
        <div
          ...
          style={{
            ...
            border: `1px solid ${showToast.color}`,
        ...
        >
          ...
          <button onClick={() => setShowToast(null)}>
            <X size={ICON_SIZE.sm} color={c.text3} />
          </button>
        </div>
      )}
      ...
      <TrCard variant="hero" rounded="lg" className="mx-5 mt-4 p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          >
            {isActive ? (
              <Play size={ICON_SIZE.sm} color="#10B981" />
            ) : isPaused ? (
              <Pause size={ICON_SIZE.sm} color="#F59E0B" />
            ) : (
              <Power size={ICON_SIZE.sm} color={c.text3} />
            )}
            <span
              style={{
                color: isActive ? '#10B981' : isPaused ? '#F59E0B' : c.text3,
        ...
        <div className="flex gap-3">
          ...
          <TrCardStat className="flex-1">
            ...
            <p
              style={{
                color: pendingCount > 0 ? '#3B82F6' : c.text1,
                fontSize: FONT_SCALE.base,
                fontWeight: FONT_WEIGHT.bold,
              }}
            >
        ...
        <PageContent padding="compact" gap="default">
          ...
          <PageSection label="Modules đang hoạt động">
            <div className="flex flex-col gap-2">
              {[
                {
                  label: 'DCA tự động',
                  desc: `${MODE_CONFIG[config.mode].dcaFreq} · ${fmtUsd(config.monthlyBudget)}/tháng`,
                ...
                {
                  label: 'Tái cân bằng',
                  desc: `Ngưỡng: ${config.rebalanceThreshold}% drift`,
                  enabled: config.rebalanceEnabled,
                  icon: RefreshCw,
                  color: '#3B82F6',
                ...
                {
                  label: 'Smart Switch',
                  desc: `Min APY gain: ${config.switchMinAPYGain}%`,
                  enabled: config.smartSwitchEnabled,
                  icon: Sparkles,
                  color: '#8B5CF6',
                ...
                },
                {
                  ...
                  desc: 'Tự động tái đầu tư lãi',
                  enabled: config.compoundEnabled,
                  icon: Zap,
                  color: '#06B6D4',
                ...
                {
                  label: 'Risk Guard',
                  desc: `Max single-asset: ${config.maxSingleAssetPct}%`,
                  enabled: config.riskGuardEnabled,
                  icon: Shield,
                  color: '#EF4444',
                ...
                <button
                  key={mod.label}
                  onClick={() => {
                    navigate(`${prefix}${mod.route}`);
                    hapticSelection();
                  }}
                  className="flex items-center gap-3 p-3 rounded-xl text-left"
                  style={{
                    background: c.surface2,
                    border: `1px solid ${mod.enabled ? 'transparent' : c.borderSolid}`,
                  }}
                >
                  <div
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
              AutoPilot kết hợp DCA + Rebalance + Smart Suggestions + Lãi kép thành một hệ thống tự
              động. Bạn có thể tạm dừng bất kỳ lúc nào và mọi hành động đều được ghi nhận.
            </p>
          </div>
        </PageContent>
      )}
                        ...
                        <div className="flex items-start gap-3 mb-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              ...
                              <span
                                className="px-1.5 py-0.5 rounded animate-pulse"
                                style={{
        ...
        <PageContent padding="compact" gap="default">
                ...
                return (
                  ...
                  >
                    <div
                      className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
                      style={{
                        background: config.mode === key ? m.color + '22' : c.borderSolid + '66',
                      }}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2 flex-wrap">
                        ...
                        <span style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>
                          Rebalance: {m.rebalanceThreshold}
                        </span>
                        <span style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>
                          Switch: {m.switchMinGain}
                        </span>
                      </div>
                    </div>
                    <div
          ...
          <PageSection label="Ngân sách hằng tháng (USD)">
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  updateConfig('monthlyBudget', Math.max(0, Number(e.target.value) || 0))
                }
                style={{
            ...
            <div className="flex gap-2 mt-2">
              {[500, 1000, 2000, 5000].map((v) => (
                <button
                  key={v}
                  onClick={() => updateConfig('monthlyBudget', v)}
          ...
          <PageSection label="Modules tự động">
            <div className="flex flex-col gap-2">
              {[
                {
                  key: 'dcaEnabled' as const,
                  label: 'DCA tự động',
                  desc: 'Tự động gửi tiết kiệm theo lịch',
                  icon: Repeat,
                  color: '#10B981',
                },
                {
                  key: 'rebalanceEnabled' as const,
                  label: 'Tái cân bằng tự động',
                  desc: 'Cân bằng lại danh mục khi lệch',
                  icon: RefreshCw,
                  color: '#3B82F6',
                },
                {
                  key: 'smartSwitchEnabled' as const,
                  label: 'Smart Switch',
                  desc: 'Chuyển sản phẩm khi APY tốt hơn',
                  icon: Sparkles,
                  color: '#8B5CF6',
                },
                {
                  key: 'compoundEnabled' as const,
                  label: 'Lãi kép tự động',
                  desc: 'Tự động tái đầu tư tiền lãi',
                  icon: Zap,
                  color: '#06B6D4',
                },
                {
                  key: 'riskGuardEnabled' as const,
                  label: 'Risk Guard',
                  desc: 'Bảo vệ chống tập trung rủi ro',
                  icon: Shield,
                  color: '#EF4444',
                },
              ].map((mod) => (
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
                    Yêu cầu xác nhận trước khi chuyển SP
                  </p>
                </div>
                <Toggle
                  on={config.approvalRequired}
                  onChange={(v) => updateConfig('approvalRequired', v)}
                />
              </div>
              <div
                className="flex items-center gap-3 p-3 rounded-xl"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
                    Nhận thông báo khi AutoPilot hành động
                  </p>
                </div>
                <Toggle
                  on={config.notificationsEnabled}
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="p-3 rounded-xl" style={{ background: c.surface2 }}>
                ...
                <input
                  ...
                  max={80}
                  step={5}
                  value={config.maxSingleAssetPct}
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
              AutoPilot tự động thực hiện giao dịch với tài sản của bạn. Kết quả phụ thuộc vào điều
              kiện thị trường và thay đổi APY. Đây không phải lời khuyên tài chính. Bạn có thể tắt
              bất kỳ lúc nào.
            </p>
          </div>
        </PageContent>
