...
import { ChartGradientDefs } from '@/shared/ui/charts/ChartGradientDefs';
...
const SCENARIOS: ScenarioConfig[] = [
  {
    id: 'apy_crash',
    label: 'APY sụt giảm',
    desc: 'APY toàn bộ sản phẩm giảm 40-60% do thị trường lạnh',
    icon: TrendingDown,
    color: '#EF4444',
    apyMultiplier: 0.45,
    volatility: 0.15,
  ...
  },
  {
    id: 'apy_spike',
    label: 'APY tăng vọt',
    desc: 'APY tăng 50-80% do nhu cầu vay tăng mạnh',
    icon: TrendingUp,
    color: '#10B981',
    apyMultiplier: 1.65,
    volatility: 0.2,
  ...
  },
  {
    ...
    desc: 'Ngân hàng trung ương cắt giảm lãi suất, APY on-chain giảm từ từ',
    icon: Snowflake,
    color: '#3B82F6',
    apyMultiplier: 0.7,
    volatility: 0.05,
  ...
  },
  {
    id: 'market_stress',
    label: 'Stress test',
    desc: 'Khủng hoảng thanh khoản — APY giảm mạnh, biến động cao, rủi ro phá sản',
    icon: CloudLightning,
    color: '#F59E0B',
    apyMultiplier: 0.25,
    volatility: 0.35,
  ...
  },
  {
    id: 'bull_run',
    label: 'Bull market',
    desc: 'Thị trường tăng mạnh, APY tăng đều, thanh khoản dồi dào',
    icon: Flame,
    color: '#F97316',
    apyMultiplier: 2.0,
    volatility: 0.1,
  ...
  },
  {
    id: 'custom',
    label: 'Tùy chỉnh',
    desc: 'Tự chỉnh APY multiplier và volatility theo ý bạn',
    icon: Sliders,
    color: '#8B5CF6',
    apyMultiplier: 1.0,
    volatility: 0.1,
...
function simulateScenario(
  ...
  const baseVariance = [
    0.02, -0.01, 0.03, -0.02, 0.01, -0.01, 0.02, -0.015, 0.025, -0.01, 0.015, -0.02,
  ];
  const scenarioVariance = [
    -0.05, 0.08, -0.12, 0.06, -0.03, 0.1, -0.08, 0.04, -0.06, 0.07, -0.04, 0.05,
  ];
  ...
  for (let i = 0; i < months; i++) {
    ...
    if (scenValue < totalValue && hasRecovered) {
      hasRecovered = false;
      recoveryMonths = 0;
    }
    if (!hasRecovered && scenValue >= totalValue) {
      hasRecovered = true;
    }
...
export function SavingsWhatIfPage() {
  ...
  const weightedAPY = MOCK_PORTFOLIO.reduce(
    (s, p) => s + (p.currentAPY * p.amount) / totalPortfolioValue,
    0,
  );
  ...
  }, [hasRun, customMultiplier, customVolatility, currentScenario, isCustom]);
  ...
  return (
    <PageLayout>
      ...
      <BottomSheetV2
        ...
        onClose={() => setShowScenarioDetail(null)}
            ...
            return (
              <div className="flex flex-col gap-4">
                ...
                <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
                  <BottomSheetRow
                    label="APY thay đổi"
                    value={`×${effMult.toFixed(2)} (${effMult >= 1 ? '+' : ''}${((effMult - 1) * 100).toFixed(0)}%)`}
                    valueColor={effMult >= 1 ? '#10B981' : '#EF4444'}
                  />
                  <BottomSheetRow
                    label="Biến động"
                    value={`${(effVol * 100).toFixed(0)}%`}
                  ...
                  <BottomSheetRow
                    label="APY mô phỏng"
                    value={`${(weightedAPY * effMult).toFixed(2)}%`}
                    valueColor={effMult >= 1 ? '#10B981' : '#EF4444'}
                  />
                </div>
              </div>
      ...
      {/* ─── Asset Impact Sheet ─── */}
      <BottomSheetV2
        open={showAssetImpact}
        onClose={() => setShowAssetImpact(false)}
            ...
            {result.assetImpact.map((ai) => (
              <div key={ai.asset} className="p-3 rounded-xl" style={{ background: c.surface2 }}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  >
                    <span style={{ color: ai.color, fontSize: 8, fontWeight: FONT_WEIGHT.bold }}>
                      {ai.asset}
                    </span>
                  </div>
                  <span
        ...
        <PageContent padding="compact" gap="default">
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background:
                          selectedScenario === s.id ? s.color + '22' : c.borderSolid + '66',
                      }}
                    >
                      <Icon
                        size={ICON_SIZE.sm}
                        color={selectedScenario === s.id ? s.color : c.text3}
                      />
                    </div>
                      ...
                      {s.id !== 'custom' && (
                        <div className="flex items-center gap-3 mt-2">
                          <span style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>
                            APY: {s.apyMultiplier >= 1 ? '+' : ''}
                            {((s.apyMultiplier - 1) * 100).toFixed(0)}%
                          </span>
                          <span style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>
                            Vol: {(s.volatility * 100).toFixed(0)}%
                          </span>
                          <span style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>
                            {s.durationMonths}M
                          </span>
              ...
              <div className="flex flex-col gap-3">
                <div className="p-3 rounded-xl" style={{ background: c.surface2 }}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      style={{
                        color: customMultiplier >= 1 ? '#10B981' : '#EF4444',
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.bold,
                      }}
                    >
                      ×{customMultiplier.toFixed(2)} ({customMultiplier >= 1 ? '+' : ''}
                      {((customMultiplier - 1) * 100).toFixed(0)}%)
                    </span>
                  </div>
                  <input
                    type="range"
                    ...
                    className="w-full"
                    style={{ height: 4, accentColor: '#8B5CF6' }}
                  />
                ...
                <div className="p-3 rounded-xl" style={{ background: c.surface2 }}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      style={{
                        color: customVolatility > 0.2 ? '#EF4444' : '#F59E0B',
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.bold,
                      }}
                    >
                  ...
                  <input
                    ...
                    className="w-full"
                    style={{ height: 4, accentColor: '#F59E0B' }}
                  />
                  <div className="flex justify-between mt-1">
              ...
              <div className="flex flex-col gap-2">
                {MOCK_PORTFOLIO.map((p) => (
                  ...
                  >
                    ...
                    >
                      <span style={{ color: p.color, fontSize: 8, fontWeight: FONT_WEIGHT.bold }}>
                        {p.asset}
                      </span>
                    </div>
                    ...
                    <span
                      style={{ color: c.text2, fontSize: FONT_SCALE.xs, fontFamily: 'monospace' }}
                    >
                      {fmtUsd(p.amount)}
                    </span>
                    <span
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
              Đây là mô phỏng giả định. Kết quả thực tế phụ thuộc vào nhiều yếu tố không thể dự
              đoán. Không phải lời khuyên đầu tư.
            </p>
          </div>
          ...
          {!result ? (
            ...
            <>
              ...
              <TrCard
                className="p-4"
                accentBorder={
                  result.difference >= 0 ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'
                }
              >
                <div className="flex items-center gap-2 mb-3">
                  {React.createElement(currentScenario.icon, {
                    size: ICON_SIZE.sm,
                    color: currentScenario.color,
                  })}
                  <span
                  ...
                  >
                    Kịch bản: {currentScenario.label}
                  </span>
                  <span
                    className="px-1.5 py-0.5 rounded"
                    style={{
                      background: RISK_COLORS[currentScenario.riskLevel] + '15',
                      color: RISK_COLORS[currentScenario.riskLevel],
                      fontSize: FONT_SCALE.micro,
                      fontWeight: FONT_WEIGHT.bold,
                    }}
                  >
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  ...
                  <div
                    ...
                    style={{
                      background:
                        result.difference >= 0 ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)',
                    }}
                  >
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    <p
                      style={{
                        color: result.interestDiff >= 0 ? '#10B981' : '#EF4444',
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.bold,
                        fontFamily: 'monospace',
                      }}
                    >
                      {result.interestDiff >= 0 ? '+' : ''}
                      {fmtUsd(result.interestDiff)}
                    </p>
                  </div>
                  <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    <p
                      style={{
                        color: result.maxDrawdown > 1 ? '#EF4444' : '#F59E0B',
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.bold,
                      }}
                    >
                  ...
                  <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                    ...
                    <p
                      style={{
                        color: result.recoveryMonths === 0 ? '#10B981' : '#F59E0B',
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.bold,
                      }}
                    >
              ...
              <TrCard className="p-4">
                ...
                <div className="flex justify-center gap-4 mt-2">
                  <div className="flex items-center gap-1">
                    <div
                      className="w-3 h-0.5"
                      style={{ background: '#6B7280', borderBottom: '1px dashed #6B7280' }}
                    />
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Tooltip
                        ...
                        formatter={(value: number, name: string) => [
                          `${value.toFixed(2)}%`,
                          name === 'apyBaseline' ? 'APY cơ bản' : 'APY kịch bản',
                        ]}
                      />
                      <Line
                        key="line-base"
              ...
              <div className="grid grid-cols-2 gap-3">
                <TrCard className="p-3">
                  ...
                  <p style={{ color: c.text3, fontSize: FONT_SCALE.micro, marginTop: 4 }}>
                    {currentScenario.durationMonths} tháng
                  </p>
                </TrCard>
                <TrCard className="p-3">
                  ...
                  <p
                    style={{
                      color:
                        result.scenarioInterest >= result.baselineInterest ? '#10B981' : '#EF4444',
                      fontSize: FONT_SCALE.base,
                      fontWeight: FONT_WEIGHT.bold,
                      fontFamily: 'monospace',
                    }}
                  >
                    {result.scenarioInterest >= 0 ? '+' : ''}
                    {fmtUsd(result.scenarioInterest)}
                  </p>
      ...
      {tab === 'stress' && (
        <PageContent padding="compact" gap="default">
          ...
          <TrCard className="p-4">
            ...
            <p
              style={{ color: c.text3, fontSize: FONT_SCALE.xs, lineHeight: 1.5, marginBottom: 12 }}
            >
              Chạy tất cả kịch bản đồng thời để so sánh ảnh hưởng lên danh mục hiện tại (
              {fmtUsd(totalPortfolioValue)}).
            </p>
              ...
              {stressResults.map((sr) => (
                <div key={sr.scenario.id} className="flex items-center gap-1">
                  <div
                    ...
                    style={{ background: sr.scenario.color }}
                  />
                  <span style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>
                    {sr.scenario.label}
                  </span>
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            <span
                              className="px-1.5 py-0.5 rounded"
                              style={{
                                background: RISK_COLORS[sr.scenario.riskLevel] + '15',
                                color: RISK_COLORS[sr.scenario.riskLevel],
                                fontSize: FONT_SCALE.micro,
                                fontWeight: FONT_WEIGHT.bold,
                              }}
                            >
                          ...
                          <p
                            style={{ color: c.text3, fontSize: FONT_SCALE.micro, lineHeight: 1.3 }}
                          >
                            {sr.scenario.desc}
                          </p>
                        </div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        ...
                        <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                          ...
                          <p
                            style={{
                              color: r.maxDrawdown > 1 ? '#EF4444' : '#F59E0B',
                              fontSize: FONT_SCALE.xs,
                              fontWeight: FONT_WEIGHT.bold,
                            }}
                          >
                        ...
                        <div className="p-2 rounded-xl" style={{ background: c.surface2 }}>
                          ...
                          <p
                            style={{
                              color: r.recoveryMonths === 0 ? '#10B981' : '#F59E0B',
                              fontSize: FONT_SCALE.xs,
                              fontWeight: FONT_WEIGHT.bold,
                            }}
                          >
          ...
          {/* Resilience score */}
          {(() => {
            const worstCase = [...stressResults].sort(
              (a, b) => a.result.difference - b.result.difference,
            )[0];
            const bestCase = [...stressResults].sort(
              (a, b) => b.result.difference - a.result.difference,
            )[0];
            const avgDiff =
              stressResults.reduce((s, sr) => s + sr.result.differencePct, 0) /
              stressResults.length;
            const resilienceScore = Math.max(0, Math.min(100, 50 + avgDiff * 5));
            const resColor =
              resilienceScore >= 60 ? '#10B981' : resilienceScore >= 40 ? '#F59E0B' : '#EF4444';
              ...
              <TrCard className="p-4" accentBorder={`${resColor}40`}>
                ...
                <div className="flex items-center gap-4 mb-3">
                  <div
                    ...
                    style={{
                      background: `conic-gradient(${resColor} ${resilienceScore}%, ${c.borderSolid} ${resilienceScore}%)`,
                    }}
                  >
                    <div
                  ...
                  <div className="flex-1">
                    ...
                    >
                      Trường hợp xấu nhất ({worstCase.scenario.label}):{' '}
                      {worstCase.result.differencePct.toFixed(1)}%. Trường hợp tốt nhất (
                      {bestCase.scenario.label}): +{bestCase.result.differencePct.toFixed(1)}%.
                    </p>
                  </div>
                </div>
          ...
          >
            ...
            <div>
              ...
              >
                Gợi ý tăng kháng chịu
              </p>
              <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
                Đa dạng hóa tài sản và pha trộn sản phẩm linh hoạt với cố định giúp giảm rủi ro
                trong kịch bản bất lợi. Cần nhắc tăng tỷ trọng trong stablecoin để đảm bảo ổn định.
              </p>
            </div>
          </div>
          ...
          >
            <AlertTriangle size={ICON_SIZE.sm} color="#F59E0B" className="mt-0.5 shrink-0" />
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
              Stress test chỉ là mô phỏng giả định. Thị trường thực tế có thể biến động hoàn toàn
              khác biệt. Không sử dụng kết quả này làm cơ sở duy nhất cho quyết định tài chính.
            </p>
          </div>
        </PageContent>
      )}
