...
export const p2pKycApi = {
  ...
  uploadIdentity: async (
    ...
    success: boolean;
    ocrData?: { fullName: string; idNumber: string; dateOfBirth: string; address: string };
  }> => {
   ...
   */
  uploadAddressProof: async (_file: File): Promise<{ success: boolean }> => {
    await delay(1000);
    return { success: true };
  },
   ...
   */
  submitSelfie: async (_file: File): Promise<{ success: boolean; faceMatchScore?: number }> => {
   ...
   */
  bookVideoKYC: async (_slotId: string): Promise<{ success: boolean; appointmentId: string }> => {
    await delay(500);
    return {
      success: true,
...
export const p2pSecurityApi = {
   ...
   * Disable 2FA
   */
  disable2FA: async (_password: string): Promise<{ success: boolean }> => {
    await delay(500);
    return { success: true };
  },
...
export const p2pComplianceApi = {
   ...
   */
  submitSourceOfFunds: async (_data: {
    source: string;
    details: string;
  }): Promise<{ success: boolean }> => {
    await delay(500);
   ...
   */
  submitLargeTransactionJustification: async (_data: {
    amount: number;
    purpose: string;
    details: string;
