...
vi.mock('@/features/trading', () => ({
  useCopyProvidersQuery: (query: { sort?: string } = {}) => ({
    isPending: false,
    isError: false,
    refetch: vi.fn(),
    data: {
      items: [...providers].sort((a, b) =>
...
describe('CopyTradingPage', () => {
  ...
  it('renders provider metrics and risk disclosure from the query boundary', () => {
    renderWithRouter(<CopyTradingPage />);
    expect(screen.getByText('Copy Trading')).toBeInTheDocument();
    expect(screen.getByText(/Copy Trading có rủi ro cao/i)).toBeInTheDocument();
    expect(screen.getByText('RiskMaster_88')).toBeInTheDocument();
  });

  it('changes server-side ranking and opens provider detail', async () => {
    const user = userEvent.setup();
    renderWithRouter(<CopyTradingPage />);
    await user.click(screen.getByRole('button', { name: /Nhiều copier/i }));
    await waitFor(() =>
      expect((document.body.textContent ?? '').indexOf('WhaleWatcher')).toBeLessThan(
        (document.body.textContent ?? '').indexOf('RiskMaster_88'),
      ),
    );
    await user.click(screen.getAllByRole('button', { name: /Xem chi tiết/i })[0]);
    await waitFor(() =>
      expect(mockNavigate).toHaveBeenCalledWith(expect.stringContaining('/trade/copy-provider/')),
    );
  });

  it('shows past-performance disclaimer', () => {
    renderWithRouter(<CopyTradingPage />);
    expect(
