...
export function TPSLForm({
  ...
  const calc = useMemo(() => {
    ...
    const tpPnl = side === 'buy' ? (tpNum - entryPrice) * amount : (entryPrice - tpNum) * amount;
    const slPnl = side === 'buy' ? (slNum - entryPrice) * amount : (entryPrice - slNum) * amount;

    const tpPct =
      entryPrice > 0 ? ((tpNum - entryPrice) / entryPrice) * 100 * (side === 'buy' ? 1 : -1) : 0;
    const slPct =
      entryPrice > 0 ? ((slNum - entryPrice) / entryPrice) * 100 * (side === 'buy' ? 1 : -1) : 0;
    ...
    <div className="flex flex-col gap-2">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span
            style={{ color: values.enabled ? sideColor : c.text2, fontSize: 13, fontWeight: 600 }}
          >
          ...
          {isBracket && (
            ...
            >
              ...
              <span style={{ color: '#8B5CF6', fontSize: 11, lineHeight: 1.4 }}>
                Bracket Order: cả Take Profit và Stop Loss sẽ được đặt đồng thời. Khi một lệnh khớp,
                lệnh còn lại tự hủy.
              </span>
            </div>
          )}
          ...
          {isBracket && values.enabled && (tpNum === 0 || slNum === 0) && (
            ...
            >
              ...
              <span style={{ color: '#F59E0B', fontSize: 11, lineHeight: 1.4 }}>
                Bracket mode yêu cầu cả TP và SL. {tpNum === 0 && 'Thiếu Take Profit. '}
                {slNum === 0 && 'Thiếu Stop Loss.'}
              </span>
            </div>
          )}
          ...
          <div>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  onChange({ ...values, tpPrice: e.target.value.replace(/[^\d.]/g, '') })
                }
                style={{
          ...
          <div>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  onChange({ ...values, slPrice: e.target.value.replace(/[^\d.]/g, '') })
                }
                style={{
                  background: 'transparent',
                  border: 'none',
            ...
            <TrCard rounded="sm" className="p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  style={{
                    color:
                      calc.rrRatio >= 2 ? '#10B981' : calc.rrRatio >= 1 ? '#F59E0B' : '#EF4444',
                    fontSize: 14,
                    fontWeight: 700,
                    fontFamily: 'monospace',
                  }}
              ...
              <div className="flex justify-between mt-1">
                <span style={{ color: '#EF4444', fontSize: 10 }}>Risk: {fmtUsd(calc.risk)}</span>
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[
                  ...
                  const tpTarget =
                    side === 'buy' ? entryPrice + diff * preset.tp : entryPrice - diff * preset.tp;
                  const slTarget =
                    side === 'buy' ? entryPrice - diff * preset.sl : entryPrice + diff * preset.sl;
                  return (
                    <button
                      key={preset.label}
                      onClick={() =>
