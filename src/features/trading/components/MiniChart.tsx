...
import { useMarketCandlesQuery, type MarketCandleInterval } from '@/features/market';
import {
  buildMiniChartConfig,
  getChartGradient,
  getResponsiveChartHeight,
} from '@/shared/lib/charts/chartTheme';
import { ChartSkeleton } from './ChartSkeleton';
import { TimeframePills } from './TimeframeSelector';
import type { MiniChartProps } from '@/shared/types/charts';

const CANDLE_LIMITS: Record<MarketCandleInterval, number> = {
  '5m': 288,
  '15m': 96,
  '1h': 24,
  '4h': 42,
...
export function MiniChart({
  ...
  const [timeframe, setTimeframe] = useState<MarketCandleInterval>('1h');
  const candlesQuery = useMarketCandlesQuery(pairId, {
    interval: timeframe,
    limit: CANDLE_LIMITS[timeframe],
  });
  const candleData = useMemo(() => candlesQuery.data?.items ?? [], [candlesQuery.data?.items]);
  const hasData = candleData.length >= 2;
  const isLoading = candlesQuery.isPending || !hasData;
  ...
  useEffect(() => {
    ...
    const chartCandles = candleData.map(({ time, open, high, low, close }) => ({
      time,
      open,
      high,
      low,
    ...
    const isPositive = candleData[candleData.length - 1].close >= candleData[0].open;
    ...
    candlestickSeries.setData(chartCandles as CandlestickData<UTCTimestamp>[]);
    ...
    if (showCurrentPrice) {
      ...
      priceLine.setData([
        { time: chartCandles[0].time as UTCTimestamp, value: currentPrice },
  ...
  if (candlesQuery.isError) {
    return (
      <div
        ...
        style={{ height, background: c.surface, border: `1px solid ${c.borderSolid}` }}
      >
        <span style={{ color: c.text3, fontSize: 11 }}>Không tải được dữ liệu biểu đồ</span>
        <button
          type="button"
