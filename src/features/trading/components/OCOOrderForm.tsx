...
export function OCOOrderForm({
  ...
  // OCO legs are mutually exclusive, so the available balance must cover the
  // larger leg rather than the sum of both legs.
  const requiredBalance =
    side === 'buy' ? Math.max(tpTotal, slTotal) : Math.max(tpAmountNum, slAmountNum);
  const hasSufficientBalance = Number.isFinite(available) && available >= requiredBalance;
  ...
  const canSubmit = canWriteTrading && isValidTP && isValidSL && hasSufficientBalance;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // The parent owns API error reporting; keep the form available for retry.
    } finally {
    ...
    <div className="flex flex-col gap-4">
      ...
      <TrCard className="p-4">
        <div className="flex items-start justify-between mb-3">
          ...
          <button
            type="button"
            aria-label="Thông tin về lệnh OCO"
            onClick={() => {
      ...
      {isValidTP && isValidSL && !hasSufficientBalance && (
        <p role="alert" className="text-sm" style={{ color: '#EF4444' }}>
          Số dư {side === 'buy' ? 'USDT' : baseAsset} khả dụng không đủ cho nhánh OCO lớn hơn.
        </p>
      )}
      {!canWriteTrading && (
        <p role="alert" className="text-sm" style={{ color: '#EF4444' }}>
          ...
          style={{
            ...
            background:
              canSubmit && !isSubmitting
                ...
                : c.surface2,
            boxShadow: canSubmit && !isSubmitting ? '0 4px 16px rgba(59,130,246,0.3)' : 'none',
