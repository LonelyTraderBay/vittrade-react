 ...
 *
 * @module features/dca/model/dca-funnel-service
...
class ConversionFunnelTrackerService {
  ...
  getFunnelAnalytics(funnelId: string): FunnelAnalytics {
    ...
    return {
      ...
      completionRate: allSessions.length > 0 ? completedSessions.length / allSessions.length : 0,
  ...
  private calculateStepAnalytics(
    ...
    return funnel.steps.map((step, index) => {
      ...
      const completed = sessions.filter(
        (s) => s.steps.length > index && s.steps[index].stepId === step.id,
      ).length;
      ...
      const dropouts = sessions.filter((s) => !s.completed && s.steps.length === index).length;
  ...
  private calculateDropoutAnalysis(
    ...
    for (const session of dropouts) {
      ...
      const dropoutStep = lastStepIndex >= 0 ? funnel.steps[lastStepIndex].id : 'entry';
    ...
    return Array.from(dropoutByStep.entries()).map(([step, data]) => ({
      ...
      avgTimeBeforeDropout:
        data.times.length > 0 ? data.times.reduce((sum, t) => sum + t, 0) / data.times.length : 0,
    }));
  }
  ...
  private calculateAvgCompletionTime(sessions: FunnelSession[]): number {
    ...
    const durations = sessions.map((s) => s.duration).filter((d): d is number => d !== undefined);
  ...
  private saveToStorage(): void {
    try {
      ...
      browserStorage.local.setItem('funnel_tracker', JSON.stringify(data));
    } catch (error) {
  ...
  private loadFromStorage(): void {
    try {
      const stored = browserStorage.local.getItem('funnel_tracker');
      if (!stored) return;
...
export type { StepAnalytics, DropoutAnalysis };
