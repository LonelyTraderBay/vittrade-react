...
import { fmtAmount, fmtFee } from '@/shared/lib/formatNumber';
import { Copy, CheckCircle, Clock, XCircle, MessageSquare, Check, AlertCircle } from 'lucide-react';
...
} from '@/features/wallet/model/wallet-types';

const TYPE_LABELS: Record<WalletTransactionType, { label: string; color: string; icon: string }> = {
...
export function TransactionDetailPage() {
    ...
    return (
      <PageLayout>
        ...
        <div className="flex flex-col items-center py-16 gap-4">
          ...
          <button
            type="button"
            onClick={() => navigate(`${prefix}/wallet/history`)}
  ...
  const steps: TransactionProgressStep[] = [
    ...
    {
      label: 'Đang xử lý',
      time: tx.status !== 'failed' ? tx.createdAt : undefined,
      done: tx.status !== 'failed',
    },
    {
      label:
        tx.status === 'completed'
          ...
          : tx.status === 'failed'
            ? 'Thất bại'
            : 'Đang chờ...',
      time: tx.status === 'completed' ? tx.createdAt : undefined,
  ...
  if (tx.fee !== undefined && tx.fee > 0)
    details.push({ label: 'Phí giao dịch', value: fmtFee(tx.fee) });
      ...
      <PageContent gap="default">
        <TrCard className="p-5 flex flex-col items-center gap-3">
          ...
          >
            ...
            <span style={{ color: statusInfo.color, fontSize: 13, fontWeight: 600 }}>
              {statusInfo.label}
            </span>
          </div>
        </TrCard>
        ...
        <TrCard className="p-4">
          ...
          <div className="flex flex-col gap-0">
            {steps.map((step, i) => {
              ...
              const dotColor = step.failed ? '#EF4444' : step.done ? '#10B981' : c.borderSolid;
              return (
                <div key={step.label} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    ...
                    {!isLast && (
                      <div
                        ...
                        style={{ background: step.done ? '#10B981' : c.borderSolid, minHeight: 24 }}
                      />
                    )}
                  </div>
                  <div className={isLast ? '' : 'pb-4'}>
                    <p
                      style={{
                        color: step.done || step.failed ? c.text1 : c.text3,
                        fontSize: 14,
                        fontWeight: 600,
                      }}
                    >
                    ...
                    {step.time && (
                      <p style={{ color: c.text3, fontSize: 12, marginTop: 2 }}>{step.time}</p>
                    )}
                  </div>
                </div>
              );
        ...
        <TrCard overflow>
          <p className="px-5 pt-4 pb-2" style={{ color: c.text1, fontSize: 14, fontWeight: 700 }}>
            Thông tin chi tiết
          </p>
          {details.map((row) => (
            <div
            ...
            >
              <span className="shrink-0" style={{ color: c.text2, fontSize: 13 }}>
                {row.label}
              </span>
