...
function performSearch(query: string): SearchResults {
  ...
  if (!q)
    return { predictions: [], arenaModes: [], arenaRooms: [], creators: [], tradingPairs: [] };

  return {
    predictions: PREDICTION_EVENTS.filter(
      (e) =>
        e.title.toLowerCase().includes(q) ||
        e.category.toLowerCase().includes(q) ||
        e.tags.some((t) => t.toLowerCase().includes(q)),
    ),
    arenaModes: ARENA_MODES.filter(
      (m) =>
        m.title.toLowerCase().includes(q) ||
        m.description.toLowerCase().includes(q) ||
        m.tags.some((t) => t.toLowerCase().includes(q)),
    ),
    arenaRooms: ARENA_ROOMS.filter(
      (r) => r.title.toLowerCase().includes(q) || r.format.toLowerCase().includes(q),
    ),
    creators: ARENA_CREATORS.filter(
      (c) => c.name.toLowerCase().includes(q) || (c.bio && c.bio.toLowerCase().includes(q)),
    ),
    tradingPairs: CRYPTO_PAIRS.filter(
      (p) =>
...
function PredictionCard({ event }: { event: PredictionEvent }) {
  ...
  return (
    <TrCard
      className="p-3.5 active:opacity-70"
      onClick={() => {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            ...
            {creator.fairPlayBadge && <Shield size={10} color="#10B981" />}
          </div>
          <div className="flex items-center gap-2">
            <span style={{ color: c.text3, fontSize: 10 }}>
        ...
        <span style={{ color: c.text2, fontSize: φ.xs, fontWeight: 600 }}>Xem creator →</span>
      </div>
    </TrCard>
  );
}
...
function TradingPairCard({ pair }: { pair: CryptoPair }) {
  ...
  return (
    <TrCard
      className="p-3.5 active:opacity-70"
      onClick={() => {
        navigate(`${prefix}/market/${pair.id}`);
        hapticSelection();
      }}
    >
...
export function UnifiedSearchPage() {
  ...
  const totalResults =
    ...
    results.arenaModes.length +
    results.arenaRooms.length +
    results.creators.length +
    results.tradingPairs.length;
            ...
            {recentSearches.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={handleClearRecent} style={{ color: c.text3, fontSize: 10 }}>
                    Xoá tất cả
                  </button>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {TRENDING_QUERIES.map((t) => (
                  ...
                  >
                    ...
                    <span style={{ color: c.text1, fontSize: φ.xs, fontWeight: 500 }}>
                      {t.label}
                    </span>
                  </button>
                ))}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p style={{ color: c.text3, fontSize: 10 }}>
                        Thị trường dự đoán · Vị thế thực · USDT
                      </p>
                    </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p style={{ color: c.text3, fontSize: 10 }}>
                        Creator modes · Thách đấu · Arena Points only
                      </p>
                    </div>
                    <ArrowRight size={14} color={c.text3} />
                  </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p style={{ color: c.text3, fontSize: 10 }}>
                        Khám phá theo chủ đề · Crypto, Sports, Macro...
                      </p>
                    </div>
                    <ArrowRight size={14} color={c.text3} />
                  </div>
          ...
          <>
            ...
            <div className="flex items-center gap-2">
              <span style={{ color: c.text2, fontSize: φ.xs }}>
                Tìm thấy <strong style={{ color: c.text1 }}>{totalResults}</strong> kết quả cho "
                {query}"
              </span>
            </div>
            ...
            {results.arenaModes.length > 0 && (
              <ResultSection
                ...
                color="#F59E0B"
              >
                {results.arenaModes.map((m) => (
                  <ArenaModeCard key={m.id} mode={m} />
                ))}
              </ResultSection>
            )}
            ...
            {results.arenaRooms.length > 0 && (
              <ResultSection
                ...
                color="#F59E0B"
              >
                {results.arenaRooms.map((r) => (
                  <ArenaRoomCard key={r.id} room={r} />
                ))}
              </ResultSection>
            )}
            ...
            {results.tradingPairs.length > 0 && (
              <ResultSection
                ...
                color="#3B82F6"
              >
                {results.tradingPairs.map((p) => (
                  <TradingPairCard key={p.symbol} pair={p} />
                ))}
              </ResultSection>
            )}
            ...
            >
              <p style={{ color: c.text3, fontSize: 9, lineHeight: 1.5 }}>
                <strong style={{ color: c.text2 }}>Lưu ý:</strong> Prediction Markets sử dụng USDT
                thật. Arena Challenges chỉ dùng Arena Points (không liên quan ví). Đây là trang khám
                phá — không phải trang giao dịch.
              </p>
            </div>
          </>
        )}
