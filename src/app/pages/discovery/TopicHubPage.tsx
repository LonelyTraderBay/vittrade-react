...
import { SHARED_TOPICS, mapCategoryToTopic } from '../../components/bridges/ArenaPredictionBridges';
...
function getArenaModesByTopic(topicId: string): ArenaMode[] {
  return ARENA_MODES.filter((m) => m.tags.some((t) => t.toLowerCase().includes(topicId)));
}
...
function TopicHero({ topicId }: { topicId: string }) {
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(135deg, ${topic.color}12, ${topic.color}04)`,
    ...
    >
      ...
      <div className="grid grid-cols-4 gap-2">
        {stats.map((s) => (
          ...
          >
            <p
              style={{ color: s.color, fontSize: φ.base, fontWeight: 700, fontFamily: 'monospace' }}
            >
              {s.value}
            </p>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <span style={{ color: '#8B5CF6', fontSize: φ.xs, fontWeight: 600 }}>Xem thị trường →</span>
      </div>
    </TrCard>
  );
}
...
function ArenaRoomRow({ room }: { room: ArenaRoom }) {
  ...
  const statusColor =
    room.status === 'in_progress' ? '#10B981' : room.status === 'waiting' ? '#F59E0B' : c.text3;
  const statusLabel =
    room.status === 'in_progress' ? 'Live' : room.status === 'waiting' ? 'Chờ' : 'Xong';
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span style={{ fontSize: 10 }}>
            {room.creator.avatar} {room.creator.name}
          </span>
        </div>
          ...
          <>
            ...
            {creators.length > 0 && (
              <section>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 style={{ color: c.text1, fontSize: φ.sm, fontWeight: 700 }}>Top Creators</h3>
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p style={{ color: c.text3, fontSize: 10, lineHeight: 1.4 }}>
                    Tạo thách đấu Arena Points liên quan đến{' '}
                    {SHARED_TOPICS.find((t) => t.id === selectedTopic)?.label}
                  </p>
                </div>
                <button
                  onClick={() => {
            ...
            >
              <p style={{ color: c.text3, fontSize: 9, lineHeight: 1.6 }}>
                ...
                thật (vị thế thực). Arena Challenges chỉ dùng Arena Points (không phải tài sản tài
                chính). Topic Hub là trang khám phá — 2 module hoàn toàn riêng biệt.
              </p>
            </div>
          </>
        )}
