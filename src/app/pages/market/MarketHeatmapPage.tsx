...
export function MarketHeatmapPage() {
  ...
  const filteredCoins =
    category === 'Tất cả' ? HEATMAP_COINS : HEATMAP_COINS.filter((c) => c.category === category);
  ...
  const topGainers = sortedByChange
    .filter((c) => (metric === '24h' ? c.change24h : c.change7d) > 0)
    .slice(0, 3);
  ...
  const avgChange =
    filteredCoins.reduce((s, c) => s + (metric === '24h' ? c.change24h : c.change7d), 0) /
      ...
      <PageContent padding="compact">
        ...
        <div className="mt-3 flex gap-3">
          ...
          <TrCard rounded="sm" className="flex-1 p-3">
            ...
            <p style={{ color: c.primary, fontSize: FONT_SCALE.sm, fontWeight: FONT_WEIGHT.bold }}>
                  ...
                  return (
                    ...
                    >
                      <span
                        style={{
                          color: '#FFF',
                          fontSize: span >= 2 ? FONT_SCALE.base : FONT_SCALE.xs,
                          fontWeight: FONT_WEIGHT.bold,
                      ...
                      <span
                        style={{
                          ...
                          fontSize: span >= 2 ? FONT_SCALE.xs : FONT_SCALE.micro,
                          fontWeight: FONT_WEIGHT.semibold,
                          textShadow: '0 1px 2px rgba(0,0,0,0.5)',
                        }}
                      >
          ...
          <TrCard className="mt-3 p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                ...
                <p style={{ color: c.text2, fontSize: FONT_SCALE.xs }}>
                  {selected.symbol}/USDT • {selected.category}
                </p>
              </div>
              <button
                onClick={() => navigate(`${prefix}/pair/${selected.id}usdt`)}
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                {
                  label: '24h',
                  value: fmtPct(selected.change24h),
                  color: selected.change24h >= 0 ? '#10B981' : '#EF4444',
                },
                {
                  label: '7d',
                  value: fmtPct(selected.change7d),
                  color: selected.change7d >= 0 ? '#10B981' : '#EF4444',
                },
                {
                  label: 'MCap',
                  value: fmtCompact(selected.marketCap, { prefix: '$' }),
                  color: c.text2,
                },
              ].map((d) => (
                <div key={d.label}>
            ...
            {topGainers.map((coin, i) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderBottom: i < topGainers.length - 1 ? `1px solid ${c.divider}` : 'none',
                  }}
                >
            ...
            {topLosers.map((coin, i) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center justify-between py-2"
                  style={{
                    borderBottom: i < topLosers.length - 1 ? `1px solid ${c.divider}` : 'none',
                  }}
                >
                  <div className="flex items-center gap-2">
                    <span style={{ color: c.text3, fontSize: FONT_SCALE.micro }}>{i + 1}.</span>
