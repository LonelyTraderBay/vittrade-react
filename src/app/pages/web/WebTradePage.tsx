...
import {
  ChevronDown, Info, ArrowLeftRight, BarChart3, Repeat,
  Star, Settings, Maximize2, Keyboard, CandlestickChart, LineChart,
  Minus, TrendingUp, Trash2, PenTool, Columns2, LayoutGrid, Layers,
  X, Bell, Calculator, ExternalLink, ScanLine, PieChart,
} from 'lucide-react';
import {
...
import {
  CRYPTO_PAIRS, USER_ASSETS, OPEN_ORDERS, ORDER_HISTORY,
  generateRecentTrades, generateChartData,
} from '../../data/mockData';
...
const TIMEFRAME_POINTS: Record<string, number> = {
  '1m': 60, '5m': 60, '15m': 48, '1H': 48, '4H': 48, '1D': 60, '1W': 52,
};
...
const SPLIT_PAIRS = CRYPTO_PAIRS.filter(p => ['btcusdt', 'ethusdt', 'solusdt', 'bnbusdt'].includes(p.id));
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function TradingChart({ pair, timeframe, chartType, compact }: {
  ...
  useEffect(() => {
    ...
    if (chartType === 'candle') {
      const candleSeries = chart.addSeries(CandlestickSeries, {
        upColor: ct.candle.upColor, downColor: ct.candle.downColor,
        borderUpColor: ct.candle.borderUpColor, borderDownColor: ct.candle.borderDownColor,
        wickUpColor: ct.candle.wickUpColor, wickDownColor: ct.candle.wickDownColor,
      });
      ...
      const areaSeries = chart.addSeries(AreaSeries, {
        topColor: ct.area.topColor, bottomColor: ct.area.bottomColor,
        lineColor: ct.area.lineColor, lineWidth: ct.area.lineWidth,
      });
  ...
  useEffect(() => {
    ...
    let currentBar: { time: any; open: number; high: number; low: number; close: number } | null = null;
          ...
          if (volumeSeriesRef.current) {
            volumeSeriesRef.current.update({
              ...
              color: currentBar.close >= currentBar.open
                ? (theme === 'dark' ? 'rgba(16,185,129,0.25)' : 'rgba(16,185,129,0.2)')
...
function DrawingToolbar({
  activeTool, onToolChange, drawingCount, isDark, panelBorder,
}: {
  ...
  return (
    <div className="flex items-center gap-0.5 px-1 py-0.5 rounded-lg"
      style={{ background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)', border: `1px solid ${panelBorder}` }}>
      {tools.map(t => {
        const Icon = t.icon;
        ...
        return (
          <button key={t.id}
            ...
            style={{
              background: active ? (t.id === 'eraser' ? 'rgba(239,68,68,0.15)' : 'rgba(59,130,246,0.15)') : 'transparent',
            }}
            title={`${t.label} (${t.key})`}>
            <Icon size={13}
              color={active
                ? (t.id === 'eraser' ? '#EF4444' : '#3B82F6')
                : c.text3
              } />
          </button>
        );
...
function LayoutSelector({
  layout, onLayoutChange, isDark, panelBorder,
}: {
  layout: LayoutMode;
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function CompactOrderForm({ pair, side: controlledSide, onSideChange, onSubmit }: {
  pair: typeof CRYPTO_PAIRS[0];
  side: 'buy' | 'sell';
  ...
  useEffect(() => { setAmount(''); setActivePct(null); }, [controlledSide]);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex rounded-lg overflow-hidden" style={{ border: `1px solid ${inputBorder}` }}>
        <button onClick={() => onSideChange('buy')}
          className="flex-1 py-2.5 flex items-center justify-center transition-colors"
          style={{ background: side === 'buy' ? '#10B981' : 'transparent', color: side === 'buy' ? '#fff' : c.text3, fontSize: 14, fontWeight: 700 }}>
          MUA <span className="ml-1.5 opacity-50" style={{ fontSize: 10 }}>B</span>
        </button>
        <button onClick={() => onSideChange('sell')}
          className="flex-1 py-2.5 flex items-center justify-center transition-colors"
          style={{ background: side === 'sell' ? '#EF4444' : 'transparent', color: side === 'sell' ? '#fff' : c.text3, fontSize: 14, fontWeight: 700 }}>
          BÁN <span className="ml-1.5 opacity-50" style={{ fontSize: 10 }}>S</span>
        </button>
      </div>
        ...
        {ORDER_TYPES.map(type => (
          <button key={type} onClick={() => setOrderType(type)}
            ...
            style={{
              ...
              fontSize: 12, fontWeight: orderType === type ? 700 : 500,
            }}>{type}</button>
        ))}
      </div>
      ...
      {orderType !== 'Thị trường' && (
        <div>
          <label style={{ color: c.text3, fontSize: 11, fontWeight: 600, display: 'block', marginBottom: 5 }}>
            {orderType === 'Dừng lỗ' ? 'Giá kích hoạt' : 'Giá'} (USDT)
          </label>
          <div className="flex items-center rounded-lg px-3.5"
            ...
            <input type="number" inputMode="decimal" placeholder="0.00" value={limitPrice}
              ...
              style={{ color: c.text1, fontSize: 14, fontWeight: 600, fontVariantNumeric: 'tabular-nums' }} />
      ...
      <div>
        ...
        <div className="flex items-center rounded-lg px-3.5"
          ...
          <input type="number" inputMode="decimal" placeholder="0.000000" value={amount}
            onChange={e => { setAmount(fmt(e.target.value)); setActivePct(null); }}
            className="flex-1 bg-transparent outline-none"
            style={{ color: c.text1, fontSize: 14, fontWeight: 600, fontVariantNumeric: 'tabular-nums' }} />
      ...
      <div className="rounded-lg p-3" style={{ background: inputBg }}>
        <div className="flex justify-between items-center mb-1.5">
          ...
          <span style={{ color: c.text1, fontSize: 14, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{fmtUsd(total)}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="flex items-center gap-1" style={{ color: c.text3, fontSize: 11 }}>Phí (0.1%) <Info size={11} /></span>
          <span style={{ color: c.text3, fontSize: 11, fontVariantNumeric: 'tabular-nums' }}>≈ ${fee.toFixed(4)}</span>
        </div>
      </div>
      ...
      <button disabled={!canPlace}
        ...
        style={{
          height: 44,
          background: !canPlace ? inputBg : side === 'buy'
            ? 'linear-gradient(135deg, #10B981 0%, #059669 100%)'
            : 'linear-gradient(135deg, #EF4444 0%, #dc2626 100%)',
          color: !canPlace ? c.text3 : '#fff',
          fontSize: 14, fontWeight: 700,
          boxShadow: canPlace ? (side === 'buy' ? '0 4px 12px rgba(16,185,129,0.3)' : '0 4px 12px rgba(239,68,68,0.3)') : 'none',
        }}
        onClick={() => {
          if (onSubmit) {
            onSubmit({
      ...
      <div className="flex items-center justify-center gap-2.5 pt-2 flex-wrap">
        {[{ k: 'B', l: 'Mua' }, { k: 'S', l: 'Bán' }, { k: 'D', l: 'Draw' }, { k: 'A', l: 'Alert' }, { k: 'P', l: 'PnL' }, { k: 'Esc', l: 'Reset' }].map(h => (
          <span key={h.k} className="flex items-center gap-1" style={{ color: c.text3, fontSize: 10 }}>
            <kbd className="px-1.5 py-0.5 rounded" style={{ background: inputBg, border: `1px solid ${inputBorder}`, fontSize: 10 }}>{h.k}</kbd> {h.l}
          </span>
        ))}
      </div>
    </div>
...
function RecentTradesPanel({ price }: { price: number }) {
  ...
  return (
    <div className="flex flex-col">
      <div className="grid items-center py-2 px-4"
        ...
        <span style={{ color: c.text3, fontSize: 11, fontWeight: 600, textAlign: 'right' }}>KL</span>
        <span style={{ color: c.text3, fontSize: 11, fontWeight: 600, textAlign: 'right' }}>Giờ</span>
      </div>
      <div className="flex-1 overflow-y-auto scrollbar-none" style={{ maxHeight: 220 }}>
        {trades.slice(0, 15).map((t, i) => (
          <div key={`${t.id}-${i}`} className="grid items-center py-1 px-4"
            style={{ gridTemplateColumns: '1fr 1fr 64px' }}>
            <span style={{ color: t.side === 'buy' ? '#10B981' : '#EF4444', fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>{fmtPrice(t.price)}</span>
            <span style={{ color: c.text2, fontSize: 12, fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>{fmtAmount(t.amount, 4)}</span>
...
function ShortcutToast({ action, visible }: { action: string; visible: boolean }) {
  ...
  return (
    <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-xl shadow-lg"
      style={{
        background: 'rgba(0,0,0,0.85)', color: '#fff', fontSize: 12, fontWeight: 600,
        backdropFilter: 'blur(12px)', animation: 'fadeInOut 1.2s ease-in-out forwards',
      }}>
      <span className="flex items-center gap-2"><Keyboard size={14} /> {action}</span>
    </div>
  );
}
...
function LiveIndicator() {
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg" style={{ background: 'rgba(16,185,129,0.08)' }}>
      <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#10B981', animation: 'livePulse 2s ease-in-out infinite' }} />
...
export function WebTradePage() {
  ...
  const [orderSide, setOrderSide] = useState<'buy' | 'sell'>((searchParams.get('side') ?? 'buy') as any);
  ...
  // Toast
  const [toast, setToast] = useState<{ action: string; visible: boolean }>({ action: '', visible: false });
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'b': case 'B':
          e.preventDefault(); setOrderSide('buy'); showToast('Chuyển sang MUA (B)'); break;
        case 's': case 'S':
          e.preventDefault(); setOrderSide('sell'); showToast('Chuyển sang BÁN (S)'); break;
        case 'd': case 'D':
          e.preventDefault();
        ...
        case '1':
          ...
          e.preventDefault(); setDrawingTool('trendline'); showToast('Tool: Trendline (1)'); break;
        case '2':
          if (drawingTool !== 'none' && drawingTool !== 'trendline') break;
          e.preventDefault(); setDrawingTool('fibonacci'); showToast('Tool: Fibonacci (2)'); break;
        case '3':
          e.preventDefault(); setDrawingTool('hline'); showToast('Tool: H-Line (3)'); break;
        case '4':
          e.preventDefault(); setDrawingTool('eraser'); showToast('Tool: Eraser (4)'); break;
        case 'Escape':
          e.preventDefault();
          if (confirmModal) { setConfirmModal(false); setPendingOrder(null); showToast('Đóng xác nhận (Esc)'); }
          else if (showAlerts) { setShowAlerts(false); showToast('Đóng cảnh báo (Esc)'); }
          else if (showPnL) { setShowPnL(false); showToast('Đóng PnL (Esc)'); }
          else if (drawingTool !== 'none') { setDrawingTool('none'); showToast('Drawing OFF (Esc)'); }
          else { setOrderSide('buy'); showToast('Reset form (Esc)'); }
          break;
        case 'a': case 'A':
          e.preventDefault(); setShowAlerts(p => !p); setShowPnL(false);
          showToast(showAlerts ? 'Đóng cảnh báo' : 'Mở cảnh báo giá (A)'); break;
        case 'p': case 'P':
          e.preventDefault(); setShowPnL(p => !p); setShowAlerts(false);
          showToast(showPnL ? 'Đóng PnL' : 'Mở PnL Calculator (P)'); break;
      }
    };
  ...
  const renderChartArea = () => {
    ...
    if (layoutMode === 'split-h') {
      return (
        <div className="flex flex-1 min-h-0">
          <div className="relative flex-1 min-w-0" style={{ background: panelBg, borderRight: `1px solid ${panelBorder}` }}>
            {/* Label */}
            <div className="absolute top-2 left-3 z-10 flex items-center gap-1.5 px-2 py-1 rounded-lg"
              style={{ background: isDark ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.85)', backdropFilter: 'blur(4px)' }}>
              <span style={{ color: pair.logoColor, fontSize: 11, fontWeight: 700 }}>{pair.symbol}</span>
            </div>
          ...
          <div className="relative flex-1 min-w-0" style={{ background: panelBg }}>
            <div className="absolute top-2 left-3 z-10 flex items-center gap-2 px-2 py-1 rounded-lg"
              style={{ background: isDark ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.85)', backdropFilter: 'blur(4px)' }}>
              <span style={{ color: splitPair.logoColor, fontSize: 11, fontWeight: 700 }}>{splitPair.symbol}</span>
              <select value={splitPairId}
                ...
                {SPLIT_PAIRS.filter(p => p.id !== pair.id).map(p => (
                  <option key={p.id} value={p.id}>{p.baseAsset}</option>
                ))}
              </select>
            </div>
    ...
    return (
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="shrink-0" style={{ height: 160, borderTop: `1px solid ${panelBorder}`, background: panelBg }}>
          <div className="px-4 py-1.5" style={{ borderBottom: `1px solid ${panelBorder}` }}>
            <span style={{ color: c.text1, fontSize: 12, fontWeight: 600 }}>Depth Chart — {pair.symbol}</span>
          </div>
  ...
  return (
    <div className="flex flex-col gap-0" style={{ height: WEB_FULL_BLEED_HEIGHT }}>
      ...
      <div className="flex items-center gap-5 px-5 py-2.5 shrink-0"
        ...
        {/* Pair */}
        <button className="flex items-center gap-2.5 shrink-0"
          onClick={() => navigate(`${prefix}/markets`)}>
          <div className="w-8 h-8 rounded-full flex items-center justify-center"
            style={{ background: pair.logoColor + '18' }}>
            <span style={{ color: pair.logoColor, fontSize: 11, fontWeight: 700 }}>{pair.baseAsset.slice(0, 3)}</span>
          </div>
        ...
        <div className="flex items-center gap-2.5">
          <span style={{
            ...
            fontSize: 20, fontWeight: 700, fontVariantNumeric: 'tabular-nums', transition: 'color 0.3s',
        ...
        <div className="flex items-center gap-5 flex-1">
          {[
            ...
            <div key={s.l} className="flex flex-col">
              ...
              <span style={{ color: s.cl, fontSize: 13, fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{s.v}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={() => setIsFavorite(!isFavorite)}
            ...
            <Star size={16} fill={isFavorite ? '#F59E0B' : 'none'} color={isFavorite ? '#F59E0B' : c.text3} />
          </button>
          ...
          <button onClick={() => navigate(`${prefix}/trade/analytics`)}
            className="web-cmd-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors"
          ...
          <button onClick={() => navigate(`${prefix}/scanner`)}
            className="web-cmd-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-colors"
          ...
          <button onClick={() => navigate(`${prefix}/trade/settings`)}
            className="web-cmd-btn w-9 h-9 rounded-lg flex items-center justify-center transition-colors">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ borderRight: `1px solid ${panelBorder}` }}>
          ...
          <div className="flex items-center gap-1.5 px-4 py-2 shrink-0"
            ...
            {TIMEFRAMES.map(tf => (
              <button key={tf} onClick={() => setTimeframe(tf)}
                ...
                style={{
                  ...
                  fontSize: 12, fontWeight: timeframe === tf ? 700 : 500,
                }}>{tf}</button>
            ))}
            ...
            <button onClick={() => setChartType('candle')}
              ...
              title="Nến"><CandlestickChart size={15} color={chartType === 'candle' ? '#3B82F6' : c.text3} /></button>
            <button onClick={() => setChartType('area')}
              className="web-cmd-btn w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
              style={{ background: chartType === 'area' ? activeBtnBg : 'transparent' }}
              title="Vùng"><LineChart size={15} color={chartType === 'area' ? '#3B82F6' : c.text3} /></button>

            <div className="w-px h-5 mx-1.5" style={{ background: panelBorder }} />

            {/* Drawing tools */}
            <DrawingToolbar activeTool={drawingTool} onToolChange={setDrawingTool} drawingCount={drawingCount}
              isDark={isDark} panelBorder={panelBorder} />

            <div className="w-px h-5 mx-1.5" style={{ background: panelBorder }} />

            {/* Layout selector */}
            <LayoutSelector layout={layoutMode} onLayoutChange={setLayoutMode}
              isDark={isDark} panelBorder={panelBorder} />
            ...
            {drawingTool !== 'none' && (
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg"
                ...
                <span style={{ color: '#3B82F6', fontSize: 11, fontWeight: 600 }}>
                  {drawingTool === 'trendline' ? 'Trendline' : drawingTool === 'fibonacci' ? 'Fibonacci' : drawingTool === 'hline' ? 'H-Line' : 'Eraser'}
                </span>
          ...
          <div className="shrink-0" style={{ borderTop: `1px solid ${panelBorder}`, maxHeight: 280 }}>
            <div className="flex items-center gap-0 px-4 py-1.5" style={{ borderBottom: `1px solid ${panelBorder}`, background: topBarBg }}>
              {[
                ...
                <button key={t.id} onClick={() => setBottomTab(t.id)}
                  ...
                  style={{
                    ...
                    fontSize: 12, fontWeight: bottomTab === t.id ? 700 : 500,
                    borderBottom: bottomTab === t.id ? '2px solid #3B82F6' : '2px solid transparent',
                  }}>{t.label}</button>
              ))}
              <div className="flex-1" />
              <button onClick={() => navigate(`${prefix}/trade/orders-history`)}
                style={{ color: '#3B82F6', fontSize: 12, fontWeight: 600 }}>Xem tất cả</button>
            </div>

            <div className="overflow-y-auto scrollbar-none" style={{ maxHeight: 220, background: panelBg }}>
              {bottomTab === 'orders' && (
                OPEN_ORDERS.length === 0 ? (
                  ...
                  <div>
                    <div className="grid items-center px-4 py-2"
                      style={{ gridTemplateColumns: '68px 1fr 1fr 1fr 1fr 68px', borderBottom: `1px solid ${panelBorder}` }}>
                      {['Loại', 'Cặp', 'Giá', 'KL / Filled', 'Tổng', ''].map(h => (
                        <span key={h} style={{ color: c.text3, fontSize: 11, fontWeight: 600 }}>{h}</span>
                      ))}
                    </div>
                    {OPEN_ORDERS.map(o => (
                      <div key={o.id} className="grid items-center px-4 py-2 web-cmd-btn transition-colors"
                        style={{ gridTemplateColumns: '68px 1fr 1fr 1fr 1fr 68px', borderBottom: `1px solid ${panelBorder}`, minHeight: 40 }}>
                        <span className="rounded px-2 py-0.5 text-center"
                          style={{ background: o.side === 'buy' ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: o.side === 'buy' ? '#10B981' : '#EF4444', fontSize: 11, fontWeight: 700 }}>
                          {o.side === 'buy' ? 'MUA' : 'BÁN'}
                        </span>
                        <span style={{ color: c.text1, fontSize: 12, fontWeight: 600 }}>{o.symbol}</span>
                        <span style={{ color: c.text2, fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>{fmtPrice(o.price)}</span>
                        <span style={{ color: c.text2, fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>{o.filled}/{o.amount}</span>
                        <span style={{ color: c.text2, fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>{fmtUsd(o.price * o.amount)}</span>
                        <button className="px-2.5 py-1 rounded text-center transition-colors"
                          style={{ background: 'rgba(239,68,68,0.08)', color: '#EF4444', fontSize: 11, fontWeight: 600 }}>Hủy</button>
                      </div>
                    ))}
                  </div>
                )
              ...
              {bottomTab === 'history' && (
                <div>
                  <div className="grid items-center px-4 py-2"
                    style={{ gridTemplateColumns: '68px 1fr 1fr 1fr 1fr 88px', borderBottom: `1px solid ${panelBorder}` }}>
                  ...
                  {ORDER_HISTORY.slice(0, 5).map(o => (
                    <div key={o.id} className="grid items-center px-4 py-2 web-cmd-btn transition-colors"
                      style={{ gridTemplateColumns: '68px 1fr 1fr 1fr 1fr 88px', borderBottom: `1px solid ${panelBorder}`, minHeight: 40 }}>
                      <span className="rounded px-2 py-0.5 text-center"
                        style={{ background: o.side === 'buy' ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: o.side === 'buy' ? '#10B981' : '#EF4444', fontSize: 11, fontWeight: 700 }}>
                        {o.side === 'buy' ? 'MUA' : 'BÁN'}
                      </span>
                      <span style={{ color: c.text1, fontSize: 12, fontWeight: 600 }}>{o.symbol}</span>
                      <span style={{ color: c.text2, fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>{fmtPrice(o.price)}</span>
                      <span style={{ color: c.text2, fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>{o.amount}</span>
                      <span style={{ color: c.text2, fontSize: 12, fontVariantNumeric: 'tabular-nums' }}>{fmtUsd(o.price * o.amount)}</span>
                      <span className="rounded px-1.5 py-0.5 text-center"
                        style={{
                          background: o.status === 'filled' ? 'rgba(16,185,129,0.1)' : o.status === 'cancelled' ? 'rgba(239,68,68,0.1)' : 'rgba(245,158,11,0.1)',
                          color: o.status === 'filled' ? '#10B981' : o.status === 'cancelled' ? '#EF4444' : '#F59E0B',
                          fontSize: 11, fontWeight: 600,
                        }}>
                        {o.status === 'filled' ? 'Khớp' : o.status === 'cancelled' ? 'Đã hủy' : 'Một phần'}
                      </span>
                    </div>
                  ))}
                </div>
        ...
        {/* MIDDLE: Orderbook */}
        <div className="flex flex-col shrink-0" style={{ width: 280, borderRight: `1px solid ${panelBorder}`, background: panelBg }}>
          <div className="px-4 py-2.5 shrink-0" style={{ borderBottom: `1px solid ${panelBorder}`, background: topBarBg }}>
        ...
        <div className="flex flex-col shrink-0" style={{ width: 340, background: panelBg }}>
          <div className="px-4 py-2.5 shrink-0 flex items-center justify-between"
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={() => { setShowAlerts(p => !p); setShowPnL(false); }}
              ...
              <button onClick={() => { setShowPnL(p => !p); setShowAlerts(false); }}
                className="web-cmd-btn w-7 h-7 rounded-md flex items-center justify-center transition-colors"
              ...
              <button onClick={() => { setShowAdvanced(p => !p); setShowAlerts(false); setShowPnL(false); }}
                className="web-cmd-btn w-7 h-7 rounded-md flex items-center justify-center transition-colors"
          ...
          <div className="relative flex-1 overflow-y-auto scrollbar-none p-4">
            <CompactOrderForm pair={pair} side={orderSide} onSideChange={setOrderSide} onSubmit={order => {
              setPendingOrder(order);
              setConfirmModal(true);
            }} />
            {/* Overlay panels */}
            <PriceAlertPanel open={showAlerts} onClose={() => setShowAlerts(false)}
              livePrice={livePrice} symbol={pair.symbol}
              onAlertTriggered={(a) => showToast(`🔔 Alert: $${a.targetPrice} ${a.direction === 'above' ? '≥' : '≤'} reached!`)} />
            <PnLCalculator open={showPnL} onClose={() => setShowPnL(false)}
              livePrice={livePrice} symbol={pair.symbol} baseAsset={pair.baseAsset} />
            <WebAdvancedOrders open={showAdvanced} onClose={() => setShowAdvanced(false)}
              livePrice={livePrice} symbol={pair.symbol} baseAsset={pair.baseAsset} />
          </div>
        </div>
      </div>
      ...
      <TradeConfirmModal
        open={confirmModal}
        order={pendingOrder}
        onConfirm={() => { setConfirmModal(false); setPendingOrder(null); showToast('Lệnh đã đặt thành công!'); }}
        onCancel={() => { setConfirmModal(false); setPendingOrder(null); }}
      />
    </div>
  );
}