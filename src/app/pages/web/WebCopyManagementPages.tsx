...
const PERF_CHART = Array.from({ length: 30 }, (_, i) => ({
  day: `D${i + 1}`,
  value: 100 + Math.sin(i / 5) * 15 + i * 0.8 + (Math.random() - 0.3) * 5,
}));
...
const AUDIT_ENTRIES = [
  {
    ...
    action: 'Trade Copied',
    detail: 'BUY 0.01 BTC @ $68,450',
    provider: 'AlphaHunter_VN',
    pnl: 0,
  },
  {
    id: 'a2',
    ts: '2026-03-08 12:15:08',
    action: 'Trade Copied',
    detail: 'SELL 0.05 ETH @ $3,850',
    provider: 'SteadyGains_Pro',
    pnl: 12.5,
  },
  {
    id: 'a3',
    ts: '2026-03-07 18:30:42',
    action: 'Stop Loss Adjusted',
    detail: 'Stop loss changed: 20% → 15%',
    provider: 'CryptoSensei',
    pnl: 0,
  },
  {
    id: 'a4',
    ts: '2026-03-07 10:00:00',
    action: 'Copy Paused',
    detail: 'Manual pause by user',
    provider: 'CryptoSensei',
    pnl: 0,
  },
  {
    id: 'a5',
    ts: '2026-03-06 22:15:30',
    action: 'Trade Copied',
    detail: 'BUY 2 SOL @ $142.30',
    provider: 'AlphaHunter_VN',
    pnl: 0,
  },
  {
    id: 'a6',
    ts: '2026-03-06 16:42:18',
    action: 'Copy Started',
    detail: 'Started copying CryptoSensei with $300',
    provider: 'CryptoSensei',
    pnl: 0,
  },
];
...
export function WebCopyActiveCopiesPage() {
  ...
  const togglePause = (id: string) => {
    setCopies((prev) =>
      prev.map((cp) =>
        cp.id === id
          ? { ...cp, status: cp.status === 'active' ? ('paused' as const) : ('active' as const) }
          : cp,
      ),
    );
      ...
      <div style={{ padding: '24px 0 40px' }}>
        ...
        >
          <StatCard
            ...
            color="#10B981"
            sub={`${copies.length} tổng`}
          />
          <StatCard
            icon={DollarSign}
            label="Tổng đầu tư"
            value={`$${totalInvested.toLocaleString()}`}
            sub="Across all copies"
          />
          <StatCard
            icon={TrendingUp}
            label="Tổng P/L"
          ...
          <StatCard
            icon={BarChart3}
            label="Tổng lệnh"
            value={copies.reduce((s, cp) => s + cp.trades, 0).toString()}
            sub="Giao dịch đã copy"
          />
        </div>
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <span style={{ color: c.text3, fontSize: WEB_FONT.sm }}>
                        Từ {cp.startDate} · SL: {cp.stopLoss}%
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2" style={{ padding: '10px 20px 14px' }}>
                  ...
                  <button
                    onClick={() => navigate(`/w/trade/copy-performance/${cp.id}`)}
                    className="flex items-center justify-center gap-2"
                    style={{
                      height: WEB_BUTTON.md,
                      padding: '0 16px',
        ...
        {confirmStop && (
          ...
          >
            ...
            >
              ...
              >
                Hành động này sẽ đóng tất cả vị thế đang mở và dừng sao chép giao dịch. Bạn không
                thể hoàn tác.
              </p>
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => {
                    setCopies((prev) => prev.filter((cp) => cp.id !== confirmStop));
                    setConfirmStop(null);
                    toast.success('Đã dừng copy');
                  }}
                  style={{
                    flex: 1,
                    height: WEB_BUTTON.lg,
  ...
  const ToggleSwitch = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
    <button
      ...
      style={{
        ...
        cursor: 'pointer',
        border: 'none',
        background: checked ? c.primary : c.surface2,
        ...
        <div className="flex flex-col gap-5">
          ...
          >
            ...
            <div className="flex flex-col gap-4">
              <div
                className="flex items-center justify-between"
                style={{ padding: '12px 0', borderBottom: `1px solid ${c.divider}` }}
              >
                <div>
                  <span
                    style={{
                  ...
                  <span style={{ color: c.text3, fontSize: WEB_FONT.sm }}>
                    Dừng tất cả copies khi tổng lỗ vượt ngưỡng
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <input
              ...
              >
                <div>
                  ...
                  <span style={{ color: c.text3, fontSize: WEB_FONT.sm }}>
                    Tái cân bằng vốn khi dừng một provider
                  </span>
                </div>
                <ToggleSwitch
              ...
              <div className="flex items-center justify-between" style={{ padding: '12px 0' }}>
                <div>
                  ...
                  <span style={{ color: c.text3, fontSize: WEB_FONT.sm }}>
                    Số provider copy đồng thời tối đa
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <input
...
const NOTIF_ITEMS = [
  {
    ...
    ts: '14:32',
    title: 'Trade copied',
    desc: 'BUY 0.01 BTC @ $68,450 từ AlphaHunter_VN',
    type: 'trade' as const,
  },
  {
    id: 'n2',
    ts: '12:15',
    title: 'P/L Alert',
    desc: 'SteadyGains_Pro đạt +$67.20 (+6.72%)',
    type: 'alert' as const,
  },
  {
    id: 'n3',
    ts: '10:00',
    title: 'Risk Warning',
    desc: 'CryptoSensei đạt daily loss limit -3%',
  ...
  },
  {
    ...
    ts: 'Hôm qua',
    title: 'Copy paused',
    desc: 'CryptoSensei tạm dừng do risk limit',
    type: 'system' as const,
  },
  {
    id: 'n5',
    ts: '2 ngày',
    title: 'New copy started',
    desc: 'Bắt đầu copy CryptoSensei với $300',
    type: 'system' as const,
  },
];
          ...
          {NOTIF_ITEMS.map((n, i) => {
            ...
            return (
              <div
                ...
                style={{
                  padding: '16px 20px',
                  borderBottom: i < NOTIF_ITEMS.length - 1 ? `1px solid ${c.divider}` : 'none',
                }}
              >
                <div
                  className="flex items-center justify-center shrink-0"
                  style={{ width: 40, height: 40, borderRadius: 10, background: cfg.color + '12' }}
                >
                ...
                <span style={{ color: c.text3, fontSize: WEB_FONT.xs, whiteSpace: 'nowrap' }}>
                  {n.ts}
                </span>
              </div>
            );
      ...
      <div style={{ padding: '24px 0 40px' }}>
        ...
        >
          ...
          <StatCard
            icon={TrendingUp}
            label="P/L"
            value={fmtSignedUsd(copy.pnl)}
            color={copy.pnl >= 0 ? '#10B981' : '#EF4444'}
            sub={`${copy.pnlPct >= 0 ? '+' : ''}${copy.pnlPct.toFixed(2)}%`}
          />
        ...
        <div className="flex gap-3">
          <button
            onClick={() => navigate(`/w/trade/copy-performance/${copyId}/attribution`)}
          ...
          <button
            onClick={() => navigate(`/w/trade/copy-audit-log/${copyId}`)}
            className="flex items-center gap-2"
            style={{
              height: WEB_BUTTON.md,
              padding: '0 20px',
...
const ATTR_DATA = [
  { asset: 'BTC/USDT', contribution: 45.2, trades: 12, pnl: 28.5 },
  { asset: 'ETH/USDT', contribution: 30.1, trades: 8, pnl: 18.2 },
  { asset: 'SOL/USDT', contribution: 15.5, trades: 5, pnl: -4.3 },
  { asset: 'BNB/USDT', contribution: 9.2, trades: 3, pnl: 0.1 },
];
          ...
          {ATTR_DATA.map((d, i) => (
            <div
              ...
              style={{
                gridTemplateColumns: '1fr 120px 80px 120px 200px',
                padding: '14px 20px',
                borderBottom: i < ATTR_DATA.length - 1 ? `1px solid ${c.divider}` : 'none',
              }}
            >
              ...
              <div
                style={{ height: 8, borderRadius: 4, background: c.surface2, overflow: 'hidden' }}
              >
                <div
                  style={{
...
export function WebCopyAuditLogPage() {
  ...
  return (
    <PageLayout>
      <Header
        title="Audit Log"
        subtitle="Nhật ký hoạt động · Copy Trading"
        back
        ...
        >
          ...
          {filtered.map((e, i) => (
            ...
            >
              ...
              <span
                style={{
                  color: e.pnl > 0 ? '#10B981' : e.pnl < 0 ? '#EF4444' : c.text3,
                  fontSize: WEB_FONT.sm,
                  fontWeight: 600,
                  fontVariantNumeric: 'tabular-nums',
                }}
...
const RISK_METRICS = [
  {
    ...
    value: '$42.50',
    color: '#EF4444',
    desc: 'Tổn thất tối đa dự kiến trong 1 ngày',
  },
  {
    label: 'Correlation Risk',
    value: 'Trung bình',
    color: '#F59E0B',
    desc: 'Mức tương quan giữa các providers',
  },
  {
    label: 'Concentration Risk',
    value: 'Thấp',
    color: '#10B981',
    desc: 'Mức tập trung vào 1 provider',
  },
  {
    label: 'Liquidity Risk',
    value: 'Thấp',
    color: '#10B981',
    desc: 'Rủi ro thanh khoản khi rút vốn',
  },
];
      ...
      <div style={{ padding: '24px 0 40px' }}>
        ...
        >
          ...
          <p style={{ color: c.text2, fontSize: WEB_FONT.sm, lineHeight: 1.6, margin: 0 }}>
            Phân tích rủi ro dựa trên dữ liệu lịch sử và mô hình thống kê. Kết quả thực tế có thể
            khác biệt đáng kể.
          </p>
        </div>
      </div>
