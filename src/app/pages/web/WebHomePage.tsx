...
import {
  TrendingUp, Eye, EyeOff, ArrowUpRight, ArrowDownLeft,
  ChevronRight, BarChart3, Globe, Zap, Target, Layers, PieChart, Award,
  ArrowLeftRight, Activity, Wallet, Shield, Clock, Users,
} from 'lucide-react';
...
/* ─── Stat Widget ─── */
function StatWidget({ label, value, sub, icon: Icon, color, onClick }: {
  label: string; value: string; sub?: string; icon: React.ComponentType<any>; color: string; onClick?: () => void;
}) {
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="flex-1 min-w-0">
        <p style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600, letterSpacing: 0.3, textTransform: 'uppercase' }}>{label}</p>
        <p style={{ color: c.text1, fontSize: WEB_FONT.xl, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{value}</p>
        {sub && <p style={{ color: c.text3, fontSize: WEB_FONT.xs, fontVariantNumeric: 'tabular-nums' }}>{sub}</p>}
      </div>
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: c.surface, border: `1px solid ${c.border}` }}>
          ...
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={{
                ...
                fontSize: WEB_FONT.sm, fontWeight: tab === t.id ? 600 : 500,
              }}>
              {t.label}
            </button>
          ))}
        </div>
        <button onClick={() => navigate('/w/markets')}
          className="flex items-center gap-1" style={{ color: '#3B82F6', fontSize: WEB_FONT.sm, fontWeight: 600 }}>
      ...
      <div className="grid items-center px-5 py-2"
        ...
        <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600 }}>Cặp giao dịch</span>
        ...
        <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600, textAlign: 'center' }}>Biểu đồ</span>
        <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600, textAlign: 'right' }}>Volume</span>
        <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600, textAlign: 'right' }}>Market Cap</span>
      </div>
      ...
      {filtered.map((pair, i) => {
        ...
        return (
          <button key={pair.id} onClick={() => navigate(`/w/trade/${pair.id}`)}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <span style={{ color: c.text3, fontSize: WEB_FONT.xs, marginLeft: 4 }}>/{pair.quoteAsset}</span>
              </div>
            </div>
...
function RecentActivityTable() {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: c.surface, border: `1px solid ${c.border}` }}>
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${c.divider}` }}>
        <span style={{ color: c.text1, fontSize: WEB_FONT.base, fontWeight: 700 }}>Hoạt động gần đây</span>
        <button onClick={() => navigate('/w/wallet/history')}
          style={{ color: '#3B82F6', fontSize: WEB_FONT.sm, fontWeight: 600 }}>Xem tất cả</button>
      </div>
      {recentTx.map((tx, i) => {
        ...
        return (
          <div key={tx.id} className="flex items-center gap-3 px-5 py-2.5"
            ...
            <div className="flex-1 min-w-0">
              ...
              <span style={{ color: c.text3, fontSize: WEB_FONT.sm, marginLeft: 6 }}>{tx.asset}</span>
            </div>
    ...
    <div className="rounded-xl overflow-hidden" style={{ background: c.surface, border: `1px solid ${c.border}` }}>
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${c.divider}` }}>
        <div className="flex items-center gap-2">
          <span style={{ color: c.text1, fontSize: WEB_FONT.base, fontWeight: 700 }}>Lệnh đang mở</span>
        ...
        <button onClick={() => navigate('/w/trade/orders-history')}
          style={{ color: '#3B82F6', fontSize: WEB_FONT.sm, fontWeight: 600 }}>Xem tất cả</button>
      </div>
      {OPEN_ORDERS.length === 0 ? (
        ...
        OPEN_ORDERS.slice(0, 4).map((order, i) => (
          <div key={order.id} className="flex items-center gap-3 px-5 py-2.5"
            style={{ borderBottom: i < Math.min(OPEN_ORDERS.length, 4) - 1 ? `1px solid ${c.divider}` : 'none' }}>
            ...
            <span style={{ color: c.text1, fontSize: WEB_FONT.sm, fontWeight: 600, flex: 1 }}>{order.symbol}</span>
            <span style={{ color: c.text2, fontSize: WEB_FONT.sm, fontVariantNumeric: 'tabular-nums' }}>
        ...
        {items.map((item, i) => {
          ...
          return (
            <button key={item.label} onClick={() => navigate(item.path)}
              ...
              style={{ borderRight: (i + 1) % 4 !== 0 ? `1px solid ${c.divider}` : 'none', borderBottom: i < 4 ? `1px solid ${c.divider}` : 'none' }}>
              <div className="w-9 h-9 rounded-lg flex items-center justify-center"
                style={{ background: item.color + '10', border: `1px solid ${item.color}20` }}>
                <Icon size={17} color={item.color} />
              </div>
              <span style={{ color: c.text2, fontSize: WEB_FONT.xs, fontWeight: 500 }}>{item.label}</span>
            </button>
          );
        })}
      </div>
      ...
      <Header
        ...
        right={
          <button onClick={toggleBalanceHidden}
            ...
            {isBalanceHidden ? <EyeOff size={14} color={c.text3} /> : <Eye size={14} color={c.text3} />}
            <span style={{ color: c.text2, fontSize: WEB_FONT.sm }}>{isBalanceHidden ? 'Hiện số dư' : 'Ẩn số dư'}</span>
          </button>
        }
      />
