...
import {
  Search, Star, Shield, X, Plus, CreditCard, BarChart3, MessageCircle,
  HelpCircle, SortDesc, CheckCircle, Clock, ChevronRight,
} from 'lucide-react';
      ...
      <Header
        ...
        right={
          <div className="flex items-center gap-2">
            <button onClick={() => navigate('/w/p2p/my-orders')}
              ...
              {processingCount > 0 && (
                <span className="rounded-full px-1.5 py-0.5" style={{ background: '#F59E0B', color: '#fff', fontSize: 10, fontWeight: 700 }}>{processingCount}</span>
              )}
            </button>
            <button onClick={() => navigate('/w/p2p/create')}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg"
              style={{ background: '#3B82F6', color: '#fff', fontSize: WEB_FONT.sm, fontWeight: 600 }}>
      ...
      <div className="flex gap-6 py-6 px-6" style={{ maxWidth: 1200 }}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mb-4">
              ...
              {['USDT', 'BTC', 'ETH'].map(a => (
                <button key={a} onClick={() => setAsset(a)}
                  ...
                  style={{
                    ...
                    fontSize: WEB_FONT.sm, fontWeight: asset === a ? 600 : 500,
                    border: asset === a ? `1px solid ${c.chipActiveBorder}` : '1px solid transparent',
                  }}>
                  {a}
                </button>
              ))}
            ...
            <div className="flex items-center gap-2 rounded-lg px-3 flex-1"
              ...
              <input type="number" inputMode="decimal" placeholder="Nhập số tiền VND..."
                value={amountInput} onChange={e => setAmountInput(e.target.value)}
                className="flex-1 bg-transparent outline-none"
                style={{ color: c.text1, fontSize: WEB_FONT.base, fontVariantNumeric: 'tabular-nums' }} />
            ...
            <div className="flex items-center gap-2 rounded-lg px-3"
              style={{ background: c.searchBg, border: `1px solid ${c.searchBorder}`, height: 38, width: 180 }}>
              <Search size={14} color={c.text3} />
              <input placeholder="Tìm merchant..." value={searchText}
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: c.surface, border: `1px solid ${c.border}` }}>
            ...
            <div className="grid items-center px-5 py-2.5"
              style={{ gridTemplateColumns: '2fr 1fr 1.5fr 1fr 120px', borderBottom: `1px solid ${c.divider}` }}>
              <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600 }}>Merchant</span>
              <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600, textAlign: 'right' }}>Giá / {asset}</span>
              <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600, textAlign: 'right' }}>Giới hạn</span>
              <span style={{ color: c.text3, fontSize: WEB_FONT.xs, fontWeight: 600, textAlign: 'center' }}>Thanh toán</span>
              <span />
            </div>
            ...
            {ads.length === 0 ? (
              <div className="flex flex-col items-center py-12 gap-2">
                ...
                <p style={{ color: c.text3, fontSize: WEB_FONT.base }}>Không tìm thấy quảng cáo phù hợp</p>
              </div>
            ) : (
              ads.map((ad, i) => {
                ...
                return (
                  <div key={ad.id}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div className="w-9 h-9 rounded-full flex items-center justify-center"
                          ...
                          <span style={{ color: '#fff', fontSize: 13, fontWeight: 700 }}>{ad.merchant.charAt(0)}</span>
                        </div>
                        {ad.isOnline && (
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span style={{ color: c.text1, fontSize: WEB_FONT.base, fontWeight: 600 }}>{ad.merchant}</span>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ color: c.text1, fontSize: WEB_FONT.md, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
                        {fmtVnd(ad.price)}
                      </p>
                      <p style={{ color: margin > 0 ? '#EF4444' : '#10B981', fontSize: WEB_FONT.xs, fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    {/* Limits */}
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ color: c.text2, fontSize: WEB_FONT.sm, fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    <div className="flex flex-wrap justify-center gap-1">
                      {ad.paymentMethods.slice(0, 2).map(pm => (
                        <span key={pm} className="px-2 py-0.5 rounded text-center"
                          style={{ background: 'rgba(59,130,246,0.08)', color: '#3B82F6', fontSize: 10, fontWeight: 600 }}>
                          {pm}
                        </span>
                      ))}
                      {ad.paymentMethods.length > 2 && (
                        <span style={{ color: c.text3, fontSize: 10 }}>+{ad.paymentMethods.length - 2}</span>
                      )}
                    </div>

                    {/* Action */}
                    <div className="flex justify-end">
                      <button onClick={() => navigate(`/w/p2p/ad/${ad.id}`)}
        ...
        <div className="flex flex-col gap-4" style={{ width: 300, flexShrink: 0 }}>
          ...
          <div className="rounded-xl p-4" style={{ background: c.surface, border: `1px solid ${c.border}` }}>
            <p style={{ color: c.text1, fontSize: WEB_FONT.base, fontWeight: 700, marginBottom: 12 }}>Bộ lọc</p>

            <label style={{ color: c.text3, fontSize: WEB_FONT.sm, fontWeight: 600, display: 'block', marginBottom: 6 }}>Sắp xếp</label>
              ...
              {[
                ...
                <button key={s.id} onClick={() => setSortBy(s.id)}
                  ...
                  style={{
                    ...
                    fontSize: 12, fontWeight: sortBy === s.id ? 600 : 500,
            ...
            <label style={{ color: c.text3, fontSize: WEB_FONT.sm, fontWeight: 600, display: 'block', marginBottom: 6 }}>Thanh toán</label>
            <div className="flex flex-wrap gap-1.5">
              <button onClick={() => setFilterPayment('')}
                ...
                style={{
                  ...
                  fontSize: 11, fontWeight: 600, border: `1px solid ${!filterPayment ? c.chipActiveBorder : c.border}`,
                }}>Tất cả</button>
              {allPaymentMethods.map(pm => (
                <button key={pm} onClick={() => setFilterPayment(pm)}
                  ...
                  style={{
                    ...
                    fontSize: 11, fontWeight: 600, border: `1px solid ${filterPayment === pm ? c.chipActiveBorder : c.border}`,
                  }}>
                  {pm}
                </button>
              ))}
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: c.surface, border: `1px solid ${c.border}` }}>
            <div className="px-4 py-3" style={{ borderBottom: `1px solid ${c.divider}` }}>
              <span style={{ color: c.text1, fontSize: WEB_FONT.base, fontWeight: 700 }}>Truy cập nhanh</span>
            </div>
            {[
              { label: 'Đơn P2P', icon: MessageCircle, path: '/w/p2p/my-orders', badge: processingCount },
              ...
              return (
                <button key={item.label} onClick={() => navigate(item.path)}
                  ...
                  <span className="flex-1 text-left" style={{ color: c.text2, fontSize: WEB_FONT.sm }}>{item.label}</span>
                  {item.badge && item.badge > 0 && (
                    <span className="rounded-full px-1.5 py-0.5"
                      style={{ background: '#F59E0B', color: '#fff', fontSize: 9, fontWeight: 700, minWidth: 18, textAlign: 'center' }}>
                      {item.badge}
                    </span>
                  )}
