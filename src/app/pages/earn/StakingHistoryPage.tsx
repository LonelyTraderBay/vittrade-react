...
export function StakingHistoryPage() {
  ...
  const totalStaked = TRANSACTIONS.filter((tx) => tx.type === 'stake').reduce(
    (sum, tx) => sum + tx.usdValue,
    0,
  );
  const totalEarned = TRANSACTIONS.filter(
    (tx) => tx.type === 'claim' || tx.type === 'compound',
  ).reduce((sum, tx) => sum + tx.usdValue, 0);
  const totalUnstaked = TRANSACTIONS.filter((tx) => tx.type === 'unstake').reduce(
    (sum, tx) => sum + tx.usdValue,
    0,
  );
  ...
  return (
    <PageLayout>
      ...
      {/* Filter Bottom Sheet */}
      <BottomSheetV2 open={showFilters} onClose={() => setShowFilters(false)} title="Bộ lọc">
        <div className="flex flex-col gap-4">
          <div>
            <p style={{ color: c.text2, fontSize: 13, marginBottom: 8 }}>Loại giao dịch</p>
            <div className="flex flex-wrap gap-2">
              {(['all', 'stake', 'unstake', 'claim', 'compound', 'penalty'] as const).map(
                (type) => (
                  <button
                    key={type}
        ...
        {selectedTx && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              >
                {React.createElement(TYPE_CONFIG[selectedTx.type].icon, {
                  size: 24,
                  color: TYPE_CONFIG[selectedTx.type].color,
                })}
              </div>
            ...
            <div className="rounded-2xl p-4" style={{ background: c.surface2 }}>
              <div className="space-y-3">
                ...
                {selectedTx.note && (
                  <div>
                    <span
                      style={{ color: c.text3, fontSize: 13, display: 'block', marginBottom: 4 }}
                    >
                      Ghi chú
                    </span>
      ...
      <PageContent>
        ...
        <TrCard className="p-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <p
                style={{ color: '#3B82F6', fontSize: 16, fontWeight: 700, fontFamily: 'monospace' }}
              >
                +{fmtUsd(totalEarned)}
              </p>
            </div>
            <div>
              ...
              <p
                style={{ color: '#F59E0B', fontSize: 16, fontWeight: 700, fontFamily: 'monospace' }}
              >
          ...
          {filtered.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {filtered.map((tx) => {
                ...
                return (
                  <TrCard key={tx.id} hover className="p-3" onClick={() => setSelectedTx(tx)}>
                    <div className="flex items-center gap-3">
                      <div
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p style={{ color: c.text3, fontSize: 10 }}>{fmtUsd(tx.usdValue)}</p>
                      </div>
                    </div>
                  </TrCard>
                );
        ...
        <div className="rounded-2xl p-4" style={{ background: c.surface2 }}>
          <p style={{ color: c.text3, fontSize: 11, lineHeight: 1.6, textAlign: 'center' }}>
            Lịch sử giao dịch được lưu trữ vĩnh viễn. Bạn có thể xuất CSV để lưu trữ hoặc khai báo
            thuế. Giá trị USD tính theo tỷ giá tại thời điểm giao dịch.
          </p>
        </div>
      </PageContent>
    </PageLayout>
