          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              <div
                ...
                style={{ background: c.borderSolid, width: '55%' }}
              />
...
export function SavingsPage() {
  ...
  /* Computed */
  const filtered =
    typeFilter === 'all' ? SAVINGS_PRODUCTS : SAVINGS_PRODUCTS.filter((p) => p.type === typeFilter);

  const totalDeposited = MY_SAVINGS.reduce(
    (s, p) =>
      s +
      (p.asset === 'USDT'
        ? p.amount
  ...
  const totalEarned = MY_SAVINGS.reduce(
    (s, p) =>
      s +
      (p.asset === 'USDT'
        ? p.earned
  ...
  const avgApy =
    MY_SAVINGS.length > 0 ? MY_SAVINGS.reduce((s, p) => s + p.apy, 0) / MY_SAVINGS.length : 0;
  ...
  const canRedeem = redeemPosition
    ? redeemNum > 0 && redeemNum <= redeemPosition.amount && redeemAgreed
    : false;
  ...
  return (
    <PageLayout>
            ...
            return (
              <div className="flex flex-col gap-5">
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(145deg, ${selectedProduct.color}${ALPHA.muted} 0%, ${selectedProduct.color}${ALPHA.ghost} 100%)`,
                    border: `1px solid ${selectedProduct.color}${ALPHA.soft}`,
                  }}
                >
                  <div className="flex items-center gap-3 p-4 pb-3">
                    <div
                      ...
                      style={{
                        background: selectedProduct.color + ALPHA.soft,
                        border: `2px solid ${selectedProduct.color}${ALPHA.border}`,
                        boxShadow: `0 0 16px ${selectedProduct.color}${ALPHA.muted}`,
                      }}
                    >
                      <span
                        style={{
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        >
                          {selectedProduct.type === 'flexible'
                            ? 'Linh hoạt'
                            : `Cố định ${selectedProduct.lockDays}D`}
                        </span>
                        <span
                          className="px-1.5 py-0.5 rounded-md"
                          style={{
                            background: `${RISK_COLORS[risk]}${ALPHA.muted}`,
                            fontSize: 9,
                            fontWeight: FONT_WEIGHT.semibold,
                  ...
                  <div className="text-center px-4 pb-4 pt-1">
                    ...
                    {selectedProduct.apyBonus && (
                      ...
                      >
                        <TrendingUp size={10} strokeWidth={ICON_STROKE.emphasis} /> Tối đa{' '}
                        {selectedProduct.apyBonus}% cho VIP
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    {
                      label: 'Tối thiểu',
                      value: `${fmtAmount(selectedProduct.minAmount)} ${selectedProduct.asset}`,
                      icon: BarChart3,
                      color: selectedProduct.color,
                    },
                    {
                      label: 'Còn lại',
                      value: selectedProduct.remainingQuota,
                      icon: Users,
                      color: '#3B82F6',
                    },
                  ].map((item) => (
                ...
                <div>
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        background: `${selectedProduct.color}${ALPHA.muted}`,
                        border: `1px solid ${selectedProduct.color}${ALPHA.soft}`,
                      }}
                    >
                      <div
                  ...
                  {amountNum > 0 && amountNum < selectedProduct.minAmount && (
                    <p
                      ...
                      style={{ color: c.error, fontSize: FONT_SCALE.xs }}
                    >
                      <AlertTriangle size={12} strokeWidth={ICON_STROKE.emphasis} /> Tối thiểu{' '}
                      {fmtAmount(selectedProduct.minAmount)} {selectedProduct.asset}
                    </p>
                  )}
                  {amountNum > balance && (
                    <p
                      ...
                      style={{ color: c.error, fontSize: FONT_SCALE.xs }}
                    >
                      <AlertTriangle size={12} strokeWidth={ICON_STROKE.emphasis} /> Vượt quá số dư
                      khả dụng
                    </p>
                  )}
                  <div className="flex gap-2 mt-3">
                    {[
                      ...
                      const isActive =
                        amount !== '' && Math.abs(parseFloat(amount) - balance * q.pct) < 0.000001;
                      return (
                        <button
                          key={q.label}
                ...
                {amountNum >= selectedProduct.minAmount && amountNum <= balance && (
                  <div
                    className="rounded-2xl overflow-hidden"
                    style={{
                      background: `linear-gradient(135deg, ${withAlpha(c.success, ALPHA.ghost)} 0%, transparent 100%)`,
                      border: `1px solid ${withAlpha(c.success, ALPHA.soft)}`,
                    }}
                  >
                    <div
                      className="px-4 py-3 flex items-center gap-2"
                      style={{ borderBottom: `1px solid ${withAlpha(c.success, ALPHA.ghost)}` }}
                    >
                      <TrendingUp
                        size={ICON_SIZE.sm}
                        color={c.success}
                    ...
                    <div className="px-4 py-2">
                      ...
                      >
                        ...
                        >
                          {selectedProduct.lockDays
                            ? `${selectedProduct.lockDays} ngày`
                            : '365 ngày (linh hoạt)'}
                        </span>
                      </div>
                ...
                {selectedProduct.type === 'locked' && (
                  <div
                    ...
                    style={{
                      background: withAlpha(c.warning, ALPHA.ghost),
                  ...
                  >
                    ...
                    <span style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.6 }}>
                      Sản phẩm cố định{' '}
                      <span style={{ fontWeight: FONT_WEIGHT.semibold, color: c.warning }}>
                        không thể rút sớm
                      </span>{' '}
                      hoặc sẽ mất lãi nếu rút trước hạn. Vui lòng đảm bảo bạn không cần sử dụng
                      khoản tiền này trong{' '}
                ...
                <button
                  ...
                  style={{
                    background: agreed ? withAlpha(c.success, ALPHA.ghost) : c.surface2,
                    border: `1px solid ${agreed ? withAlpha(c.success, ALPHA.soft) : c.border}`,
                    transition: 'all 0.2s ease',
                  }}
                >
                  <div
                  ...
                  >
                    ...
                    <span style={{ color: c.primary, fontWeight: FONT_WEIGHT.medium }}>
                      điều khoản Tiết kiệm
                    </span>{' '}
                    và{' '}
                    <span style={{ color: c.primary, fontWeight: FONT_WEIGHT.medium }}>
                      chính sách rủi ro
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3" style={{ marginTop: -4 }}>
              <div
                ...
                style={{
                  background: redeemPosition.color + '22',
                  border: `1.5px solid ${redeemPosition.color}44`,
                }}
              >
                <span
                  style={{
                    color: redeemPosition.color,
                    fontSize: FONT_SCALE.micro,
                    fontWeight: FONT_WEIGHT.bold,
                  }}
                >
            ...
            <BottomSheetRow
              label="Lãi tích luỹ"
              value={`+${fmtAmount(redeemPosition.earned)} ${redeemPosition.asset}`}
              valueColor="#10B981"
            />
            <BottomSheetRow
              label="APY hiện tại"
              value={`${redeemPosition.apy}%`}
              valueColor="#10B981"
            />
            ...
            {redeemPosition.type === 'locked' && redeemPosition.endDate && (
              <div
                ...
                style={{
                  background: withAlpha(c.error, ALPHA.ghost),
                  border: `1px solid ${withAlpha(c.error, ALPHA.soft)}`,
                }}
              >
                <AlertTriangle
                  size={ICON_SIZE.sm}
                ...
                <div>
                  ...
                  >
                    Sản phẩm cố định đáo hạn ngày {redeemPosition.endDate}. Rút trước hạn sẽ mất
                    toàn bộ lãi tích luỹ ({fmtAmount(redeemPosition.earned)} {redeemPosition.asset}
                    ).
                  </p>
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label style={{ color: c.text2, fontSize: FONT_SCALE.sm }}>
                  Số lượng rút ({redeemPosition.asset})
                </label>
                <span style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
                  Tối đa:{' '}
                  <span
              ...
              >
                ...
                />
                <span style={{ color: c.text2, fontSize: FONT_SCALE.sm }}>
                  {redeemPosition.asset}
                </span>
              </div>
              {redeemNum > redeemPosition.amount && (
                <p
                  ...
                  style={{ color: c.error, fontSize: FONT_SCALE.xs }}
                >
                  <AlertTriangle size={ICON_SIZE.sm} strokeWidth={ICON_STROKE.standard} /> Vượt quá
                  số lượng đang gửi
                </p>
              )}
              <div className="flex gap-2 mt-2">
                {[
                  ...
                  <button
                    key={q.label}
                    onClick={() => setQuickRedeemAmount(q.pct)}
            ...
            {redeemNum > 0 && redeemNum <= redeemPosition.amount && (
              <div className="rounded-2xl p-3" style={{ background: c.surface2 }}>
                <BottomSheetRow
                  label="Số lượng rút"
                  value={`${fmtAmount(redeemNum)} ${redeemPosition.asset}`}
                  highlight
                />
            ...
            >
              <div
                ...
                style={{
                  borderColor: redeemAgreed
                    ? redeemPosition.type === 'locked'
                      ? '#EF4444'
                      : '#10B981'
                    : c.borderSolid,
                  background: redeemAgreed
                    ? redeemPosition.type === 'locked'
                      ? '#EF4444'
                      : '#10B981'
                    : 'transparent',
      ...
      {showSuccess && (
        <div
          className="fixed top-24 left-4 right-4 z-50 rounded-2xl px-4 py-3 flex items-center gap-3"
          style={{
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs }}>
              Tiền sẽ về ví trong thời gian xử lý.
            </p>
          </div>
          <button onClick={() => setShowRedeemSuccess(false)}>
            <X size={ICON_SIZE.base} color={c.text3} strokeWidth={ICON_STROKE.standard} />
          </button>
        </div>
      )}
      ...
      >
        ...
        <div className="flex items-center justify-between mb-2 relative z-10">
          <span style={{ color: c.portfolioTextDim, fontSize: FONT_SCALE.sm }}>
            Tổng tiền gửi (USD)
          </span>
          <PiggyBank
            size={ICON_SIZE.base}
        ...
        <div className="flex gap-2.5 relative z-10">
          <button
            onClick={() => {
              navigate(`${prefix}/earn/savings/portfolio`);
              hapticSelection();
            }}
      ...
      <div className="flex flex-col gap-2 mx-5 mt-4">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            >
              APY có thể cao hơn 1.2%
            </p>
            <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
              3 gợi ý tối ưu danh mục cho bạn
            </p>
          </div>
        ...
        {MY_SAVINGS.some((p) => p.type === 'locked') && (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                BTC sắp đáo hạn 7 ngày nữa
              </p>
              <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
                Xem chi tiết và lên kế hoạch tái đầu tư
              </p>
            </div>
            <ChevronRight size={ICON_SIZE.sm} color={c.text3} className="shrink-0" />
          </button>
        ...
        <button
          onClick={() => {
            navigate(`${prefix}/earn/savings/dca`);
            hapticSelection();
          }}
          className="flex items-center gap-3 rounded-2xl p-3 w-full text-left"
          style={{ background: c.surface2, border: `1px solid ${c.divider}` }}
        >
          ...
          <div className="flex-1 min-w-0">
            <p
              style={{ color: c.text1, fontSize: FONT_SCALE.xs, fontWeight: FONT_WEIGHT.semibold }}
            >
              Tiết kiệm tự động mỗi tuần
            </p>
            <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
              Thiết lập DCA để tích luỹ đều đặn
            </p>
          </div>
          <ChevronRight size={ICON_SIZE.sm} color={c.text3} className="shrink-0" />
        </button>
      ...
      <div className="mx-5 mt-3">
        ...
        >
          ...
          <span
            style={{ color: c.text2, fontSize: FONT_SCALE.xs, fontWeight: FONT_WEIGHT.semibold }}
          >
            Công cụ nâng cao
          </span>
        ...
        <div className="flex flex-col gap-5">
          ...
          <ToolboxGroup label="Quản lý" color="#3B82F6" c={c}>
            ...
            <ToolboxItem
              icon={Clock}
              label="Lịch sử GD"
              desc="Giao dịch gần đây"
              color="#8B5CF6"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/history`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              icon={Download}
              label="Xuất báo cáo"
              desc="Tải file PDF/CSV"
              color="#3B82F6"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/export`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
          ...
          <ToolboxGroup label="Tối ưu hoá" color="#10B981" c={c}>
            ...
            <ToolboxItem
              icon={ArrowLeftRight}
              label="So sánh SP"
              desc="So sánh sản phẩm"
              color="#10B981"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/comparison`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              icon={RefreshCw}
              label="Lãi kép"
              desc="Tự động tái đầu tư"
              color="#3B82F6"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/auto-compound`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            ...
            <ToolboxItem
              icon={RefreshCw}
              label="Tái cân bằng"
              desc="Cân lại danh mục"
              color="#8B5CF6"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/rebalance`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
          ...
          <ToolboxGroup label="Phân tích" color="#F59E0B" c={c}>
            <ToolboxItem
              icon={BarChart3}
              label="Phân tích"
              desc="Biểu đồ & xu hướng"
              color="#10B981"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/analytics`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              icon={Lightbulb}
              label="Gợi ý AI"
              desc="Tín hiệu & xu hướng APY"
              color="#F59E0B"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/smart-suggestions`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              icon={Activity}
              label="Mô phỏng"
              desc="Backtest chiến lược"
              color="#8B5CF6"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/backtest`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              icon={Layers}
              label="Ladder"
              desc="Bậc thang đáo hạn"
              color="#F59E0B"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/ladder`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              icon={CloudLightning}
              label="What-If"
              desc="Kịch bản stress test"
              color="#EF4444"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/whatif`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
          ...
          <ToolboxGroup label="Cài đặt & An toàn" color="#8B5CF6" c={c}>
            <ToolboxItem
              icon={Target}
              label="Mục tiêu"
              desc="Đặt mục tiêu tài chính"
              color="#F59E0B"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/goals`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              icon={Brain}
              label="AutoPilot"
              desc="Quản lý tự động"
              color="#10B981"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/autopilot`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            ...
            <ToolboxItem
              icon={Bell}
              label="Cài đặt TB"
              desc="Tuỳ chỉnh thông báo"
              color="#EF4444"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/notification-preferences`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
          ...
          <ToolboxGroup label="Hỗ trợ" color="#6B7280" c={c}>
            <ToolboxItem
              icon={Info}
              label="Hướng dẫn"
              desc="Cách sử dụng Tiết kiệm"
              color="#10B981"
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/guide`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              ...
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/faq`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
            <ToolboxItem
              ...
              c={c}
              onClick={() => {
                navigate(`${prefix}/earn/savings/notifications`);
                setShowToolbox(false);
                hapticSelection();
              }}
            />
              ...
              {MY_SAVINGS.map((pos) => {
                const daysLeft = pos.endDate
                  ? Math.max(
                      0,
                      Math.ceil(
                        (new Date(pos.endDate.split('/').reverse().join('-')).getTime() -
                          Date.now()) /
                          86400000,
                      ),
                    )
                  ...
                  <TrCard key={pos.id} className="p-4">
                    ...
                    {progress !== null && daysLeft !== null && (
                      <div className="mb-3">
                        <div className="flex justify-between mb-1">
                          ...
                          <span
                            style={{
                              color: daysLeft <= 3 ? '#F59E0B' : c.text2,
                              fontSize: FONT_SCALE.xs,
                              fontWeight: FONT_WEIGHT.semibold,
                            }}
                          >
                        ...
                        <div className="h-1.5 rounded-full" style={{ background: c.borderSolid }}>
                          <div
                            ...
                            style={{ background: pos.color, width: `${progress}%` }}
                          />
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2">
                      {pos.type === 'flexible' && (
                        <button
                          onClick={() => {
                            navigate(`${prefix}/earn/savings/redeem/${pos.id}`);
                            hapticSelection();
                          }}
