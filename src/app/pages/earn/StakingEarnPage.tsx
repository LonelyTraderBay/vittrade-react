          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              <div
                ...
                style={{ background: c.borderSolid, width: '60%' }}
              />
...
export function StakingEarnPage() {
  ...
  const totalEarned = MY_POSITIONS.reduce(
    (s, p) => s + (p.asset === 'USDT' ? p.earned : p.earned * 67543),
    0,
  );
        ...
        {selectedProduct && (
          <div className="flex flex-col gap-4">
            ...
            >
              ...
              <p style={{ color: '#10B981', fontSize: 32, fontWeight: 800 }}>
                {selectedProduct.apy}%
              </p>
              {selectedProduct.apyBoost && (
                <p style={{ color: '#F59E0B', fontSize: 12 }}>
                  🔥 Boost: {selectedProduct.apyBoost}% với VIP
                </p>
              )}
            </div>
            <BottomSheetRow
              ...
              value={
                selectedProduct.type === 'fixed'
                  ? `Cố định ${selectedProduct.lockDays} ngày`
            ...
            <BottomSheetRow
              label="Tối thiểu"
              value={`${selectedProduct.minAmount} ${selectedProduct.asset}`}
            />
            <BottomSheetRow
              label="Rủi ro"
              value={RISK_LABELS[selectedProduct.riskLevel]}
              valueColor={RISK_COLORS[selectedProduct.riskLevel]}
            />
            <div>
              <label style={{ color: c.text2, fontSize: 13, display: 'block', marginBottom: 6 }}>
                Số lượng ({selectedProduct.asset})
              </label>
              <div
              ...
              >
                <input
                  type="number"
                  inputMode="decimal"
                  placeholder={`Tối thiểu ${selectedProduct.minAmount}`}
                  value={stakeAmount}
            ...
            {amountNum > 0 && (
              <div className="rounded-2xl p-3" style={{ background: c.surface2 }}>
                <BottomSheetRow
                  ...
                  value={`+${estimatedEarning.toFixed(6)} ${selectedProduct.asset}`}
                  valueColor="#10B981"
                  highlight
                />
                <BottomSheetRow
                  label="Thời gian"
                  value={
                    selectedProduct.lockDays ? `${selectedProduct.lockDays} ngày` : 'Linh hoạt'
                  }
                />
              </div>
            )}
            ...
            >
              ...
              <span style={{ color: c.text2, fontSize: 12, lineHeight: 1.5, textAlign: 'left' }}>
                ...
                <span style={{ color: '#3B82F6' }}>điều khoản Staking</span>. Tôi hiểu các rủi ro
                liên quan.
              </span>
            </button>
            <button
              ...
              style={{
                ...
                background: canStake
                  ...
                  : c.surface2,
                color: canStake ? '#fff' : c.text3,
                boxShadow: canStake ? '0 4px 20px rgba(16,185,129,0.3)' : 'none',
                fontSize: 16,
              }}
            >
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p style={{ color: c.text2, fontSize: 12 }}>
              Staking đã được kích hoạt, lãi tính từ hôm nay
            </p>
          </div>
        ...
        >
          ...
          <div className="flex gap-4 relative z-10">
            <div className="flex-1">
              ...
              <p
                style={{ color: '#34D399', fontSize: 24, fontWeight: 700, fontFamily: 'monospace' }}
              >
                +{fmtUsd(totalEarned)}
              </p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, marginTop: 2 }}>
                Tích luỹ từ trước đến nay
              </p>
            </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span
                          ...
                          style={{ color: c.text1, fontSize: 15, fontWeight: 700 }}
                        >
                          {product.name}
                        </span>
                        {product.isHot && (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span style={{ color: c.text2, fontSize: 12 }}>
                          ...
                          {product.participants.toLocaleString()} người
                        </span>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p style={{ color: '#10B981', fontSize: 20, fontWeight: 800 }}>
                        {product.apy}%
                      </p>
                      <p style={{ color: c.text3, fontSize: 10 }}>APY</p>
                      {product.apyBoost && (
                        <p style={{ color: '#F59E0B', fontSize: 10 }}>🔥 Max {product.apyBoost}%</p>
                      )}
                    </div>
                  </div>
                  <div className="mt-3">
                    <div className="flex justify-between mb-1">
                      <span style={{ color: c.text3, fontSize: 11 }}>
                        Đã staking: {product.totalStaked}
                      </span>
                      <span style={{ color: RISK_COLORS[product.riskLevel], fontSize: 11 }}>
                        Rủi ro: {RISK_LABELS[product.riskLevel]}
                      </span>
                    </div>
                    <div className="h-1.5 rounded-full" style={{ background: c.borderSolid }}>
                      <div
                        ...
                        style={{
                          ...
                          width: `${STAKING_PROGRESS[product.id] ?? 50}%`,
                        }}
                      />
                    </div>
                  </div>
            ...
            {MY_POSITIONS.length === 0 ? (
              ...
              <div className="contents">
                {MY_POSITIONS.map((pos) => {
                  ...
                  const progress =
                    daysLeft != null
                      ? Math.max(0, Math.min(100, ((pos.endDate ? 90 - daysLeft : 0) / 90) * 100))
                      : null;
                  return (
                    <TrCard key={pos.id} hover className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        >
                          <span style={{ color: pos.color, fontSize: 9, fontWeight: 700 }}>
                            {pos.asset.slice(0, 3)}
                          </span>
                        </div>
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span style={{ color: c.text2, fontSize: 11 }}>
                              {pos.type === 'fixed' ? 'Cố định' : 'Linh hoạt'}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        {[
                          { label: 'Đang staking', value: `${pos.amount} ${pos.asset}` },
                          {
                            label: 'Đã nhận',
                            value: `+${pos.earned} ${pos.asset}`,
                            color: '#10B981',
                          },
                          { label: 'Bắt đầu', value: pos.startDate },
                          {
                            label: pos.endDate ? 'Kết thúc' : 'Trạng thái',
                            value: pos.endDate ?? 'Đang hoạt động',
                          },
                        ].map((r) => (
                          <div
                            key={r.label}
                      ...
                      <div className="flex gap-2">
                        <button
                          className="flex-1 py-2 rounded-xl text-xs font-semibold hover-chip"
                          style={{
                ...
                <TrCard className="p-4" accentBorder="rgba(16,185,129,0.2)">
                  ...
                  <div className="flex justify-between">
                    {[
                      { l: 'Hàng ngày', v: '+$1.42' },
                      { l: 'Hàng tháng', v: '+$42.60' },
                      { l: 'Hàng năm', v: '+$511.20' },
                    ].map((r) => (
                      <div key={r.l}>
                        <p style={{ color: c.text3, fontSize: 11 }}>{r.l}</p>
                        <p style={{ color: '#10B981', fontSize: 14, fontWeight: 700 }}>{r.v}</p>
                      </div>
                    ))}
                  </div>
                </TrCard>
