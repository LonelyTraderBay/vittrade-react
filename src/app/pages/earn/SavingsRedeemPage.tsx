...
export function SavingsRedeemPage() {
  ...
  const daysLeft = position.endDate
    ? Math.max(
        0,
        Math.ceil(
          (new Date(position.endDate.split('/').reverse().join('-')).getTime() - Date.now()) /
            86400000,
        ),
      )
    : null;
  ...
  const processingTime =
    position.type === 'flexible' ? 'Tức thì (< 1 phút)' : 'T+1 (1 ngày làm việc)';
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            />
            <p style={{ color: c.error, fontSize: FONT_SCALE.base, fontWeight: FONT_WEIGHT.bold }}>
              Bạn sẽ mất toàn bộ lãi
            </p>
            <p style={{ color: c.text2, fontSize: FONT_SCALE.sm, marginTop: 4, lineHeight: 1.5 }}>
              Rút sớm sản phẩm cố định sẽ mất {fmtAmount(lostInterest)} {position.asset} lãi đã tích
              luỹ. Hành động này không thể hoàn tác.
            </p>
          </div>

          <BottomSheetRow
            label="Rút"
            value={`${fmtAmount(amountNum)} ${position.asset}`}
            highlight
          />
          <BottomSheetRow
            label="Lãi mất"
            value={`-${fmtAmount(lostInterest)} ${position.asset}`}
            valueColor={c.error}
            highlight
          />
          <BottomSheetRow
            label="Thực nhận"
            value={`${fmtAmount(netReceived)} ${position.asset}`}
            highlight
          />
      ...
      <PageContent gap="relaxed" grow>
        ...
        <TrCard className="p-4">
          <div className="flex items-center gap-3 mb-4">
            <div
              ...
              style={{
                background: position.color + '22',
                border: `1.5px solid ${position.color}44`,
              }}
            >
              <span
                style={{
                ...
                {position.type === 'flexible' ? (
                  <>
                    <Unlock
                      ...
                      strokeWidth={ICON_STROKE.standard}
                    />
                    <span style={{ color: c.success, fontSize: FONT_SCALE.xs }}>Linh hoạt</span>
                  </>
                ) : (
                  <>
                    <Lock
                      ...
                      strokeWidth={ICON_STROKE.standard}
                    />
                    <span style={{ color: c.warning, fontSize: FONT_SCALE.xs }}>
                      Cố định · {daysLeft} ngày còn lại
                    </span>
                  </>
                )}
              </div>
            ...
            <div className="text-right">
              <p
                style={{ color: c.success, fontSize: FONT_SCALE.lg, fontWeight: FONT_WEIGHT.bold }}
              >
                {position.apy}%
              </p>
        ...
        {isEarlyRedeem && (
          <div
            ...
            style={{
              background: withAlpha(c.error, ALPHA.ghost),
              border: `1px solid ${withAlpha(c.error, ALPHA.muted)}`,
            }}
          >
            <AlertTriangle
              size={ICON_SIZE.base}
            ...
            />
            <div>
              <p style={{ color: c.error, fontSize: FONT_SCALE.sm, fontWeight: FONT_WEIGHT.bold }}>
                Cảnh báo rút sớm
              </p>
              <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.6, marginTop: 4 }}>
                Sản phẩm cố định {position.lockDays} ngày chưa đáo hạn (còn {daysLeft} ngày). Rút
                sớm sẽ{' '}
                <span style={{ color: c.error, fontWeight: FONT_WEIGHT.semibold }}>
                  mất toàn bộ lãi tích luỹ ({fmtAmount(position.earned)} {position.asset})
                </span>
            ...
            <PageSection label={`Số lượng rút (${position.asset})`}>
              ...
              {amountNum > position.amount && (
                ...
                >
                  ...
                  số lượng đang gửi ({fmtAmount(position.amount)} {position.asset})
                </p>
              )}
        ...
        {position.type === 'flexible' && (
          ...
          >
            ...
            <span style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.6 }}>
              Sản phẩm linh hoạt cho phép rút bất cứ lúc nào mà không mất lãi. Tiền sẽ về ví chính
              trong vòng vài phút.
            </span>
          </div>
        )}
        ...
        {isEarlyRedeem ? (
          ...
          >
            {processing
              ? 'Đang xử lý...'
              : canSubmit
                ? `Xác nhận rút · ${fmtAmount(amountNum)} ${position.asset}`
                : amountNum > 0
                  ? 'Vui lòng đồng ý điều khoản'
                  : 'Nhập số lượng rút'}
          </CTAButton>
