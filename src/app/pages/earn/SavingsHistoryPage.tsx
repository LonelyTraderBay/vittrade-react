...
const TYPE_CONFIG: Record<
  TxType,
  { label: string; icon: typeof ArrowDownToLine; color: string; bg: string }
> = {
  subscribe: {
    label: 'Gửi tiết kiệm',
...
export function SavingsHistoryPage() {
  ...
  /* ─── Summary stats ─── */
  const totalSubscribed = TRANSACTIONS.filter(
    (t) => t.type === 'subscribe' && t.status === 'completed',
  ).reduce((s, t) => s + t.usdValue, 0);
  const totalInterest = TRANSACTIONS.filter(
    (t) => t.type === 'interest' && t.status === 'completed',
  ).reduce((s, t) => s + t.usdValue, 0);
  const totalRedeemed = TRANSACTIONS.filter(
    (t) => (t.type === 'redeem' || t.type === 'early_redeem') && t.status === 'completed',
  ).reduce((s, t) => s + t.usdValue, 0);
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <TrCardStat>
                ...
                <span
                  style={{ color: c.accent, fontSize: FONT_SCALE.xs, fontWeight: FONT_WEIGHT.bold }}
                >
            ...
            <div className="flex items-center gap-2">
                ...
                {(
                  ...
                  <button
                    ...
                    style={{
                      background:
                        dateRange === opt.key
                          ? withAlpha(c.primary, ALPHA.muted)
                          : c.portfolioBtnGhost,
                      color: dateRange === opt.key ? c.primary : c.text3,
                      fontSize: FONT_SCALE.micro,
                      fontWeight:
                        dateRange === opt.key ? FONT_WEIGHT.semibold : FONT_WEIGHT.regular,
                    }}
                  >
                    {opt.label}
                  </button>
              ...
              >
                <Filter
                  size={ICON_SIZE.base}
                  color={hasActiveFilters ? c.primary : c.text3}
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: FONT_SCALE.xs,
                    fontWeight:
                      statusFilter === opt.key ? FONT_WEIGHT.semibold : FONT_WEIGHT.regular,
                  }}
                >
                  {opt.label}
                </button>
              ...
              <div className="px-5 pb-8 flex flex-col gap-4">
                ...
                <TrCard>
                  <div className="flex flex-col gap-0">
                    ...
                    <BottomSheetRow
                      label="Thời gian"
                      value={`${selectedTx.date} ${selectedTx.time}`}
                    />
                    {selectedTx.apy != null && (
                      <BottomSheetRow
                        label="APY"
                        value={`${selectedTx.apy}%`}
                        valueColor="#10B981"
                      />
                    )}
                    {selectedTx.lockDays != null && (
                      <BottomSheetRow
                        label="Thời hạn"
                        value={selectedTx.lockDays ? `${selectedTx.lockDays} ngày` : 'Linh hoạt'}
                      />
                    )}
                    {selectedTx.fee != null && selectedTx.fee > 0 && (
                      <BottomSheetRow
                        label="Phí"
                        value={`${fmtAmount(selectedTx.fee)} ${selectedTx.asset}`}
                        valueColor="#EF4444"
                      />
                    )}
                    {selectedTx.note && <BottomSheetRow label="Ghi chú" value={selectedTx.note} />}
                  </div>
                </TrCard>
                ...
                >
                  ...
                  <button
                    onClick={() => handleCopy(selectedTx.refId)}
