...
export function SavingsDCAPage() {
  ...
  const totalInvested = plans
    ...
    .reduce((s, p) => {
      const price =
        p.asset === 'USDT' ? 1 : p.asset === 'BTC' ? 67543 : p.asset === 'ETH' ? 2800 : 130;
  ...
  const canCreate =
    selectedProduct && createAmountNum > 0 && createAmountNum <= createBalance && createAgreed;

  /* Frequency estimates */
  const frequencyMultiplier: Record<DCAFrequency, number> = {
    daily: 365,
    weekly: 52,
    biweekly: 26,
    monthly: 12,
  ...
  const allExecutions = useMemo(() => {
    return plans
      .flatMap((p) =>
        p.executions.map((e) => ({ ...e, planName: p.productName, planColor: p.color })),
      )
      .sort((a, b) => {
  ...
  const TABS = [
    {
      id: 'plans' as const,
      label: `Kế hoạch (${plans.filter((p) => p.status !== 'cancelled').length})`,
    },
    ...
    <PageLayout>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div>
            <label
              style={{ color: c.text2, fontSize: FONT_SCALE.xs, marginBottom: 8, display: 'block' }}
            >
              Chọn sản phẩm
            </label>
              ...
              {eligibleProducts.map((product) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      borderColor: createProduct === product.id ? '#10B981' : c.borderSolid,
                    }}
                  >
            ...
            <>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label style={{ color: c.text2, fontSize: FONT_SCALE.xs }}>
                    Số lượng mỗi lần ({selectedProduct.asset})
                  </label>
                ...
                {createAmountNum > createBalance && (
                  <p
                    ...
                    style={{ color: '#EF4444', fontSize: FONT_SCALE.xs }}
                  >
              ...
              >
                ...
                <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
                  DCA sẽ tự động gửi tiết kiệm theo lịch. Nếu số dư không đủ, lệnh sẽ được báo lại
                  và bạn sẽ nhận thông báo. APY có thể thay đổi theo thị trường.
                </p>
              </div>
              ...
              >
                ...
                >
                  Tôi đồng ý với <span style={{ color: '#3B82F6' }}>điều khoản DCA</span> và hiểu
                  rằng số tiền sẽ được tự động trừ từ ví Spot theo lịch đã chọn.
                </span>
              </button>
            ...
            return (
              <div className="flex flex-col gap-4">
                ...
                <BottomSheetRow
                  ...
                  value={`${fmtAmount(plan.amountPerPeriod)} ${plan.asset}`}
                />
                <BottomSheetRow label="Tần suất" value={FREQUENCY_LABELS[plan.frequency]} />
                <BottomSheetRow
                  label="APY hiện tại"
                  value={`${plan.currentAPY}%`}
                  valueColor="#10B981"
                />
                <BottomSheetRow
                  label="Lần thực hiện"
                  value={`${plan.successfulExecutions}/${plan.totalExecutions}`}
                />
                <BottomSheetRow label="Bắt đầu từ" value={plan.startDate} />
                <BottomSheetRow
                  label="Lần tiếp theo"
                  value={plan.nextExecution}
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <p style={{ color: '#EF4444', fontSize: 12, lineHeight: 1.5 }}>
              Hủy kế hoạch DCA sẽ dừng tất cả lệnh gửi tự động trong tương lai. Số tiền đã gửi sẽ
              vẫn được giữ và tiếp tục nhận lãi theo sản phẩm tiết kiệm.
            </p>
          </div>
          <div className="flex gap-2">
            <CTAButton
            ...
            <CTAButton
              variant="danger"
              onClick={() => showConfirmCancel && handleCancelPlan(showConfirmCancel)}
              className="flex-1"
            >
              Xác nhận hủy
            </CTAButton>
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs }}>
              DCA sẽ bắt đầu theo lịch đã chọn.
            </p>
          </div>
      ...
      >
        ...
        <p
          ...
          style={{ color: c.portfolioTextMuted, fontSize: φ.xs, marginBottom: 2 }}
        >
          Tổng đã gửi (USD)
        </p>
        ...
        <div className="flex items-center gap-2.5 mt-2 mb-5 relative z-10">
          <div className="flex items-center justify-between w-full">
            <div
              ...
              style={{
                background: totalGain >= 0 ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)',
                border: `1px solid ${totalGain >= 0 ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}`,
              }}
            >
              <TrendingUp
                size={ICON_SIZE.sm}
                color={totalGain >= 0 ? '#34D399' : '#EF4444'}
        ...
        <div className="flex gap-2.5 relative z-10 mb-4">
          ...
          >
            <span style={{ color: c.portfolioTextMuted, fontSize: FONT_SCALE.micro }}>
              Kế hoạch đang chạy
            </span>
            <span
              style={{
      ...
      {/* ─── Info Banner ─── */}
      <div
        className="flex items-start gap-2 mx-5 mt-3 p-3 rounded-xl"
        style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.12)' }}
      >
        <Info size={ICON_SIZE.sm} color="#3B82F6" className="mt-0.5 shrink-0" />
        <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
          DCA giúp bạn gửi tiết kiệm đều đặn theo lịch, giảm rủi ro timing và xây dựng thói quen
          tích lũy hiệu quả.
        </p>
      </div>
                      ...
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span style={{ color: c.text3, fontSize: 11 }}>
                              {fmtAmount(plan.amountPerPeriod)} {plan.asset} /{' '}
                              {FREQUENCY_LABELS[plan.frequency].toLowerCase()}
                            </span>
                          </div>
                        </div>
