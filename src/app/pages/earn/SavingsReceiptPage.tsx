  ...
  return segments
    .map((len) =>
      Array.from({ length: len })
        .map((_, i) => chars[(i * 7 + 3) % chars.length])
        .join(''),
    )
    .join('-');
}
    ...
    <PageLayout variant="flush">
      ...
      <PageContent gap="relaxed" grow>
        ...
        <div className="flex flex-col items-center py-6">
          ...
          <p style={{ color: statusColor, fontSize: FONT_SCALE.lg, fontWeight: FONT_WEIGHT.bold }}>
            {statusText}
          </p>
          ...
          <div className="mt-4 text-center">
            ...
            >
              {isSubscribe ? '' : ''}
              {fmtAmount(data.amount)} {data.asset}
            </p>
        ...
        <TrCard className="p-4">
          ...
          <div className="flex flex-col gap-0">
            {[
              ...
              {
                label: 'Số lượng',
                value: `${fmtAmount(data.amount)} ${data.asset}`,
                highlight: true,
              },
              ...(isSubscribe
                ? [
                    ...
                    {
                      label: 'Thời hạn',
                      value: data.lockDays ? `${data.lockDays} ngày` : 'Linh hoạt',
                    },
                    {
                      label: 'Dự kiến lãi',
                      value: data.estimatedEarning
                        ? `+${data.estimatedEarning.toFixed(6)} ${data.asset}`
                        : '—',
                ...
                : [
                    ...(data.isEarlyRedeem
                      ? [
                          ...
                          {
                            label: 'Lãi mất',
                            value: `-${fmtAmount(data.lostInterest ?? 0)} ${data.asset}`,
                            valueColor: c.error,
                            highlight: true,
                          },
                        ]
                    ...
                    {
                      label: 'Phí rút',
                      value: `${data.fee ?? 0} ${data.asset}`,
                      valueColor: c.success,
                    },
                    {
                      label: 'Thực nhận',
                      value: `${fmtAmount(data.netReceived ?? data.amount)} ${data.asset}`,
                      highlight: true,
                    },
        ...
        <TrCard className="p-4">
          <div className="flex items-center justify-between">
            ...
            >
              {copied ? (
                <>
                  <CheckCircle size={ICON_SIZE.sm} strokeWidth={ICON_STROKE.standard} /> Đã sao chép
                </>
              ) : (
                <>
                  <Copy size={ICON_SIZE.sm} strokeWidth={ICON_STROKE.standard} /> Sao chép
                </>
              )}
            </button>
          </div>
                ...
                <div className="pb-4">
                  <p
                    style={{
                      ...
                      fontSize: FONT_SCALE.sm,
                      fontWeight: step.done ? FONT_WEIGHT.semibold : FONT_WEIGHT.regular,
                    }}
                  >
                    {step.label}
                  </p>
