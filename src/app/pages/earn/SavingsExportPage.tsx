...
const EXPORT_HISTORY: ExportHistory[] = [
  {
    ...
    period: '01/01/2026 - 09/03/2026',
    rows: 47,
    fileSize: '12.3 KB',
    status: 'completed',
    createdAt: '09/03/2026 14:30',
    expiresAt: '16/03/2026 14:30',
    downloadUrl: '#',
  },
  {
    id: 'exp2',
    ...
    fileSize: '245 KB',
    status: 'completed',
    createdAt: '05/01/2026 10:00',
    expiresAt: '05/04/2026 10:00',
    downloadUrl: '#',
  },
  {
    id: 'exp3',
    ...
    period: 'Thời điểm: 09/03/2026',
    rows: 8,
    fileSize: '18.7 KB',
    status: 'completed',
    createdAt: '09/03/2026 09:15',
    expiresAt: '16/03/2026 09:15',
    downloadUrl: '#',
  },
  {
    id: 'exp4',
    ...
    period: '07/02/2026 - 09/03/2026',
    rows: 30,
    fileSize: '8.1 KB',
    status: 'completed',
    createdAt: '08/03/2026 16:00',
    expiresAt: '15/03/2026 16:00',
    downloadUrl: '#',
  },
];
...
const FORMAT_CONFIG: Record<
  ExportFormat,
  { label: string; icon: React.ComponentType<any>; color: string; bg: string; desc: string }
> = {
  csv: {
    label: 'CSV',
    ...
    desc: 'Tương thích Excel, Google Sheets',
  },
  pdf: {
    label: 'PDF',
    icon: FileText,
  ...
  xlsx: {
    ...
    desc: 'Microsoft Excel với định dạng',
  },
};
...
const REPORT_TYPES: {
...
}[] = [
  {
    ...
    desc: 'Xuất tất cả giao dịch tiết kiệm: gửi, rút, nhận lãi, lãi kép',
    icon: ArrowDownToLine,
    color: '#10B981',
    bg: 'rgba(16,185,129,0.08)',
    estimatedRows: 47,
    fields: ['Ngày', 'Loại', 'Sản phẩm', 'Số lượng', 'USD', 'APY', 'Trạng thái', 'Mã GD'],
  },
  {
    id: 'tax',
    title: 'Báo cáo thuế',
    desc: 'Tóm tắt thu nhập lãi tiết kiệm theo năm, phù hợp nộp thuế cá nhân',
    icon: Shield,
    color: '#8B5CF6',
    bg: 'rgba(139,92,246,0.08)',
    estimatedRows: 12,
    fields: ['Tháng', 'Tổng lãi', 'USD', 'Sản phẩm', 'Loại', 'Ghi chú'],
  },
  {
    id: 'portfolio',
    title: 'Ảnh chụp danh mục',
    desc: 'Trạng thái hiện tại của tất cả vị thế tiết kiệm đang hoạt động',
    icon: PiggyBank,
    color: '#3B82F6',
    bg: 'rgba(59,130,246,0.08)',
    estimatedRows: 8,
    fields: ['Sản phẩm', 'Tài sản', 'Số lượng', 'USD', 'APY', 'Loại', 'Ngày bắt đầu', 'Đáo hạn'],
  },
  {
    id: 'performance',
    title: 'Hiệu suất đầu tư',
    desc: 'Phân tích hiệu suất: lãi hằng ngày, APY trung bình, tăng trưởng',
    icon: TrendingUp,
    color: '#F59E0B',
    bg: 'rgba(245,158,11,0.08)',
    estimatedRows: 30,
    fields: ['Ngày', 'Tổng gửi', 'Tổng lãi', 'APY TB', 'Giá trị', 'Tăng trưởng'],
  },
];
...
export function SavingsExportPage() {
  ...
  const estimatedSize = useMemo(() => {
    ...
    return {
      rows,
      size: sizeKB < 1000 ? `${sizeKB.toFixed(1)} KB` : `${(sizeKB / 1000).toFixed(1)} MB`,
    };
  ...
  const handleCopyLink = useCallback(
    (exportItem: ExportHistory) => {
      ...
      // Simulate copy
      navigator.clipboard
        ?.writeText?.(`https://app.example.com/exports/${exportItem.id}`)
        .catch(() => {});
    },
    [hapticLight],
  );
  ...
  return (
    <PageLayout>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>
                {FORMAT_CONFIG[config.format].label} · {periodLabel}
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl p-3" style={{ background: c.surface2 }}>
            ...
            <BottomSheetRow
              label="Phạm vi"
              value={SCOPE_OPTIONS.find((s) => s.id === config.scope)?.label ?? 'Tất cả'}
            />
            ...
            <BottomSheetRow
              label="Bao gồm lãi"
              value={config.includeInterest ? 'Có' : 'Không'}
              valueColor={config.includeInterest ? '#10B981' : c.text3}
            />
            <BottomSheetRow
              label="Bao gồm phí"
              value={config.includeFees ? 'Có' : 'Không'}
              valueColor={config.includeFees ? '#10B981' : c.text3}
            />
            <BottomSheetRow
              label="Chi tiết sản phẩm"
              value={config.includeProductDetails ? 'Có' : 'Không'}
              valueColor={config.includeProductDetails ? '#10B981' : c.text3}
            />
            {config.emailCopy && (
              <BottomSheetRow label="Gửi email" value="Có" valueColor="#3B82F6" />
            )}
          </div>
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.micro, lineHeight: 1.5 }}>
              File xuất sẽ được lưu trữ an toàn và tự động xóa sau 7 ngày. Không chia sẻ link tải
              xuống với người khác.
            </p>
          </div>

          <CTAButton onClick={handleExport}>Xuất báo cáo</CTAButton>
        </div>
      </BottomSheetV2>
      ...
      <BottomSheetV2
        ...
        onClose={() => setShowDetailSheet(null)}
      ...
      <TrCard variant="hero" rounded="lg" className="mx-5 mt-4 p-5">
        ...
        <div className="flex items-end justify-between mb-4">
          ...
          <div className="text-right">
            ...
            <p
              style={{ color: '#3B82F6', fontSize: FONT_SCALE.base, fontWeight: FONT_WEIGHT.bold }}
            >
              4 loại
            </p>
          </div>
        ...
        <PageContent padding="compact" gap="default">
            ...
            <div className="flex flex-col gap-2">
              {REPORT_TYPES.map((rpt) => (
                <button
                  key={rpt.id}
                  onClick={() => {
                    setConfig((prev) => ({ ...prev, reportType: rpt.id }));
                    hapticLight();
                  }}
                ...
                >
                  <div
                    ...
                    style={{
                      background:
                        config.reportType === rpt.id ? rpt.color + '22' : c.borderSolid + '66',
                    }}
                  >
                    <rpt.icon
                      size={ICON_SIZE.sm}
                      color={config.reportType === rpt.id ? rpt.color : c.text3}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p
                    ...
                    <p style={{ color: c.text3, fontSize: FONT_SCALE.micro, marginTop: 4 }}>
                      ~{rpt.estimatedRows} dòng · {rpt.fields.length} cột
                    </p>
                  </div>
                  <div
                    className="w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-1"
                    style={{
                      borderColor: config.reportType === rpt.id ? rpt.color : c.borderSolid,
                    }}
                  >
              ...
              {(Object.keys(FORMAT_CONFIG) as ExportFormat[]).map((fmt) => {
                ...
                return (
                  ...
                  >
                    <info.icon
                      size={ICON_SIZE.md}
                      color={config.format === fmt ? info.color : c.text3}
                    />
                    <span
                      style={{
                        color: config.format === fmt ? info.color : c.text1,
                        fontSize: FONT_SCALE.xs,
                        fontWeight: FONT_WEIGHT.semibold,
                      }}
                    >
                    ...
                    <span
                      style={{ color: c.text3, fontSize: FONT_SCALE.micro, textAlign: 'center' }}
                    >
                      {info.desc}
                    </span>
                  </button>
            ...
            <div className="flex flex-wrap gap-2">
              {PERIOD_OPTIONS.map((p) => (
                <button
                  key={p.id}
                  onClick={() => {
                    setConfig((prev) => ({ ...prev, period: p.id }));
                    hapticLight();
                  }}
              ...
              <div className="flex flex-wrap gap-2">
                {SCOPE_OPTIONS.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => {
                      setConfig((prev) => ({ ...prev, scope: s.id }));
                      hapticLight();
                    }}
          ...
          <PageSection label="Tùy chọn thêm">
            <div className="flex flex-col gap-2">
              {[
                {
                  key: 'includeInterest' as const,
                  label: 'Bao gồm lãi tích lũy',
                  desc: 'Thêm cột lãi hằng ngày/tháng',
                  icon: TrendingUp,
                  color: '#10B981',
                },
                {
                  key: 'includeFees' as const,
                  label: 'Bao gồm phí',
                  desc: 'Thêm cột phí giao dịch (nếu có)',
                  icon: Info,
                  color: '#F59E0B',
                },
                {
                  key: 'includeAPYHistory' as const,
                  label: 'Lịch sử APY',
                  desc: 'Thêm dữ liệu APY theo ngày',
                  icon: TrendingUp,
                  color: '#8B5CF6',
                },
                {
                  key: 'includeProductDetails' as const,
                  label: 'Chi tiết sản phẩm',
                  desc: 'Thêm loại, thời hạn, rủi ro',
                  icon: PiggyBank,
                  color: '#3B82F6',
                },
                {
                  key: 'emailCopy' as const,
                  label: 'Gửi bản sao qua email',
                  desc: 'Gửi file tới email đã đăng ký',
                  icon: Mail,
                  color: '#06B6D4',
                },
              ].map((opt) => (
          ...
          <div className="rounded-2xl p-3" style={{ background: c.surface2 }}>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="p-2 rounded-xl" style={{ background: c.surface }}>
                ...
                <p
                  style={{
                    color: FORMAT_CONFIG[config.format].color,
                    fontSize: FONT_SCALE.xs,
                    fontWeight: FONT_WEIGHT.bold,
                  }}
                >
                  {FORMAT_CONFIG[config.format].label}
                </p>
              </div>
            </div>
          </div>
          ...
          >
            {exportStatus === 'generating'
              ? 'Đang tạo...'
              : `Xem trước & Xuất ${FORMAT_CONFIG[config.format].label}`}
          </CTAButton>
        </PageContent>
      )}
      ...
      {tab === 'history' && (
        <PageContent padding="compact" gap="default">
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: FONT_SCALE.xs, lineHeight: 1.5 }}>
              Báo cáo được lưu trữ an toàn trên server và tự động xóa sau 7 ngày. Tải xuống trước
              khi hết hạn để lưu trữ lâu dài.
            </p>
          </div>
        </PageContent>
      )}
