...
function ErrorBanner({ message, onRetry }: { message: string; onRetry?: () => void }) {
  const c = useThemeColors();
  return (
    <TrCard className="p-4 flex items-center gap-3 mt-4" accentBorder="rgba(239,68,68,0.3)">
      <div
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <span
                  style={{ color: c.text2, fontSize: 11, fontWeight: 600, fontFamily: 'monospace' }}
                >
      ...
      {outcomes.map((o, idx) => (
        ...
        >
          ...
          <span
            ...
            style={{ color: o.color, fontSize: 14, fontWeight: 700, fontFamily: 'monospace' }}
          >
            {o.chance}%
          </span>
          <span
            className="relative z-10 px-2.5 py-1 rounded-lg"
...
function OrderBookView({ chance }: { chance: number }) {
  ...
  const renderRow = (entry: OrderBookEntry, side: 'bid' | 'ask', idx: number) => {
    ...
    return (
      ...
      >
        ...
        <span
          className="relative z-10 w-16 text-right"
          style={{ color: c.text2, fontSize: 11, fontFamily: 'monospace' }}
        >
          {entry.shares.toLocaleString()}
        </span>
        <span
          className="relative z-10 w-20 text-right"
          style={{ color: c.text3, fontSize: 11, fontFamily: 'monospace' }}
        >
  ...
  if (bids.length === 0 && asks.length === 0) {
    return (
      <EmptyState
        icon={Layers}
        title="No orders yet"
        subtitle="Be the first to place an order in this market."
      />
    );
  }
  ...
  return (
    <div>
      <div
        className="flex items-center py-2 px-2 mb-1"
        style={{ borderBottom: `1px solid ${c.border}` }}
      >
        <span className="flex-1" style={{ color: c.text3, fontSize: 10, fontWeight: 600 }}>
          PRICE
        </span>
        <span className="w-16 text-right" style={{ color: c.text3, fontSize: 10, fontWeight: 600 }}>
          SHARES
        </span>
      ...
      <div className="mb-1">
        {[...asks]
          .reverse()
          .slice(0, 5)
          .map((e, i) => renderRow(e, 'ask', i))}
      </div>
      <div
        className="flex items-center justify-center py-2 px-2"
        style={{ background: c.surface2, borderRadius: 6 }}
      >
      ...
      <div className="mt-1">{bids.slice(0, 5).map((e, i) => renderRow(e, 'bid', i))}</div>
    </div>
  );
}
...
function TradePanel({
  ...
  const price =
    orderType === 'market' ? outcome.chance / 100 : Number(limitPrice) || outcome.chance / 100;
    ...
    <TrCard className="p-4">
              ...
              background:
                side === s
                  ? s === 'buy'
                    ...
                    : 'rgba(239,68,68,0.12)'
                  : 'transparent',
              color: side === s ? (s === 'buy' ? c.buy : c.sell) : c.text3,
              fontSize: 13,
      ...
      <div className="mb-3">
        ...
        <div className="relative">
          ...
          />
          <span
            className="absolute right-3 top-1/2 -translate-y-1/2 px-2 py-0.5 rounded"
        ...
        {amount && Number(amount) > 0 && (
          ...
          >
            <div className="flex justify-between py-1">
              ...
              <span
                style={{ color: c.text1, fontSize: 11, fontWeight: 600, fontFamily: 'monospace' }}
              >
            ...
            <div
              className="flex justify-between py-1 mt-1"
            ...
            >
              ...
              <span
                style={{ color: c.buy, fontSize: 13, fontWeight: 700, fontFamily: 'monospace' }}
              >
      ...
      <div>
        ...
        {rules.rules.map((rule, idx) => (
          <div key={idx} className="flex gap-2.5 mb-2.5">
            <span style={{ color: c.text3, fontSize: 11, fontWeight: 600, minWidth: 16 }}>
              {idx + 1}.
            </span>
    ...
    <div>
      ...
      {comments.length === 0 ? (
        <EmptyState
          ...
          subtitle="Be the first to share your thoughts on this market."
        />
      ) : (
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => {
                        setReportTarget({ user: comment.user });
                        hapticSelection();
                      }}
      ...
      <PredictionsCommentReportDialog
        ...
        onReport={() => {
          actionToast.success(TOAST.PREDICTIONS.COMMENT_REPORTED);
        }}
        onBlock={() => {
          actionToast.success(TOAST.PREDICTIONS.USER_BLOCKED);
        }}
...
function TopHoldersTab({ eventId }: { eventId: string }) {
  ...
  return (
    <div>
      {/* Header */}
      <div
        className="flex items-center py-2 px-1 mb-1"
        style={{ borderBottom: `1px solid ${c.border}` }}
      >
        <span className="w-6" style={{ color: c.text3, fontSize: 10, fontWeight: 600 }}>
          #
        </span>
        <span className="flex-1" style={{ color: c.text3, fontSize: 10, fontWeight: 600 }}>
          TRADER
      ...
      {holders.map((h, idx) => (
        ...
        >
          ...
          <span
            className="w-14 text-center px-1.5 py-0.5 rounded"
            style={{
          ...
          >
            {h.pnl >= 0 ? '+' : ''}
            {h.pnl > 999 ? `$${(h.pnl / 1000).toFixed(1)}K` : `$${h.pnl}`}
          </span>
        </motion.div>
      ))}
    </div>
  ...
  if (activity.length === 0) {
    return (
      <EmptyState
        ...
        subtitle="Activity will appear here once trading begins."
      />
    );
  }
...
function RelatedMarkets({ currentId, category }: { currentId: string; category: string }) {
  ...
  const related = PREDICTION_EVENTS.filter((e) => e.id !== currentId && e.status === 'active')
    ...
    <div>
      <SectionHeader
        ...
        right={
          <button
            onClick={() => {
              navigate(`${prefix}/markets/predictions`);
              hapticSelection();
            }}
        ...
        {related.map((ev, idx) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              onClick={() => {
                navigate(`${prefix}/markets/predictions/event/${ev.id}`);
                hapticSelection();
              }}
        ...
        {hasError ? (
          ...
          <PageContent>
            ...
            <div>
              ...
              {/* ─── Outcomes Section ─── */}
              <OutcomeCards outcomes={event.outcomes} onSelect={setSelectedOutcome} />
              ...
              {[
                {
                  icon: BarChart3,
                  label: 'Volume 24h',
                  value: fmtVolume(event.volume24h),
                  color: c.primary,
                },
                {
                  icon: Users,
                ...
                },
                {
                  icon: BarChart3,
                  label: 'Total Volume',
                  value: fmtVolume(event.totalVolume),
                  color: c.buy,
                },
            ...
            {userPosition && (
              <div>
                <TrCard
                  className="p-3"
                  accentBorder={
                    userPosition.pnl >= 0 ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'
                  }
                >
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      >
                        {userPosition.pnlPct >= 0 ? '+' : ''}
                        {userPosition.pnlPct.toFixed(1)}%
                      </p>
                    </div>
                  </div>
                </TrCard>
            ...
            <TrCard className="p-4">
              ...
              {activeTab === 'Top Holders' && (
                <div>
                  <TopHoldersTab eventId={event.id} />
                </div>
              )}
              {activeTab === 'Activity' && (
                <div>
            ...
            <div className="flex gap-2">
              <button
                onClick={() => {
                  navigate(`${prefix}/markets/predictions/rewards`);
                  hapticSelection();
                }}
              ...
              <button
                onClick={() => {
                  navigate(`${prefix}/markets/predictions/activity`);
                  hapticSelection();
                }}
                className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-xl"
                style={{ background: c.surface, border: `1px solid ${c.cardBorder}` }}
      ...
      {showArenaBridgeSheet && (
        ...
        >
          <div className="w-full max-w-md rounded-t-2xl p-5 pb-8" style={{ background: c.surface }}>
            ...
            <div
              className="px-3 py-2 rounded-lg mb-5"
              style={{
