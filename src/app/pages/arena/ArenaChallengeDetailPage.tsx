...
import {
  SafetySnapshotCard,
  TrustBreakdownSheet,
  buildTrustBreakdownFromCreator,
} from '../../components/arena/ArenaSafetyTrust';
import {
  ResultReceiptSheet,
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="min-w-0">
        <p
          style={{ color: c.text1, fontSize: size === 'md' ? φ.sm : φ.xs, fontWeight: 600 }}
          className="truncate"
        >
          {p.name}
        </p>
  ...
  return (
    <div className="flex items-center gap-3">
      <TrCard className="flex-1 p-3">
        <ParticipantCard
          p={a || { id: '?', name: 'Đang chờ...', avatar: '❓', role: 'player', status: 'waiting' }}
        />
      </TrCard>
      <span style={{ color: c.text3, fontSize: φ.sm, fontWeight: 700 }}>VS</span>
      <TrCard className="flex-1 p-3">
        <ParticipantCard
          p={b || { id: '?', name: 'Đang chờ...', avatar: '❓', role: 'player', status: 'waiting' }}
        />
      </TrCard>
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-3">
      <TrCard className="shrink-0 p-3 w-28">
        <ParticipantCard
          p={host || { id: '?', name: 'Host', avatar: '❓', role: 'host', status: 'waiting' }}
        />
      </TrCard>
    ...
    <div className="flex gap-3">
      {teams.slice(0, 2).map((team) => {
        ...
        return (
          ...
          >
            <TrCard className="p-3 h-full" accentBorder={hexToRgba(team.color, 30)}>
              ...
              {captain && (
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span
                    ...
                    style={{
                      background: hexToRgba(team.color, 15),
                      color: team.color,
                      fontSize: 8,
                      fontWeight: 600,
                    }}
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span style={{ color: c.text2, fontSize: φ.xs }}>
          {slotsFilled}/{slotsTotal} đã vào
        </span>
        <span
          style={{ color: openSlots > 0 ? '#3B82F6' : '#EF4444', fontSize: φ.xs, fontWeight: 600 }}
        >
        ...
        {waiting.slice(0, 3).map((p) => (
          <div
            ...
            style={{
              ...
              fontSize: 16,
              opacity: 0.5,
              border: `1px dashed ${c.borderSolid}`,
            }}
            title={`${p.name} (đang chờ)`}
          >
            {p.avatar}
          </div>
        ))}
...
function ChatBubble({
  ...
  isMe?: boolean;
  onReport?: (msg: ArenaChatMessage) => void;
}) {
  const c = useThemeColors();
    ...
    return (
      <div className="flex justify-center">
        <div className="px-3 py-1.5 rounded-xl" style={{ background: 'rgba(59,130,246,0.08)' }}>
          <p style={{ color: '#3B82F6', fontSize: 10, textAlign: 'center', lineHeight: 1.4 }}>
            {msg.text}
          </p>
        </div>
      </div>
...
export function ArenaChallengeDetailPage() {
    ...
    return (
      <PageLayout>
        ...
        <EmptyState
          ...
          subtitle="Challenge này không tồn tại hoặc đã bị xoá"
        />
      </PageLayout>
    );
  }
  ...
  const data: ArenaChallenge = challenge || {
    ...
    leaderboard: [],
    challengeState:
      room!.status === 'waiting' ? 'open' : room!.status === 'in_progress' ? 'live' : 'resolved',
          ...
          <PageContent gap="default">
            ...
            <div>
              ...
              {data.modeName && (
                <button
                  onClick={() => {
                    navigate(`${prefix}/arena/mode/${data.modeId}`);
                    hapticSelection();
                  }}
                  className="flex items-center gap-1 active:opacity-70"
                  style={{ minHeight: 28 }}
                >
                  <span style={{ color: '#8B5CF6', fontSize: φ.sm, fontWeight: 600 }}>
                    {data.modeName}
                  </span>
              ...
              {(() => {
                ...
                const isFromPrediction =
                  mode?.templateId === 'prediction' || mode?.templateId === 'closest_guess';
                  ...
                  <div className="mt-3">
                    <TrCard className="p-3.5" accentBorder="rgba(59,130,246,0.3)">
                      ...
                      <div className="flex items-center justify-between">
                        <button
                          onClick={() => {
                            navigate(`${prefix}/markets/predictions/event/${linkedEvent.id}`);
                            hapticSelection();
                          }}
                          className="flex items-center gap-1 active:opacity-70"
                          style={{ minHeight: 28 }}
                ...
                <div style={{ borderTop: `1px solid ${c.divider}`, paddingTop: 12 }}>
                  ...
                  <div className="h-2 rounded-full" style={{ background: c.surface2 }}>
                    <motion.div
                      ...
                      style={{
                        background:
                          fillPct >= 90 ? '#EF4444' : fillPct >= 60 ? '#F59E0B' : '#3B82F6',
                      }}
                    />
                  </div>
                ...
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    <div key={row.label} className="flex items-start justify-between gap-3">
                      <span style={{ color: c.text3, fontSize: φ.xs, minWidth: 90 }}>
                        {row.label}
                      </span>
                      <span
            ...
            <div>
              <RuleClarityCard
                score={computeClarityScore({
                  ...
                  evidenceRequired:
                    (data.evidenceRequirement || '').toLowerCase() !== 'không yêu cầu' &&
                    (data.evidenceRequirement || '').toLowerCase() !== 'không cần',
                  joinStyle:
                    data.privacy === 'public'
                      ? 'public'
            ...
            <div>
              <TrCard
                ...
                as="button"
                onClick={() => {
                  navigate(`${prefix}/arena/creator/${data.creator.id}`);
                  hapticSelection();
                }}
            ...
            <div>
              <HostActionsPanel
                ...
                onCancel={() => {
                  setCancelSheetOpen('cancel');
                  hapticSelection();
                }}
                onVoid={() => {
                  setCancelSheetOpen('void');
                  hapticSelection();
                }}
              />
            </div>
            ...
            {/* ─── 5-Tab Navigation ─── */}
            <div className="flex gap-1.5 overflow-x-auto px-5 no-scrollbar">
              {DETAIL_TABS.map((t) => {
                    ...
                    <div className="contents">
                      {participants.map((p, i) => (
                        <div
                          ...
                          style={{
                            borderBottom:
                              i < participants.length - 1 ? `1px solid ${c.divider}` : 'none',
                            minHeight: 52,
                          }}
                        >
                          <div
                  ...
                  <div className="flex flex-col gap-3">
                    {data.rules.map((rule, i) => (
                      <div key={i} className="flex items-start gap-2">
                        ...
                        <span style={{ color: c.text1, fontSize: φ.sm, lineHeight: 1.6 }}>
                          {rule}
                        </span>
                      </div>
                    ))}
                    ...
                    {ARENA_ACTIVITY_FEED.slice()
                      ...
                      .map((event, i) => (
                        <div
                          ...
                          style={{
                            borderBottom:
                              i < ARENA_ACTIVITY_FEED.length - 1
                                ? `1px solid ${c.divider}`
                                : 'none',
                          }}
                        >
                    ...
                    {isLive && data.evidenceRequirement !== 'Không cần' && (
                      <button
                        ...
                        style={{
                          ...
                          border: `1.5px solid ${c.chipActiveBorder}`,
                  ...
                  <div className="p-3 max-h-80 overflow-y-auto" style={{ minHeight: 200 }}>
                    {messages.map((msg) => (
                      <ChatBubble
                        key={msg.id}
                        msg={msg}
                        isMe={msg.sender.name === 'Bạn'}
                        onReport={(m) => setMsgReportOpen({ text: m.text, sender: m.sender.name })}
                      />
                    ))}
                    <div ref={chatEndRef} />
                  </div>
                ...
                <TrCard overflow>
                  {data.leaderboard.slice(0, 5).map((entry, i) => (
                    <div
                      ...
                      style={{
                        borderBottom:
                          i < Math.min(5, data.leaderboard.length) - 1
                            ? `1px solid ${c.divider}`
                            : 'none',
                        minHeight: 48,
                      }}
            ...
            <div className="px-5 flex flex-col gap-2">
              <WarningBanner
                text="Arena Points chỉ dùng trong Open Arena, không phải tài sản tài chính."
                type="info"
              />
              <WarningBanner text="Không thỏa thuận giao dịch ngoài nền tảng." type="warning" />
              {cState === 'under_review' && (
                <WarningBanner
                  text="Challenge này đang được xem xét bởi đội ngũ kiểm duyệt."
                  type="warning"
                />
              )}
              {cState === 'reported' && (
                <WarningBanner
                  text="Nội dung đã bị báo cáo. Kết quả có thể bị tạm giữ."
                  type="danger"
                />
              )}
            ...
            <div className="px-5 flex flex-col gap-3">
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  ...
                  {cState === 'full' && <CTAButton disabled>Đã đầy</CTAButton>}
                  {isLive && (
                    <CTAButton
                      onClick={() => {
                        setEvidenceUploadOpen(true);
                  ...
                  {isResolved && (
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        onClick={() => {
                          actionToast.success(TOAST.ARENA.REMATCH_SENT);
                          hapticSelection();
                        }}
                        className="w-full py-3 rounded-2xl active:opacity-70"
                        style={{
                  ...
                  {cState === 'under_review' && <CTAButton disabled>Đang xem xét</CTAButton>}
                  {cState === 'reported' && <CTAButton disabled>Đã bị báo cáo</CTAButton>}
                  {cState === 'offline' && <CTAButton disabled>Ngoại tuyến</CTAButton>}
                </div>
              </div>
            ...
            <BottomSheetV2
              open={boundarySheetOpen}
              onClose={() => setBoundarySheetOpen(false)}
              title="Ranh giới Arena & Prediction"
              maxHeight="70vh"
            >
              <div className="flex flex-col gap-3">
                ...
                <div className="mt-1">
                  ...
                  <BoundaryInfoRow
                    icon={Shield}
                    text="Prediction Markets tách biệt hoàn toàn về ví và số dư."
                    color="#8B5CF6"
                  />
                  <BoundaryInfoRow
                    icon={Info}
                    text="Chỉ chia sẻ bối cảnh (topic, event title). Không bao giờ chia sẻ giá trị."
                    color="#6B7280"
                  />
                </div>
              </div>
