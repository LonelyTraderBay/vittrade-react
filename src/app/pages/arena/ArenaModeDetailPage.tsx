...
export function ArenaModeDetailPage() {
  ...
  if (!mode) {
    return (
      <PageLayout>
        ...
        <EmptyState
          icon={AlertTriangle}
          title="Không tìm thấy mode"
          subtitle="Mode này không tồn tại hoặc đã bị xoá"
        />
      </PageLayout>
    );
  }
  ...
  const relatedModes = ARENA_MODES.filter(
    (m) => m.templateId === mode.templateId && m.id !== mode.id,
  ).slice(0, 4);
      ...
      {isLoading ? (
        ...
        <PageContent padding="compact" gap="default">
          ...
          >
            ...
            <button
              onClick={() => {
                navigate(`${prefix}/arena/creator/${mode.creator.id}`);
                hapticSelection();
              }}
            ...
            >
              ...
              {mode.creator.badge && (
                <span
                  ...
                  style={{
                    background: hexToRgba(badgeColor(mode.creator.badge), 0.08),
                    color: badgeColor(mode.creator.badge),
                    fontSize: 10,
                    fontWeight: 600,
                  }}
                >
            ...
            <button
              onClick={() => {
                navigate(`${prefix}/arena/trust/${mode.creator.id}`);
                hapticSelection();
              }}
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {cx && (
                <span
                  ...
                  style={{
                    background: hexToRgba(cx.color, 0.08),
                    color: cx.color,
                    fontSize: 10,
                    fontWeight: 600,
                  }}
            ...
            <TrCard className="p-4">
              <div className="flex flex-col gap-3">
                {[
                  {
                    label: 'Formats',
                    value: mode.allowedFormats?.join(', ') || tpl?.formatTags.join(', ') || '—',
                  },
                ...
                {drl && (
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      ...
                      style={{
                        background: hexToRgba(drl.color, 0.08),
                        color: drl.color,
                        fontSize: 10,
                        fontWeight: 600,
                      }}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                {
                  label: 'Fair Play',
                  value: mode.fairPlay ? 'Đạt' : 'Chưa đạt',
                  color: mode.fairPlay ? '#10B981' : '#F59E0B',
                  icon: Shield,
                },
                {
                  label: 'Tỷ lệ báo cáo',
                  value:
                    mode.reportRate !== undefined ? `${(mode.reportRate * 100).toFixed(0)}%` : '—',
                  color: (mode.reportRate || 0) < 0.05 ? '#10B981' : '#EF4444',
                  icon: Flag,
                },
                {
                  label: 'Dùng lại',
                  value: mode.repeatUsage !== undefined ? `${mode.repeatUsage} lần/người` : '—',
                  color: '#3B82F6',
                  icon: RefreshCw,
                },
              ].map((s) => (
                ...
                {relatedRooms.map((room, i) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderBottom:
                          i < relatedRooms.length - 1 ? `1px solid ${c.divider}` : 'none',
                        minHeight: 52,
                      }}
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p style={{ color: c.text3, fontSize: φ.xs }}>
                          {room.slotsFilled}/{room.slotsTotal} slots · {fmtPoints(room.entryPoints)}{' '}
                          pts
                        </p>
                      </div>
                      <span
                ...
                {relatedModes.map((rm) => {
                  ...
                  return (
                    <button
                      key={rm.id}
                      onClick={() => {
                        navigate(`${prefix}/arena/mode/${rm.id}`);
                        hapticSelection();
                      }}
          ...
          {(() => {
            ...
            if (
              mode.templateId !== 'prediction' &&
              mode.templateId !== 'closest_guess' &&
              !hasPredictionTopic
            )
              return null;
