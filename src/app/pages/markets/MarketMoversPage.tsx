...
const MoverRow = React.memo(function MoverRow({
  ...
  const changeKey =
    timeframe === '1h' ? 'change1h' : timeframe === '24h' ? 'change24h' : 'change7d';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        >
          <span
            style={{ fontSize: FONT_SCALE.micro, fontWeight: FONT_WEIGHT.bold, color: mover.color }}
          >
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          <span className="block truncate" style={{ fontSize: FONT_SCALE.micro, color: c.text3 }}>
            {mover.name}
          </span>
        </div>
      </div>
...
export function MarketMoversPage() {
  ...
  const movers = useMemo(() => {
    ...
    switch (tab) {
      case 'Tăng mạnh':
        list = getTopGainers(timeframe, 20);
        break;
      case 'Giảm mạnh':
        list = getTopLosers(timeframe, 20);
        break;
      case 'Hoạt động':
        list = getMostActive(20);
        break;
      case 'KL bất thường':
        list = getUnusualVolume(20);
        break;
      case 'Mới niêm yết':
        list = getNewListings();
        break;
      ...
      <PageContent padding="compact" gap="default">
        ...
        {tab !== 'Mới niêm yết' && (
          <div className="flex items-center gap-2">
            <span
              style={{ fontSize: FONT_SCALE.xs, color: c.text3, fontWeight: FONT_WEIGHT.medium }}
            >
              Khung thời gian:
            </span>
            {TIMEFRAMES.map((tf) => (
              <button
                ...
                style={{
                  ...
                  boxShadow: timeframe === tf ? '0 1px 3px rgba(59,130,246,0.15)' : 'none',
                }}
              >
                {tf}
              </button>
        ...
        >
          <div className="flex items-center gap-2">
            <span
              style={{ fontSize: FONT_SCALE.xs, fontWeight: FONT_WEIGHT.medium, color: c.text3 }}
            >
              Danh mục:
            </span>
            <span
                ...
                style={{
                  ...
                  border:
                    sortBy === s.id
                      ? `2px solid ${c.chipActiveBorder}`
                      : `1.5px solid ${c.borderSolid}`,
                  boxShadow: sortBy === s.id ? '0 1px 3px rgba(59,130,246,0.15)' : 'none',
                }}
              >
                {s.label}
              </button>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span style={{ fontSize: FONT_SCALE.xs, color: c.text3 }}>{movers.length} kết quả</span>
      ...
      <Sheet open={showCategorySheet} onOpenChange={setShowCategorySheet}>
        <SheetContent side="bottom" className="p-0">
          <SheetHeader className="p-5 pb-3">
            <SheetTitle
              style={{ fontSize: FONT_SCALE.base, fontWeight: FONT_WEIGHT.bold, color: c.text1 }}
            >
              Chọn danh mục
            </SheetTitle>
