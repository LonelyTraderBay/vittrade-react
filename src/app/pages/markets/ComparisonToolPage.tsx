...
import { Plus, X, Search, ArrowUpRight, ArrowDownRight, Scale } from 'lucide-react';
...
const METRICS: MetricRow[] = [
  ...
  { label: 'Thấp nhất 24h', key: 'low', format: 'price', getValue: (p) => p.low24h },
  {
    ...
    key: 'range',
    format: 'pct',
    getValue: (p) => ((p.high24h - p.low24h) / p.low24h) * 100,
  },
  {
    label: 'Vol/MCap',
    key: 'volmcap',
    format: 'pct',
    getValue: (p) => (p.volume24h / p.marketCap) * 100,
  },
];
...
export function ComparisonToolPage() {
  ...
  const selectedPairs = useMemo(
    () =>
      selectedIds
        .map((id) => CRYPTO_PAIRS.find((p) => p.id === id))
        .filter(Boolean) as CryptoPair[],
    [selectedIds],
  );
            ...
            <div className="flex flex-col gap-1 max-h-48 overflow-y-auto">
              {filteredPairs.map((pair) => (
                ...
                >
                  ...
                  >
                    <span
                      style={{ color: pair.logoColor, fontSize: 10, fontWeight: FONT_WEIGHT.bold }}
                    >
                      {pair.baseAsset.slice(0, 2)}
                    </span>
                  </div>
        ...
        {selectedPairs.length >= 2 && (
          <TrCard className="p-4">
            ...
            <div
              ...
              style={{ gridTemplateColumns: `repeat(${selectedPairs.length}, 1fr)` }}
            >
              {selectedPairs.map((pair) => (
                      ...
                      return (
                        <div key={pair.id} className="flex items-center gap-1.5">
                          <div
                            ...
                            style={{ background: pair.logoColor }}
                          />
