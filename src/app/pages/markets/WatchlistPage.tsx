...
export function WatchlistPage() {
  ...
  const watchlistPairs = watchlist
    .map((w) => {
  ...
  const handleAddNote = (id: string) => {
    ...
    if (note !== null) {
      setWatchlist(
        watchlist.map((w) => (w.id === id ? { ...w, note: note.trim() || undefined } : w)),
      );
    }
  };
              ...
              return (
                <TrCard key={item.id} hover className="mb-3 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        ...
                        <p style={{ color: c.text3, fontSize: FONT_SCALE.xs }}>{pair.symbol}</p>
                      </button>
                    </div>
                  ...
                  {item.note && (
                    ...
                    >
                      <p style={{ color: '#3B82F6', fontSize: 12 }}>📝 {item.note}</p>
                    </div>
                  )}
