...
function generateChartData(
  ...
  to: string,
  tf: Timeframe,
): { time: string; price: number; vol: number }[] {
      ...
      <TrCard className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span style={{ color: c.text1, fontSize: FS.sm, fontWeight: 700 }}>
              {from}/{to}
            </span>
        ...
        <div className="mb-2">
          ...
          <span style={{ color: c.text3, fontSize: FS.xs, marginLeft: 6 }}>
            {to}/{from}
          </span>
        </div>
    ...
    >
      <TrCard className="p-4">
        ...
        <div className="flex items-center mb-2">
          <span className="flex-1" style={{ color: c.text3, fontSize: FS.micro }}>
            PRICE ({to})
          </span>
          <span className="w-16 text-right" style={{ color: c.text3, fontSize: FS.micro }}>
            AMOUNT
          </span>
          <span className="w-16 text-right" style={{ color: c.text3, fontSize: FS.micro }}>
            TOTAL
        ...
        {[...ob.asks].reverse().map((ask, i) => (
          <div key={`a-${i}`} className="flex items-center py-1 relative">
            <div
              className="absolute inset-0 right-0 rounded-sm"
              style={{
            ...
            <span
              ...
              style={{ color: '#EF4444', fontSize: FS.xs, fontFamily: 'monospace' }}
            >
              {ask.price.toFixed(6)}
            </span>
            <span
              className="w-16 text-right relative z-10"
              style={{ color: c.text2, fontSize: FS.xs, fontFamily: 'monospace' }}
            >
              {ask.amount.toFixed(2)}
            </span>
            <span
              className="w-16 text-right relative z-10"
              style={{ color: c.text3, fontSize: FS.xs, fontFamily: 'monospace' }}
            >
              {ask.total.toFixed(2)}
            </span>
          </div>
        ...
        >
          <span
            style={{ color: c.text1, fontSize: FS.sm, fontWeight: 700, fontFamily: 'monospace' }}
          >
            {ob.mid.toFixed(6)}
          </span>
        ...
        {ob.bids.map((bid, i) => (
          <div key={`b-${i}`} className="flex items-center py-1 relative">
            ...
            <span
              className="flex-1 relative z-10"
              style={{ color: '#10B981', fontSize: FS.xs, fontFamily: 'monospace' }}
            >
              {bid.price.toFixed(6)}
            </span>
            <span
    ...
    >
      ...
      >
        ...
        <div className="mx-5 mb-3 rounded-xl px-4 py-3" style={{ background: c.surface2 }}>
          ...
          <p
            style={{ color: c.text1, fontSize: FS.base, fontWeight: 700, fontFamily: 'monospace' }}
          >
          ...
          {pairAlerts.map((alert) => (
            ...
            >
              <div
                ...
                style={{
                  background:
                    alert.direction === 'above' ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)',
                }}
              >
...
export function TxDetailSheet({ tx, onClose }: TxDetailSheetProps) {
  ...
  const statusColor =
    tx.status === 'success' ? '#10B981' : tx.status === 'pending' ? '#F59E0B' : '#EF4444';
  const statusLabel =
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Amount display */}
        <div
          className="mx-5 rounded-2xl p-4 flex items-center justify-center gap-4"
          style={{ background: c.surface2 }}
        >
  ...
  return (
    ...
    >
      <TrCard className="p-4">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p style={{ color: c.text3, fontSize: FS.micro }}>
                Chống front-running & sandwich attacks
              </p>
            </div>
          </div>
          <button
            ...
            style={{
              background: mevProtection ? '#10B981' : c.surface2,
              border: `1px solid ${mevProtection ? '#10B981' : c.borderSolid}`,
            }}
          >
            <motion.div
        ...
        >
          <div className="flex items-center gap-2">
            <Gauge size={14} color={autoSlippage ? '#3B82F6' : c.text3} />
            <div>
              <p style={{ color: c.text1, fontSize: FS.xs, fontWeight: 600 }}>Auto Slippage</p>
              <p style={{ color: c.text3, fontSize: FS.micro }}>
                Tự điều chỉnh theo điều kiện thị trường
              </p>
            </div>
          </div>
          <button
            onClick={onToggleAutoSlippage}
            className="w-10 h-5 rounded-full transition-all active:scale-95 flex items-center"
            style={{
              background: autoSlippage ? '#3B82F6' : c.surface2,
              border: `1px solid ${autoSlippage ? '#3B82F6' : c.borderSolid}`,
            }}
          >
            <motion.div
        ...
        <div className="flex items-center justify-center gap-4 mt-2">
          ...
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ background: '#F59E0B', borderTop: '1px dashed #F59E0B' }}
            />
...
export function PairInfoCard({ from, to }: PairInfoProps) {
  ...
  const rows = [
    {
      label: 'Volume 24h',
      value: fmtUsd(stats.volume24h, { decimals: 0 }),
      icon: <BarChart3 size={12} color="#3B82F6" />,
    },
    {
      label: 'High 24h',
      value: stats.high24h.toFixed(6),
      icon: <TrendingUp size={12} color="#10B981" />,
    },
    {
      label: 'Low 24h',
      value: stats.low24h.toFixed(6),
      icon: <TrendingDown size={12} color="#EF4444" />,
    },
    {
      label: 'Spread',
      value: `${stats.spreadPct.toFixed(2)}% (${stats.spread.toFixed(6)})`,
      icon: <ArrowDownUp size={12} color="#F59E0B" />,
    },
    {
      label: 'Liquidity',
      value: fmtUsd(stats.liquidity, { decimals: 0 }),
      icon: <Droplets size={12} color="#06B6D4" />,
    },
    {
      label: 'Trades 24h',
      value: stats.trades24h.toLocaleString(),
      icon: <Activity size={12} color="#8B5CF6" />,
    },
    {
      label: 'Avg Trade',
      value: fmtUsd(stats.avgTradeSize, { decimals: 0 }),
      icon: <Layers size={12} color={c.text2} />,
    },
  ];
        ...
        <div className="px-5 pb-6 flex flex-col gap-3">
          ...
          >
            ...
            <div className="flex-1 text-left">
              ...
              <p style={{ color: c.text3, fontSize: FS.xs }}>
                {history.length} giao dịch · .csv file
              </p>
            </div>
  ...
  return (
    ...
    >
      ...
      <span style={{ color: c.text2, fontSize: FS.xs }}>
        1 {displayFrom} ={' '}
        <span style={{ fontWeight: 600, color: c.text1, fontFamily: 'monospace' }}>
