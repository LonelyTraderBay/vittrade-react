...
async function completeToResults(
  ...
  const resultsBtn = await screen.findByRole(
    'button',
    { name: /Xem kết quả/ },
    { timeout: 6000 },
  );
...
describe('PreCopyAssessmentPage', () => {
  ...
  it('should reveal doc content on expand and auto-complete after 3 seconds', async () => {
    ...
    expect(
      screen.getByText(/mọi lệnh của họ sẽ được tự động sao chép/i),
    ).toBeInTheDocument();
    expect(screen.getByText(/Slippage: Lệnh của bạn execute sau provider 0\.5-3s/i)).toBeInTheDocument();
  ...
  it('should block unsuitable users and send them back to the list', async () => {
    ...
    expect(screen.getByText(/không phù hợp với kiến thức và kinh nghiệm hiện tại/i)).toBeInTheDocument();
  ...
  it('should include the mandated risk disclosures in education docs', async () => {
    ...
    expect(
      screen.getByText(/Provider KHÔNG phải investment advisor/i),
    ).toBeInTheDocument();
