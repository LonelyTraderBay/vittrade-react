          ...
          return (
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p style={{ color: c.text3, fontSize: 11 }}>{tx.createdAt}</p>
              </div>
  ...
  const mainContent = (
    <div className="flex flex-col flex-1">
          ...
          {[
            {
              ...
              label: 'Nạp',
              color: '#10B981',
              action: () => navigate(`${prefix}/wallet/deposit/USDT`),
            },
            {
              icon: Upload,
              label: 'Rút',
              color: '#EF4444',
              action: () => navigate(`${prefix}/wallet/withdraw/USDT`),
            },
            {
              icon: ShoppingCart,
              label: 'Mua',
              color: '#3B82F6',
              action: () => navigate(`${prefix}/wallet/buy-crypto`),
            },
            {
              icon: ArrowDownUp,
              label: 'Chuyển',
              color: '#8B5CF6',
              action: () => navigate(`${prefix}/wallet/transfer`),
            },
            {
              icon: Clock,
              label: 'Lịch sử',
              color: '#8B95B3',
              action: () => navigate(`${prefix}/wallet/history`),
            },
          ].map((btn) => (
            <button
              key={btn.label}
      ...
      <div className="flex gap-2 mb-4">
        <button
          onClick={() => navigate(`${prefix}/wallet/portfolio-analytics`)}
        ...
        <button
          onClick={() => navigate(`${prefix}/wallet/address-book`)}
          className="px-3 py-1.5 rounded-xl text-xs"
          style={{
        ...
        <span className="ml-auto" style={{ color: c.text3, fontSize: 12 }}>
          {USER_ASSETS.length} tài sản
        </span>
      </div>
      ...
      <TrCard overflow>
        <div
          ...
          style={{ borderBottom: `1px solid ${c.divider}` }}
        >
          <span className="flex-1" style={{ color: c.text3, fontSize: 11, fontWeight: 600 }}>
            Tài sản
          </span>
          <span
            style={{
          ...
          <span
            style={{ color: c.text3, fontSize: 11, fontWeight: 600, width: 80, textAlign: 'right' }}
          >
            24h
          </span>
          <span style={{ width: 24 }} />
        ...
        {USER_ASSETS.map((asset, i) => (
          <button
            key={asset.id}
            onClick={() => navigate(`${prefix}/wallet/asset/${asset.id}`)}
          ...
          >
            ...
            >
              <span style={{ color: asset.logoColor, fontSize: 11, fontWeight: 700 }}>
            ...
            {/* Balance */}
            <div
              className={isDesktop ? 'flex flex-col items-end' : 'flex flex-col items-end'}
              style={isDesktop ? { width: 120 } : undefined}
            >
              <span
                style={{ color: c.text1, fontSize: 13, fontWeight: 600, fontFamily: 'monospace' }}
              >
            ...
            <div
              ...
              style={isDesktop ? { width: 80 } : { marginLeft: 8 }}
            >
              <span
                style={{
