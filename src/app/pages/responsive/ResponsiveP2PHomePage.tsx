    ...
    <TrCard hover className="p-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <span
          ...
          style={{
            background: ad.isOnline ? 'rgba(16,185,129,0.15)' : 'rgba(74,85,104,0.2)',
            color: ad.isOnline ? '#10B981' : c.text3,
          }}
        >
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        {ad.priceType === 'floating' && (
          <span
            ...
            style={{ background: 'rgba(168,85,247,0.1)', color: '#A855F7', fontSize: 10 }}
          >
            Thả nổi
          </span>
        )}
      ...
      <div className="flex justify-between text-xs mb-3">
        ...
        <div>
          ...
          <span style={{ color: c.text1, fontFamily: 'monospace', fontWeight: 600 }}>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <span style={{ color: '#F59E0B', fontSize: 10 }}>
            {ad.counterpartyRequirements.minKycLevel
              ? `KYC ${ad.counterpartyRequirements.minKycLevel}+`
              : ''}
            {ad.counterpartyRequirements.minCompletedTrades
              ? ` ${ad.counterpartyRequirements.minCompletedTrades}+ đơn`
              : ''}
          </span>
        </div>
      )}
  ...
  return (
    <div className="flex flex-col gap-4" style={{ width: 320, flexShrink: 0 }}>
      ...
      <TrCard className="p-4">
        ...
        <label style={{ color: c.text2, fontSize: 12, marginBottom: 6, display: 'block' }}>
          <SortDesc size={10} className="inline mr-1" />
          Sắp xếp theo
        </label>
        <div className="flex gap-2 mb-4">
          {[
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {allPaymentMethods.map((pm) => (
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg text-xs font-semibold"
              style={{
                background: filterPayment === pm ? c.chipActiveBg : c.chipBg,
                color: filterPayment === pm ? c.chipActiveText : c.chipText,
                border: `1px solid ${filterPayment === pm ? c.chipActiveBorder : c.chipBorder}`,
              }}
            >
              {pm}
            </button>
      ...
      <TrCard className="p-4">
        ...
        {[
          ...
          >
            ...
            <span className="flex-1 text-left" style={{ color: c.text2, fontSize: 13 }}>
              {item.label}
            </span>
            {item.badge > 0 && (
              <span
  ...
  const mainContent = (
    <PageLayout style={{ flex: 1 }}>
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-2">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate(`${prefix}/p2p/my-ads`)}
      ...
      {/* Escrow Info */}
      <div
        className="mx-5 mb-3 rounded-2xl p-3"
        style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)' }}
      >
        <p style={{ color: '#10B981', fontSize: 12 }}>
          <Shield size={10} className="inline mr-1" />
          Giao dịch P2P được bảo vệ bởi hệ thống Escrow VitTrade. Tài sản an toàn cho đến khi xác
          nhận.
        </p>
      </div>
      ...
      {/* Amount input */}
      <div
        className="mx-5 mb-3 flex items-center gap-3 rounded-2xl px-4"
        style={{
      ...
      >
        <input
          type="number"
          inputMode="decimal"
          placeholder="Nhập số tiền VND muốn giao dịch..."
          value={amountInput}
      ...
      {!isDesktop && (
        <div className="flex gap-2 px-5 mb-4">
          ...
          <button
            onClick={() => navigate(`${prefix}/p2p/payment-methods`)}
            className="flex-1 flex items-center justify-between px-3 py-2.5 rounded-xl"
            style={{ background: c.surface2, border: `1px solid ${c.borderSolid}` }}
          >
          ...
          <button
            onClick={() => navigate(`${prefix}/p2p/reviews`)}
            className="flex-1 flex items-center justify-between px-3 py-2.5 rounded-xl"
            style={{ background: c.surface2, border: `1px solid ${c.borderSolid}` }}
          >
      ...
      {/* Ads list */}
      <div
        className={`flex flex-col gap-3 ${isDesktop ? '' : 'px-5'}`}
        style={
          isDesktop
      ...
      >
        {ads.length === 0 ? (
          <div className="flex flex-col items-center py-12 gap-3 col-span-2">
            ...
            <p style={{ color: c.text2, fontSize: 15, fontWeight: 600 }}>
              Không tìm thấy quảng cáo
            </p>
            <p style={{ color: c.text3, fontSize: 13 }}>Thử thay đổi bộ lọc hoặc tiền tệ</p>
          </div>
        ) : (
          ads.map((ad) => <AdCard key={ad.id} ad={ad} tradeType={tab} prefix={prefix} />)
        )}
      </div>
    </PageLayout>
  );
