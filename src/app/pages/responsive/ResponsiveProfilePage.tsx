...
import {
  ...
  CheckCircle, MonitorSmartphone, Smartphone,
  Globe, ClipboardList, Zap, Bot, Star, User,
} from 'lucide-react';
  ...
  const menuItems = [
    {
      section: 'Tài khoản',
      items: [
        { icon: ShieldCheck, label: 'Xác minh danh tính (KYC)', sub: KYC_LABELS[USER_PROFILE.kycStatus], subColor: KYC_COLORS[USER_PROFILE.kycStatus], action: () => navigate(`${prefix}/profile/kyc`) },
        { icon: Shield, label: 'Bảo mật', sub: USER_PROFILE.has2FA ? '2FA đang bật' : '2FA chưa bật', subColor: USER_PROFILE.has2FA ? '#10B981' : '#EF4444', action: () => navigate(`${prefix}/profile/security`) },
        { icon: Crown, label: 'VIP Program', sub: 'VIP 1 — Maker 0.09%', subColor: '#F59E0B', action: () => navigate(`${prefix}/profile/vip`) },
    ...
    },
    {
      ...
      items: [
        ...
        { icon: theme === 'dark' ? Moon : Sun, label: 'Giao diện', sub: theme === 'dark' ? 'Tối' : 'Sáng', action: () => setTheme(theme === 'dark' ? 'light' : 'dark') },
        { icon: Settings, label: 'Cài đặt chung', sub: '', action: () => navigate(`${prefix}/profile/settings`) },
      ],
    },
    {
      section: 'Khác',
      items: [
        ...
        { icon: RefreshCw, label: 'Mua định kỳ (DCA)', sub: 'Tự động mua crypto theo lịch', subColor: '#8B5CF6', action: () => navigate(`${prefix}/dca`) },
        ...
        { icon: HelpCircle, label: 'Trung tâm hỗ trợ', sub: '', action: () => navigate(`${prefix}/support`) },
        { icon: FileText, label: 'Tin tức & Thông báo', sub: '', action: () => navigate(`${prefix}/news`) },
  ...
  return (
    <PageLayout style={{ maxWidth: maxW, margin: isDesktop ? '0 auto' : undefined }}>
      ...
      <div className="mx-5 rounded-3xl p-5"
        style={{ background: 'linear-gradient(135deg, #1a2550 0%, #0d1b3e 100%)', border: '1px solid rgba(59,130,246,0.2)' }}>
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            <p style={{ color: '#F0F4FF', fontSize: 18, fontWeight: 700 }}>{USER_PROFILE.fullName}</p>
          ...
          <button onClick={() => navigate(`${prefix}/profile/edit`)} className="w-9 h-9 flex items-center justify-center rounded-xl"
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleCopyRef} className="flex-1 rounded-2xl p-3" style={{ background: 'rgba(255,255,255,0.05)' }}>
            <p style={{ color: c.text3, fontSize: 11 }}>Mã giới thiệu</p>
            <div className="flex items-center gap-1">
              <p style={{ color: '#3B82F6', fontSize: 13, fontWeight: 600, fontFamily: 'monospace' }}>
                {USER_PROFILE.referralCode}
              </p>
              {copiedRef ? <CheckCircle size={12} color="#10B981" /> : <Copy size={12} color="#3B82F6" />}
            </div>
          </button>
        </div>
      </div>
      ...
      <TrCard className="mx-5 mt-3 p-4">
        ...
        <div className="h-2 rounded-full mb-2" style={{ background: c.borderSolid }}>
          <div className="h-full rounded-full" style={{ background: 'linear-gradient(90deg, #F59E0B, #F97316)', width: '35%' }} />
        </div>
        <p style={{ color: c.text3, fontSize: 11 }}>Volume 30 ngày: $18,450 / $50,000 để lên VIP 2</p>
      </TrCard>
      ...
      {menuItems.map(section => (
        <div key={section.section} className="mx-5 mt-4">
          <p style={{ color: c.text3, fontSize: 12, fontWeight: 600, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>
            {section.section}
          </p>
          {isDesktop ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {section.items.map((item, i) => (
                <TrCard key={item.label} as="button" hover
                  onClick={item.action}
                  ...
                  <div className="flex-1 text-left">
                    ...
                    {item.sub && <p style={{ color: item.subColor ?? c.text2, fontSize: 12 }}>{item.sub}</p>}
                  </div>
    ...
    <div className="w-full text-left">
      <div className="flex items-center gap-3 mb-3">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
          style={{ background: `linear-gradient(135deg, ${currentTier.color}22, ${currentTier.color}15)`, border: `1px solid ${currentTier.color}33` }}>
          <span style={{ fontSize: 18 }}>{currentTier.icon}</span>
        </div>
        <div className="flex-1">
          <p style={{ color: c.text1, fontSize: 14, fontWeight: 700 }}>Giới thiệu bạn bè</p>
          <p style={{ color: c.text3, fontSize: 12 }}>Hạng {currentTier.name} · {currentTier.commission}% hoa hồng</p>
        </div>
        <ChevronRight size={16} color={c.text3} />
      </div>
      <div className="flex gap-2">
        <div className="flex-1 rounded-xl p-2.5" style={{ background: c.surface2 }}>
          ...
          <p style={{ color: '#3B82F6', fontSize: 14, fontWeight: 700, fontFamily: 'monospace' }}>{stats.totalFriends}</p>
        </div>
        <div className="flex-1 rounded-xl p-2.5" style={{ background: c.surface2 }}>
          <p style={{ color: c.text3, fontSize: 10 }}>Hoa hồng</p>
          <p style={{ color: '#10B981', fontSize: 14, fontWeight: 700, fontFamily: 'monospace' }}>{fmtUsd(stats.totalCommission)}</p>
        </div>
        <div className="flex-1 rounded-xl p-2.5" style={{ background: c.surface2 }}>
          <p style={{ color: c.text3, fontSize: 10 }}>Tháng này</p>
          <p style={{ color: '#F59E0B', fontSize: 14, fontWeight: 700, fontFamily: 'monospace' }}>+{fmtUsd(stats.thisMonthCommission)}</p>
        </div>
      </div>
    </div>
  );
