...
import { Zap, Lock, AlertTriangle, Shield, CheckCircle } from 'lucide-react';
...
export function P2PExpressConfirmPage() {
  ...
  return (
    <PageLayout>
      <Header
        title={`Xác nhận ${tradeType === 'buy' ? 'mua' : 'bán'} nhanh`}
      ...
      <TrCard className="p-4" accentBorder={tradeColor + '30'}>
        {[
          ...
          {
            label: tradeType === 'buy' ? 'Cần thanh toán' : 'Sẽ nhận được',
            value: `${fmtVnd(Math.round(fiatAmount))} VND`,
            bold: true,
            color: tradeColor,
          },
          ...
          <div className="flex-1">
            <div className="flex items-center gap-1.5">
              ...
              {bestAd.merchantVerified && (
                <Shield size={11} color="#3B82F6" fill="rgba(59,130,246,0.2)" />
              )}
            </div>
      ...
      >
        ...
        <p style={{ color: '#10B981', fontSize: 11, lineHeight: 1.6 }}>
          {fmtAmount(cryptoAmount)} {asset} sẽ được khóa trong Escrow. Bạn có 15 phút để hoàn tất
          thanh toán sau khi tạo đơn.
        </p>
      </div>
      ...
      >
        ...
        <p style={{ color: '#D97706', fontSize: 11, lineHeight: 1.6 }}>
          Chỉ bấm xác nhận khi bạn đã sẵn sàng thanh toán. Hủy đơn nhiều lần sẽ ảnh hưởng đến uy
          tín.
        </p>
      </div>
