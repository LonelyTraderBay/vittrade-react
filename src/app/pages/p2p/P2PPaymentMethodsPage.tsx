    ...
    >
      <div className="flex items-start gap-3">
        <div
          ...
          style={{
            background: pm.type === 'bank' ? 'rgba(59,130,246,0.1)' : 'rgba(168,85,247,0.1)',
          }}
        >
          {pm.type === 'bank' ? (
            <CreditCard size={φIcon.md} color="#3B82F6" />
          ) : (
            <Smartphone size={φIcon.md} color="#A855F7" />
          )}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <p style={{ color: c.text1, fontSize: φ.sm, fontFamily: 'monospace', fontWeight: 600 }}>
            {pm.accountNumber}
          </p>
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => {
              navigate(`${prefix}/p2p/payment-method/add?type=${pm.type}`);
              hapticSelection();
            }}
    ...
    <PageLayout>
      ...
      <PageContent gap="default">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => {
              navigate(`${prefix}/p2p/payment-method/add?type=bank`);
              hapticSelection();
            }}
          ...
          <button
            onClick={() => {
              navigate(`${prefix}/p2p/payment-method/add?type=ewallet`);
              hapticSelection();
            }}
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm"
            style={{
        ...
        {ewalletMethods.length > 0 && (
          <div className="flex flex-col gap-3">
            <h3 style={{ color: c.text2, fontSize: φ.sm, fontWeight: 600 }}>
              ...
              Ví điện tử ({ewalletMethods.length})
            </h3>
        ...
        >
          <p style={{ color: '#3B82F6', fontSize: 11, lineHeight: 1.6 }}>
            ...
            Thông tin thanh toán chỉ hiển thị cho đối tác khi đơn hàng P2P được tạo. VitTrade không
            lưu trữ mật khẩu ngân hàng.
          </p>
        </div>
      </PageContent>
