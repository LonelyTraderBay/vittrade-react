              ...
              <div className="flex gap-0.5 mt-1">
                {Array.from({ length: 5 }, (_, i) => (
                  <Star
                    key={i}
                    size={12}
                    fill={i < Math.round(parseFloat(avgRating)) ? '#F59E0B' : c.surface2}
                    color={i < Math.round(parseFloat(avgRating)) ? '#F59E0B' : c.surface2}
                  />
                ))}
              </div>
              <span style={{ color: c.text3, fontSize: φ.xs, marginTop: 4 }}>
              ...
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                return (
                  <div key={star} className="flex items-center gap-2">
                    ...
                    <span style={{ color: c.text3, fontSize: 10, width: 16, textAlign: 'right' }}>
                      {count}
                    </span>
                  </div>
                );
                  ...
                  <div className="flex gap-0.5 ml-1">
                    {Array.from({ length: 5 }, (_, i) => (
                      <Star
                        key={i}
                        size={10}
                        fill={i < review.rating ? '#F59E0B' : c.surface2}
