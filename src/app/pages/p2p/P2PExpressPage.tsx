...
function findBestAd(
  ...
  let candidates = P2P_ADS.filter(
    (ad) => ad.type === adType && ad.asset === asset && ad.status === 'active',
...
function findTopAds(tradeType: 'buy' | 'sell', asset: string, fiatAmount: number): P2PAd[] {
  const adType = tradeType === 'buy' ? 'sell' : 'buy';
  let candidates = P2P_ADS.filter(
    (ad) => ad.type === adType && ad.asset === asset && ad.status === 'active',
  );
    ...
    <PageLayout>
      ...
      <div className="flex-1 px-5 py-4 flex flex-col gap-4 pb-6">
        ...
        <TrCard className="p-4">
          ...
          >
            <span style={{ color: c.text3, fontSize: φ.md, fontWeight: 600, marginRight: 8 }}>
              ₫
            </span>
            <TrInput
              type="number"
              ...
              value={fiatInput}
              onChange={(value) => setFiatInput(value)}
              style={{
                flex: 1,
          ...
          <div className="flex flex-wrap gap-2">
            {(userPayments.length > 0
              ...
              <button
                key={pm}
                onClick={() => {
                  setSelectedPayment(selectedPayment === pm ? '' : pm);
                  hapticSelection();
                }}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl"
                style={{
              ...
              <TrCard className="p-4" accentBorder={tradeColor + '40'}>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span
                        className="truncate"
                        style={{ color: c.text1, fontSize: φ.sm, fontWeight: 700 }}
                      >
                        {bestAd.merchant}
                      </span>
                      {bestAd.merchantVerified && (
                  ...
                  <button
                    onClick={() => {
                      navigate(`${prefix}/p2p/merchant/${bestAd.merchantId}`);
                      hapticSelection();
                    }}
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
                    ...
                    >
                      {priceDiff >= 0 ? '+' : ''}
                      {priceDiff.toFixed(2)}% vs thị trường
                    </p>
                  </div>
                  <div className="rounded-xl p-3" style={{ background: c.surface2 }}>
        ...
        <TrCard className="p-4">
          ...
          <div className="flex flex-col gap-2.5">
            {[
              ...
              {
                step: '3',
                text: `Xác nhận → Tạo đơn → ${tradeType === 'buy' ? 'Thanh toán' : 'Nhận tiền'}`,
                icon: '✅',
              },
            ].map((item) => (
