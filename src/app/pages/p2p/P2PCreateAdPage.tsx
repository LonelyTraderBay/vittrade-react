  ...
  return (
    <PageLayout variant="flush">
      ...
      <PageContent gap="relaxed" grow>
        {/* Ad Type */}
        <div>
          <label style={{ color: c.text2, fontSize: 13, marginBottom: 8, display: 'block' }}>
            Loại quảng cáo
          </label>
          <div className="flex rounded-xl p-1" style={{ background: c.surface2 }}>
            <button
              ...
              style={{
                background: adType === 'buy' ? c.buy : 'transparent',
                color: adType === 'buy' ? '#fff' : c.text3,
                fontWeight: 700,
                fontSize: 13,
              }}
            >
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-lg flex items-center justify-center"
              style={{
                background: adType === 'sell' ? c.sell : 'transparent',
                color: adType === 'sell' ? '#fff' : c.text3,
                fontWeight: 700,
                fontSize: 13,
              }}
            >
            ...
            <div className="flex gap-2">
              {['VND', 'USD'].map((cr) => (
                <button
                  ...
                  className="px-3 py-2 rounded-xl text-sm font-semibold"
                  style={{
                    background: currency === cr ? c.chipActiveBg : c.chipBg,
                    color: currency === cr ? c.chipActiveText : c.chipText,
                    border: `1px solid ${currency === cr ? c.chipActiveBorder : c.chipBorder}`,
                  }}
                >
                  {cr}
                </button>
        ...
        <div>
          ...
          <div className="flex rounded-xl p-1" style={{ background: c.surface2 }}>
            <button
              ...
              style={{
                background: priceType === 'fixed' ? c.chipActiveBg : 'transparent',
                color: priceType === 'fixed' ? c.chipActiveText : c.text3,
              }}
            >
              Cố định
            </button>
            <button
              ...
              className="flex-1 py-2 rounded-lg text-sm font-semibold"
              style={{
                background: priceType === 'floating' ? 'rgba(168,85,247,0.15)' : 'transparent',
                color: priceType === 'floating' ? '#A855F7' : c.text3,
              }}
            >
              Thả nổi %
            </button>
        ...
        {priceType === 'fixed' ? (
          <InputRow
            label={`Giá (${currency}/${asset})`}
            hint={`Giá thị trường: ${fmtVnd(marketPrice)} ${currency}`}
            required
          >
            <InputField
              type="number"
              placeholder={fmtVnd(marketPrice)}
              value={price}
              onChange={(e) => setPrice(e.target.value)}
              suffix={<span style={{ color: c.text3, fontSize: 13 }}>{currency}</span>}
              style={{ fontSize: 17, fontFamily: 'monospace', fontWeight: 700 }}
            />
            {effectivePrice > 0 && (
              <div className="flex items-center gap-1 mt-2">
                <span
                  style={{ color: priceDiff >= 0 ? c.buy : c.sell, fontSize: 12, fontWeight: 600 }}
                >
          ...
          <InputRow
            label="Biên độ giá (%)"
            hint={`Giá = Thị trường × (1 + biên độ%). Giá hiện tại: ${fmtVnd(effectivePrice)} ${currency}`}
            required
          >
            <InputField
              type="number"
            ...
            <div className="flex gap-2 mt-2">
              {[-1, -0.5, 0, 0.5, 1, 2].map((m) => (
                <button
                  key={m}
                  onClick={() => {
                    setPriceMargin(m.toString());
                    hapticSelection();
                  }}
                  className="px-2 py-1 rounded-lg text-xs font-semibold"
                  style={{
                    background: priceMargin === m.toString() ? 'rgba(168,85,247,0.15)' : c.surface2,
                    color: priceMargin === m.toString() ? '#A855F7' : c.text3,
                  }}
                >
                  {m >= 0 ? '+' : ''}
                  {m}%
        ...
        <InputRow label={`Tổng ${asset} giao dịch`} required>
          <InputField
            ...
            suffix={<span style={{ color: c.text3, fontSize: 13 }}>{asset}</span>}
            style={{ fontSize: 17, fontFamily: 'monospace', fontWeight: 700 }}
          />
        </InputRow>
        ...
        <InputRow
          ...
          hint={`Đã chọn ${selectedPayments.length}/5`}
          required
        >
            ...
            {PAYMENT_OPTIONS.map((pm) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSelected ? c.primaryAlpha12 : c.surface2,
                    color: isSelected ? c.primary : c.text2,
                    border: `1px solid ${isSelected ? c.primaryAlpha12 : c.borderSolid}`,
                  }}
                >
                  {isSelected && <CheckCircle size={12} color={c.primary} />}
                  {pm}
                </button>
              );
            })}
          ...
          <div className="flex gap-2">
            {['15', '30', '60'].map((min) => (
              <button
                key={min}
                onClick={() => {
                  setPaymentWindow(min);
                  hapticSelection();
                }}
                className="flex-1 py-2 rounded-xl text-sm font-semibold"
                style={{
                  background: paymentWindow === min ? c.chipActiveBg : c.chipBg,
                  color: paymentWindow === min ? c.chipActiveText : c.chipText,
                  border: `1px solid ${paymentWindow === min ? c.chipActiveBorder : c.chipBorder}`,
                }}
              >
                {min} phút
              </button>
          ...
          <div className="flex gap-2">
            {['24/7', '08:00 - 22:00', '08:00 - 17:00'].map((h) => (
              <button
                ...
                style={{
                  background: tradingHours === h ? c.chipActiveBg : c.chipBg,
                  color: tradingHours === h ? c.chipActiveText : c.chipText,
                  border: `1px solid ${tradingHours === h ? c.chipActiveBorder : c.chipBorder}`,
                }}
              >
                {h}
              </button>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                style={{
                  background: reqKyc ? c.primary : c.surface2,
                  border: `1px solid ${reqKyc ? c.primary : c.borderSolid}`,
                }}
              >
                <div
              ...
              <div className="flex gap-2">
                {['1', '2', '3'].map((lvl) => (
                  <button
                    ...
                    style={{
                      background: reqKycLevel === lvl ? c.primaryAlpha12 : c.surface2,
                      color: reqKycLevel === lvl ? c.primary : c.text3,
                    }}
                  >
                    Cấp {lvl}
                  </button>
            ...
            <InputField
              ...
              placeholder="0"
              value={reqMinDays}
              onChange={(e) => setReqMinDays(e.target.value)}
              containerStyle={{ height: 40 }}
              style={{ fontSize: 13 }}
            />
          </div>
        ...
        <InputRow label="Điều kiện giao dịch (tuỳ chọn)">
          <textarea
            ...
            placeholder="VD: Chỉ giao dịch với tài khoản đã xác minh KYC..."
            rows={3}
        ...
        <InputRow label="Tin nhắn tự động (tuỳ chọn)" hint="Gửi tự động khi đối tác tạo đơn">
          <textarea
            ...
            placeholder="VD: Cảm ơn bạn! Vui lòng chuyển khoản theo thông tin bên dưới."
            rows={2}
            className="w-full rounded-2xl px-4 py-3"
            style={{
              background: c.surface2,
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <p style={{ color: c.warn, fontSize: 12, lineHeight: 1.6 }}>
        ...
        <LivePreviewSection
          ...
          onToggle={() => {
            setLivePreviewExpanded(!livePreviewExpanded);
            hapticSelection();
          }}
          adType={adType}
          asset={asset}
      ...
      <StickyFooter>
        ...
        >
          {isLoading
            ? 'Đang đăng...'
            : `Đăng quảng cáo ${adType === 'buy' ? 'MUA' : 'BÁN'} ${asset}`}
        </CTAButton>
      </StickyFooter>
      ...
      <P2PConfirmationModal
        ...
        summaryRows={[
          {
            label: 'Loại',
            value: adType === 'buy' ? 'Quảng cáo MUA' : 'Quảng cáo BÁN',
            highlight: true,
          },
          ...
          {
            label: 'Loại giá',
            value: priceType === 'fixed' ? 'Cố định' : `Thả nổi ${priceMargin || '0'}%`,
          },
          {
            label: 'Giới hạn',
            value: `${fmtVnd(parseFloat(minLimit || '0'))} – ${fmtVnd(parseFloat(maxLimit || '0'))} ${currency}`,
            mono: true,
          },
        ...
        warnings={[
          {
            text: 'Quảng cáo sẽ được xem xét tự động. Vi phạm chính sách có thể dẫn đến đình chỉ tài khoản.',
            type: 'warning',
          },
          {
            text:
              adType === 'sell'
                ? `${totalAmount || '0'} ${asset} sẽ bị khóa trong Escrow khi có người đặt đơn.`
                : 'Bạn cần đảm bảo đủ số dư để thanh toán khi có đơn hàng.',
            type: 'info',
          },
        ]}
