...
export function P2PMyAdsPage() {
  ...
  const toggleAdStatus = (id: string) => {
    ...
    setAds((prev) =>
      prev.map((a) =>
        a.id === id ? ({ ...a, status: a.status === 'active' ? 'paused' : 'active' } as P2PAd) : a,
      ),
    );
    actionToast.success(ad?.status === 'active' ? TOAST.P2P.AD_PAUSED : TOAST.P2P.AD_RESUMED, {
      haptic: 'selection',
    });
  };
  ...
  return (
    <PageLayout>
        ...
        <PageContent padding="compact">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              {
                label: 'KL 30 ngày',
                value: fmtCompact(totalVolume, { prefix: '$' }),
                color: '#3B82F6',
              },
            ].map((stat) => (
          ...
          >
            <div key={filter} className="flex flex-col gap-3">
              {filtered.map((ad, idx) => (
                <div key={ad.id}>
                  <TrCard className="p-4" style={{ opacity: ad.status === 'paused' ? 0.7 : 1 }}>
                    {/* Header */}
                    ...
                    <div className="mb-3">
                      ...
                      <span style={{ color: c.text3, fontSize: φ.sm, marginLeft: 4 }}>
                        {ad.currency}/{ad.asset}
                      </span>
                      {ad.priceMargin !== undefined && (
                        <span
                          style={{
                            color: ad.priceMargin >= 0 ? '#10B981' : '#EF4444',
                            fontSize: φ.xs,
                            marginLeft: 8,
                          }}
                        >
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3 text-xs">
                      ...
                      <div>
                        ...
                        <p style={{ color: c.text1, fontFamily: 'monospace', fontSize: 10 }}>
                          {fmtVnd(ad.minLimit)}-{fmtVnd(ad.maxLimit)}
                        </p>
                      </div>
                      <div>
                        <p style={{ color: c.text3 }}>Thanh toán</p>
                        <p style={{ color: c.text1, fontSize: 10 }}>
                    ...
                    >
                      <button
                        onClick={() => navigate(`${prefix}/p2p/ad-analytics/${ad.id}`)}
                ...
                {[
                  {
                    icon: Settings,
                    label: 'Cài đặt P2P',
                    desc: 'Tùy chọn giao dịch, thông báo',
                    color: '#6B7280',
                    path: `${prefix}/p2p/settings`,
                  },
                  {
                    icon: UserX,
                    ...
                    color: '#EF4444',
                    path: `${prefix}/p2p/blacklist`,
                  },
                  {
                    icon: BookOpen,
                    label: 'Hướng dẫn',
                    desc: 'Mẹo đăng quảng cáo hiệu quả',
                    color: '#8B5CF6',
                    path: `${prefix}/p2p/guide`,
                  },
                ].map((link, i) => (
                  <button
                    key={link.label}
      ...
      <ConfirmationDialog
        ...
        onConfirm={() => {
          if (deleteConfirm) deleteAd(deleteConfirm);
        }}
        variant="danger"
