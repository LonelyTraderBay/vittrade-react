        ...
        <TrCard hover className="p-4" accentBorder={isFav ? 'rgba(245,158,11,0.25)' : undefined}>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {isFav && <Heart size={14} color="#F59E0B" fill="#F59E0B" className="shrink-0" />}
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  hapticSelection();
...
export function P2PHomePage() {
  ...
  const ads = useMemo(() => {
    ...
    if (merchantType !== 'all') {
      if (merchantType === 'elite')
        filtered = filtered.filter((ad) => ad.merchantBadge === 'elite');
      else if (merchantType === 'pro')
        filtered = filtered.filter((ad) => ad.merchantBadge === 'pro');
    ...
    >
      ...
      {hasError && (
        <div className="px-5 py-8">
          <ErrorState title="Không thể tải P2P" />
        </div>
      )}
            ...
            <div className="grid grid-cols-3 gap-0">
              ...
              <div className="flex flex-col items-center py-1.5">
                ...
                <span style={{ color: c.text3, fontSize: 10, marginTop: 1 }}>
                  avg {P2P_PLATFORM_STATS.avgCompletionTime}
                </span>
              </div>
            </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span style={{ color: c.text2, fontSize: 11 }}>
                  <span style={{ fontWeight: 700, fontFamily: 'monospace', color: c.text1 }}>
                    {P2P_PLATFORM_STATS.totalTrades24h.toLocaleString()}
                  </span>{' '}
      ...
      <ContextMenuSheet
        ...
        onToggleFav={() => {
          if (contextMenuAd) handleToggleFav(contextMenuAd);
        }}
