...
export default function FunnelDashboard() {
  ...
  const funnelAnalytics = useMemo(() => {
    ...
    const steps: FunnelStep[] = funnel.steps.map((step, idx) => {
      const stepAnalytics = analytics.stepAnalytics.find((s) => s.stepId === step.id);
  ...
  const bottleneck = steps.reduce(
    (max, step) => (step.dropoutRate > (max?.dropoutRate || 0) ? step : max),
    steps[0],
  );
  ...
  return (
    <PageLayout>
      ...
      <PageContent gap="default">
        {/* Funnel Selector */}
        <div>
          <p style={{ color: c.text3, fontSize: 11, marginBottom: 8 }}>Chọn funnel</p>
          <div className="grid grid-cols-2 gap-2">
            {CONVERSION_FUNNELS.map((f) => (
              <button
                ...
                style={{
                  background: selectedFunnel === f.id ? c.surface : c.surface2,
                  border: selectedFunnel === f.id ? `2px solid #8B5CF6` : `2px solid transparent`,
                }}
              >
                <p style={{ color: c.text1, fontSize: 13, fontWeight: 600 }}>{f.name}</p>
                <p style={{ color: c.text3, fontSize: 10 }}>{f.steps.length} bước</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <TrCard className="p-4">
            ...
            <p style={{ color: c.text3, fontSize: 11 }}>{completedSessions} hoàn thành</p>
          </TrCard>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p style={{ color: c.text2, fontSize: 12 }}>
                  <span style={{ fontWeight: 600 }}>{bottleneck.label}</span> có tỷ lệ rời bỏ cao
                  nhất ({(bottleneck.dropoutRate * 100).toFixed(1)}%)
                </p>
              </div>
            </div>
          </TrCard>
        ...
        <TrCard className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 style={{ color: c.text1, fontSize: φ.base, fontWeight: 600 }}>Funnel Waterfall</h2>
          </div>
                ...
                <div key={step.step}>
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      {isBottleneck && <AlertTriangle size={14} color="#F59E0B" />}
                    </div>
                    <div className="text-right">
                      ...
                      <p style={{ color: c.text3, fontSize: 10 }}>{widthPercent.toFixed(0)}%</p>
                    </div>
                  </div>

                  {/* Progress Bar */}
                  <div
                    className="h-8 rounded-lg overflow-hidden mb-1"
              ...
              <Bar key="bar-dropout" dataKey="dropout" name="Tỷ lệ rời bỏ" radius={[4, 4, 0, 0]}>
                {dropoutChartData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={
                      entry.dropout > 30 ? '#F59E0B' : entry.dropout > 20 ? '#3B82F6' : '#10B981'
                    }
                  />
                ))}
              </Bar>
        ...
        <TrCard className="p-4">
          ...
          <div className="space-y-2">
            {steps.map((step, idx) => (
              <div key={step.step} className="p-3 rounded-lg" style={{ background: c.surface2 }}>
