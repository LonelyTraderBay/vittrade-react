...
import { MOCK_AIRDROP_CLAIMS, calculateAirdropStats, type AirdropClaim } from './launchpadData';
...
export function LaunchpadAirdropClaimPage() {
  ...
  const filtered =
    tab === 'claimable'
      ? claims.filter((c) => c.status === 'claimable')
      : tab === 'claimed'
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                {
                  label: 'Claimable',
                  value: stats.totalClaimable + ' tokens',
                  sublabel: stats.totalClaimableUSD,
                  color: '#10B981',
                },
                {
                  label: 'Claimed',
                  value: stats.totalClaimed + ' tokens',
                  sublabel: stats.totalClaimedUSD,
                  color: '#3B82F6',
                },
              ].map((s) => (
                <div
            ...
            {stats.expiringSoon > 0 && (
              ...
              >
                ...
                <span style={{ color: '#EF4444', fontSize: 11, fontWeight: 600 }}>
                  {stats.expiringSoon} airdrop{stats.expiringSoon > 1 ? 's' : ''} hết hạn trong 7
                  ngày
                </span>
              </div>
            )}
      ...
      <button className="w-full text-left p-4" onClick={() => onSelect(claim)}>
        ...
        <div className="flex items-start gap-3 mb-3">
          <div
            ...
            style={{
              background: claim.projectLogoColor + '22',
              border: `2px solid ${claim.projectLogoColor}44`,
              color: claim.projectLogoColor,
            }}
          >
            {claim.projectLogo}
          </div>
...
function ClaimDetailSheet({ claim, onClose }: { claim: AirdropClaim; onClose: () => void }) {
  const c = useThemeColors();
  const statusConfig = STATUS_CONFIG[claim.status];
          ...
          <PageSection label="Claim Details" accentColor={claim.projectLogoColor}>
            <div className="rounded-2xl p-3" style={{ background: c.surface2 }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  style={{ color: c.text1, fontSize: 14, fontWeight: 700, fontFamily: 'monospace' }}
                >
                  {claim.claimableAmount.toLocaleString()} {claim.projectSymbol}
                </span>
              </div>
              ...
              {claim.claimConditions.map((cond) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p
                      style={{ color: cond.met ? c.text1 : c.text3, fontSize: 12, fontWeight: 600 }}
                    >
            ...
            <>
              ...
              >
                ...
                <p style={{ color: c.text2, fontSize: 11, lineHeight: 1.5 }}>
                  Tất cả conditions đã được thỏa. Bạn có thể claim airdrop này trước{' '}
                  {claim.claimDeadline}.
                </p>
              </div>

              <CTAButton>
                Claim {claim.claimableAmount.toLocaleString()} {claim.projectSymbol}
              </CTAButton>
            </>
          )}
