...
export function WithdrawPage() {
  ...
  const networks =
    WITHDRAW_NETWORKS[asset as keyof typeof WITHDRAW_NETWORKS] ?? WITHDRAW_NETWORKS.USDT;
  ...
  const isValidAmount =
    ...
    amountNum <= Math.min(selectedNet.maxWithdraw, assetData.available);

  const validate = () => {
    const e: Record<string, string> = {};
    if (!address.trim()) e.address = 'Vui lòng nhập địa chỉ ví';
    if (address.length < 20) e.address = 'Địa chỉ ví không hợp lệ';
    if (requiresMemo && !memo.trim())
      e.memo = `${memoLabel} là bắt buộc cho mạng ${selectedNet.name}`;
    if (!amount || amountNum <= 0) e.amount = 'Vui lòng nhập số tiền';
    if (amountNum < selectedNet.minWithdraw)
      e.amount = `Tối thiểu ${selectedNet.minWithdraw} ${asset}`;
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="text-center">
              ...
              <p style={{ color: c.text2, fontSize: 14, marginTop: 8, lineHeight: 1.6 }}>
                ...
                đang được xử lý. Thời gian ước tính:{' '}
                <strong style={{ color: c.text1 }}>~30 phút</strong>.
              </p>
            </div>
  ...
  return (
    <PageLayout>
      ...
      <QRScannerOverlay
        ...
        onScan={(result) => {
          setAddress(result);
          actionToast.success(`Đã dán địa chỉ — ${result.slice(0, 16)}...`, { haptic: 'success' });
        }}
        title={`Quét QR — ${asset}`}
      />
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl p-4 flex flex-col gap-1" style={{ background: c.surface2 }}>
            ...
            />
            {requiresMemo && memo && <BottomSheetRow label={memoLabel} value={memo} />}
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <p style={{ color: c.warningText, fontSize: 12, lineHeight: 1.5 }}>
                  ...
                  . Sai {memoLabel} sẽ khiến giao dịch không được ghi nhận.
                </p>
              </div>
            </div>
          )}
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <p style={{ color: '#F87171', fontSize: 13, lineHeight: 1.6 }}>
                <strong>Kiểm tra kỹ địa chỉ ví!</strong> Giao dịch blockchain không thể hoàn tác sau
                khi xác nhận.
              </p>
            </div>
          </div>
            ...
            >
              <div>
                <div className="flex items-center gap-2">
                  <p style={{ color: c.text1, fontSize: 14, fontWeight: 600, textAlign: 'left' }}>
                    {net.name}
                  </p>
                ...
                <p style={{ color: c.text3, fontSize: 12 }}>
                  Phí: {net.fee} {asset} • Tối thiểu: {net.minWithdraw}
                </p>
              </div>
      ...
      <PageContent gap="default">
        ...
        <div>
          ...
          <button
            onClick={() => {
              setShowNetPicker(true);
              hapticSelection();
            }}
            className="w-full flex items-center justify-between px-4 rounded-2xl hover-ghost"
            style={{
              background: c.surface2,
              border: `1.5px solid ${errors.network ? c.error : c.borderSolid}`,
              height: 52,
              borderRadius: 14,
            }}
          >
            <div className="flex flex-col items-start">
              <span style={{ color: c.text1, fontSize: 15, fontWeight: 600 }}>
                {selectedNet.name}
              </span>
              <span style={{ color: c.text2, fontSize: 11 }}>
                Phí: {selectedNet.fee} {asset} • Tối thiểu: {selectedNet.minWithdraw}
              </span>
            </div>
        ...
        <div>
          ...
          <div
            className="flex flex-col rounded-2xl px-4 py-3 gap-2"
            style={{
              background: c.surface2,
              border: `1.5px solid ${errors.address ? c.error : c.borderSolid}`,
            }}
          >
            <input
              type="text"
              placeholder={`Nhập địa chỉ ${asset} (${selectedNet.name.split(' ')[0]})`}
              value={address}
          ...
          {errors.address && (
            <p style={{ color: c.error, fontSize: 12, marginTop: 4 }}>{errors.address}</p>
          )}
        ...
        {requiresMemo && (
          <div>
            ...
            <div
              ...
              style={{
                background: c.surface2,
                border: `1.5px solid ${errors.memo ? c.error : c.borderSolid}`,
              }}
            >
              <input
                type="text"
            ...
            {errors.memo && (
              <p style={{ color: c.error, fontSize: 12, marginTop: 4 }}>{errors.memo}</p>
            )}
          </div>
        )}
        ...
        <div>
          <div className="flex justify-between mb-2">
            ...
            <button
              onClick={() => {
                setAmount(assetData.available.toFixed(6));
                hapticSelection();
              }}
              style={{ color: c.primary, fontSize: 12, fontWeight: 600 }}
            >
              Tất cả
            </button>
          </div>
          <div
            ...
            style={{
              background: c.surface2,
              border: `1.5px solid ${errors.amount ? c.error : c.borderSolid}`,
              height: 52,
              borderRadius: 14,
            }}
          >
          ...
          {errors.amount && (
            <p style={{ color: c.error, fontSize: 12, marginTop: 4 }}>{errors.amount}</p>
          )}
