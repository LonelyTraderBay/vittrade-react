...
const WALLETS = [
  ...
  { id: 'funding', name: 'Ví Funding', icon: Wallet, balance: 8450.2 },
  { id: 'futures', name: 'Ví Futures', icon: Banknote, balance: 3200.0 },
];
...
export function TransferPage() {
  ...
  const handleConfirmTransfer = () => {
    ...
    hapticSuccess();
    actionToast.success(
      `Chuyển thành công! ${fmtAmount(amountNum)} ${selectedAsset} đã chuyển từ ${fromW.name} sang ${toW.name}`,
      { haptic: 'success' },
    );
        ...
        {WALLETS.filter((w) => w.id !== exclude).map((w) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <span style={{ color: c.text2, fontSize: 12, marginLeft: 8 }}>
                {fmtUsd(w.balance)}
              </span>
            </div>
          </button>
    ...
    <PageLayout>
      ...
      <PageContent>
        ...
        <TrCard className="p-4 relative">
          ...
          >
            <div
              ...
              style={{
                background: (asset?.logoColor ?? c.primary) + '22',
                border: `1.5px solid ${asset?.logoColor ?? c.primary}44`,
              }}
            >
              <span style={{ color: asset?.logoColor ?? c.primary, fontSize: 10, fontWeight: 700 }}>
                {selectedAsset.slice(0, 3)}
              </span>
            </div>
            <div className="flex-1 text-left">
              <p style={{ color: c.text1, fontSize: 15, fontWeight: 600 }}>{selectedAsset}</p>
              <p style={{ color: c.text2, fontSize: 12 }}>
                Khả dụng: {fmtAmount(available)} {selectedAsset}
              </p>
            </div>
            <ChevronDown size={18} color={c.text3} />
          </button>
          ...
          {showAssetPicker && (
            <div
              className="absolute top-full left-0 right-0 mt-1 rounded-2xl overflow-hidden z-10 max-h-60 overflow-y-auto"
              style={{ background: c.surface2, border: `1px solid ${c.borderSolid}` }}
            >
              {USER_ASSETS.map((a) => (
                <button
                ...
                >
                  ...
                  >
                    <span style={{ color: a.logoColor, fontSize: 9, fontWeight: 700 }}>
                      {a.symbol.slice(0, 3)}
                    </span>
                  </div>
                  ...
                  <span
                    className="ml-auto"
                    style={{ color: c.text2, fontSize: 12, fontFamily: 'monospace' }}
                  >
                    {fmtAmount(a.available)}
                  </span>
                </button>
              ))}
            </div>
        ...
        <TrCard className="p-4">
          ...
          {amount && (
            <p style={{ color: c.text3, fontSize: 12, marginTop: 4 }}>≈ {fmtUsd(usdEquiv)}</p>
          )}
        </TrCard>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl p-4 flex flex-col gap-1" style={{ background: c.surface2 }}>
            ...
            <BottomSheetRow
              label="Số lượng"
              value={`${fmtAmount(amountNum)} ${selectedAsset}`}
              highlight
            />
