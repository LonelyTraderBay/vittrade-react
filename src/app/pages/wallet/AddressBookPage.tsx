...
export function AddressBookPage() {
  ...
  const toggleFavorite = (id: string) => {
    ...
    actionToast.info(
      addr?.isFavorite
        ? TOAST.FAVORITE.removed(addr.label)
        : TOAST.FAVORITE.added(addr?.label ?? ''),
    );
  };
    ...
    <TrCard className="p-4">
      ...
      <div className="flex gap-2 mt-3">
        ...
        <button
          ...
          style={{
            background: addr.isFavorite ? 'rgba(245,158,11,0.1)' : c.surface2,
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => {
                ...
                actionToast.info(
                  !whitelistOnly
                    ? 'Whitelist đã bật — Chỉ rút tới địa chỉ whitelist. Địa chỉ mới cần 24h.'
                    : 'Whitelist đã tắt — Bạn có thể rút tới bất kỳ địa chỉ nào.',
                );
              }}
          ...
          {whitelistOnly && (
            ...
            >
              ...
              <p style={{ color: '#D97706', fontSize: 10, lineHeight: 1.5 }}>
                <strong>Bảo vệ nâng cao:</strong> Địa chỉ mới thêm vào whitelist cần chờ{' '}
                <strong>24 giờ</strong> trước khi dùng được. Tắt whitelist mode cũng cần xác nhận
                2FA.
              </p>
            </div>
          )}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              {
                label: 'Yêu thích',
                value: addresses.filter((a) => a.isFavorite).length.toString(),
                color: '#F59E0B',
              },
              {
                label: 'Whitelist',
                value: addresses.filter((a) => a.isWhitelisted).length.toString(),
                color: '#10B981',
              },
            ].map((s) => (
          ...
          >
            ...
            <p style={{ color: c.text2, fontSize: 12, lineHeight: 1.5 }}>
              <span style={{ color: '#3B82F6', fontWeight: 600 }}>Bảo mật:</span> Địa chỉ whitelist
              được bảo vệ bởi 2FA. Chỉ có thể rút tới địa chỉ đã được xác minh.
            </p>
          </div>
        </div>
      </PageContent>
