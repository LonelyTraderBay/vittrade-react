...
export function PnLCalculator({ open, onClose, livePrice, symbol, baseAsset }: PnLCalculatorProps) {
  ...
  const calc = useMemo(() => {
    ...
    // PnL before fees
    const rawPnL = direction === 'long' ? (exit - entry) * qty * lev : (entry - exit) * qty * lev;
  ...
  const InputField = ({
    ...
    <div>
      ...
      >
        ...
        />
        {suffix && (
          <span style={{ color: c.text3, fontSize: 10, fontWeight: 600, marginLeft: 4 }}>
            {suffix}
          </span>
        )}
      </div>
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none px-3 py-2 flex flex-col gap-2.5">
        ...
        {calc ? (
          ...
          >
            ...
            >
              ...
              >
                {calc.isProfit ? '+' : ''}
                {calc.netPnL < 0 ? '-' : ''}$
                {Math.abs(calc.netPnL).toLocaleString('en-US', {
            ...
            <div className="px-3 py-2" style={{ borderTop: `1px solid ${panelBorder}` }}>
              ...
              <div className="flex justify-between py-1">
                ...
                <span
                  style={{ color: '#F59E0B', fontSize: 11, fontVariantNumeric: 'tabular-nums' }}
                >
          ...
          <div
            className="rounded-lg py-6 flex flex-col items-center justify-center gap-1.5"
        ...
        {/* Use live price hint */}
        <button
          onClick={() => setExitPrice(livePrice.toFixed(2))}
