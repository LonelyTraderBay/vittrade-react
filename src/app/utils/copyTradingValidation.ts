...
export function validateCopyTrader(trader: CopyTrader): ValidationResult {
  ...
  if (trader.copiers > trader.maxCopiers) {
    errors.push(
      `${trader.name}: copiers (${trader.copiers}) exceeds maxCopiers (${trader.maxCopiers})`,
    );
  }
  ...
  if (trader.weeklyPnl.length !== 7) {
    errors.push(
      `${trader.name}: weeklyPnl should have exactly 7 days, got ${trader.weeklyPnl.length}`,
    );
  }
  ...
  if (difference > 50) {
    // Allow some variance for mock data
    errors.push(
      `${trader.name}: totalPnlPct (${trader.totalPnlPct}%) doesn't match totalPnl/AUM calculation (${calculatedPct.toFixed(1)}%)`,
    );
  }
  ...
  if (trader.sharpeRatio < 0 || trader.sharpeRatio > 5) {
    errors.push(
      `${trader.name}: sharpeRatio (${trader.sharpeRatio}) seems unrealistic (expected 0-5)`,
    );
  }
  ...
  if (!validRiskLevels.includes(trader.riskLevel)) {
    errors.push(
      `${trader.name}: invalid riskLevel "${trader.riskLevel}" (expected: ${validRiskLevels.join(', ')})`,
    );
  }
